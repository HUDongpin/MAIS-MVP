...
export function normalizeNextDistDir(
  ...
  if (
    ...
    throw new Error(`NEXT_DIST_DIR must stay strictly inside the project root: ${raw}`);
  }
  return relative;
}

const distDir = normalizeNextDistDir(process.env.NEXT_DIST_DIR);

export function normalizeNextTsconfigPath(
  input: string | undefined,
  projectRoot = process.cwd()
) {
  const raw = input?.trim();
  ...
  if (
    ...
    throw new Error(`NEXT_TSCONFIG_PATH must stay inside the project root: ${raw}`);
  }
  if (
    path.posix.dirname(relative) !== "." ||
    !/^tsconfig(?:[.][a-zA-Z0-9_-]+)*[.]json$/.test(path.posix.basename(relative))
  ) {
    throw new Error(
      `NEXT_TSCONFIG_PATH must name one root tsconfig*.json file: ${raw}`
    );
  }
  return relative;
}

const explicitTsconfigPath = normalizeNextTsconfigPath(
