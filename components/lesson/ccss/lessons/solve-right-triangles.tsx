...
export default function Lesson() {
  ...
  const rawHeight = dist * Math.tan(rad);
  const rawLineOfSight = dist / Math.cos(rad);
  const height = r2(rawHeight);
  const lineOfSight = r2(rawLineOfSight);
  const heightRelation = relationForDisplayedValue(rawHeight, height);
  const lineOfSightRelation = relationForDisplayedValue(rawLineOfSight, lineOfSight);
  ...
  return (
    <div className="prose-lesson max-w-none">
      <p>
        Trigonometry <em>measures the unreachable</em>. From a ground-level
        observation point {dist} m from a building, measure the{" "}
        <strong>angle of elevation</strong>{" "}to its top, and you can compute the
        height without climbing — using tan θ = opposite / adjacent.
      </p>

      <Figure caption="From a ground-level observation point, a known distance and angle of elevation let tangent give the building height.">
        <div className="flex flex-col items-center gap-6">
          <svg width={260} height={180} viewBox="0 0 260 180" role="img" aria-label={`A ${angle} degree angle of elevation measured from a ground-level point ${dist} meters from a building, giving a height that ${spokenRelationForDisplayedValue(rawHeight, height)} ${height} meters`}>
            {/* ground */}
            ...
            <path d={`M ${observerX + arcR} ${groundY} A ${arcR} ${arcR} 0 0 0 ${observerX + arcR * Math.cos(rad)} ${groundY - arcR * Math.sin(rad)}`} fill="none" stroke="var(--band-middle)" strokeWidth={2} />
            ...
            <text x={buildingX > 185 ? buildingX - 6 : buildingX + 6} y={(groundY + topY) / 2} textAnchor={buildingX > 185 ? "end" : "start"} fontSize={12} fontWeight={800} fill={ACCENT}>h {heightRelation} {height}</text>
          </svg>

          <div className="grid grid-cols-2 gap-4 text-center font-mono text-sm">
            <div className="rounded-lg bg-[var(--surface-2)] px-4 py-2">height = {dist}·tan {angle}°<br /><strong style={{ color: ACCENT }}>{heightRelation} {height} m</strong></div>
            <div className="rounded-lg bg-[var(--surface-2)] px-4 py-2">line of sight = {dist}/cos {angle}°<br /><strong>{lineOfSightRelation} {lineOfSight} m</strong></div>
          </div>
      ...
      <p>
        ...
        <strong>tangent</strong>: height = {dist}·tan {angle}° {heightRelation} {height} m. Need the
        slanted line of sight instead? That&apos;s the hypotenuse, so use cosine — or
        the Pythagorean theorem once two sides are known. If an observer measures
        from eye or instrument height instead, this calculation gives the vertical
        rise above that point; add the observation height to obtain the full
        building height.
      </p>
