...
export default function Lesson() {
  ...
  const rawS = Math.sin(rad);
  const s = r2(rawS);
  const sineRelation = relationForDisplayedValue(rawS, s);
  const sineSpokenRelation = spokenRelationForDisplayedValue(rawS, s);
  ...
  const rawPrincipal = (Math.asin(rawS) * 180) / Math.PI;
  const principal = r2(rawPrincipal);
  const principalRelation = relationForDisplayedValue(rawPrincipal, principal);
  const principalSpokenRelation = spokenRelationForDisplayedValue(rawPrincipal, principal);
  ...
  return (
    <div className="prose-lesson max-w-none">
      ...
      <Figure caption="sin(θ) = value has many solutions; arcsin returns the one principal angle in [−90°, 90°].">
        <div className="flex flex-col items-center gap-6">
          <div className="rounded-2xl border-2 px-8 py-4 text-center font-mono" style={{ borderColor: ACCENT }}>
            <div className="text-lg" aria-label={`sine of ${deg} degrees ${sineSpokenRelation} ${s}, to the nearest hundredth`}>sin({deg}°) {sineRelation} <strong style={{ color: ACCENT }}>{s}</strong>{" "}(nearest hundredth)</div>
            <div className="mt-2 text-lg" aria-label={`arcsine of sine of ${deg} degrees ${principalSpokenRelation} ${principal} degrees`}>arcsin(sin({deg}°)) {principalRelation} <strong style={{ color: ACCENT }}>{principal}°</strong></div>
          </div>

          <div className="rounded-xl bg-[var(--surface-2)] px-6 py-3 text-center text-sm">
            <div className="font-bold">Solving sin(x) = sin({deg}°) on [0°, 360°):</div>
      ...
      <p>
        Because sine repeats, sin(x) = sin({deg}°) has infinitely many solutions. The
