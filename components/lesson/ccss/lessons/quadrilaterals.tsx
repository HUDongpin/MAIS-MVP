...
const SHAPES: Shape[] = [
  ...
  { key: "trap", name: "Trapezoid", points: "55,30 135,30 165,120 25,120", attrs: "this example has exactly one pair of parallel sides" },
];
...
export default function Lesson() {
  ...
  return (
    <div className="prose-lesson max-w-none">
      <p>
        A <strong>quadrilateral</strong>{" "}is a closed, flat shape made from
        <strong> exactly 4 straight sides</strong>{" "}joined end to end, with no gaps
        or crossings. Those shared attributes make a whole category — and inside
        it are special kinds like rectangles, rhombuses, and squares.
      </p>

      <Figure caption="Every shape here is closed and has 4 straight sides joined without crossings, so all are quadrilaterals. Each also has its own special traits.">
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-wrap justify-center gap-2">
            {SHAPES.map((s, i) => (
              <button key={s.key} type="button" onClick={() => setIdx(i)} aria-pressed={idx === i} className="rounded-lg border px-3 py-1.5 text-sm font-bold" style={idx === i ? { background: FILL, color: "white", borderColor: FILL } : { borderColor: "var(--line)", color: "var(--ink-soft)" }}>{s.name}</button>
            ))}
          </div>
      ...
      <MathCheck>
        <p>
          ...
          (3.G.A.1). Being closed with exactly 4 straight, noncrossing sides
          defines the quadrilaterals; adding
