...
export default function Lesson() {
  ...
  const mean = samples.length ? Math.round(samples.reduce((a, b) => a + b, 0) / samples.length) : 0;

  return (
    <div className="prose-lesson max-w-none">
      <p>
        Statistics reasons from a <strong>sample</strong>{" "}to the whole{" "}
        <strong>population</strong>. A single sample won&apos;t exactly match the truth
      ...
      <Figure caption="Each draw of 20 gives a sample proportion. They scatter around the true 60% — that's sampling variability.">
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="grid grid-cols-2 gap-4 text-center font-mono text-sm">
            ...
            <div className="rounded-lg bg-[var(--surface-2)] px-4 py-2">average of samples<br /><strong style={{ color: ACCENT }}>{samples.length ? mean + "%" : "—"}</strong></div>
          </div>
        </div>
      </Figure>
      ...
      <p>
        No single sample equals the population, but their average homes in on the true
        60% as you collect more — and bigger samples vary less. This is the basis of{" "}
        ...
        claimed fair produced results far from what random chance ever gives, the
        model is suspect.
      </p>
