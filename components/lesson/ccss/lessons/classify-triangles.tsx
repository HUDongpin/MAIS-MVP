      ...
      <Figure caption="Tap a type to see it. The little square marks a right angle.">
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-wrap justify-center gap-2">
            {TRIS.map((t, i) => (
              <button key={t.key} type="button" onClick={() => setIdx(i)} className="rounded-lg border px-3 py-1.5 text-sm font-bold" style={idx === i ? { background: FILL, color: "white", borderColor: FILL } : { borderColor: "var(--line)", color: "var(--ink-soft)" }}>{t.name}</button>
            ))}
          </div>
