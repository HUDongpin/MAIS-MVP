...
export default function Lesson() {
  ...
  // both steppers reach −4, which rendered "x² − -3x − -4 = 0" — the exact
  // double-sign form students are being taught to simplify.
  const term = (coef: number, suffix: string) => `${coef < 0 ? "+" : "−"} ${Math.abs(coef)}${suffix}`;
  ...
  // Affine, not clamped. Clamping inside the mapper rotated the line instead of
  // clipping it — at m = 4, k = 8 the legend read "y = 4x + 8" while the drawn
  // line passed through (0, 1) — and pinned solution dots to the top edge.
  ...
  return (
    <div className="prose-lesson max-w-none">
      ...
      <p>
        Since both equal y, set x² = {m}x {addend(k)}, i.e. x² {term(m, "x")} {term(k, "")} = 0. Its
