...
export default function Lesson() {
  ...
  const mag = r2(Math.sqrt(cx * cx + cy * cy));
  const dir = r2((Math.atan2(cy, cx) * 180) / Math.PI);
  ...
  return (
    <div className="prose-lesson max-w-none">
      ...
      <Figure caption="An arrow from the tail to the tip. Components are Δx and Δy; magnitude is the length.">
        <div className="flex flex-col items-center gap-6">
          <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} className="max-w-full" style={{ maxHeight: 340 }} role="img" aria-label="vector on a grid">
          ...
          <div className="grid w-full max-w-md grid-cols-3 gap-2 font-mono text-center text-sm">
            ...
            <Info label="magnitude" value={`${mag}`} />
            <Info label="direction" value={`${dir}°`} />
          </div>
      ...
      <p>
        ...
        = ⟨{cx}, {cy}⟩. The magnitude √(Δx² + Δy²) = {mag} is the Pythagorean
        length, and the direction {dir}° is the angle it points. Same arrow, same
        vector, no matter where you place its tail.
      </p>
