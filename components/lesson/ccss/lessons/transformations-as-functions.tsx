      ...
      <Figure caption="Each transformation is a rule on coordinates. Apply it to every vertex to get the image.">
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-wrap justify-center gap-2">
            {TRANSFORMS.map((tr, i) => (
              <button key={tr.name} type="button" onClick={() => setIdx(i)} className="rounded-lg border px-3 py-1.5 text-sm font-bold" style={idx === i ? { background: IMG, color: "white", borderColor: IMG } : { borderColor: "var(--line)", color: "var(--ink-soft)" }}>{tr.name}</button>
            ))}
          </div>

          <svg width={SIZE} height={SIZE} viewBox={`0 0 ${SIZE} ${SIZE}`} className="max-w-full" style={{ maxHeight: 330 }} role="img" aria-label="transformation of a shape">
