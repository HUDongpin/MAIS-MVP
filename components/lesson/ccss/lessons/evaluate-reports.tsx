      ...
      <Figure caption="For each claim, ask the critical question before believing the number.">
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-wrap justify-center gap-2">
            {CLAIMS.map((cl, i) => (
              <button key={i} type="button" onClick={() => setIdx(i)} aria-pressed={idx === i} className="rounded-lg border px-3 py-1.5 text-sm font-bold" style={idx === i ? { background: ACCENT, color: "white", borderColor: ACCENT } : { borderColor: "var(--line)", color: "var(--ink-soft)" }}>Claim {i + 1}</button>
            ))}
          </div>
