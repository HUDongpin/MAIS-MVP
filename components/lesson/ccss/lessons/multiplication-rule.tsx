  ...
  return (
    <div className="prose-lesson max-w-none">
      ...
      <Figure caption="Draw two aces without replacement: the second probability shrinks because a card is gone.">
        <div className="flex flex-col items-center gap-6">
          <div className="flex flex-col items-center gap-1 font-mono text-lg">
            <span>P(1st ace) = {aces}/{deck} = {r2(p1)}</span>
            <span>P(2nd ace | 1st ace) = {aces - 1}/{deck - 1} = {r2(p2)}</span>
            <span className="mt-1 text-xl font-black" style={{ color: ACCENT }}>P(both) = {r2(p1)} × {r2(p2)} = {both}</span>
          </div>

          <div className="rounded-xl bg-[var(--surface-2)] px-6 py-2 text-center text-sm">
            With replacement (independent), it would be {r2(aces / deck)} × {r2(aces / deck)} = {withRepl} — slightly higher.
          </div>
      ...
      <p>
        ...
        so the second probability {r2(p2)} is smaller than the first. Multiplying gives
        P(both) = {both}. This <strong>general multiplication rule</strong>{" "}works for
