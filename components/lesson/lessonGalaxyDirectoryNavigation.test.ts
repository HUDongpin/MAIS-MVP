...
test("lesson content omits the oversized next-item action and its CTA-only visualization API", () => {
  ...
  // ConfiguredVisualizationLab keeps its optional controlFooterAction slot: the
  // CA lab presentation slice on main builds on that API. Removing the lesson
  // CTA only requires that LessonView no longer feeds the slot (asserted above).
