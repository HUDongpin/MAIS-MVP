...
export type LearningAnalyticsRecordOptions = {
  ...
  preservePhysicalCommit?: boolean;
};
...
export function AppProviders({ children }: { children: ReactNode }) {
  ...
  const analyticsPhysicalCommitSequenceRef = useRef(0);
  ...
  const recordLearningEvent = useCallback((
    ...
    if (
      options.preservePhysicalCommit === true &&
    ...
    const publicCreatedEvent: LearningAnalyticsEvent = {
      ...generatedEvent,
      ...(options.eventId ? { id: options.eventId } : {}),
      ...(options.eventTimestamp ? { timestamp: options.eventTimestamp } : {})
    };
    const createdEvent = options.preservePhysicalCommit === true
      ? createPhysicalLearningAnalyticsCommit(
          publicCreatedEvent,
          analyticsPhysicalCommitSequenceRef.current++,
    ...
    if (isHighFrequencyLearningAnalyticsEvent(analyticsEvent)) {
      if (isPreservedPhysicalLearningAnalyticsCommit(analyticsEvent)) {
        const persistedAs = appendLearningEventsToQueues(
          [analyticsEvent],
          recordingIdentity
        );
        if (persistedAs === "confirmed") {
          const currentImmediateDrain =
            analyticsImmediateDeliveryRequestedRef.current;
          analyticsImmediateDeliveryRequestedRef.current =
            currentImmediateDrain &&
            isCurrentLearningAnalyticsIdentity(
              ...
              ? {
                  ...currentImmediateDrain,
                  eventIds: new Set([
                    ...currentImmediateDrain.eventIds,
                    analyticsEvent.id
                  ])
                }
              : {
                  ...recordingIdentity,
                  eventIds: new Set([analyticsEvent.id])
                };
    ...
    return appendLearningEventsToQueues([analyticsEvent], recordingIdentity);
  }, [analyticsOwnerIdentity.clientEpoch, analyticsOwnerIdentity.userId, analyticsOwnerServerGeneration, appendLearningEventsToQueues, clearAnalyticsDeliveryHandle, currentUser?.role, scheduleHighFrequencyLearningEventFlush, selectedGrade, settingsReady]);
  ...
  const freezeLearningAnalyticsIdentity = useCallback((options: {
    persistQueuedEvents?: boolean;
  } = {}) => {
    ...
    const bufferedEvents = takeBufferedHighFrequencyLearningEvents();
    if (frozenIdentity.userId && options.persistQueuedEvents !== false) {
  ...
  const adoptLearningAnalyticsForwardGeneration = useCallback((
    ...
    try {
      ...
      // Every queue entry was already written to its physical protocol
      // revision before it entered React state. Re-persisting the old in-memory
      // queue after installing the forward marker would falsely stamp
      // pre-clear rows with the transition token and revive them in the new
      // generation. Newly-created terminal rows are written directly through
      // the active transition gate by finalizeVisibleAnalytics above.
  ...
  useEffect(() => {
    ...
    const handleGenerationChange = (event: StorageEvent) => {
      ...
      if (
        ...
        // The peer marker is already the physical writer boundary. Existing
        // React queue rows were persisted before that boundary; writing them
        // again here would stamp them with the peer token and could revive
        // pre-clear data in the next generation. Terminal rows created by
        // finalization still persist directly through the active marker gate.
        freezeLearningAnalyticsIdentity({ persistQueuedEvents: false });
      } else {
        invalidateLearningAnalyticsGenerationReadiness();
        analyticsDeliveryPausedRef.current = true;
          ...
          if (clearFence) {
            if (
              ...
              adoptLearningAnalyticsForwardGeneration(
                ...
                mismatch.currentGeneration,
                mismatch.clearedAt,
                `clear-${clearFence.requestId}-${mismatch.currentGeneration}`
              )
            ) return;
  ...
  useEffect(() => {
    ...
    analyticsDeliveryHandleRef.current = window.setTimeout(() => {
      ...
      const events = selectLearningAnalyticsDeliveryBatch(
        pendingLearningEvents.filter(
          (event) =>
        ...
        .then(async (response) => {
          ...
          if (mismatch) {
            if (mismatch.direction === "forward") {
              ...
              if (!adopted) {
                // Forward adoption may already have installed its transition
                // marker before a later durable step failed. Do not restamp
                // the pre-marker React queue during the handshake fallback.
                freezeLearningAnalyticsIdentity({ persistQueuedEvents: false });
                requestLearningAnalyticsGenerationHandshake();
              }
            } else {
  ...
  useEffect(() => {
    ...
    const flushUserId = currentUser.id;
    if (analyticsIdentityRef.current.userId !== flushUserId) return;
    let cancelled = false;
    let inFlight = false;
    let retryAttempt = 0;
    let retryHandle: number | null = null;
    ...
    const flushOutbox = async () => {
      ...
      const flushIdentity = analyticsIdentityRef.current;
      if (!effectUserIsCurrent() || flushIdentity.userId !== flushUserId) return;
  ...
  useEffect(() => {
    ...
    const outcome = recordLearningEvent({ type: "page-view", source, topicId });
  ...
  const publicLearningAnalyticsEvents = useMemo(
    () => learningAnalyticsEventsForPublicProjection(learningAnalyticsEvents),
  ...
  const value = useMemo<SettingsContextValue>(
    () => ({
      ...
      revalidateSession,
      mistakeRecords,
      learningAnalyticsEvents: publicLearningAnalyticsEvents,
