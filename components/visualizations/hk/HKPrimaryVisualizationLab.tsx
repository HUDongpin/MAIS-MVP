...
export const HK_PRIMARY_VISUALIZATION_CONTRACTS = {
  ...
  "p6-pre-secondary-problem-solving": {
    ...
    invariants: ["Each workflow step uses the same quantities.", "The check uses positive remaining or positive overspend and balances both sides."],
...
const HK_PRIMARY_CANONICAL_STATES = {
  ...
  "p5-charts-averages": { selectedCategory: "B", firstSeriesValue: 8, secondSeriesValue: 7 },
    ...
    <div className={theme.paddedSurfaceClassName}>
      ...
      >
        <svg
          ...
          className="block h-auto min-h-[360px] w-full min-w-[640px]"
        >
    ...
    <ModelShell
      ...
      surface={
        <SvgFrame label={t(localized("Number bond counters and part-whole diagram", "數的組合計數物和部分—整體圖", "数的组合计数物和部分—整体图"))}>
          ...
          <path data-viz-mark data-viz-name="known-connector" d="M260 238 L190 282" fill="none" stroke={semantic.main} strokeWidth="7" strokeLinecap="round" />
          <path data-viz-mark data-viz-name="missing-connector" d="M380 238 L450 282" fill="none" stroke={semantic.change} strokeWidth="7" strokeLinecap="round" />
          <g data-viz-mark data-viz-name="whole-node">
            <circle cx="320" cy="220" r="42" fill={semantic.result} stroke={theme.pointStroke} strokeWidth="3" />
            <text x="320" y="232" textAnchor="middle" fill="#052e16" fontSize="32" fontWeight="900">{total}</text>
          </g>
          <g data-viz-mark data-viz-name="known-part-node">
            <circle cx="170" cy="300" r="38" fill={semantic.main} stroke={theme.pointStroke} strokeWidth="3" />
            <text x="170" y="311" textAnchor="middle" fill="#082f49" fontSize="28" fontWeight="900">{known}</text>
          </g>
          <g data-viz-mark data-viz-name="missing-part-node">
            <circle cx="470" cy="300" r="38" fill={semantic.change} stroke={theme.pointStroke} strokeWidth="3" />
            <text x="470" y="311" textAnchor="middle" fill="#451a03" fontSize="28" fontWeight="900">{missing}</text>
          </g>
        </SvgFrame>
      }
      controls={
      ...
      surface={mode === "measure" ? (
        <SvgFrame label={t(localized("Equal-unit ruler and two compared lengths", "相等單位直尺和兩個比較長度", "相等单位直尺和两个比较长度"))}>
          ...
          <g data-viz-mark data-viz-name="length-bar" data-viz-units={length}>
            <rect x="74" y="96" width={length * 41} height="54" rx="14" fill={semantic.main} stroke={theme.pointStroke} strokeWidth="3" />
            <text x={74 + (length * 41) / 2} y="130" textAnchor="middle" fill="#082f49" fontSize="20" fontWeight="900">{length}</text>
          </g>
          <g data-viz-mark data-viz-name="reference-bar" data-viz-units="5">
            <rect x="74" y="178" width={5 * 41} height="54" rx="14" fill={semantic.change} stroke={theme.pointStroke} strokeWidth="3" />
            <text x={74 + (5 * 41) / 2} y="212" textAnchor="middle" fill="#451a03" fontSize="20" fontWeight="900">5</text>
          </g>
        </SvgFrame>
      ) : (
    ...
    <ModelShell
      ...
      surface={
        <SvgFrame label={t(localized("Base-ten blocks and hundreds-tens-ones table", "十進制積木和百、十、個數位表", "十进制积木和百、十、个数位表"))}>
          {isThousand ? (
            <g data-viz-mark data-viz-name="thousand-regrouping" data-viz-value="1000">
              ...
              <g data-viz-mark data-viz-name="thousand-cube" data-viz-value="1000">
                <rect x="350" y="68" width="190" height="220" rx="20" fill={semantic.result} stroke={theme.pointStroke} strokeWidth="5" />
                ...
                <text x="445" y="194" textAnchor="middle" fill="#052e16" fontSize="36" fontWeight="900">1000</text>
              </g>
            </g>
          ) : (
            <g data-viz-mark data-viz-name="place-value-table" data-viz-number={number}>
              {columns.map((column) => (
                <g key={column.key} data-viz-mark data-viz-name={`${column.key}-column`} data-viz-value={column.value}>
                  <rect x={column.x} y="70" width="140" height="228" rx="20" fill={theme.softFill} stroke={column.color} strokeWidth="3" />
                  <text x={column.x + 70} y="102" textAnchor="middle" fill={column.color} fontSize="14" fontWeight="900">{column.key === "hundreds" ? "100" : column.key === "tens" ? "10" : "1"}</text>
                  ...
                  <g data-viz-mark data-viz-name="place-digit" data-viz-digit={column.value}>
                    <circle cx={column.x + 70} cy="318" r="20" fill={column.color} stroke={theme.pointStroke} strokeWidth="3" />
                    <text x={column.x + 70} y="326" textAnchor="middle" fill="#0f172a" fontSize="22" fontWeight="900">{column.value}</text>
                  </g>
                </g>
              ))}
            </g>
      ...
      surface={mode === "money" ? (
        <SvgFrame label={t(localized("Hong Kong dollar payment and change bar", "港幣付款和找續長條", "港币付款和找零长条"))}>
          <g data-viz-mark data-viz-name="hk-dollar-notes">
            {[0, 1, 2].map((index) => (
              <rect key={index} x={86 + index * 150} y={72 + index * 12} width="176" height="84" rx="12" fill={index === 0 ? semantic.main : index === 1 ? semantic.parameter : semantic.result} stroke={theme.pointStroke} strokeWidth="3" opacity="0.92" />
            ))}
            <text x="174" y="124" textAnchor="middle" fill="#0f172a" fontSize="22" fontWeight="900">HK$</text>
          </g>
          <rect data-viz-mark data-viz-name="payment-bar" data-viz-payment={safePayment} x="76" y="228" width={paymentBarWidth} height="70" rx="16" fill={theme.emptyFill} stroke={theme.axisStrong} strokeWidth="3" />
          <g
        ...
        <SvgFrame label={t(localized("Metre estimation and strict one-to-one pictogram", "米的估測和嚴格一對一象形圖", "米的估测和严格一对一象形图"))}>
          {mode === "metres" ? (
            <g data-viz-mark data-viz-name="metre-measurement" data-viz-target={target} data-viz-tool={tool} data-viz-tool-suitable={String(toolIsSuitable)}>
              ...
              <g data-viz-mark data-viz-name="measuring-tool-choice" data-viz-tool={tool} data-viz-expected-tool={definition.expectedTool} data-viz-suitable={String(toolIsSuitable)}>
                <rect x="148" y="266" width="344" height="58" rx="18" fill={toolIsSuitable ? "rgba(52,211,153,.26)" : "rgba(251,113,133,.22)"} stroke={toolIsSuitable ? semantic.result : semantic.attention} strokeWidth="3" />
                <text x="320" y="302" textAnchor="middle" fill={svgTextColor(theme.mode)} fontSize="16" fontWeight="900">{t(metreTools[tool])} · {toolIsSuitable ? "✓" : "↺"}</text>
              </g>
            </g>
          ) : (
    ...
    <ModelShell
      ...
      surface={
        <SvgFrame label={t(localized("Multiplication array and equal sharing", "乘法陣列與平均分", "乘法阵列与平均分"))}>
          ...
          <g data-viz-mark data-viz-name="total-tray" data-viz-total={total}>
            <rect x="218" y="294" width="204" height="42" rx="16" fill={semantic.result} stroke={theme.pointStroke} strokeWidth="3" />
            <text x="320" y="322" textAnchor="middle" fill="#052e16" fontSize="19" fontWeight="900">{total}</text>
          </g>
        </SvgFrame>
      }
      controls={
          ...
          {model === "measurement" ? (
            <>
              ...
              <g data-viz-mark data-viz-name="conversion-marker" data-viz-quantity={quantityType} data-viz-unit={unit} data-viz-displayed-unit={displayedUnit} data-viz-displayed-value={formatDecimal(displayedValue, 3)}>
                <circle cx="220" cy="294" r="40" fill={unit === "small" ? semantic.main : theme.emptyFill} stroke={semantic.main} strokeWidth="4" />
                <circle cx="420" cy="294" r="40" fill={unit === "large" ? semantic.result : theme.emptyFill} stroke={semantic.result} strokeWidth="4" />
                ...
                <text x="220" y="301" textAnchor="middle" fill={unit === "small" ? "#082f49" : svgTextColor(theme.mode)} fontSize="17" fontWeight="900">{definition.small}</text>
                <text x="420" y="301" textAnchor="middle" fill={unit === "large" ? "#052e16" : svgTextColor(theme.mode)} fontSize="17" fontWeight="900">{definition.large}</text>
              </g>
            </>
          ) : (
          ...
          >
            ...
            {definition.points.map((point, index) => (
              <g key={`vertex-${index}`} data-viz-mark data-viz-name="shape-vertex" data-viz-vertex-index={index + 1}>
                ...
                <text x={point.x} y={point.y < 60 ? point.y + 30 : point.y - 18} textAnchor="middle" fill={theme.text} fontSize="15" fontWeight="900">{String.fromCharCode(65 + index)}</text>
              </g>
            ))}
          </g>
          <g data-viz-mark data-viz-name="shape-property-counts" data-viz-sides={sideCount} data-viz-vertices={vertexCount} data-viz-parallel-pairs={parallelPairs}>
            <rect x="82" y="306" width="476" height="38" rx="14" fill={theme.labelFill} stroke={theme.panelStroke} strokeWidth="2" />
            <text x="320" y="331" textAnchor="middle" fill={theme.text} fontSize="16" fontWeight="900">
...
function FactorsMultiplesModel({ controlFooterAction }: { controlFooterAction?: ReactNode }) {
  const { t } = useSettings();
  const theme = useVisualizationTheme();
  const [mode, setMode] = useState<FactorsMultiplesMode>("factor-pairs");
  const [firstNumber, setFirstNumber] = useState(24);
  const [secondNumber, setSecondNumber] = useState(18);
  const [candidateDivisor, setCandidateDivisor] = useState(6);
  ...
  function chooseMode(next: FactorsMultiplesMode) {
    setMode(next);
    if (next === "common-hcf-lcm") {
      setFirstNumber((current) => Math.min(current, 20));
      setSecondNumber((current) => Math.min(current, 20));
    }
  }

  function chooseFirstNumber(value: number) {
    setFirstNumber(value);
    setCandidateDivisor((current) => Math.min(current, value));
  }

  function reset() {
    setMode("factor-pairs");
    setFirstNumber(24);
    setSecondNumber(18);
    setCandidateDivisor(6);
  }
    ...
    <ModelShell
          ...
          {mode === "factor-pairs" ? (
            <g data-viz-mark data-viz-name="factor-pair-array" data-viz-number={firstNumber} data-viz-complete="true" data-viz-pair-count={factorPairs.length}>
              {factorPairs.map(([left, right], index) => {
                ...
                return (
                  <g key={left} data-viz-mark data-viz-name="factor-pair" data-viz-left={left} data-viz-right={right}>
                    <rect x={x} y={y} width="150" height="48" rx="14" fill={theme.softFill} stroke={semantic.main} strokeWidth="3" />
                    <text x={x + 75} y={y + 31} textAnchor="middle" fill={svgTextColor(theme.mode)} fontSize="18" fontWeight="900">{left} × {right}</text>
                  </g>
                );
              })}
              <g data-viz-mark data-viz-name="number-classification" data-viz-classification={classification}>
                <rect x="172" y="207" width="296" height="44" rx="14" fill={classification === "prime" ? "rgba(52,211,153,.24)" : classification === "composite" ? "rgba(167,139,250,.24)" : theme.softFill} stroke={classification === "prime" ? semantic.result : classification === "composite" ? semantic.parameter : semantic.reference} strokeWidth="3" />
                <text x="320" y="235" textAnchor="middle" fill={svgTextColor(theme.mode)} fontSize="16" fontWeight="900">{firstNumber}: {t(classificationLabel)}</text>
              </g>
              <g data-viz-mark data-viz-name="remainder-test" data-viz-dividend={firstNumber} data-viz-divisor={safeDivisor} data-viz-quotient={quotient} data-viz-remainder={remainder} data-viz-is-factor={String(remainder === 0)}>
                <rect x="116" y="273" width="408" height="58" rx="18" fill={remainder === 0 ? "rgba(52,211,153,.25)" : "rgba(251,113,133,.2)"} stroke={remainder === 0 ? semantic.result : semantic.attention} strokeWidth="3" />
                <text x="320" y="308" textAnchor="middle" fill={svgTextColor(theme.mode)} fontSize="18" fontWeight="900">{firstNumber} ÷ {safeDivisor} = {quotient} r {remainder} · {remainder === 0 ? `✓ ${t(localized("factor", "因數", "因数"))}` : `✕ ${t(localized("not a factor", "不是因數", "不是因数"))}`}</text>
              </g>
            </g>
          ) : (
                ...
                {[
                  ...
                  <g key={row.id} data-viz-mark data-viz-name="factor-list" data-viz-number={row.value} data-viz-factors={row.factors.join(",")}>
                    ...
                    {row.factors.map((factor, index) => {
                      ...
                      return (
                        <g key={factor} data-viz-mark data-viz-name={common ? "common-factor" : "factor-list-item"} data-viz-value={factor}>
                          <circle cx={108 + index * 54} cy={row.y} r={factor === hcf ? 20 : 16} fill={common ? semantic.result : theme.emptyFill} stroke={factor === hcf ? semantic.attention : row.color} strokeWidth={factor === hcf ? 5 : 3} />
                          <text x={108 + index * 54} y={row.y + 5} textAnchor="middle" fill={common ? "#052e16" : svgTextColor(theme.mode)} fontSize="13" fontWeight="900">{factor}</text>
                        </g>
                      );
                    })}
                  </g>
      ...
      controls={
        <>
          ...
          {mode === "factor-pairs" ? (
            <RangeControl controlId="candidateDivisor" label={t(localized("Test divisor", "檢驗除數", "检验除数"))} value={safeDivisor} min={1} max={firstNumber} onChange={setCandidateDivisor} />
          ) : (
            <RangeControl controlId="secondNumber" label={t(localized("Second positive integer", "第二個正整數", "第二个正整数"))} value={secondNumber} min={1} max={20} onChange={setSecondNumber} />
          )}
    ...
    <ModelShell
      ...
      surface={
        <SvgFrame label={t(localized("Decimal number line with a magnified hundredths interval", "帶百分位放大區間的小數數線", "带百分位放大区间的小数数线"))}>
          ...
          <g data-viz-mark data-viz-name="hundredths-interval" data-viz-lower={lowerTenth.toFixed(1)} data-viz-hundredth={hundredthIndex}>
          ...
          {mode === "families" ? (
            <g data-viz-mark data-viz-name="quadrilateral-families" data-viz-selected-shape={familyShape}>
              ...
              <g data-viz-mark data-viz-name="family-inclusion-map" data-viz-direction="subset-to-superset">
                <g data-viz-mark data-viz-name="family-node" data-viz-family="parallelogram"><rect x="420" y="48" width="168" height="42" rx="13" fill={semantic.parameter} /><text x="504" y="75" textAnchor="middle" fill="#2e1065" fontSize="14" fontWeight="900">{t(quadrilateralDefinitions.parallelogram.label)}</text></g>
                <g data-viz-mark data-viz-name="family-node" data-viz-family="rectangle"><rect x="352" y="150" width="126" height="42" rx="13" fill={semantic.main} /><text x="415" y="177" textAnchor="middle" fill="#082f49" fontSize="14" fontWeight="900">{t(quadrilateralDefinitions.rectangle.label)}</text></g>
                <g data-viz-mark data-viz-name="family-node" data-viz-family="rhombus"><rect x="510" y="150" width="112" height="42" rx="13" fill={semantic.change} /><text x="566" y="177" textAnchor="middle" fill="#451a03" fontSize="14" fontWeight="900">{t(quadrilateralDefinitions.rhombus.label)}</text></g>
                <g data-viz-mark data-viz-name="family-node" data-viz-family="square"><rect x="428" y="258" width="132" height="42" rx="13" fill={semantic.result} /><text x="494" y="285" textAnchor="middle" fill="#052e16" fontSize="14" fontWeight="900">{t(quadrilateralDefinitions.square.label)}</text></g>
    ...
    <ModelShell
      ...
      surface={
        <SvgFrame label={t(localized("Equal price groups, one-item price, and scaled target group", "等價價格組、一件價格和倍增目標組", "等价价格组、一件价格和倍增目标组"))}>
          ...
          <g data-viz-mark data-viz-name="one-item-price" data-viz-unit-price={formatDecimal(unitPrice)}>
            <rect x="140" y="210" width="360" height="64" rx="18" fill={semantic.result} stroke={theme.pointStroke} strokeWidth="3" />
            <text x="320" y="250" textAnchor="middle" fill="#052e16" fontSize="16" fontWeight="900">1 = {quotientDisplay(totalPrice, itemCount, "HK$")}</text>
          </g>
          ...
          {mode === "equivalence" ? (
            <>
              ...
              <g data-viz-mark data-viz-name="equivalent-representations">
                ...
                <g data-viz-mark data-viz-name="fraction-value" data-viz-value={`${fractionNumerator}/${fractionDenominator}`}>
                  <circle cx="414" cy="190" r="44" fill={semantic.result} stroke={theme.pointStroke} strokeWidth="3" />
                  <text x="414" y="197" textAnchor="middle" fill="#052e16" fontSize="17" fontWeight="900">{fractionNumerator}/{fractionDenominator}</text>
                </g>
                <g data-viz-mark data-viz-name="decimal-marker" data-viz-value={decimal.toFixed(2)}>
                  <circle cx="520" cy="190" r="44" fill={semantic.change} stroke={theme.pointStroke} strokeWidth="3" />
                  <text x="520" y="197" textAnchor="middle" fill="#451a03" fontSize="18" fontWeight="900">{decimal.toFixed(2)}</text>
                </g>
                <g data-viz-mark data-viz-name="percent-value" data-viz-value={percent}>
                  <rect x="380" y="264" width="174" height="62" rx="18" fill={semantic.parameter} stroke={theme.pointStroke} strokeWidth="3" />
                  <text x="467" y="304" textAnchor="middle" fill="#2e1065" fontSize="28" fontWeight="900">{percent}%</text>
                </g>
              </g>
            </>
          ) : (
            <g data-viz-mark data-viz-name="given-percent-change" data-viz-direction={changeDirection} data-viz-base={baseAmount} data-viz-percent={percent} data-viz-change={formatDecimal(changeAmount, 2)} data-viz-final={formatDecimal(finalAmount, 2)}>
              ...
              <g data-viz-mark data-viz-name="given-change-values">
                <rect x="118" y="177" width="174" height="40" rx="12" fill={theme.labelFill} stroke={theme.axisStrong} strokeWidth="2" />
                <text data-viz-mark data-viz-name="base-value-readout" data-viz-value={baseAmount} x="205" y="205" textAnchor="middle" fill={theme.text} fontSize="14" fontWeight="900">{t(localized("Base", "基數", "基数"))} = {baseAmount}</text>
                <rect x="348" y="177" width="174" height="40" rx="12" fill={theme.labelFill} stroke={theme.axisStrong} strokeWidth="2" strokeDasharray={changeDirection === "increase" ? undefined : "7 5"} />
                <text data-viz-mark data-viz-name="change-value-readout" data-viz-value={formatDecimal(changeAmount, 2)} x="455" y="205" textAnchor="middle" fill={theme.text} fontSize="14" fontWeight="900">{t(localized("Change", "改變量", "改变量"))} = {changeDirection === "increase" ? "+" : "−"}{formatDecimal(changeAmount, 2)}</text>
              </g>
              <g data-viz-mark data-viz-name="final-amount" data-viz-value={formatDecimal(finalAmount, 2)} data-viz-direction={changeDirection}>
                ...
                <text data-viz-mark data-viz-name="final-value-readout" data-viz-value={formatDecimal(finalAmount, 2)} x="320" y="278" textAnchor="middle" fill={theme.text} fontSize="18" fontWeight="900">{t(localized("Final", "最後數量", "最后数量"))} = {formatDecimal(finalAmount, 2)}</text>
              </g>
              <g data-viz-mark data-viz-name="given-change-formula" data-viz-base={baseAmount} data-viz-percent={percent} data-viz-change={formatDecimal(changeAmount, 2)}>
                <rect x="122" y="300" width="396" height="42" rx="14" fill={theme.softFill} stroke={theme.panelStroke} strokeWidth="2" />
                <text x="320" y="327" textAnchor="middle" fill={theme.text} fontSize="15" fontWeight="900">{baseAmount} × {percent}/100 = {formatDecimal(changeAmount, 2)}</text>
              </g>
            </g>
          )}
        </SvgFrame>
...
function AveragesLineGraphsModel({ controlFooterAction }: { controlFooterAction?: ReactNode }) {
  ...
  const seriesA = useMemo(() => [value1, value2, value3, value4], [value1, value2, value3, value4]);
  const seriesB = useMemo(() => seriesA.map((value) => value + seriesBShift), [seriesA, seriesBShift]);
  ...
  const times = [9, 10, 11, 12] as const;
  const graphTopY = 34;
  const graphBottomY = 276;
  const graphX = (index: number) => 94 + index * 92;
        ...
        <SvgFrame label={t(localized("Fair-share mean and ordered broken-line graph", "公平分配平均數與按序折線圖", "公平分配平均数与有序折线图"))}>
          {mode === "mean-fair-share" ? (
            <g data-viz-mark data-viz-name="mean-fair-share" data-viz-total={totalA} data-viz-count="4" data-viz-mean={formatDecimal(meanA, 2)}>
              ...
              <rect data-viz-mark data-viz-name="series-mean" data-viz-series="A" data-viz-total={totalA} data-viz-count="4" data-viz-value={formatDecimal(meanA, 2)} x="224" y="24" width="192" height="42" rx="14" fill="rgba(52,211,153,.25)" stroke={semantic.result} strokeWidth="3" />
              <text x="320" y="51" textAnchor="middle" fill={svgTextColor(theme.mode)} fontSize="16" fontWeight="900">{t(localized("mean", "平均數", "平均数"))} = {formatDecimal(meanA, 2)}</text>
            </g>
          ) : (
            <g data-viz-mark data-viz-name="broken-line-graph" data-viz-series-count={seriesCount} data-viz-order="continuous-time" data-viz-extrapolation="none" data-viz-unit="degrees-celsius">
              ...
              <g data-viz-mark data-viz-name="broken-line-series" data-viz-series="A" data-viz-values={seriesA.join(",")} data-viz-mean={formatDecimal(meanA, 2)}>
                {seriesA.slice(0, -1).map((value, index) => <line key={index} data-viz-mark data-viz-name="adjacent-line-segment" data-viz-from-hour={times[index]} data-viz-to-hour={times[index + 1]} x1={graphX(index)} y1={graphY(value)} x2={graphX(index + 1)} y2={graphY(seriesA[index + 1])} stroke={semantic.main} strokeWidth="5" />)}
                {seriesA.map((value, index) => <circle key={index} data-viz-mark data-viz-name="series-point" data-viz-series="A" data-viz-hour={times[index]} data-viz-value={value} cx={graphX(index)} cy={graphY(value)} r="8" fill={semantic.main} stroke={theme.pointStroke} strokeWidth="3" />)}
                <line data-viz-mark data-viz-name="series-mean" data-viz-series="A" data-viz-value={formatDecimal(meanA, 2)} x1="94" x2="370" y1={graphY(meanA)} y2={graphY(meanA)} stroke={semantic.main} strokeWidth="2" strokeDasharray="6 5" />
              </g>
              {seriesCount === 2 ? (
                <g data-viz-mark data-viz-name="broken-line-series" data-viz-series="B" data-viz-values={seriesB.join(",")} data-viz-mean={formatDecimal(meanB, 2)}>
                  {seriesB.slice(0, -1).map((value, index) => <line key={index} data-viz-mark data-viz-name="adjacent-line-segment" data-viz-from-hour={times[index]} data-viz-to-hour={times[index + 1]} x1={graphX(index)} y1={graphY(value)} x2={graphX(index + 1)} y2={graphY(seriesB[index + 1])} stroke={semantic.change} strokeWidth="5" />)}
                  {seriesB.map((value, index) => <circle key={index} data-viz-mark data-viz-name="series-point" data-viz-series="B" data-viz-hour={times[index]} data-viz-value={value} cx={graphX(index)} cy={graphY(value)} r="8" fill={semantic.change} stroke={theme.pointStroke} strokeWidth="3" />)}
                  <line data-viz-mark data-viz-name="series-mean" data-viz-series="B" data-viz-value={formatDecimal(meanB, 2)} x1="94" x2="370" y1={graphY(meanB)} y2={graphY(meanB)} stroke={semantic.change} strokeWidth="2" strokeDasharray="6 5" />
                </g>
              ) : null}
              <g data-viz-mark data-viz-name="data-table" data-viz-series-count={seriesCount}>
                ...
                {times.map((time, index) => (
                  <g key={time} data-viz-mark data-viz-name="data-table-row" data-viz-hour={time} data-viz-series-a={seriesA[index]} data-viz-series-b={seriesCount === 2 ? seriesB[index] : undefined}>
                    <line x1="416" x2="600" y1={91 + index * 38} y2={91 + index * 38} stroke={theme.grid} strokeWidth="1.5" />
                    <text x="442" y={116 + index * 38} textAnchor="middle" fill={theme.tickText} fontSize="12">{time}:00</text>
                    <text x="506" y={116 + index * 38} textAnchor="middle" fill={theme.text} fontSize="13" fontWeight="900">{seriesA[index]}</text>
                    {seriesCount === 2 ? <text x="572" y={116 + index * 38} textAnchor="middle" fill={theme.text} fontSize="13" fontWeight="900">{seriesB[index]}</text> : null}
                  </g>
                ))}
              </g>
              <g data-viz-mark data-viz-name="series-mean-readout" data-viz-series-a-mean={formatDecimal(meanA, 2)} data-viz-series-b-mean={seriesCount === 2 ? formatDecimal(meanB, 2) : undefined}>
        ...
        <>
          ...
          {mode === "broken-line" ? (
            <>
              ...
              {seriesCount === 2 ? <RangeControl controlId="seriesBShift" label={t(localized("Series B positive offset", "數列 B 正偏移量", "数列 B 正偏移量"))} value={seriesBShift} min={0} max={2} onChange={setSeriesBShift} /> : null}
            </>
          ) : null}
...
function ProblemSolvingModel({ controlFooterAction }: { controlFooterAction?: ReactNode }) {
  ...
  const remaining = budget - itemCost - extraCost;
      ...
      surface={
        <SvgFrame label={t(localized("Four-stage multi-step problem-solving model", "四階段多步解難模型", "四阶段多步解题模型"))}>
          {workflowStep === "represent" ? (
            <g data-viz-mark data-viz-name="bar-model" data-viz-budget={budget} data-viz-item-cost={itemCost} data-viz-extra-cost={extraCost} data-viz-remaining={remaining} data-viz-scale-total={absoluteTotal}>
              <rect data-viz-mark data-viz-name="comparison-scale" x="85" y="128" width={barWidth} height="92" rx="18" fill={theme.emptyFill} stroke={theme.axisStrong} strokeWidth="3" />
              <g
              ...
              >
                <rect data-viz-mark data-viz-overlap-member="mark" x="85" y="128" width={itemWidth} height="92" rx="18" fill={semantic.main} />
              ...
              >
                <rect data-viz-mark data-viz-overlap-member="mark" x={85 + itemWidth} y="128" width={extraWidth} height="92" fill={semantic.change} />
              ...
              {withinBudget ? (
                ...
                >
                  <rect data-viz-mark data-viz-overlap-member="mark" x={85 + itemWidth + extraWidth} y="128" width={remainderWidth} height="92" rx="18" fill={semantic.result} />
            ...
            <g data-viz-mark data-viz-name="strategy-table">
              {[{ x: 72, value: count, color: semantic.parameter }, { x: 212, value: unitPrice, color: semantic.main }, { x: 352, value: itemCost, color: semantic.result }, { x: 492, value: extraCost, color: semantic.change }].map((node, index) => (
                <g key={index} data-viz-mark data-viz-name="strategy-node">
                  <circle cx={node.x} cy="176" r="46" fill={node.color} stroke={theme.pointStroke} strokeWidth="3" />
                  <text x={node.x} y="184" textAnchor="middle" fill="#0f172a" fontSize="22" fontWeight="900">{node.value}</text>
            ...
            <g data-viz-mark data-viz-name="calculation-chain">
              {(withinBudget
                ...
                return <g key={index} data-viz-mark data-viz-name={index === 5 ? "calculation-result" : "calculation-step"}><rect x={x - 70} y={y - 34} width="140" height="68" rx="16" fill={index === 5 ? (withinBudget ? semantic.result : semantic.attention) : index % 2 === 0 ? theme.softFill : semantic.main} stroke={theme.pointStroke} strokeWidth="3" /><text x={x} y={y + 7} textAnchor="middle" fill={index === 5 && withinBudget ? "#052e16" : svgTextColor(theme.mode)} fontSize="18" fontWeight="900">{value}</text></g>;
              })}
            </g>
          ) : (
            <g data-viz-mark data-viz-name="check-balance" data-viz-left={withinBudget ? totalSpending + remaining : budget + overspend} data-viz-right={withinBudget ? budget : totalSpending} data-viz-status={withinBudget ? "within-budget" : "over-budget"}>
              <line x1="320" x2="320" y1="92" y2="282" stroke={theme.axisStrong} strokeWidth="7" />
              <line x1="128" x2="512" y1="246" y2="246" stroke={withinBudget ? semantic.result : semantic.attention} strokeWidth="10" strokeLinecap="round" />
              <rect x="106" y="126" width="172" height="72" rx="16" fill={semantic.main} stroke={theme.pointStroke} strokeWidth="3" />
              <rect x="362" y="126" width="172" height="72" rx="16" fill={withinBudget ? semantic.result : semantic.attention} stroke={theme.pointStroke} strokeWidth="3" />
              <text x="192" y="170" textAnchor="middle" fill="#082f49" fontSize="17" fontWeight="900">{withinBudget ? `${totalSpending}+${remaining}` : `${budget}+${overspend}`}</text>
              <text x="448" y="170" textAnchor="middle" fill={withinBudget ? "#052e16" : "#4c0519"} fontSize="20" fontWeight="900">{withinBudget ? budget : totalSpending}</text>
            </g>
          )}
        </SvgFrame>
      }
