...
export const HK_SECONDARY_MODEL_CONTRACTS = {
  ...
  "linear-equations": {
    learningObjective: enZh("Solve ax + b = c by applying inverse operations to both sides.", "透過在等號兩邊施行逆運算解 ax + b = c。"),
    coreRelation: enZh("ax + b = c implies x = (c - b)/a.", "ax + b = c 表示 x = (c − b)/a。", "ax + b = c 表示 x = (c − b)/a。"),
    ...
    <div className="min-w-0">
      ...
      >
        <svg
          ...
          className="aspect-[8/5] h-auto w-full min-w-[640px]"
        >
          <rect width={WIDTH} height={HEIGHT} rx="24" fill={theme.svgBackground} />
          {children}
        </svg>
      </div>
      <p data-viz-pan-hint className="mt-2 text-xs font-semibold text-slate-500 dark:text-white/55 sm:hidden">{panHint}</p>
    </div>
  );
}
  ...
  return {
    ...
    surface: (
      <Surface name="integer-number-line" label={t(enZh("Signed movement on an integer number line", "整數數線上的有向移動"))} theme={theme}>
        ...
        <line data-viz-mark data-viz-name="integer-zero" x1={map(0)} x2={map(0)} y1="266" y2="302" stroke={GREEN} strokeWidth="5" strokeLinecap="round" />
      </Surface>
    )
  };
}
  ...
  return {
    ...
    surface: (
      <Surface name="algebra-symbolic-balance" label={t(enZh("Symbolic equation balance with inverse operations", "帶逆運算的符號方程天平"))} theme={theme}>
        ...
        <path data-viz-mark data-viz-name="algebra-left-pan" d="M 92 222 L 66 318 H 264 L 238 222" fill={theme.softFill} stroke={BLUE} strokeWidth="4" />
        <path data-viz-mark data-viz-name="algebra-right-pan" d="M 402 222 L 376 318 H 574 L 548 222" fill={theme.softFill} stroke={GREEN} strokeWidth="4" />
        ...
        <rect data-viz-mark data-viz-name="algebra-x-tile" data-viz-solution={solution} x="92" y="252" width="70" height="52" rx="12" fill={BLUE} opacity="0.86" stroke={theme.pointStroke} strokeWidth="3" />
        <text data-viz-label data-viz-name="algebra-symbol-label" x="127" y="285" textAnchor="middle" fill="#0f172a" className="text-lg font-black">x</text>
        ...
        <g data-viz-mark data-viz-name="algebra-inverse-operation" data-viz-operation={-state.a} data-viz-applied-both-sides="true" opacity={stepIndex >= 1 ? 1 : 0.28}>
          <path d="M 150 126 V 184 M 490 126 V 184" stroke={AMBER} strokeWidth="8" strokeLinecap="round" />
          <circle cx="150" cy="104" r="18" fill={ROSE} stroke={theme.pointStroke} strokeWidth="3" />
          <circle cx="490" cy="104" r="18" fill={ROSE} stroke={theme.pointStroke} strokeWidth="3" />
          <text data-viz-label data-viz-name="algebra-inverse-left-label" x="150" y="109" textAnchor="middle" fill="#0f172a" className="text-xs font-black">−a</text>
          <text data-viz-label data-viz-name="algebra-inverse-right-label" x="490" y="109" textAnchor="middle" fill="#0f172a" className="text-xs font-black">−a</text>
        </g>
  ...
  for (const candidateCompanionId of companionIds) {
    ...
    for (let offset = companionStep; offset <= companionMax - companionMin + companionStep; offset += companionStep) {
      for (const direction of [1, -1]) {
        ...
        if (triangleStateIsValid(candidate)) return candidate;
      }
    }
  }
  ...
  return {
    ...
    surface: (
      <Surface name="ratio-scaling" label={t(enZh("Equivalent and scaled ratio bars", "等值與縮放比條"))} theme={theme}>
        ...
        <g data-viz-mark data-viz-name="ratio-base" data-viz-a={state.a} data-viz-b={state.b} opacity={activeMode === "scale" ? 0.56 : 1}>
          <rect x="105" y="90" width={baseWidth} height="64" rx="18" fill={theme.emptyFill} stroke={theme.axis} strokeWidth="3" />
          <rect x="105" y="90" width={aWidth} height="64" rx="18" fill={BLUE} opacity="0.82" />
          <rect x={105 + aWidth} y="90" width={baseWidth - aWidth} height="64" rx="18" fill={AMBER} opacity="0.82" />
          <text data-viz-label data-viz-name="ratio-base-a-label" x={105 + aWidth / 2} y="129" textAnchor="middle" fill="#0f172a" className="text-sm font-black">A={formatNumber(state.a, 0)}</text>
          <text data-viz-label data-viz-name="ratio-base-b-label" x={105 + aWidth + (baseWidth - aWidth) / 2} y="129" textAnchor="middle" fill="#0f172a" className="text-sm font-black">B={formatNumber(state.b, 0)}</text>
        </g>
    ...
    surface: (
      <Surface name="linear-equation-balance" label={t(enZh("Equation balance and inverse-operation steps", "方程天平與逆運算步驟"))} theme={theme}>
        <g transform={`rotate(${beamRotation} 320 210)`}>
          ...
          <path data-viz-mark data-viz-name="equation-left-pan" d="M 108 216 L 78 302 H 236 L 206 216" fill={theme.softFill} stroke={BLUE} strokeWidth="4" />
          <path data-viz-mark data-viz-name="equation-right-pan" d="M 434 216 L 404 302 H 562 L 532 216" fill={theme.softFill} stroke={GREEN} strokeWidth="4" />
        </g>
        ...
        <rect data-viz-mark data-viz-name="equation-right-side" data-viz-value={state.c} x="450" y="248" width="66" height="40" rx="10" fill={GREEN} opacity="0.86" />
        <text data-viz-label data-viz-name="equation-right-label" x="483" y="273" textAnchor="middle" fill="#0f172a" className="text-sm font-black">c={formatNumber(state.c, 0)}</text>
        ...
        <g data-viz-mark data-viz-name="coordinate-original-points" data-viz-points={original.map((point) => `${point.label}:${point.x},${point.y}`).join("|")}>
          {original.map((point, index) => (
            <g key={point.label}>
              ...
              <text data-viz-label data-viz-name={`coordinate-label-${point.label.toLowerCase()}`} x={originalScreen[index].x + [20, -20, 0][index]} y={originalScreen[index].y + [-20, -20, 32][index]} textAnchor="middle" fill={theme.text} className="text-sm font-black">{point.label}</text>
            </g>
          ))}
        </g>
        {hasImage ? (
            ...
            <g data-viz-mark data-viz-name={activeMode === "translate" ? "coordinate-translation-guides" : "coordinate-reflection-guides"}>
              ...
              {imageScreen.map((point, index) => (
                <g key={index}>
                  ...
                  <text data-viz-label data-viz-name={`coordinate-image-label-${original[index].label.toLowerCase()}`} x={point.x + [-20, 20, 0][index]} y={point.y + [28, 28, -24][index]} textAnchor="middle" fill={theme.text} className="text-sm font-black">{original[index].label}′</text>
                </g>
              ))}
            </g>
          </>
  ...
  return {
    ...
    surface: (
      <Surface name="coordinate-transformations" label={t(enZh("Original and transformed triangle", "原圖形與變換後三角形"))} theme={theme}>
        ...
        <polygon data-viz-mark data-viz-name="transform-original" data-viz-points={pointsAttribute(original)} points={pointsAttribute(originalScreen)} fill={BLUE} opacity="0.55" stroke={BLUE} strokeWidth="5" />
        <polygon data-viz-mark data-viz-name="transform-image" data-viz-points={pointsAttribute(image)} data-viz-mode-kind={activeMode === "enlarge" ? "enrichment" : "core"} points={pointsAttribute(imageScreen)} fill={GREEN} opacity="0.52" stroke={GREEN} strokeWidth="5" />
        <text data-viz-label data-viz-name="transform-original-label" x={originalScreen[0].x - 18} y={originalScreen[0].y - 16} textAnchor="middle" fill={theme.text} className="text-sm font-black">A</text>
        <text data-viz-label data-viz-name="transform-original-label-b" x={originalScreen[1].x + 18} y={originalScreen[1].y - 16} textAnchor="middle" fill={theme.text} className="text-sm font-black">B</text>
        <text data-viz-label data-viz-name="transform-original-label-c" x={originalScreen[2].x} y={originalScreen[2].y - 20} textAnchor="middle" fill={theme.text} className="text-sm font-black">C</text>
        <text data-viz-label data-viz-name="transform-image-label" x={imageScreen[0].x - 18} y={imageScreen[0].y + 28} textAnchor="middle" fill={theme.text} className="text-sm font-black">A′</text>
        <text data-viz-label data-viz-name="transform-image-label-b" x={imageScreen[1].x + 18} y={imageScreen[1].y + 28} textAnchor="middle" fill={theme.text} className="text-sm font-black">B′</text>
        <text data-viz-label data-viz-name="transform-image-label-c" x={imageScreen[2].x} y={imageScreen[2].y + 30} textAnchor="middle" fill={theme.text} className="text-sm font-black">C′</text>
    ...
    surface: (
      <Surface name="polynomial-area-model" label={t(enZh("Area model for expanding and factoring a polynomial", "多項式展開與因式分解的面積模型"))} theme={theme}>
        <g opacity={activeMode === "factor" ? 0.82 : 1}>
          <rect data-viz-mark data-viz-name="polynomial-x2" data-viz-area={state.x ** 2} x={left} y={top} width={xSize} height={xSize} fill={BLUE} opacity="0.72" stroke={theme.pointStroke} strokeWidth="3" />
          <rect data-viz-mark data-viz-name="polynomial-px" data-viz-area={state.p * state.x} x={left + xSize} y={top} width={pSize} height={xSize} fill={AMBER} opacity="0.74" stroke={theme.pointStroke} strokeWidth="3" />
          <rect data-viz-mark data-viz-name="polynomial-qx" data-viz-area={state.q * state.x} x={left} y={top + xSize} width={xSize} height={qSize} fill={PURPLE} opacity="0.7" stroke={theme.pointStroke} strokeWidth="3" />
          <rect data-viz-mark data-viz-name="polynomial-pq" data-viz-area={constant} x={left + xSize} y={top + xSize} width={pSize} height={qSize} fill={GREEN} opacity="0.74" stroke={theme.pointStroke} strokeWidth="3" />
          <text data-viz-label data-viz-name="polynomial-x2-label" x={left + xSize / 2} y={top + xSize / 2 + 6} textAnchor="middle" fill="#0f172a" className="text-base font-black">x²</text>
          <text data-viz-label data-viz-name="polynomial-px-label" x={left + xSize + pSize / 2} y={top + xSize / 2 + 6} textAnchor="middle" fill="#0f172a" className="text-sm font-black">{formatNumber(state.p, 0)}x</text>
          <text data-viz-label data-viz-name="polynomial-qx-label" x={left + xSize / 2} y={top + xSize + qSize / 2 + 6} textAnchor="middle" fill="#0f172a" className="text-sm font-black">{formatNumber(state.q, 0)}x</text>
          <text data-viz-label data-viz-name="polynomial-pq-label" x={left + xSize + pSize / 2} y={top + xSize + qSize / 2 + 6} textAnchor="middle" fill="#0f172a" className="text-sm font-black">{formatNumber(constant, 0)}</text>
        </g>
        <rect data-viz-mark data-viz-name="polynomial-whole" data-viz-width-factor={`x+${state.p}`} data-viz-height-factor={`x+${state.q}`} data-viz-area={totalArea} x={left} y={top} width={xSize + pSize} height={xSize + qSize} fill="none" stroke={activeMode === "factor" ? GREEN : theme.axisStrong} strokeWidth="7" />
        <path data-viz-mark data-viz-name="polynomial-factor-braces" d={`M ${left} ${top - 18} H ${left + xSize + pSize} M ${left - 18} ${top} V ${top + xSize + qSize}`} stroke={activeMode === "factor" ? AMBER : theme.axis} strokeWidth="5" strokeLinecap="round" />
      </Surface>
    )
  };
}
  ...
  return {
    ...
    surface: (
      <Surface name="soh-cah-toa" label={t(enZh("Right triangle with SOH CAH TOA side bindings", "綁定 SOH CAH TOA 邊長的直角三角形"))} theme={theme}>
        ...
        <text data-viz-label data-viz-name={referenceIsLeft ? "trig-opposite-label" : "trig-adjacent-label"} x={right.x + 34} y={(right.y + top.y) / 2} textAnchor="middle" fill={theme.text} className="text-sm font-black">{t(referenceIsLeft ? enZh("opposite", "對邊", "对边") : enZh("adjacent", "鄰邊", "邻边"))} {formatNumber(verticalLength, 2)}</text>
        <text data-viz-label data-viz-name="trig-hypotenuse-label" x={(origin.x + top.x) / 2 - 24} y={(origin.y + top.y) / 2 - 18} textAnchor="middle" fill={theme.text} className="text-sm font-black">{t(enZh("hypotenuse", "斜邊", "斜边"))} {formatNumber(state.hyp, 2)}</text>
      </Surface>
    ),
    details: (
      <div className="grid grid-cols-3 gap-2">
  ...
  return {
    ...
    surface: (
      <Surface name="circle-geometry" label={t(enZh("Circle geometry with chord tangent arc and same-arc angles", "包含弦、切線、弧和同弧角的圓幾何", "包含弦、切线、弧和同弧角的圆几何"))} theme={theme}>
        <circle data-viz-mark data-viz-name="circle-geometry-circle" data-viz-r={state.r} data-viz-radius={state.r} data-viz-pixel-radius={formatNumber(radiusPx, 6)} cx={cx} cy={cy} r={radiusPx} fill={theme.softFill} stroke={BLUE} strokeWidth="7" />
  ...
  return {
    ...
    surface: (
      <Surface name="function-machine-table-graph" label={t(enZh("Input-output machine linked to a function graph", "連接函數圖像的輸入輸出機器"))} theme={theme}>
        <rect data-viz-mark data-viz-name="function-input" data-viz-x={state.input} x="42" y="90" width="78" height="64" rx="18" fill={BLUE} opacity="0.84" stroke={theme.pointStroke} strokeWidth="3" />
        <text data-viz-label data-viz-name="function-input-label" x="81" y="128" textAnchor="middle" fill="#0f172a" className="text-sm font-black">x={formatNumber(state.input, 1)}</text>
        <path data-viz-mark data-viz-name="function-machine-flow" d="M 124 122 H 156" stroke={AMBER} strokeWidth="7" strokeLinecap="round" />
        <rect data-viz-mark data-viz-name="function-rule" data-viz-family={activeMode} data-viz-a={state.a} data-viz-b={state.b} x="160" y="70" width="76" height="104" rx="24" fill={PURPLE} opacity="0.68" stroke={theme.pointStroke} strokeWidth="3" />
        <text data-viz-label data-viz-name="function-rule-label" x="198" y="127" textAnchor="middle" fill="#0f172a" className="text-base font-black">f</text>
        <path data-viz-mark data-viz-name="function-machine-flow" d="M 240 122 H 266" stroke={AMBER} strokeWidth="7" strokeLinecap="round" />
        <rect data-viz-mark data-viz-name="function-output" data-viz-y={formatNumber(output, 6)} x="42" y="218" width="194" height="70" rx="20" fill={GREEN} opacity="0.76" stroke={theme.pointStroke} strokeWidth="3" />
        <text data-viz-label data-viz-name="function-output-label" x="139" y="260" textAnchor="middle" fill="#0f172a" className="text-sm font-black">f(x)={formatNumber(output, 2)}</text>
  ...
  return {
    ...
    surface: (
      <Surface name="coordinate-geometry" label={t(enZh("Gradient distance and midpoint on one segment", "同一線段上的斜率、距離與中點"))} theme={theme}>
        ...
        <text data-viz-label data-viz-name="coordinate-point-a-label" x={a.x - 24} y={a.y - 20} textAnchor="middle" fill={theme.text} className="text-sm font-black">A</text>
        <text data-viz-label data-viz-name="coordinate-point-b-label" x={b.x + 24} y={b.y - 20} textAnchor="middle" fill={theme.text} className="text-sm font-black">B</text>
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(290px,350px)]">
        ...
        <aside className="min-w-0 space-y-4">
          ...
          <div data-viz-formula data-viz-formula-topic={dedicatedId} className="rounded-3xl border border-cyan-200/70 bg-cyan-50/75 p-4 dark:border-cyan-300/15 dark:bg-cyan-300/[0.055]">
            ...
            <p data-viz-formula-pan-hint className="mt-1 text-xs font-semibold text-slate-500 dark:text-white/55 sm:hidden">{formulaPanHint}</p>
