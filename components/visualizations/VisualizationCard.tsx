...
export function VisualizationCard({
  ...
  /** Optional disambiguated accessible name when the visible title has a nearby subtitle badge. */
  ...
  const notifyExplored = useCallback(() => {
    try {
      ...
      // Host callbacks are observational and cannot invalidate a durable ACK.
    }
  }, [moduleId, onExplored]);
  ...
  const recordExplored = useCallback(() => {
    ...
    if (!currentUser && typeof window !== "undefined") {
      ...
      try {
        ...
        // The guest explored marker is durable even if analytics degrades.
      }
      notifyExplored();
      if (activeStateScopeKeyRef.current === requestScopeKey) setSaveState("saved");
      return;
    }
    ...
    try {
      ...
      // The session is already durable. Leave telemetry fail-closed without
      // suppressing the provider wakeup for this exact queued session.
    }
  ...
  useEffect(() => {
    ...
    const handleAcknowledged = (event: Event) => {
      ...
      notifyExplored();
      if (activeStateScopeKeyRef.current === stateScopeKey) setSaveState("saved");
    };
  ...
  const [dwellSatisfiedScopeKey, setDwellSatisfiedScopeKey] =
    useState<string | null>(null);
  const [interactionScopeKey, setInteractionScopeKey] =
    useState<string | null>(null);
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 shadow-sm shadow-slate-900/5 dark:border-white/10 dark:bg-slate-900 dark:shadow-none sm:p-6"
    >
      <div className="mb-5 min-w-0">
        <h2
          aria-label={accessibleTitle && accessibleTitle !== title ? accessibleTitle : undefined}
          data-viz-card-accessible-title={accessibleTitle && accessibleTitle !== title ? accessibleTitle : undefined}
