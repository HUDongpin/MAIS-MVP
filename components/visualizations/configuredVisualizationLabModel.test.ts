...
test("calculus probe reaches both slider ends and its area lower limit stays before the probe", () => {
  assert.equal(calculusAreaLowerLimit, -3.6);
  assert.equal(calculusProbeX(1), -3.2);
  assert.equal(calculusProbeX(9), 3.2);
  for (const comparison of sliderRange(1, 9)) {
    assert.ok(calculusProbeX(comparison) > calculusAreaLowerLimit);
  }
});

test("base-ten grade bounds and labels match the reachable place values", () => {
  for (const [grade, max] of [["K", 19], ["P1", 99], ["P2", 999]] as const) {
    const bounds = sliderBoundsForThreeDTemplate("base-ten", grade);
    const state = baseTenState(bounds.valueMax, bounds.comparisonMax);
    assert.equal(state.total, max, `${grade} must reach its intended maximum`);
    for (const lab of visualizationLabCatalog.filter((item) => item.templateId === "base-ten" && item.grade === grade)) {
      if (grade === "K" || grade === "P1") {
        assert.doesNotMatch(lab.templateConfig.formula?.en ?? "", /100 x hundreds/, `${lab.topicId} must not promise hundreds`);
      }
    }
  }
});
