...
function calculusState(value: number, comparison: number) {
  const a = value / 10;
  const x0 = calculusProbeX(comparison);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <TemplateMarks accent={accent} comparison={comparison} gradeBand={gradeBand} hasHundreds={hasHundreds} mode={mode} templateId={templateId} userPoints={userPoints} vizTheme={vizTheme} value={value} />
        {showStandardGraphOverlay ? (
...
function TemplateMarks({
  ...
  if (templateId === "base-ten") {
    ...
    return (
      <g data-viz-active-mode={mode}>
        ...
        {hasHundreds && Array.from({ length: state.hundreds }, (_, index) => (
        ...
        <text data-viz-overlap-ok x="104" y="288" fill={vizTheme.labelText} className="text-xs font-black">
          {hasHundreds ? `${state.hundreds} hundreds + ` : ""}{state.tens} tens + {state.ones} ones = {state.total}
        </text>
      </g>
    );
  }
  ...
  if (templateId === "calculus-rate-area") {
    ...
    const areaLeft = calculusAreaLowerLimit;
...
function ConfiguredVisualizationLabSurface({
  ...
  const [rawValue, setValue] = useState(5);
  const [rawComparison, setComparison] = useState(4);
  ...
  const hasHundreds = templateId !== "base-ten" || (lab?.grade !== "K" && lab?.grade !== "P1");
  const sliderBounds = useMemo(() => sliderBoundsForThreeDTemplate(templateId, lab?.grade), [templateId, lab?.grade]);
  const value = clamp(rawValue, sliderBounds.valueMin, sliderBounds.valueMax);
  const comparison = clamp(rawComparison, sliderBounds.comparisonMin, sliderBounds.comparisonMax);
  ...
  const controlCopy = useMemo(() => {
    ...
    if (templateId === "base-ten") {
      const state = baseTenState(value, comparison);
      const hundredsEn = hasHundreds ? `${state.hundreds} hundreds + ` : "";
      const hundredsZh = hasHundreds ? `${state.hundreds} 個百 + ` : "";
      const hundredsZhHans = hasHundreds ? `${state.hundreds} 个百 + ` : "";
      return {
        modeLabels: hasHundreds
          ? [t({ en: "Hundreds", zh: "百位", zhHans: "百位" }), t({ en: "Tens", zh: "十位", zhHans: "十位" }), t({ en: "Ones", zh: "個位", zhHans: "个位" })]
          : [t({ en: "All", zh: "全部", zhHans: "全部" }), t({ en: "Tens", zh: "十位", zhHans: "十位" }), t({ en: "Ones", zh: "個位", zhHans: "个位" })],
        valueLabel: hasHundreds ? t({ en: "Hundreds and tens", zh: "百位與十位", zhHans: "百位与十位" }) : t({ en: "Tens", zh: "十位", zhHans: "十位" }),
        comparisonLabel: t({ en: "Ones", zh: "個位數", zhHans: "个位数" }),
        comparisonNote: t({
          en: `${hundredsEn}${state.tens} tens + ${state.ones} ones = ${state.total}.`,
          zh: `${hundredsZh}${state.tens} 個十 + ${state.ones} 個一 = ${state.total}。`,
          zhHans: `${hundredsZhHans}${state.tens} 个十 + ${state.ones} 个一 = ${state.total}。`
        })
      };
    }
  ...
  }, [cappedMode, comparison, coordinateModeCount, gradeBand, hasHundreds, mode, modelMode, t, templateId, usesGradeOneSetControls, value]);
  const sliderDisplay: { comparison?: string; value?: string } = useMemo(() => {
    ...
    if (templateId === "base-ten") {
      // Show the places controlled by the value dial for this grade.
      const state = baseTenState(value, comparison);
      return {
        comparison: `${state.ones}`,
        value: hasHundreds ? `${state.hundreds}h ${state.tens}t` : `${state.tens}t`
      };
    }
    ...
    return {};
  }, [comparison, gradeBand, hasHundreds, mode, templateId, value]);
