...
function ArrayMarks({ accent, state, strings, theme }: { accent: string; state: ArrayState; strings: Required<SemanticPrimaryLocalizedStrings>; theme: VisualizationTheme }) {
  ...
  const visibleRows = Math.min(state.rows, 8);
  ...
  const additionLines = state.family === "equal-groups-array" ? repeatedAdditionLines(state) : [];
  return (
    <>
      <rect data-viz-mark data-viz-name="array outline" x={startX - 6} y={startY - 6} width={visibleColumns * cell + 12} height={visibleRows * cell + 12} rx="9" fill={theme.emptyFill} stroke={accent} strokeWidth="3" />
...
function FractionMarks({ accent, state, theme }: { accent: string; state: FractionState; theme: VisualizationTheme }) {
  ...
  const bar = (
    ...
    return (
      ...
      >
        <rect x={barX} y={y} width={barWidth} height="42" rx="12" fill={theme.emptyFill} stroke={theme.axisStrong} strokeWidth={active ? 3 : 2} />
        <rect x={barX} y={y} width={(barWidth * safeNumerator) / denominator} height="42" rx="12" fill={accent} fillOpacity={active ? 0.76 : 0.42} />
        {range(1, denominator - 1).map((index) => <line key={index} x1={barX + (barWidth * index) / denominator} x2={barX + (barWidth * index) / denominator} y1={y} y2={y + 42} stroke={theme.axisStrong} strokeOpacity="0.55" />)}
