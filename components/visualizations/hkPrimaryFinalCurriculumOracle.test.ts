...
import { hkPrimaryAveragesGraphProjection } from "./hk/HKPrimaryVisualizationLab";
...
test("P6 average source shares four values across fair-share and adjacent broken-line marks", () => {
  ...
  for (const [series, points, segments] of [
    ["A", projection.pointsA, projection.segmentsA],
    ["B", projection.pointsB, projection.segmentsB]
  ] as const) {
    assert.equal(points.length, 4, `series ${series} must project four ordered points`);
    assert.equal(segments.length, 3, `series ${series} must project exactly three adjacent segments`);
    assert.deepEqual(
      segments,
      ...
      `series ${series} segments must join each point only to its immediate successor`
    );
  }

  assert.match(section, /type AveragesGraphMode = "mean-fair-share" \| "broken-line"/);
  assert.match(section, /export function hkPrimaryAveragesSeriesProjection/);
  assert.match(section, /export function hkPrimaryAveragesGraphProjection/);
  assert.match(section, /segmentsA:\s*segmentsFor\(pointsA\)/);
  assert.match(section, /segmentsB:\s*segmentsFor\(pointsB\)/);
  assert.match(section, /hkPrimaryAveragesSeriesProjection\([\s\S]*?\[value1, value2, value3, value4\][\s\S]*?seriesBShift/);
  assert.match(section, /const seriesB = seriesA\.map\(\(value\) => value \+ seriesBShift\)/);
  ...
  for (const [collection, series] of [
    ["segmentsA", "A"],
    ["segmentsB", "B"]
  ] as const) {
    const renderer = section.match(
      new RegExp(`\\{${collection}\\.map\\(\\(segment, index\\) => <line[\\s\\S]*?\\/>\\)\\}`)
    )?.[0];
    assert.ok(renderer, `${collection} must render one line per projected segment`);
    assert.match(renderer, /data-viz-name="adjacent-line-segment"/);
    assert.match(renderer, new RegExp(`data-viz-series="${series}"`));
    for (const [attribute, property] of [
      ["data-viz-from-hour", "fromHour"],
      ["data-viz-to-hour", "toHour"],
      ["data-viz-from-value", "fromValue"],
      ["data-viz-to-value", "toValue"],
      ["x1", "x1"],
      ["y1", "y1"],
      ["x2", "x2"],
      ["y2", "y2"]
      ...
      assert.match(
        renderer,
        new RegExp(`${attribute}=\\{segment\\.${property}\\}`),
        `${collection} must expose exact ${attribute} from its pure projected segment`
      );
    }
  }
  ...
  assert.match(section, /localized\("Series B offset", "數列 B 偏移量", "数列 B 偏移量"\)/);
  assert.doesNotMatch(section, /Series B positive offset|數列 B 正偏移量|数列 B 正偏移量/);
