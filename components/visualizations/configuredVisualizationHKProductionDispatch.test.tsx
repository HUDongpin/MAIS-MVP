...
import {
  ...
  isConfiguredVisualizationSessionInteraction,
...
test("lesson-only session records are exact per user and topic while standalone keeps the outer card owner", async () => {
  ...
  assert.match(configuredSource, /onChange=\{recordLessonSessionFromInteraction\}/);
  assert.match(configuredSource, /onChangeCapture=\{recordLessonSessionFromInteraction\}/);
  assert.match(configuredSource, /onClickCapture=\{recordLessonSessionFromInteraction\}/);
  assert.match(configuredSource, /onInput=\{recordLessonSessionFromInteraction\}/);
  assert.match(configuredSource, /onInputCapture=\{recordLessonSessionFromInteraction\}/);
  ...
  assert.match(configuredSource, /control\.getAttribute\("aria-disabled"\) === "true"/);
...
test("lesson session ownership accepts pointer, assistive click, input, and change interactions exactly through one classifier", () => {
  for (const eventType of ["pointerup", "click", "input", "change"]) {
    assert.equal(isConfiguredVisualizationSessionInteraction(eventType), true, eventType);
  }
  assert.equal(isConfiguredVisualizationSessionInteraction("keyup", "Enter"), true);
  assert.equal(isConfiguredVisualizationSessionInteraction("keyup", "ArrowRight"), true);
  assert.equal(isConfiguredVisualizationSessionInteraction("keyup", "Tab"), false);
  assert.equal(isConfiguredVisualizationSessionInteraction("focus"), false);
  assert.equal(isConfiguredVisualizationSessionInteraction("blur"), false);
  assert.equal(isConfiguredVisualizationSessionInteraction("keydown", "Enter"), false);
});
