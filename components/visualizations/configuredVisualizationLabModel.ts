...
/** Both the SVG area strips and the 3D accumulation use this lower limit. */
export const calculusAreaLowerLimit = -3.6;

/** The probe slider spans comparisons 1..9, corresponding to x = -3.2..3.2. */
export function calculusProbeX(comparison: number) {
  return (comparison - 5) / 1.25;
}
 ...
 * its maximum was 99 even for grade 2 and later three-digit lessons. The
 * component applies grade-specific slider bounds, keeping K and P1 within
 * their taught ranges while letting P2 and later reach hundreds.
 */
