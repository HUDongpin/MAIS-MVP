...
test("ThreeDLabCanvas suspends the continuous render loop while Manim playback is paused", () => {
  const source = fs.readFileSync("components/visualizations/three/ThreeDLabCanvas.tsx", "utf8");

  assert.match(
    source,
    /<Canvas[\s\S]*frameloop=\{manimPlaybackState === "playing" \? "always" : "demand"\}/,
    "a paused lesson visualization must not consume a continuous mobile render loop"
  );
});
...
test("ThreeDLabCanvas exposes a MAIS Manim scene-spec selector for authoring", () => {
  ...
  assert.match(canvasSource, /from "\.\/manim\/mathSceneSelectorCatalog"/);
  for (const symbol of [
    ...
    assert.match(canvasSource, new RegExp(`\\b${symbol}\\b`));
  }
  ...
  assert.match(canvasSource, /setManimSceneSelectorCatalog\(entries\.slice\(\)\)/);
