...
test("topic visualization labs select curated signature benches or the configured renderer", () => {
  const source = fs.readFileSync("data/visualizationLabs.ts", "utf8");

  assert.doesNotMatch(source, /legacyModuleByTopicId\[topic\.id\]/);
  assert.match(source, /import \{ hasSignatureLab \} from "\.\/signatureLabAssignments"/);
...
test("active visualization module types and page registry support configured and signature labs", () => {
  ...
  assert.match(pageSource, /SignatureLabRoutes\[assignment\.primary\] \?\? ConfiguredVisualizationLab/);
  assert.match(pageSource, /return labComponentRegistry\[lab\.moduleId\] \?\? ConfiguredVisualizationLab/);
});
