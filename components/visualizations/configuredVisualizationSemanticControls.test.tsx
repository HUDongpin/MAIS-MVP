...
import * as semanticControlModule from "./configuredVisualizationSemanticControls";
...
test("HK pass-through reset plans are explicit per topic and contain no generic 5/4/0 fallback", () => {
  const module = semanticControlModule as unknown as Record<string, unknown>;
  assert.equal(
    typeof module.getConfiguredVisualizationSemanticResetPlan,
    "function",
    "Product reset must consume one pure topic-aware reset-plan API."
  );
  assert.ok(
    module.configuredVisualizationPassThroughResetPlans
      && typeof module.configuredVisualizationPassThroughResetPlans === "object",
    "The seven HK pass-through resets must be frozen in an explicit topic table."
  );

  const plans = module.configuredVisualizationPassThroughResetPlans as Readonly<Record<string, unknown>>;
  ...
  const resetPlan = module.getConfiguredVisualizationSemanticResetPlan as (
    contract: ConfiguredVisualizationSemanticControlContract | undefined,
    topicId: string
  ) => { comparison: number; height: number; mode: number; value: number };
  const exactFamilies = {
  ...
  for (const [topicId, family] of Object.entries(exactFamilies)) {
    const contract = getConfiguredVisualizationSemanticControlContract(family, topicId);
    assert.deepEqual(
      resetPlan(contract, topicId),
      (plans as Readonly<Record<string, unknown>>)[topicId],
      topicId
    );
  }
});

test("advanced-functions exposes only a positive quadratic coefficient a with a trilingual exact label", () => {
  const contract = getConfiguredVisualizationSemanticControlContract(
    "function-properties",
    "advanced-functions"
  );
  assert.ok(contract);
  const coefficient = contract.sliders.find(({ id }) => id === "value");
  assert.deepEqual(coefficient, {
    id: "value",
    initial: 5,
    label: {
      en: "Quadratic coefficient a",
      zh: "二次項係數 a",
      zhHans: "二次项系数 a"
    },
    max: 10,
    min: 1,
    role: "quadratic-coefficient-a",
    step: 1
  });
  const projected = projectConfiguredVisualizationSemanticControlState(
    contract,
    { comparison: 5, value: 0 },
    0
  );
  assert.equal(projected.values.value, 1);
  assert.equal(projected.bounds.value?.min, 1);
  ...
  assert.equal(Number(state.metrics.scaleParameter), 1 / 6);
  assert.ok(Number(state.metrics.scaleParameter) > 0);
});

test("the calculus coefficient label is not misnamed as curvature in any supported locale", () => {
  for (const variant of ["differentiation-intro", "calculus"]) {
    ...
    const coefficient = contract?.sliders.find(({ id }) => id === "value");
    assert.deepEqual(coefficient?.label, {
      en: "Quadratic coefficient a",
      zh: "二次項係數 a",
      zhHans: "二次项系数 a"
    });
...
test("Hong Kong pass-through variants expose topic-accurate reset controls and modes", () => {
  ...
  for (const variant of ["differentiation-intro", "calculus"]) {
    ...
    assert.deepEqual(
      ...
      [
        { id: "value", initial: 5, max: 10, min: 0, role: "quadratic-coefficient-a" },
