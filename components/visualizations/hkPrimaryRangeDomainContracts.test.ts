...
import * as HKPrimaryVisualizationModule from "./hk/HKPrimaryVisualizationLab";
...
test("P4 atomically persists a clamped divisor when the number or mode lowers its domain", () => {
  type FactorsState = {
  ...
  type FactorsUpdate = Partial<FactorsState>;
  const transition = (HKPrimaryVisualizationModule as unknown as {
    hkPrimaryTransitionFactorsMultiplesState?: (state: FactorsState, update: FactorsUpdate) => FactorsState;
  }).hkPrimaryTransitionFactorsMultiplesState;

  assert.ok(transition, "P4 must expose the pure state transition used by its live controls.");

  const highDivisor: FactorsState = {
    mode: "factor-pairs",
    firstNumber: 48,
    secondNumber: 18,
    candidateDivisor: 48
  };
  const commonMode = transition(highDivisor, { mode: "common-hcf-lcm" });
  assert.deepEqual(commonMode, {
    mode: "common-hcf-lcm",
    firstNumber: 20,
    secondNumber: 18,
    candidateDivisor: 20
  });
  const backToFactors = transition(commonMode, { mode: "factor-pairs" });
  const raisedAfterModeClamp = transition(backToFactors, { firstNumber: 30 });
  assert.deepEqual(
    raisedAfterModeClamp,
    { mode: "factor-pairs", firstNumber: 30, secondNumber: 18, candidateDivisor: 20 },
    "Raising the number after a mode clamp must not resurrect the old divisor as 30."
  );

  const loweredDirectly = transition(highDivisor, { firstNumber: 12 });
  const raisedAfterDirectClamp = transition(loweredDirectly, { firstNumber: 30 });
  assert.equal(loweredDirectly.candidateDivisor, 12);
  assert.equal(
    raisedAfterDirectClamp.candidateDivisor,
    12,
    "Raising the number after a direct clamp must keep the persisted divisor."
  );
});
