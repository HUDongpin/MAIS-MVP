...
type TriangleCoordinateId = keyof typeof TRIANGLE_COORDINATE_BOUNDS;
type TriangleState = Record<TriangleCoordinateId, number>;

function triangleGeometry(state: TriangleState) {
  const vertices = [
  ...
  const angles = vertices.map((vertex, index) => angleAt(
    vertex,
    vertices[(index + 1) % vertices.length],
    vertices[(index + 2) % vertices.length]
  ));
  return { angles, area, minimumSide };
}

function assertHonestTriangleState(
  state: TriangleState,
  requestedCoordinateId: TriangleCoordinateId,
  requestedValue: number,
  context: string
) {
  assert.equal(
    state[requestedCoordinateId],
    requestedValue,
    `${context}: the requested coordinate must remain exactly reachable.`
  );
  for (const [coordinateId, [minimum, maximum]] of Object.entries(TRIANGLE_COORDINATE_BOUNDS)) {
    const value = state[coordinateId as TriangleCoordinateId];
    assert.ok(Number.isFinite(value), `${context}: ${coordinateId} must be finite.`);
    assert.ok(value >= minimum && value <= maximum, `${context}: ${coordinateId} must stay in range.`);
    assert.ok(
      Math.abs((value - minimum) / 0.25 - Math.round((value - minimum) / 0.25)) <= 1e-9,
      `${context}: ${coordinateId} must remain on the declared 0.25 grid.`
    );
  }
  const { angles, area, minimumSide } = triangleGeometry(state);
  assert.ok(Number.isFinite(area) && area >= 0.25, `${context}: triangle area must be at least 0.25.`);
  assert.ok(
    Number.isFinite(minimumSide) && minimumSide >= 2.5,
    `${context}: the minimum side must preserve distinct 48px targets.`
  );
  ...
  assert.ok(
    ...
    `${context}: honest triangle angles must sum to 180 degrees.`
  );
}
...
test("all 22 teaching contracts contain one complete field set with no duplicate or omitted topic", () => {
  ...
  const linearEquationsContract = contractBlock("linear-equations");
  const exactNonzeroConditionCopy = [
  ...
  assert.deepEqual(
    exactNonzeroConditionCopy.filter((expectedCopy) => !linearEquationsContract.includes(expectedCopy)),
    [],
    "linear-equations: the learner objective and core relation must state a ≠ 0 exactly"
  );
});
...
test("learner surface uses local focusable horizontal pan and 44px control contracts", () => {
  ...
  assert.doesNotMatch(surface, /sm:hidden/);
  ...
  assert.doesNotMatch(surface, /data-viz-overlap-ok/);
});

test("intentional mark-label containment uses only exact two-member overlap owners", () => {
  const expectedOwners = [
    "algebra-x-tile-label",
    "algebra-inverse-left-label",
    "algebra-inverse-right-label",
    "ratio-base-a-label",
    "ratio-base-b-label",
    "equation-right-side-label",
    "polynomial-x2-label",
    "polynomial-px-label",
    "polynomial-qx-label",
    "polynomial-pq-label",
  ...
  assert.equal((code.match(/data-viz-overlap-ok=/g) ?? []).length, expectedOwners.length);
  assert.equal((code.match(/data-viz-overlap-member="mark"/g) ?? []).length, expectedOwners.length);
  assert.equal((code.match(/data-viz-overlap-member="label"/g) ?? []).length, expectedOwners.length);
  assert.equal((code.match(/data-viz-overlap-reason=/g) ?? []).length, expectedOwners.length);
  for (const owner of expectedOwners) {
    const ownerPattern = new RegExp(
      `<g\\s+data-viz-overlap-ok="${owner}"\\s+data-viz-overlap-reason="[^"]+">` +
      `[\\s\\S]*?data-viz-overlap-member="mark"[\\s\\S]*?` +
      `data-viz-overlap-member="label"[\\s\\S]*?<\\/g>`
    );
    assert.match(code, ownerPattern, `${owner} must contain one explicit mark followed by one explicit label.`);
  }
  assert.doesNotMatch(code, /<g\b[^>]*data-viz-overlap-ok=[^>]*data-viz-mark/);
});
...
test("angle projection preserves an honest triangle through the exact production four-step regression", () => {
  const requests = [
    ["ax", -8],
    ["bx", -8],
    ["cy", 0],
    ["cx", -8]
  ] as const satisfies readonly (readonly [TriangleCoordinateId, number])[];
  let state: TriangleState = { ax: -4, ay: -2, bx: 4, by: -2, cx: 1, cy: 3 };

  for (const [index, [coordinateId, requestedValue]] of requests.entries()) {
    state = projectValidTriangleCoordinate(state, coordinateId, requestedValue) as TriangleState;
    assertHonestTriangleState(
      state,
      coordinateId,
      requestedValue,
      `production regression step ${index + 1}: ${coordinateId}=${requestedValue}`
    );
  }
});

test("angle projection keeps every reordering of the adversarial four-step requests valid", () => {
  const requests = [
    ["ax", -8],
    ["bx", -8],
    ["cy", 0],
  ...
  for (const [permutationIndex, requestOrder] of permutations(requests).entries()) {
    let state: TriangleState = { ax: -4, ay: -2, bx: 4, by: -2, cx: 1, cy: 3 };
    for (const [stepIndex, [coordinateId, requestedValue]] of requestOrder.entries()) {
      state = projectValidTriangleCoordinate(state, coordinateId, requestedValue) as TriangleState;
      assertHonestTriangleState(
        state,
        coordinateId,
        requestedValue,
        `request permutation ${permutationIndex + 1}, step ${stepIndex + 1}: ${coordinateId}=${requestedValue}`
      );
    }
  }
});
...
test("angle coordinate controls project every declared endpoint to an honest non-degenerate triangle", () => {
  ...
  assert.match(projection, /mirroredBoundaryCandidates/);
  assert.match(projection, /squaredDisplacement/);
  assert.match(projection, /candidateOrder/);
  assert.match(projection, /if\s*\(triangleStateIsValid\(current\)\)\s*return\s+current/);
  assert.match(projection, /throw\s+new\s+Error/);
  assert.doesNotMatch(projection, /\n\s*return\s+requested;\s*\n\}/);
  ...
  assert.match(runtime, /onClick=\{\(\)\s*=>\s*setActiveMode\(item\.id\)\}/);
  assert.match(runtime, /function\s+resetModel\(\)\s*\{[\s\S]*?setState\(initialStateFor\(dedicatedId\)\);[\s\S]*?setActiveMode\(contract\.modes\[0\]\.id\);/);
...
test("coordinate and transformation vertex labels stay outside active axes, edges, and filled regions", () => {
  const coordinates = functionBlock("renderCoordinates");
  const transformations = functionBlock("renderTransformations");

  assert.match(coordinates, /const\s+coordinateOriginalLabelPosition\s*=/);
  assert.match(coordinates, /activeMode\s*===\s*"reflect"[\s\S]*?point\.label\s*===\s*"B"/);
  assert.match(coordinates, /const\s+coordinateImageLabelPosition\s*=/);
  assert.match(coordinates, /activeMode\s*===\s*"reflect"[\s\S]*?index\s*===\s*1[\s\S]*?x:\s*point\.x\s*\+\s*28[\s\S]*?y:\s*point\.y\s*-\s*26/);

  assert.match(transformations, /const\s+transformationSegments\s*=/);
  assert.match(transformations, /polygonSegments\(originalScreen\)/);
  assert.match(transformations, /polygonSegments\(imageScreen\)/);
  assert.match(transformations, /const\s+transformationObstacles\s*=/);
  assert.match(transformations, /const\s+originalLabelPositions\s*=\s*labelPositionsOutsidePolygon\(originalScreen,\s*transformationObstacles\)/);
  assert.match(transformations, /const\s+imageLabelPositions\s*=\s*labelPositionsOutsidePolygon\(imageScreen,\s*transformationObstacles\)/);
});

test("decorative polygon and disk fills are separate from exact marked boundaries", () => {
  const transformations = functionBlock("renderTransformations");
  const circles = functionBlock("renderCircleGeometry");

  assert.match(transformations, /data-viz-name="transform-original-fill"[\s\S]*?stroke="none"/);
  assert.match(transformations, /data-viz-mark\s+data-viz-name="transform-original"[\s\S]*?fill="none"/);
  assert.match(transformations, /data-viz-name="transform-image-fill"[\s\S]*?stroke="none"/);
  assert.match(transformations, /data-viz-mark\s+data-viz-name="transform-image"[\s\S]*?fill="none"/);
  assert.doesNotMatch(transformations, /data-viz-mark\s+data-viz-name="transform-(?:original|image)-fill"/);

  assert.match(circles, /data-viz-name="circle-geometry-fill"[\s\S]*?stroke="none"/);
  assert.match(circles, /data-viz-mark\s+data-viz-name="circle-geometry-circle"[\s\S]*?fill="none"/);
  assert.doesNotMatch(circles, /data-viz-mark\s+data-viz-name="circle-geometry-fill"/);
});
...
test("formula overflow is a keyboard-focusable localized region with a visible pan hint", () => {
  ...
  assert.doesNotMatch(runtime, /data-viz-formula-pan-hint[^>]*sm:hidden/);
});
