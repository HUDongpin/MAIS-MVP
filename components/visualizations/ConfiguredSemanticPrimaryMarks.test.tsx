...
test("P2 multiplication foundations shows countable rows, columns, and object total without formal area or repeated addition", () => {
  ...
  const state = buildEqualGroupsArrayState({
    ...input,
    comparison: 5,
    value: 4,
  ...
  const markup = renderPrimary("equal-groups-array", {
    comparison: 5,
    value: 4,
    variant: "p2-multiplication-foundations"
  });
  ...
  assert.doesNotMatch(markup, /data-viz-name="equal groups repeated addition"/);
  assert.doesNotMatch(markup, /\bArea\b|square units?|5 \+ 5 \+ 5 \+ 5/i);

  const outline = openingTagsByVizName(markup, "array outline")[0];
  assert.ok(outline);
  assert.equal(attribute(outline, "data-viz-rows"), "4");
  assert.equal(attribute(outline, "data-viz-columns"), "5");
  assert.equal(attribute(outline, "data-viz-object-total"), "20");
  const items = openingTagsByVizName(markup, "array item");
  assert.deepEqual(
    new Set(items.map((tag) => attribute(tag, "data-viz-row"))),
    new Set(["1", "2", "3", "4"])
  );
  assert.deepEqual(
    new Set(items.map((tag) => attribute(tag, "data-viz-column"))),
    new Set(["1", "2", "3", "4", "5"])
  );
  assert.equal(
    new Set(items.map((tag) => `${attribute(tag, "data-viz-row")}:${attribute(tag, "data-viz-column")}`)).size,
    20
  );

  for (const [rows, columns] of [[1, 1], [4, 5], [10, 10]] as const) {
    const boundaryMarkup = renderPrimary("equal-groups-array", {
      comparison: columns,
      mode: 99,
      value: rows,
      variant: "p2-multiplication-foundations"
    });
    assert.equal((boundaryMarkup.match(/data-viz-name="array item"/g) ?? []).length, rows * columns);
    assert.match(boundaryMarkup, new RegExp(`data-viz-name="equal groups total"[^>]*data-viz-total="${rows * columns}"`, "u"));
    assert.doesNotMatch(boundaryMarkup, /data-viz-name="equal groups repeated addition"|\bArea\b|square units?/iu);
  }
});
...
test("HK P3 raw denominator-minus-one and dynamic numerator cover zero, whole, reset, and every mode", () => {
  ...
  const wholeFill = openingTagsByVizName(compareMarkup, "whole bar fill")[0];
  const equivalentFill = openingTagsByVizName(compareMarkup, "equivalent bar fill")[0];
  assert.ok(wholeFill && equivalentFill, "both fraction bars need machine-visible fill geometry");
  assert.equal(attribute(wholeFill, "data-viz-numerator"), "4");
  assert.equal(attribute(wholeFill, "data-viz-denominator"), "6");
  assert.equal(attribute(wholeFill, "data-viz-bar-width"), "456");
  assert.equal(Number(attribute(wholeFill, "width")), 456 * 4 / 6);
  assert.equal(attribute(equivalentFill, "data-viz-numerator"), "8");
  assert.equal(attribute(equivalentFill, "data-viz-denominator"), "12");
  assert.equal(Number(attribute(equivalentFill, "width")), 456 * 8 / 12);
