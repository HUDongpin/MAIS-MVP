...
function buildFunctionPropertiesState(
  ...
  if (input.variant === "advanced-functions") {
    const scaleParameter = clamp(finite(input.value, 5), 1, 10) / 6;
    const verticalShift = clamp(finite(input.comparison, 5), 0, 10) - 5;
    // The current HK lesson exposes one selected curve and no family-mode
    // control. Clamp legacy/stale mode inputs to the proven default surface.
    const activeMode = 0;
    ...
    const domain = Array.from(
      { length: 33 },
      (_, index) => -2.4 + index * (5.4 / 32)
    );
    const series = domain.map((x) => ({ x, y: evaluate(primaryFamily, x) }));

    return makeState(input, {
      check: `selected ${primaryFamily} · a=${fixed(scaleParameter)} · k=${fixed(verticalShift)}`,
      formula: `y=${formulaFor(primaryFamily)}`,
...
function buildDerivativeState(
  ...
  if (input.variant === "differentiation-intro" || input.variant === "calculus") {
    ...
    const activeModeIndex = 0;
    const activeMode = "tangent";
    const functionFormula = `f(x)=0.5·a·x²+0.35, a=${fixed(curvature)}`;
    const readNumber = (value: number) => fixed(value).replace(/^-/, "−");
    const check = `x₀=${readNumber(probeX)} · tangent slope=${readNumber(tangentSlope)}`;
    const formula = `${functionFormula} · f′(x)=a·x`;
    const baseSeries = Array.from({ length: 33 }, (_, index) => {
      const sampleX = -4 + index * 0.25;
      return { x: sampleX, y: evaluate(sampleX) };
    });
    const tangent = { x: probeX, y: evaluate(probeX) };
    const series = baseSeries.some((point) => Math.abs(point.x - probeX) < EPSILON)
      ? baseSeries
      : [...baseSeries, tangent].sort((left, right) => left.x - right.x);
  ...
  switch (input.family) {
    ...
    case "function-properties":
      if (input.variant === "advanced-functions") {
        ...
        mode = String(state.metrics.primaryFamily);
      } else {
        value = displaySigned("a", number("coefficient"));
...
export function configuredSemanticSecondaryRequiredModeCount(
  ...
  if (input.family === "function-properties" && input.variant === "advanced-functions") return 0;
  if (
    input.family === "derivative-rate-area" &&
    (input.variant === "differentiation-intro" || input.variant === "calculus")
  ) return 1;
  if (
...
export type ConfiguredSemanticSecondaryPassThroughViewportPlan = Readonly<{
  id: string;
  viewport: Readonly<Required<Omit<SemanticViewport, "preserveAspectRatio">>>;
  xTicks: readonly number[];
  yTicks: readonly number[];
}>;

const distributionSummaryFixedViewport = Object.freeze({
...
} as const satisfies ConfiguredSemanticSecondaryPassThroughViewportPlan);

export const configuredSemanticSecondaryPassThroughViewportPlans = Object.freeze({
  "statistics-s1": distributionSummaryFixedViewport,
  "data-handling": distributionSummaryFixedViewport,
...
} as const satisfies Readonly<Record<string, ConfiguredSemanticSecondaryPassThroughViewportPlan>>);

function passThroughViewportPlanForState(
  state: ConfiguredSemanticSecondaryMathState
): ConfiguredSemanticSecondaryPassThroughViewportPlan | null {
  return configuredSemanticSecondaryPassThroughViewportPlans[
    state.variant as keyof typeof configuredSemanticSecondaryPassThroughViewportPlans
  ] ?? null;
}
...
function viewportForState(
  ...
  const fixedPassThroughPlan = passThroughViewportPlanForState(state);
  if (fixedPassThroughPlan) return fixedPassThroughPlan.viewport;
...
type ConfiguredSemanticVisibleLocale = "en" | "zh" | "zh-Hans";

function configuredSemanticVisibleLocale(
  strings: ConfiguredSemanticSecondaryStrings | undefined
): ConfiguredSemanticVisibleLocale {
  if (strings?.locale) return strings.locale;
  return strings?.checkLabel === "驗證"
    ? "zh"
...
function localizedPassThroughVisibleText(
  state: ConfiguredSemanticSecondaryMathState,
  locale: ConfiguredSemanticVisibleLocale
): { check: string; formula: string; readout: string | null } | null {
  if (state.variant === "statistics-s1" || state.variant === "data-handling") {
    const mean = fixed(metric(state, "mean"));
    const spread = fixed(metric(state, "spread"));
    if (locale === "zh") {
      return {
  ...
  if (state.variant === "advanced-functions") {
    const coefficient = fixed(metric(state, "scaleParameter"));
    const shift = fixed(metric(state, "verticalShift"));
    const family = locale === "en" ? "quadratic" : locale === "zh" ? "二次函數" : "二次函数";
    const selected = locale === "en" ? "selected " : locale === "zh" ? "已選" : "已选";
    return {
      check: `${selected}${family} · a=${coefficient} · k=${shift}`,
      formula: state.formula,
      readout: `${family} · a=${coefficient} · k=${shift}`
    };
  }

  if (state.variant === "differentiation-intro" || state.variant === "calculus") {
    const probeX = fixed(metric(state, "probeX")).replace(/^-/, "−");
    const slope = fixed(metric(state, "tangentSlope")).replace(/^-/, "−");
    const slopeLabel = locale === "en" ? "tangent slope" : locale === "zh" ? "切線斜率" : "切线斜率";
    return {
      check: `x₀=${probeX} · ${slopeLabel}=${slope}`,
      formula: state.formula,
      readout: `x₀=${probeX} · ${slopeLabel}=${slope}`
    };
  }
...
function renderFixedPassThroughViewport(
  theme: ConfiguredSemanticSecondaryVizTheme,
  plan: ConfiguredSemanticSecondaryPassThroughViewportPlan
) {
  return (
    <g aria-hidden="true">
      <rect
        data-viz-mark
        data-viz-name="semantic fixed viewport"
        data-viz-viewport-id={plan.id}
        data-viz-domain-x-min={plan.viewport.xMinimum}
        data-viz-domain-x-max={plan.viewport.xMaximum}
        data-viz-domain-y-min={plan.viewport.yMinimum}
        data-viz-domain-y-max={plan.viewport.yMaximum}
        data-viz-x-ticks={plan.xTicks.join(",")}
        data-viz-y-ticks={plan.yTicks.join(",")}
        x={secondaryFrame.left}
        y={secondaryFrame.top}
        width={secondaryFrame.right - secondaryFrame.left}
        height={secondaryFrame.bottom - secondaryFrame.top}
        fill="none"
        stroke={theme.grid}
        strokeWidth="1"
      />
      {plan.xTicks.map((tick) => {
        const x = mapPoint({ x: tick, y: 0 }, plan.viewport).x;
        return (
          <g key={`fixed-x-${tick}`}>
            <line
              data-viz-mark
              data-viz-name="semantic x tick"
              data-viz-tick-axis="x"
              data-viz-tick-value={tick}
              x1={x}
              x2={x}
              y1={secondaryFrame.top}
              y2={secondaryFrame.bottom}
              stroke={theme.grid}
              strokeWidth="1"
            />
            <text x={x} y={secondaryFrame.bottom - 4} textAnchor="middle" fill={theme.textMuted} className="text-[8px] font-bold">{fixed(tick)}</text>
          </g>
        );
      })}
      {plan.yTicks.map((tick) => {
        const y = mapPoint({ x: 0, y: tick }, plan.viewport).y;
        return (
          <g key={`fixed-y-${tick}`}>
            <line
              data-viz-mark
              data-viz-name="semantic y tick"
              data-viz-tick-axis="y"
              data-viz-tick-value={tick}
              x1={secondaryFrame.left}
              x2={secondaryFrame.right}
              y1={y}
              y2={y}
              stroke={theme.grid}
              strokeWidth="1"
            />
            <text x={secondaryFrame.left + 4} y={y - 3} fill={theme.textMuted} className="text-[8px] font-bold">{fixed(tick)}</text>
          </g>
        );
      })}
      {renderAxes(theme, plan.viewport)}
    </g>
  );
}
...
function renderSemanticBody(
  ...
  const noModelLabel = strings?.noModelLabel;
  if (state.active) return renderSemanticBody(state.active, accent, theme, strings);
  ...
  if (["advanced-functions", "linear", "reciprocal", "quadratic", "properties", "exp-log", "sequence", "calculus", "regression", "distribution"].includes(state.kind)) {
    ...
    const fixedPassThroughViewport = passThroughViewportPlanForState(state);
    const visibleLocale = configuredSemanticVisibleLocale(strings);
    const localizedPassThrough = localizedPassThroughVisibleText(state, visibleLocale);
    ...
    const leftSpreadPoint = summaryDistribution ? state.points.leftSpread : null;
    const rightSpreadPoint = summaryDistribution ? state.points.rightSpread : null;
    const mappedLeftSpreadPoint = leftSpreadPoint ? mapPoint(leftSpreadPoint, viewport) : null;
    const mappedRightSpreadPoint = rightSpreadPoint ? mapPoint(rightSpreadPoint, viewport) : null;
    const advancedSampleIndexes = state.kind === "advanced-functions" && state.series.length
      ? [0, Math.floor(state.series.length / 2), state.series.length - 1]
      : [];
    return (
      <g>
        {fixedPassThroughViewport
          ? renderFixedPassThroughViewport(theme, fixedPassThroughViewport)
          : renderAxes(theme, viewport)}
        {summaryDistribution && mappedLeftSpreadPoint && mappedRightSpreadPoint ? (
          <rect
            data-viz-mark
            data-viz-name="distribution spread band"
            data-viz-mean={metric(state, "mean")}
            data-viz-spread={metric(state, "spread")}
            data-viz-left-value={leftSpreadPoint?.x}
            data-viz-right-value={rightSpreadPoint?.x}
            data-viz-left-x={mappedLeftSpreadPoint.x}
            data-viz-right-x={mappedRightSpreadPoint.x}
            x={mappedLeftSpreadPoint.x}
            y={secondaryFrame.bottom - 13}
            width={mappedRightSpreadPoint.x - mappedLeftSpreadPoint.x}
            height="10"
            rx="5"
            fill={accent}
            fillOpacity="0.24"
        ...
        {summaryDistribution
          ? <path data-viz-mark data-viz-name="distribution summary curve" data-viz-mean={metric(state, "mean")} data-viz-spread={metric(state, "spread")} data-viz-symmetry-residual={metric(state, "symmetryResidual")} data-viz-sample-count={state.series.length} data-viz-left-spread-x={mappedLeftSpreadPoint?.x} data-viz-left-spread-y={mappedLeftSpreadPoint?.y} data-viz-right-spread-x={mappedRightSpreadPoint?.x} data-viz-right-spread-y={mappedRightSpreadPoint?.y} d={path} fill="none" stroke={accent} strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
          : state.kind === "advanced-functions"
            ? <path data-viz-mark data-viz-name="advanced primary curve" data-viz-function-family={String(state.metrics.primaryFamily)} data-viz-scale-parameter={metric(state, "scaleParameter")} data-viz-vertical-shift={metric(state, "verticalShift")} data-viz-formula={String(state.metrics.primaryFormula)} data-viz-sample-count={state.series.length} data-viz-domain-min={state.series[0]?.x} data-viz-domain-max={state.series[state.series.length - 1]?.x} data-viz-sample-start-x={points[0]?.x} data-viz-sample-start-y={points[0]?.y} data-viz-sample-middle-x={points[Math.floor(points.length / 2)]?.x} data-viz-sample-middle-y={points[Math.floor(points.length / 2)]?.y} data-viz-sample-end-x={points[points.length - 1]?.x} data-viz-sample-end-y={points[points.length - 1]?.y} d={path} fill="none" stroke={accent} strokeWidth="5" strokeLinecap="round" strokeLinejoin="round" />
          ...
          : state.kind === "reciprocal"
            ...
            : <path
                data-viz-mark
                data-viz-name={`${state.kind} curve`}
                data-viz-curvature={hkCalculus ? metric(state, "curvature") : undefined}
                data-viz-vertical-shift={hkCalculus ? metric(state, "verticalShift") : undefined}
                data-viz-domain-min={hkCalculus ? state.series[0]?.x : undefined}
                data-viz-domain-max={hkCalculus ? state.series[state.series.length - 1]?.x : undefined}
                data-viz-sample-count={hkCalculus ? state.series.length : undefined}
                data-viz-probe-mapped-x={hkCalculus ? mappedTangentPoint?.x : undefined}
                data-viz-probe-mapped-y={hkCalculus ? mappedTangentPoint?.y : undefined}
                d={path}
                fill="none"
                stroke={accent}
                strokeWidth="5"
                ...
                strokeLinejoin="round"
              />}
        {state.kind === "advanced-functions" ? advancedSampleIndexes.map((index) => {
          const logical = state.series[index];
          const point = points[index];
          return <circle key={index} data-viz-mark data-viz-name="advanced curve sample" data-viz-sample-index={index} data-viz-x={logical.x} data-viz-y={logical.y} data-viz-mapped-x={point.x} data-viz-mapped-y={point.y} cx={point.x} cy={point.y} r="4" fill={theme.labelText} stroke={theme.pointStroke} strokeWidth="1.5" />;
        }) : null}
        {summaryDistribution && leftSpreadPoint && mappedLeftSpreadPoint ? <circle data-viz-mark data-viz-name="distribution left spread marker" data-viz-series-index="8" data-viz-x-value={leftSpreadPoint.x} data-viz-density={leftSpreadPoint.y} data-viz-series-y={state.series[8]?.y} data-viz-mapped-x={mappedLeftSpreadPoint.x} data-viz-mapped-y={mappedLeftSpreadPoint.y} cx={mappedLeftSpreadPoint.x} cy={mappedLeftSpreadPoint.y} r="6" fill={accent} stroke={theme.pointStroke} strokeWidth="2" /> : null}
        {summaryDistribution && rightSpreadPoint && mappedRightSpreadPoint ? <circle data-viz-mark data-viz-name="distribution right spread marker" data-viz-series-index="16" data-viz-x-value={rightSpreadPoint.x} data-viz-density={rightSpreadPoint.y} data-viz-series-y={state.series[16]?.y} data-viz-mapped-x={mappedRightSpreadPoint.x} data-viz-mapped-y={mappedRightSpreadPoint.y} cx={mappedRightSpreadPoint.x} cy={mappedRightSpreadPoint.y} r="6" fill={accent} stroke={theme.pointStroke} strokeWidth="2" /> : null}
        {state.kind === "advanced-functions" ? <text data-viz-mark data-viz-name="advanced function readout" data-viz-primary-family={String(state.metrics.primaryFamily)} data-viz-selected-curve-only="true" data-viz-scale-parameter={metric(state, "scaleParameter")} data-viz-vertical-shift={metric(state, "verticalShift")} data-viz-formula={String(state.metrics.primaryFormula)} data-viz-visible-locale={visibleLocale} data-viz-visible-readout={localizedPassThrough?.readout ?? state.check} data-viz-token-a={fixed(metric(state, "scaleParameter"))} data-viz-token-k={fixed(metric(state, "verticalShift"))} x="320" y="108" textAnchor="middle" fill={theme.text} className="text-[11px] font-black">{localizedPassThrough?.readout ?? state.check}</text> : null}
        {summaryDistribution ? <line data-viz-mark data-viz-name="distribution mean" data-viz-mean={metric(state, "mean")} data-viz-series-index="12" x1={mapPoint({ x: metric(state, "mean"), y: 0 }, viewport).x} x2={mapPoint({ x: metric(state, "mean"), y: 1 }, viewport).x} y1={mapPoint({ x: metric(state, "mean"), y: 0 }, viewport).y} y2={mapPoint({ x: metric(state, "mean"), y: 1 }, viewport).y} stroke={theme.labelText} strokeWidth="3" strokeDasharray="5 4" /> : null}
        {summaryDistribution ? <text data-viz-mark data-viz-name="distribution summary readout" data-viz-mean={metric(state, "mean")} data-viz-spread={metric(state, "spread")} data-viz-visible-locale={visibleLocale} data-viz-visible-readout={localizedPassThrough?.readout ?? state.check} data-viz-token-mean={fixed(metric(state, "mean"))} data-viz-token-spread={fixed(metric(state, "spread"))} x="320" y="108" textAnchor="middle" fill={theme.text} className="text-[11px] font-black">{localizedPassThrough?.readout ?? state.check}</text> : null}
        ...
        {state.kind === "calculus" && tangentLine.length && (!hkCalculus || activeCalculusMode === "tangent") ? <path data-viz-mark data-viz-name="tangent line" data-viz-slope={metric(state, "tangentSlope")} data-viz-anchor-x={tangentPoint?.x} data-viz-anchor-y={tangentPoint?.y} data-viz-rendered-start-x={mapPoint(tangentLine[0], viewport).x} data-viz-rendered-start-y={mapPoint(tangentLine[0], viewport).y} data-viz-rendered-end-x={mapPoint(tangentLine[1], viewport).x} data-viz-rendered-end-y={mapPoint(tangentLine[1], viewport).y} d={pathForSeries(tangentLine, viewport)} fill="none" stroke={theme.labelText} strokeWidth="4" /> : null}
        ...
        {hkCalculus && mappedTangentPoint ? <circle data-viz-mark data-viz-name="calculus probe point" data-viz-x0={metric(state, "x0")} data-viz-y0={metric(state, "functionValue")} data-viz-tangent-slope={metric(state, "tangentSlope")} data-viz-mapped-x={mappedTangentPoint.x} data-viz-mapped-y={mappedTangentPoint.y} cx={mappedTangentPoint.x} cy={mappedTangentPoint.y} r="7" fill={accent} stroke={theme.pointStroke} strokeWidth="2" /> : null}
        ...
        {hkCalculus ? <text data-viz-mark data-viz-name="calculus mode readout" data-viz-active-mode={activeCalculusMode} data-viz-curvature={metric(state, "curvature")} data-viz-x0={metric(state, "probeX")} data-viz-y0={metric(state, "functionValue")} data-viz-tangent-slope={metric(state, "tangentSlope")} data-viz-formula={state.formula} data-viz-visible-locale={visibleLocale} data-viz-visible-readout={localizedPassThrough?.readout ?? state.check} data-viz-token-a={fixed(metric(state, "curvature"))} data-viz-token-x0={fixed(metric(state, "probeX")).replace(/^-/, "−")} data-viz-token-slope={fixed(metric(state, "tangentSlope")).replace(/^-/, "−")} data-viz-x1={activeCalculusMode === "secant" ? metric(state, "x1") : undefined} data-viz-secant-slope={activeCalculusMode === "secant" ? metric(state, "secantSlope") : undefined} data-viz-lower-bound={activeCalculusMode === "area" ? metric(state, "lowerBound") : undefined} data-viz-exact-integral={activeCalculusMode === "area" ? metric(state, "exactIntegral") : undefined} data-viz-midpoint-approximation={activeCalculusMode === "area" ? metric(state, "midpointApproximation") : undefined} data-viz-signed-error={activeCalculusMode === "area" ? metric(state, "signedError") : undefined} data-viz-absolute-error={activeCalculusMode === "area" ? metric(state, "absoluteError") : undefined} x="320" y="108" textAnchor="middle" fill={theme.text} className="text-[10px] font-black">{localizedPassThrough?.readout ?? state.check}</text> : null}
      </g>
    );
  }
...
function visibleSecondaryStateText(
  ...
  const locale = configuredSemanticVisibleLocale(strings);
  const localizedPassThrough = localizedPassThroughVisibleText(state, locale);
  if (localizedPassThrough) {
    return {
      check: localizedPassThrough.check,
      formula: localizedPassThrough.formula
    };
  }
  if (state.kind !== "relative-position") {
...
export function ConfiguredSemanticSecondaryMarks(
  ...
  const visibleLocale = configuredSemanticVisibleLocale(props.strings);
  ...
  return (
    ...
    >
      ...
      <g data-viz-mark data-viz-name="semantic model body">
        {externalMarks ?? renderSemanticBody(state, props.accent, props.vizTheme, props.strings)}
      </g>
      {!externalComposite ? (
        <>
          <g
            data-viz-overlap-ok="semantic-formula-overlap-pair"
            data-viz-overlap-reason="formula label is intentionally contained by its dedicated formula background"
          >
            <path
              data-viz-mark
              data-viz-overlap-member="mark"
              data-viz-name="semantic formula background"
              d="M 87 286 H 553 Q 568 286 568 301 V 313 H 72 V 301 Q 72 286 87 286 Z"
              fill={props.vizTheme.labelFill}
              stroke="none"
            />
            <text
              data-viz-label
              data-viz-overlap-member="label"
              data-viz-name="semantic formula"
              data-viz-visible-formula={visibleStateText.formula}
              data-viz-visible-locale={visibleLocale}
              x="320"
              y="307"
              textAnchor="middle"
          ...
          <g
            data-viz-overlap-ok="semantic-check-overlap-pair"
            data-viz-overlap-reason="check label is intentionally contained by its dedicated check background"
          >
            <path
              data-viz-mark
              data-viz-overlap-member="mark"
              data-viz-name="semantic invariant check background"
              d="M 72 313 H 568 V 325 Q 568 340 553 340 H 87 Q 72 340 72 325 Z"
              fill={props.vizTheme.labelFill}
              stroke="none"
            />
            <text
          ...
          <rect
            aria-hidden="true"
            data-viz-name="semantic formula and check outline"
            x="72"
            y="286"
            width="496"
            height="54"
