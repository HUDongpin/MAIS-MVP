...
const hongKongAdvancedFunctionsSliders = [
  slider(
    "quadratic-coefficient-a",
    "value",
    localized("Quadratic coefficient a", "二次項係數 a", "二次项系数 a"),
    1,
    10,
    5
  ),
...
export type ConfiguredVisualizationSemanticResetPlan = Readonly<{
  comparison: number;
  height: number;
  mode: number;
  value: number;
...
export const configuredVisualizationPassThroughResetPlans = Object.freeze({
...
} as const satisfies Readonly<Record<string, ConfiguredVisualizationSemanticResetPlan>>);

/**
 * Resolve one deterministic reset vector from an explicit topic plan or the
 * declared control initials. No UI-owned 5/4/0 fallback is permitted here.
 */
export function getConfiguredVisualizationSemanticResetPlan(
  contract: ConfiguredVisualizationSemanticControlContract | undefined,
  topicId: string
): ConfiguredVisualizationSemanticResetPlan {
  const exact = configuredVisualizationPassThroughResetPlans[
    topicId as keyof typeof configuredVisualizationPassThroughResetPlans
  ];
  if (exact) return exact;

  const initial = (id: ConfiguredVisualizationSemanticSliderInput) =>
    contract?.sliders.find((control) => control.id === id)?.initial ?? 0;
...
export function getConfiguredVisualizationSemanticControlContract(
  ...
  if (base?.family === "function-properties" && variant === "advanced-functions") {
    return { ...base, modes: [], sliders: hongKongAdvancedFunctionsSliders };
  }
