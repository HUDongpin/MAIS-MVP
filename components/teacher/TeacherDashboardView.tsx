  ...
  return (
    <Link href={item.href} className="focus-ring soft-panel block p-4 transition hover:-translate-y-0.5 hover:shadow-glow">
      ...
      <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-slate-500 dark:text-slate-400">
        ...
        <span>{t({ en: "Updated", zh: "更新" })} <LocalDateTime value={item.createdAt} language={language} /></span>
      </div>
    </Link>
  );
}
...
export function TeacherDashboardView({ dashboard }: { dashboard: TeacherDashboardData }) {
  ...
  const enterpriseWorkflows: {
    ...
    },
    {
      ...
      title: t({ en: "Send school notices", zh: "發送校務通知" }),
      detail: t({ en: "Create school notices, track delivery, and collect parent or student acknowledgements.", zh: "建立校務通知，追蹤送達情況，收集家長或學生回執。" }),
      signal: t({ en: `${dashboard.kpis.unrepliedMessages} open`, zh: `${dashboard.kpis.unrepliedMessages} 未回覆` }),
      href: "/teacher/operations/notices",
      cta: t({ en: "Open school notices", zh: "前往校務通知" }),
      zone: "records",
      icon: "inbox",
      signalStatus: countStatus(dashboard.kpis.unrepliedMessages)
    },
    {
      eyebrow: t({ en: "Wrap up", zh: "結算" }),
      title: t({ en: "Save reports and archive", zh: "保存報告與歸檔" }),
      detail: t({ en: "Export bilingual reports, save term snapshots, and keep organized class records.", zh: "匯出雙語報告，保存學期快照，維持整齊的班級記錄。" }),
      signal: t({ en: "Records ready", zh: "記錄齊備" }),
      ...
      <section className="glass-panel p-6 sm:p-8">
        ...
        <div className="mt-4 grid gap-6 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-end">
          ...
          <p className="rounded-full border border-slate-200/80 bg-white/75 px-4 py-2 text-xs font-black text-slate-500 dark:border-white/10 dark:bg-white/[0.07] dark:text-slate-300">
            {t({ en: "Updated", zh: "更新" })} <LocalDateTime value={dashboard.generatedAt} language={language} />
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_390px]">
        <div data-tour="kpis">
          <div className="mb-4">
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">{t({ en: "Today's numbers", zh: "今日數據" })}</h2>
          </div>
        ...
        <aside data-tour="action-queue">
          <div className="mb-4">
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">{t({ en: "What needs attention", zh: "需要跟進事項" })}</h2>
          </div>
        ...
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.22em] text-slate-500 dark:text-slate-400">
              {t({ en: "Teaching workflow", zh: "教學工作流" })}
            </p>
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">
              {t({ en: "Plan, teach, and follow up", zh: "備課、授課、跟進" })}
            </h2>
      ...
      <section className="glass-panel p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">{t({ en: "Reward queue", zh: "積分獎勵隊列" })}</h2>
          </div>
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">{t({ en: "Class learning status", zh: "班級學習狀態" })}</h2>
          </div>
        </div>
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-bold text-slate-950 dark:text-white">{t({ en: "Weak topics by class", zh: "按班級顯示薄弱課題" })}</h2>
          </div>
        </div>
