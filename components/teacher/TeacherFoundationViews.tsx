...
export function TeacherOverview({ foundation }: { foundation: TeacherFoundationData }) {
  ...
  const metricCards = [
    {
      label: t({ en: "Classes", zh: "班級" }),
      value: foundation.totals.classes,
      detail: t({ en: "Active teacher spaces", zh: "可管理教學空間" })
    },
    {
      label: t({ en: "Students", zh: "學生" }),
      value: foundation.totals.students,
      detail: t({ en: "Enrolled across classes", zh: "已加入班級學生" })
    },
    {
      label: t({ en: "Active assignments", zh: "進行中作業" }),
      value: foundation.totals.activeAssignments,
      detail: t({ en: "Ready for completion tracking", zh: "可追蹤完成進度" })
    },
    {
      label: t({ en: "Unread inbox", zh: "未讀私信" }),
      value: foundation.totals.unreadMessages,
      detail: t({ en: "Needs teacher response", zh: "需要教師回覆" })
    }
  ];
    ...
    <div className="grid gap-6">
      <section className="glass-panel overflow-hidden p-6 sm:p-8">
        <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">
          {t({ en: "Teacher foundation", zh: "教師端基礎架構" })}
        </p>
        <div className="mt-4 grid gap-6 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
          ...
          >
            {t({ en: "Review assignments", zh: "查看作業" })}
          </Link>
        </div>
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="glass-panel p-5 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Recent assignments", zh: "最近作業" })}</h2>
            <Link href="/teacher/assignments" className="text-sm font-black text-cyan-700 dark:text-cyan-200">
              {t({ en: "Open all", zh: "查看全部" })}
            </Link>
          </div>
          <div className="mt-5 grid gap-3">
            {foundation.recentAssignments.length ? (
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                        {text(assignmentContentTypeLabels[assignment.contentType])} · {t({ en: "Due", zh: "截止" })} {formatDate(assignment.dueAt, language)}
                      </p>
                    </div>
              ...
              <EmptyState title={t({ en: "No assignments yet", zh: "尚未有作業" })} body={t({ en: "New assignments will appear here.", zh: "新作業會顯示在這裡。" })} />
            )}
          </div>
        </div>

        <aside className="glass-panel p-5 sm:p-6">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Inbox preview", zh: "私信預覽" })}</h2>
          <div className="mt-5 grid gap-3">
            {foundation.inboxPreview.length ? (
              foundation.inboxPreview.map((message) => <MessageCard key={message.id} message={message} />)
            ) : (
              <EmptyState title={t({ en: "Inbox clear", zh: "暫無待覆私信" })} body={t({ en: "Student messages will land here.", zh: "學生私信會顯示在這裡。" })} />
            )}
          </div>
        </aside>
      </section>
    ...
    <section className="glass-panel p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-white">{t({ en: "Teacher class spaces", zh: "教師班級空間" })}</h1>
        </div>
  ...
  return (
    <section className="glass-panel p-5 sm:p-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">
            {t({ en: "Assignments", zh: "作業" })}
          </p>
          <h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-white">{t({ en: "Assignment foundation", zh: "作業分派基礎" })}</h1>
        </div>
        <button className="focus-ring rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="button">
          {t({ en: "New assignment", zh: "新增作業" })}
        </button>
      </div>
          ...
          assignments.map((assignment) => (
            <article key={assignment.id} className="soft-panel p-4">
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_150px_120px] lg:items-center">
                ...
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{t({ en: "Due", zh: "截止" })}</p>
                ...
                <div className="lg:text-right">
                  ...
                  <p className="text-xs font-bold text-slate-500 dark:text-slate-400">{t({ en: "completed", zh: "已完成" })}</p>
                </div>
              </div>
            </article>
          ))
        ) : (
          <EmptyState title={t({ en: "No assignments yet", zh: "尚未有作業" })} body={t({ en: "Create the first teacher assignment from this workspace.", zh: "可在此工作台建立第一份教師作業。" })} />
        )}
      </div>
    </section>
  );
  ...
  return (
    <article className="soft-panel p-4">
      ...
      <p className="mt-3 text-xs font-bold text-slate-500 dark:text-slate-400">
        {message.priority === "urgent" ? t({ en: "Urgent", zh: "緊急" }) : t({ en: "Normal priority", zh: "一般優先" })}
      </p>
    </article>
  );
}
...
export function TeacherInboxView({ messages }: { messages: TeacherMessage[] }) {
  ...
  return (
    <section className="glass-panel p-5 sm:p-6">
      <div>
        <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">
          {t({ en: "Student inbox", zh: "學生私信" })}
        </p>
        <h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-white">{t({ en: "Teacher response queue", zh: "教師回覆佇列" })}</h1>
      </div>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {messages.length ? (
          ...
          <EmptyState title={t({ en: "No messages", zh: "暫無私信" })} body={t({ en: "Student help requests will appear here.", zh: "學生求助會顯示在這裡。" })} />
        )}
      </div>
    </section>
  );
