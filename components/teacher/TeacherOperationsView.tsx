...
const tabs: Array<{ id: OperationsTab; label: { en: string; zh: string } }> = [
  { id: "notices", label: { en: "Notice receipts", zh: "通知回執" } },
  { id: "reminders", label: { en: "Missing work", zh: "未交提醒" } },
  { id: "roster", label: { en: "Roster & seats", zh: "花名冊與座位" } },
  { id: "collaboration", label: { en: "Collaboration", zh: "協作與備課組" } },
  { id: "archive", label: { en: "Term archive", zh: "學期歸檔" } },
  { id: "ai-governance", label: { en: "AI governance", zh: "AI 治理" } }
];
...
const prepTeamShareKindOptions: Array<{ value: PrepTeamShareKind; label: { en: string; zh: string } }> = [
  { value: "resource", label: { en: "Resource", zh: "教學資源" } },
  { value: "assessment", label: { en: "Assessment", zh: "測驗評估" } },
  { value: "lesson-kit", label: { en: "Lesson kit", zh: "備課包" } },
  { value: "note", label: { en: "Note", zh: "備註" } }
];
  ...
  return (
    <article id={`notice-${notice.id}`} className="soft-panel min-w-0 scroll-mt-24 p-4">
      ...
      <div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-4">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Ack", zh: "回執" })}</p>
        ...
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Due", zh: "截止" })}</p>
          <p className="mt-1 text-sm font-bold text-slate-600 dark:text-slate-300">{formatDate(notice.dueAt, language)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Last send", zh: "最近發送" })}</p>
      ...
      <div className="mt-4 min-w-0 max-w-full overflow-x-auto">
        <table className="w-full min-w-[560px] text-left text-xs">
          <thead className="uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
            <tr>
              <th className="py-2">{t({ en: "Student", zh: "學生" })}</th>
              <th>{t({ en: "Guardian", zh: "家長" })}</th>
              <th>{t({ en: "Status", zh: "狀態" })}</th>
              <th>{t({ en: "Acknowledged", zh: "確認時間" })}</th>
            </tr>
          </thead>
  ...
  return (
    <section className="glass-panel min-w-0 border-emerald-300/45 bg-emerald-400/10 p-4">
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">{t({ en: "Notice ready", zh: "通知已就緒" })}</p>
          ...
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {notice.className} · {notice.acknowledgement.pending}/{notice.acknowledgement.total} {t({ en: "pending acknowledgements", zh: "待回執" })}
          </p>
        </div>
      ...
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">
            {latestShare ? t({ en: "Share added", zh: "共享已加入" }) : t({ en: "Prep team ready", zh: "備課組已就緒" })}
          </p>
          <p className="mt-1 break-words font-black text-slate-950 dark:text-white">{text(team.name)}</p>
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {latestShare ? text(latestShare.title) : `${team.members.length} ${t({ en: "members", zh: "成員" })} · ${team.shares.length} ${t({ en: "shares", zh: "共享資料" })}`}
          </p>
        </div>
        <div className="flex min-w-0 flex-wrap gap-2">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {firstItem
              ...
              : t({ en: "No eligible missing-work reminders were generated.", zh: "本次沒有符合條件的未交提醒。" })}
          </p>
        </div>
      ...
      <div className="mt-4 flex min-w-0 flex-wrap gap-2">
        {firstItem ? (
          <a href={`#${missingWorkAnchorId(firstItem.assignmentId, firstItem.studentId)}`} className="focus-ring rounded-full bg-slate-950 px-4 py-2 text-xs font-black text-white dark:bg-white dark:text-slate-950">
            {t({ en: "Open missing work", zh: "查看未交項" })}
          </a>
        ) : null}
        {firstRun?.noticeId ? (
    ...
    <div className="min-w-0 max-w-full overflow-x-auto">
      <table className="w-full min-w-[860px] text-left text-sm">
        <thead className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
          <tr>
            <th className="py-3">{t({ en: "Student", zh: "學生" })}</th>
            <th>{t({ en: "Assignment", zh: "作業" })}</th>
            <th>{t({ en: "Due", zh: "截止" })}</th>
            <th>{t({ en: "Next threshold", zh: "下一提醒" })}</th>
            <th>{t({ en: "Last reminder", zh: "上次提醒" })}</th>
            <th className="text-right">{t({ en: "Action", zh: "操作" })}</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200/70 dark:divide-white/10">
          {items.map((item) => (
          ...
          {!items.length ? (
            <tr>
              <td colSpan={6} className="py-8 text-center text-sm font-bold text-slate-500 dark:text-slate-400">
                {t({ en: "No missing work for this class.", zh: "此班暫無未交作業。" })}
              </td>
            </tr>
          ) : null}
        </tbody>
...
function RosterCredentialPanel({
  ...
  return (
    <section className="glass-panel min-w-0 overflow-hidden p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300">{t({ en: "Account handoff", zh: "帳號交付" })}</p>
          <h2 className="mt-2 break-words text-xl font-black text-slate-950 dark:text-white">{t({ en: "Temporary login slips", zh: "臨時登入單" })}</h2>
          <p className="mt-2 break-words text-sm font-bold leading-6 text-slate-500 dark:text-slate-400">
            {t({
              ...
              zh: "這些登入資料只在本次導入結果中顯示；離開頁面前請複製或下載。"
            })}
          </p>
        </div>
      ...
      <div className="mt-5 min-w-0 max-w-full overflow-x-auto">
        <table className="w-full min-w-[820px] text-left text-sm">
          <thead className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
            <tr>
              <th className="py-3">{t({ en: "Role", zh: "角色" })}</th>
              <th>{t({ en: "Name", zh: "姓名" })}</th>
              <th>{t({ en: "Username", zh: "用戶名" })}</th>
              <th>{t({ en: "Temporary password", zh: "臨時密碼" })}</th>
              <th>{t({ en: "School / class", zh: "學校 / 班級" })}</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-200/70 dark:divide-white/10">
    ...
    <tr>
      ...
      <td colSpan={5}>
        <form onSubmit={submit} className="grid min-w-0 gap-2 sm:grid-cols-[90px_90px_80px_80px_80px_auto]">
          <input name="studentNo" defaultValue={row.studentNo ?? ""} aria-label={t({ en: "Student number", zh: "學號" })} className="focus-ring min-w-0 rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
          <input name="seatLabel" defaultValue={row.seatLabel ?? ""} aria-label={t({ en: "Seat label", zh: "座位" })} className="focus-ring min-w-0 rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
          <input name="seatRow" type="number" min="1" defaultValue={row.seatRow ?? ""} aria-label={t({ en: "Seat row", zh: "座位行" })} className="focus-ring min-w-0 rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
          <input name="seatColumn" type="number" min="1" defaultValue={row.seatColumn ?? ""} aria-label={t({ en: "Seat column", zh: "座位列" })} className="focus-ring min-w-0 rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
          <input name="displayOrder" type="number" min="1" defaultValue={row.displayOrder} aria-label={t({ en: "Display order", zh: "排序" })} className="focus-ring min-w-0 rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
          <button disabled={saving} className="focus-ring min-w-0 rounded-full bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950" type="submit">
            {saving ? t({ en: "Saving", zh: "儲存中" }) : t({ en: "Save", zh: "儲存" })}
          </button>
        </form>
      </td>
    </tr>
  ...
  return (
    <section className="mt-4 rounded-3xl border border-emerald-300/50 bg-emerald-400/10 p-4">
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">
            {t({ en: "Governance policy applied", zh: "治理策略已生效" })}
          </p>
          <p className="mt-1 break-words text-sm font-bold text-emerald-900 dark:text-emerald-100">
            {t({ en: "Server-confirmed policy is reloaded and ready for audit.", zh: "已重新載入服務端確認的策略，可供審計。" })}
          </p>
        </div>
      ...
      <div className="mt-4 grid min-w-0 gap-2 sm:grid-cols-2">
        <div className="soft-panel min-w-0 p-3">
          <p className="break-words text-xs font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Roles", zh: "角色" })}</p>
          <p className="mt-2 break-words text-sm font-black text-slate-950 dark:text-white">{policy.allowedRoles.join(", ") || "-"}</p>
        </div>
        <div className="soft-panel min-w-0 p-3">
          <p className="break-words text-xs font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Surfaces", zh: "範圍" })}</p>
          <p className="mt-2 break-words text-sm font-black text-slate-950 dark:text-white">{policy.enabledSurfaces.length} {t({ en: "enabled", zh: "已啟用" })}</p>
        </div>
        <div className="soft-panel min-w-0 p-3">
          <p className="break-words text-xs font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Limits", zh: "限制" })}</p>
          <p className="mt-2 break-words text-sm font-black text-slate-950 dark:text-white">
            {policy.maxSelectionLength} {t({ en: "chars", zh: "字元" })} · {policy.retentionDays} {t({ en: "days", zh: "日" })}
          </p>
        </div>
        <div className="soft-panel min-w-0 p-3">
          <p className="break-words text-xs font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Audit", zh: "審計" })}</p>
          <p className="mt-2 break-words text-sm font-black text-slate-950 dark:text-white">
            {policy.blockedPatterns.length} {t({ en: "blocked terms", zh: "阻擋詞" })} · {runCount} {t({ en: "runs loaded", zh: "run 已載入" })}
          </p>
        </div>
      </div>

      <div className="mt-4 flex min-w-0 flex-wrap gap-2">
        {(changedFields.length ? changedFields : [t({ en: "no field delta", zh: "無欄位差異" })]).map((field) => (
      ...
      <div className="mt-4 flex min-w-0 flex-wrap items-center justify-between gap-3">
        <p className="break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
          {t({ en: "Updated", zh: "更新時間" })}: {formatDate(policy.updatedAt, language)}
        </p>
        <div className="flex min-w-0 flex-wrap gap-2">
...
function NovaLensGovernancePanel() {
  ...
  async function loadGovernance(nextMessage = "") {
    ...
    try {
      ...
      setStatus("error");
      setMessage(t({ en: "Could not load AI Tutor governance data.", zh: "暫時未能載入 AI Tutor 治理資料。" }));
      return null;
    }
  }
  ...
  async function savePolicy(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const nextMessage = response.ok
        ? t({ en: "AI Tutor policy saved. Current settings reloaded.", zh: "AI Tutor 策略已儲存，現行設定已重新載入。" })
        : t({ en: "Could not save AI Tutor policy. Current settings reloaded.", zh: "暫時未能儲存 AI Tutor 策略，現行設定已重新載入。" });
      ...
      setStatus("error");
      setMessage(t({ en: "Could not save AI Tutor policy.", zh: "暫時未能儲存 AI Tutor 策略。" }));
    }
  }
  ...
  return (
    <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
      <div className="glass-panel min-w-0 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <h2 className="break-words text-xl font-black text-slate-950 dark:text-white">{t({ en: "AI Tutor governance", zh: "AI Tutor 治理" })}</h2>
            <p className="mt-1 break-words text-sm font-bold text-slate-500 dark:text-slate-400">
              {isAdmin
                ? t({ en: "Admin policy controls and redacted agent runs.", zh: "管理員策略控制與已遮蔽 agent run。" })
                : t({ en: "Read-only redacted AI Tutor agent history.", zh: "只讀已遮蔽 AI Tutor agent 紀錄。" })}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
          <OperationsMetric label={t({ en: "Completed", zh: "已完成" })} value={completedRuns} />
          <OperationsMetric label={t({ en: "Blocked", zh: "已阻擋" })} value={blockedRuns} />
          <OperationsMetric label={t({ en: "Fallback", zh: "後備回覆" })} value={fallbackRuns} />
        </div>
        ...
        {state && isAdmin ? (
          <form id="nova-policy-form" key={state.policy.updatedAt} onSubmit={savePolicy} className="mt-5 grid scroll-mt-24 gap-4">
            <label className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200/80 bg-white/70 p-3 text-sm font-black text-slate-800 dark:border-white/10 dark:bg-white/[0.06] dark:text-slate-100">
              <span>{t({ en: "AI Tutor enabled", zh: "啟用 AI Tutor" })}</span>
              <input name="enabled" type="checkbox" defaultChecked={state.policy.enabled} className="h-5 w-5 accent-cyan-600" />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-2">
                <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Max selection", zh: "最長選取" })}</span>
              ...
              <label className="grid gap-2">
                <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Retention days", zh: "保留日數" })}</span>
            ...
            <div className="grid gap-2">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Allowed roles", zh: "允許角色" })}</p>
              <div className="flex flex-wrap gap-2">
            ...
            <div className="grid gap-2">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Enabled surfaces", zh: "啟用範圍" })}</p>
              <div className="flex flex-wrap gap-2">
            ...
            <label className="grid gap-2">
              <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Blocked patterns", zh: "阻擋字詞" })}</span>
            ...
            <button type="submit" disabled={status === "saving"} className="focus-ring rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950">
              {status === "saving" ? t({ en: "Saving…", zh: "儲存中…" }) : t({ en: "Save policy", zh: "儲存策略" })}
            </button>
          </form>
        ) : null}
      </div>
      ...
      <div id="nova-run-history" className="glass-panel min-w-0 scroll-mt-24 overflow-hidden p-5">
        <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Redacted run history", zh: "已遮蔽 run 紀錄" })}</h2>
        <div className="mt-4 min-w-0 max-w-full overflow-x-auto">
          <table className="w-full min-w-[920px] text-left text-sm">
            <thead className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
              <tr>
                <th className="py-3">{t({ en: "Time", zh: "時間" })}</th>
                <th>{t({ en: "User", zh: "用戶" })}</th>
                <th>{t({ en: "Surface", zh: "範圍" })}</th>
                <th>{t({ en: "Action", zh: "動作" })}</th>
                <th>{t({ en: "Status", zh: "狀態" })}</th>
                <th>{t({ en: "Preview", zh: "預覽" })}</th>
                <th>{t({ en: "Flags", zh: "旗標" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200/70 dark:divide-white/10">
              {state?.runs.map((run) => (
              ...
              {state && !state.runs.length ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-sm font-bold text-slate-500 dark:text-slate-400">
                    {t({ en: "No AI Tutor runs yet.", zh: "暫無 AI Tutor run。" })}
                  </td>
                </tr>
              ) : null}
              {!state && status === "loading" ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-sm font-bold text-slate-500 dark:text-slate-400">
                    {t({ en: "Loading AI Tutor runs…", zh: "正在載入 AI Tutor run…" })}
                  </td>
                </tr>
              ) : null}
              {!state && status === "error" ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-sm font-bold text-slate-500 dark:text-slate-400">
                    {t({ en: "AI Tutor governance data is unavailable.", zh: "AI Tutor 治理資料暫時未能使用。" })}
                  </td>
                </tr>
              ) : null}
            </tbody>
...
function ArchiveReadyPanel({ archive }: { archive: TermArchive }) {
  ...
  return (
    <section className="glass-panel min-w-0 border-emerald-300/45 bg-emerald-400/10 p-4">
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">{t({ en: "Archive ready", zh: "歸檔已就緒" })}</p>
          <p className="mt-1 break-words font-black text-slate-950 dark:text-white">{archive.termLabel}</p>
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {archive.className} · {archive.snapshot.studentCount} {t({ en: "students", zh: "學生" })} · {archive.snapshot.assignmentCount} {t({ en: "assignments", zh: "作業" })}
          </p>
        </div>
        <div className="flex min-w-0 flex-wrap gap-2">
...
export function TeacherOperationsView({
    ...
    setMessage(response.ok
      ? commit
        ? nextCredentials.length
          ? t({ en: `Roster import committed. ${nextCredentials.length} login slips are ready.`, zh: `花名冊已導入，已生成 ${nextCredentials.length} 份登入單。` })
          : t({ en: "Roster import committed. No new temporary accounts were created.", zh: "花名冊已導入，沒有新增臨時帳號。" })
        : t({ en: "Roster CSV validated.", zh: "CSV 驗證完成。" })
      : t({ en: "Roster import needs attention.", zh: "花名冊導入需要修正。" }));
  ...
  async function copyImportCredentials() {
    ...
    try {
      ...
      setCredentialStatus(t({ en: "Credential CSV copied.", zh: "登入單 CSV 已複製。" }));
    } catch {
      setCredentialStatus(t({ en: "Could not copy. Use Download CSV instead.", zh: "暫時未能複製，請改用下載 CSV。" }));
    }
  }
  ...
  function downloadImportCredentials() {
    ...
    setCredentialStatus(t({ en: "Credential CSV downloaded.", zh: "登入單 CSV 已下載。" }));
  }
  ...
  async function createPrepTeam(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(response.ok ? t({ en: "Prep team created.", zh: "備課組已建立。" }) : t({ en: "Could not create prep team.", zh: "暫時未能建立備課組。" }));
  ...
  async function createPrepTeamShare(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage(response.ok ? t({ en: "Prep team share added.", zh: "備課組共享已加入。" }) : t({ en: "Could not add prep team share.", zh: "暫時未能加入共享資料。" }));
    if (response.ok && payload?.team) {
    ...
    <div className="grid min-w-0 gap-6">
      <section className="glass-panel min-w-0 overflow-hidden p-5 sm:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="break-words text-sm font-black uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300 sm:tracking-[0.24em]">{t({ en: "Operations", zh: "校務落地" })}</p>
            <h1 className="mt-2 break-words text-2xl font-black text-slate-950 dark:text-white sm:text-3xl">{selectedClass ? selectedClass.name : t({ en: "Teacher operations", zh: "教師校務工作台" })}</h1>
          ...
          <div className="max-w-full rounded-2xl border border-cyan-300/45 bg-cyan-400/10 px-4 py-3 text-sm font-black text-cyan-800 dark:text-cyan-100">
            {data.wecom.enabled ? t({ en: "WeCom enabled", zh: "企業微信已啟用" }) : t({ en: "WeCom disabled", zh: "企業微信未啟用" })}
          </div>
        </div>

        <div className="mt-5 grid min-w-0 gap-3 md:grid-cols-5">
          <OperationsMetric label={t({ en: "Notices", zh: "通知" })} value={data.totals.notices} />
          <OperationsMetric label={t({ en: "Pending ack", zh: "待回執" })} value={data.totals.pendingAcknowledgements} />
          <OperationsMetric label={t({ en: "Missing work", zh: "未交" })} value={data.totals.missingWork} />
          <OperationsMetric label={t({ en: "Collaborators", zh: "協作者" })} value={data.totals.collaborators} />
          <OperationsMetric label={t({ en: "Archives", zh: "歸檔" })} value={data.totals.archives} />
        </div>

        <div className="mt-5 flex min-w-0 max-w-full gap-2 overflow-x-auto overscroll-x-contain pb-1" role="tablist" aria-label={t({ en: "Operations tabs", zh: "校務分頁" })}>
          {tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
      ...
      {activeTab === "notices" ? (
        <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
          <form onSubmit={createNotice} className="glass-panel grid min-w-0 h-fit gap-4 p-5">
            <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Create notice", zh: "建立通知" })}</h2>
            <label className="grid min-w-0 gap-2">
              <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "WeCom channel", zh: "企業微信群" })}</span>
            ...
            <label className="grid min-w-0 gap-2">
              <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Audience", zh: "對象" })}</span>
              <select name="audience" defaultValue="parents" className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-3 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]">
                <option value="parents">{t({ en: "Parents", zh: "家長" })}</option>
                <option value="students">{t({ en: "Students", zh: "學生" })}</option>
                <option value="both">{t({ en: "Parents and students", zh: "家長與學生" })}</option>
              </select>
            </label>
            <input name="subject" required placeholder={t({ en: "Notice subject", zh: "通知標題" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
            <textarea name="body" required rows={5} placeholder={t({ en: "Message body", zh: "通知內容" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
        ...
        <div className="grid min-w-0 gap-4">
          <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
            <div className="glass-panel grid min-w-0 h-fit gap-4 p-5">
              <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "CSV import", zh: "CSV 導入" })}</h2>
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={() => validateRoster(false)} className="focus-ring min-w-0 rounded-full border border-slate-200/80 bg-white/75 px-4 py-2 text-sm font-black text-slate-700 dark:border-white/10 dark:bg-white/[0.06] dark:text-slate-200">{t({ en: "Validate", zh: "驗證" })}</button>
                <button type="button" onClick={() => validateRoster(true)} className="focus-ring min-w-0 rounded-full bg-slate-950 px-4 py-2 text-sm font-black text-white dark:bg-white dark:text-slate-950">{t({ en: "Commit import", zh: "確認導入" })}</button>
              </div>
              {validation ? (
                <div className="soft-panel min-w-0 break-words p-4 text-sm font-bold text-slate-600 dark:text-slate-300">
                  {validation.totals.valid}/{validation.totals.rows} {t({ en: "valid rows", zh: "有效行" })} · {validation.totals.creates} {t({ en: "creates", zh: "新增" })} · {validation.totals.updates} {t({ en: "updates", zh: "更新" })}
                </div>
              ) : null}
            </div>
            <div className="glass-panel min-w-0 overflow-hidden p-5">
              <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Student numbers and seats", zh: "學號與座位表" })}</h2>
              <div className="mt-4 min-w-0 max-w-full overflow-x-auto">
                <table className="w-full min-w-[860px] text-left text-sm">
                  <thead className="text-xs uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">
                    <tr>
                      <th className="py-3">{t({ en: "Student", zh: "學生" })}</th>
                      <th>{t({ en: "Parent link", zh: "家長綁定" })}</th>
                      <th>{t({ en: "Student no / seat / row / column / order", zh: "學號 / 座位 / 行 / 列 / 排序" })}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-200/70 dark:divide-white/10">
      ...
      {activeTab === "collaboration" ? (
        <section className="grid min-w-0 gap-4 xl:grid-cols-2">
          <form onSubmit={addCollaborator} className="glass-panel grid min-w-0 h-fit gap-4 p-5">
            <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Class collaborators", zh: "任課教師協作" })}</h2>
            <input name="teacherUsername" required placeholder={t({ en: "Teacher username or email", zh: "教師用戶名或電郵" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
            <select name="role" defaultValue="co-teacher" className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]">
              <option value="co-teacher">{t({ en: "Co-teacher", zh: "協作教師" })}</option>
              <option value="viewer">{t({ en: "Viewer", zh: "只讀" })}</option>
            </select>
            <button className="focus-ring min-w-0 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="submit">{t({ en: "Save collaborator", zh: "儲存協作者" })}</button>
          ...
          <div className="grid min-w-0 gap-4">
            <form onSubmit={createPrepTeam} className="glass-panel grid min-w-0 h-fit gap-4 p-5">
              <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Prep team sharing", zh: "備課組共享" })}</h2>
              <input name="name" required placeholder={t({ en: "Prep team name", zh: "備課組名稱" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
              <input name="description" placeholder={t({ en: "Description", zh: "描述" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
              <select name="grade" defaultValue={selectedClass?.grade ?? ""} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]">
                <option value="">{t({ en: "No grade lock", zh: "不限年級" })}</option>
              ...
              <button className="focus-ring min-w-0 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="submit">{t({ en: "Create prep team", zh: "建立備課組" })}</button>
            </form>
            ...
            {selectedPrepTeam ? (
              <form onSubmit={createPrepTeamShare} className="glass-panel grid min-w-0 gap-4 p-5">
                <div className="min-w-0">
                  <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-cyan-600 dark:text-cyan-300">{t({ en: "Share handoff", zh: "共享交付" })}</p>
                  <h2 className="mt-2 break-words text-xl font-black text-slate-950 dark:text-white">{t({ en: "Add to prep team", zh: "加入備課組" })}</h2>
                </div>
                <label className="grid min-w-0 gap-2">
                  <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Team", zh: "備課組" })}</span>
                ...
                <label className="grid min-w-0 gap-2">
                  <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Kind", zh: "類型" })}</span>
                ...
                <input name="title" required placeholder={t({ en: "Shared item title", zh: "共享項目標題" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
                <input name="targetId" placeholder={t({ en: "Optional resource, assessment, or lesson-kit id", zh: "可選資源、評估或備課包 ID" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
                <button className="focus-ring min-w-0 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="submit">{t({ en: "Add share", zh: "加入共享" })}</button>
              </form>
            ) : null}
            ...
            <section className="glass-panel min-w-0 p-5">
              <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-cyan-600 dark:text-cyan-300">{t({ en: "Prep workspace", zh: "備課工作區" })}</p>
                  <h2 className="mt-2 break-words text-xl font-black text-slate-950 dark:text-white">{t({ en: "Shared teams and materials", zh: "共享備課組與材料" })}</h2>
                </div>
                <p className="shrink-0 text-xs font-black text-slate-500 dark:text-slate-400">{visiblePrepTeams.length} {t({ en: "teams", zh: "備課組" })}</p>
              </div>
                ...
                {visiblePrepTeams.map((team) => (
                  <article id={`prep-team-${team.id}`} key={team.id} className="soft-panel min-w-0 scroll-mt-24 p-4">
                    ...
                    <div className="mt-3 grid min-w-0 gap-2">
                      ...
                      {!team.shares.length ? (
                        <p className="rounded-2xl border border-dashed border-slate-300/80 px-3 py-4 text-sm font-bold text-slate-500 dark:border-white/15 dark:text-slate-400">{t({ en: "No shared materials yet.", zh: "尚未有共享材料。" })}</p>
                      ) : null}
                    </div>
                  </article>
                ))}
                {!visiblePrepTeams.length ? (
                  <div className="soft-panel min-w-0 p-5 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "Create a prep team to start sharing materials.", zh: "建立備課組後即可共享材料。" })}</div>
                ) : null}
              </div>
            </section>
          </div>
      ...
      {activeTab === "archive" ? (
        <section className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,420px)_minmax(0,1fr)]">
          <form onSubmit={createArchive} className="glass-panel grid min-w-0 h-fit gap-4 p-5">
            <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Create read-only snapshot", zh: "建立只讀快照" })}</h2>
            <input name="termLabel" required placeholder={t({ en: "2025-2026 Term 1", zh: "2025-2026 上學期" })} className="focus-ring min-w-0 w-full rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 text-sm font-semibold dark:border-white/10 dark:bg-white/[0.06]" />
            <button className="focus-ring min-w-0 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="submit">{t({ en: "Archive term", zh: "歸檔學期" })}</button>
          </form>
            ...
            {visibleArchives.map((archive) => (
              <article id={`archive-${archive.id}`} key={archive.id} className="glass-panel min-w-0 scroll-mt-24 p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <Link href={archive.exportUrl} className="focus-ring shrink-0 rounded-full bg-slate-950 px-4 py-2 text-sm font-black text-white dark:bg-white dark:text-slate-950">{t({ en: "Download", zh: "下載" })}</Link>
                </div>
