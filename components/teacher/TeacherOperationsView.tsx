    ...
    <article id={`notice-${notice.id}`} className="soft-panel min-w-0 scroll-mt-24 p-4">
      ...
      <div className="mt-4 grid min-w-0 gap-3 sm:grid-cols-4">
        ...
        <div className="min-w-0 sm:text-right">
          ...
          >
            {busy ? t({ en: "Queueing…", zh: "正在加入隊列…", zhHans: "正在加入队列…" }) : notice.status === "sent" ? t({ en: "Sent", zh: "已發送", zhHans: "已发送" }) : notice.status === "draft" ? t({ en: "Send", zh: "發送", zhHans: "发送" }) : t({ en: "Retry", zh: "重試", zhHans: "重试" })}
          </button>
        </div>
      </div>
    ...
    <section className="glass-panel min-w-0 border-emerald-300/45 bg-emerald-400/10 p-4">
      <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex min-w-0 flex-wrap gap-2">
          ...
          >
            {busy ? t({ en: "Queueing…", zh: "正在加入隊列…", zhHans: "正在加入队列…" }) : notice.status === "sent" ? t({ en: "Sent", zh: "已發送", zhHans: "已发送" }) : notice.status === "draft" ? t({ en: "Send now", zh: "立即發送", zhHans: "立即发送" }) : t({ en: "Retry send", zh: "重新發送", zhHans: "重新发送" })}
          </button>
  ...
  return (
    <section role="status" className="mt-5 rounded-2xl border border-emerald-300/55 bg-emerald-400/12 p-4">
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">
            {t({ en: "Reminder requests queued", zh: "提醒請求已加入隊列", zhHans: "提醒请求已加入队列" })}
          </p>
          <p className="mt-1 break-words font-black text-slate-950 dark:text-white">
            {attemptedCount} {t({ en: "requests created", zh: "項請求已建立", zhHans: "项请求已建立" })}{skippedCount ? ` · ${skippedCount} ${t({ en: "skipped", zh: "已跳過", zhHans: "已跳过" })}` : ""}
          </p>
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {firstItem
          ...
          {items.map((item) => (
            <tr id={missingWorkAnchorId(item.assignmentId, item.studentId)} key={`${item.assignmentId}-${item.studentId}`} className="scroll-mt-24">
              ...
              <td className="text-right">
                ...
                >
                  {busy ? t({ en: "Running…", zh: "執行中…", zhHans: "执行中…" }) : t({ en: "Manual send", zh: "手動補發", zhHans: "手动补发" })}
                </button>
              </td>
            </tr>
          ))}
      ...
      if (result.ok) {
        ...
        setMessage(result.payload.email.status === "no-eligible"
          ? t({
              ...
              zh: "通知請求已獲接受，但沒有符合條件的家庭電郵收件人。請在通知詳情中查看渠道狀態。", zhHans: "通知请求已获接受，但没有符合条件的家庭邮箱收件人。请在通知详情中查看渠道状态。"
            })
          : t({
              en: "Notice accepted and queued. Delivery has not been confirmed yet.",
              zh: "通知已獲接受並加入隊列，尚未確認完成送達。", zhHans: "通知已获接受并加入队列，尚未确认完成送达。"
            }), "status");
        router.refresh();
      } else {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setMessage(t({
          ...
          zh: "提醒請求已加入隊列，尚未確認完成送達。", zhHans: "提醒请求已加入队列，尚未确认完成送达。"
        }), "status");
        router.refresh();
      } else {
        ...
        <section className="glass-panel min-w-0 overflow-hidden p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button
              ...
              type="button"
            >
              {reminderBusy ? t({ en: "Running…", zh: "執行中…", zhHans: "执行中…" }) : t({ en: "Run due reminders", zh: "執行到期提醒", zhHans: "执行到期提醒" })}
            </button>
          </div>
