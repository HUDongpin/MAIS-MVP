  ...
  return (
    <div className="grid min-w-0 gap-6">
      <section className="glass-panel min-w-0 overflow-hidden p-5 sm:p-6">
        <p className="break-words text-sm font-black uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300 sm:tracking-[0.24em]">{t({ en: "Resources", zh: "資料庫" })}</p>
        <h1 className="mt-2 break-words text-2xl font-black text-slate-950 dark:text-white sm:text-3xl">{t({ en: "Teaching resources and papers", zh: "課件與試卷資料庫" })}</h1>
      ...
      <section className="glass-panel min-w-0 overflow-hidden p-5 sm:p-6">
        <h2 className="break-words text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Upload resource", zh: "上載資源" })}</h2>
          ...
          <div className="mt-4 rounded-2xl border border-emerald-300/60 bg-emerald-400/12 p-4 text-sm text-emerald-900 dark:text-emerald-100">
            <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                <p className="break-words text-xs font-black uppercase tracking-[0.14em]">{t({ en: "Resource ready", zh: "資源已就緒" })}</p>
              ...
              <div className="flex min-w-0 flex-wrap gap-2">
                ...
                <Link href={assignmentHrefForResource(uploadedResource)} className="focus-ring rounded-full bg-slate-950 px-4 py-2 text-xs font-black text-white dark:bg-white dark:text-slate-950">
                  {t({ en: "Create assignment", zh: "建立作業" })}
                </Link>
              </div>
            </div>
          </div>
      ...
      <section className="glass-panel min-w-0 overflow-hidden p-5 sm:p-6">
        <div className="flex min-w-0 flex-wrap items-end justify-between gap-4">
          <h2 className="break-words text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Resource library", zh: "資源列表" })}</h2>
          <div className="grid min-w-0 w-full gap-3 md:w-auto md:grid-cols-4">
            ...
            <label className="soft-panel flex min-w-0 items-center gap-2 px-3 py-2 text-sm font-black">
              ...
              <span className="min-w-0 break-words">{t({ en: "Recent", zh: "最近" })}</span>
            </label>
          </div>
        </div>
          ...
          <table className="w-full min-w-[1040px] text-left text-sm">
            <thead className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">
              <tr>
                ...
                <th className="py-3 pr-4">{t({ en: "Submissions", zh: "提交" })}</th>
                <th className="py-3">{t({ en: "Actions", zh: "操作" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200/80 dark:divide-white/10">
              {data.assessments.map((assessment) => {
                ...
                const assignmentHref = assignmentHrefForAssessment(
                  assessment,
                  t({ en: `${assessmentTitle} assignment`, zh: `${assessmentTitle} 測驗分派` })
                );
                return (
...
export function TeacherAssessmentNewView({ data, initialAssessment }: { data: TeacherAssessmentCreateData; initialAssessment?: Assessment }) {
  ...
  function addSection() {
    updateSections((current) => [...current, {
      ...
      title: { en: `Section ${current.length + 1}`, zh: `第 ${current.length + 1} 部分` },
      instructions: { en: "", zh: "" },
  ...
  function addManualQuestion() {
    if (!manualPrompt.trim() || !manualAnswer.trim()) {
      setError(t({ en: "Manual question and answer are required.", zh: "請填寫手動題目與答案。" }));
      return;
    }
  ...
  function importManualQuestions() {
    ...
    if (!parsed.length) {
      setError(t({ en: "No valid manual questions found.", zh: "未找到有效手動題目。" }));
      return;
    }
  ...
  async function generateQuestions() {
    ...
    if (!response.ok || !payload?.questions) {
      setError(payload?.error ?? t({ en: "AI question generation is unavailable.", zh: "AI 題目生成暫時不可用。" }));
      return;
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setError("");
    if (!currentItemCount) {
      setError(t({ en: "Add at least one question before saving.", zh: "請先加入至少一題。" }));
      return;
    }
    const trimmedTitle = assessmentTitle.trim();
    if (!trimmedTitle) {
      setActiveStep(0);
      setError(t({ en: "Assessment title is required.", zh: "請填寫測驗標題。" }));
      return;
    }
	  ...
	  return (
	    <section className="grid gap-6">
	      ...
	      <div className="mt-4 flex flex-wrap items-end justify-between gap-4">
	        <div>
	          <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">{t({ en: "Free paper builder", zh: "自由組卷" })}</p>
	          <h1 className="mt-2 text-3xl font-black text-slate-950 dark:text-white">{initialAssessment ? t({ en: "Edit assessment", zh: "編輯測驗" }) : t({ en: "Create assessment", zh: "建立測驗" })}</h1>
	        </div>
	        <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
	          <StatCard label={t({ en: "Questions", zh: "題目" })} value={String(currentItemCount)} detail={t({ en: "In paper", zh: "已加入" })} />
	          <StatCard label={t({ en: "Points", zh: "分數" })} value={String(currentTotalPoints)} detail={t({ en: "Total", zh: "總分" })} />
	        </div>
	      </div>
	      <div className="mt-5 flex flex-wrap gap-2">
	        {[
	          t({ en: "1. Setup", zh: "1. 設定" }),
	          t({ en: "2. Select", zh: "2. 選題" }),
	          t({ en: "3. Custom", zh: "3. 自訂" }),
	          t({ en: "4. Preview", zh: "4. 預覽" })
	        ].map((label, index) => (
	      ...
	      {!data.classes.length ? (
            ...
            <label className="grid gap-2 lg:col-span-2">
	            ...
	            <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Opens", zh: "開始" })}</span><input name="opensAt" type="datetime-local" value={opensAt} onChange={(event) => setOpensAt(event.target.value)} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
	            <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Closes", zh: "結束" })}</span><input name="closesAt" type="datetime-local" value={closesAt} onChange={(event) => setClosesAt(event.target.value)} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
		            <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Time limit", zh: "時限" })}</span><input name="timeLimitMinutes" type="number" min="0" value={timeLimitMinutes} onChange={(event) => setTimeLimitMinutes(event.target.value)} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
	            <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Weight", zh: "比重" })}</span><input name="gradeWeight" type="number" min="0" value={gradeWeight} onChange={(event) => setGradeWeight(event.target.value)} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
		            <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Attempts", zh: "可嘗試次數" })}</span><input name="maxAttempts" type="number" min="1" value={maxAttempts} onChange={(event) => setMaxAttempts(event.target.value)} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
	          </div>
	          <div className="flex flex-wrap gap-3 lg:col-span-2">
		            <label className="soft-panel flex items-center gap-2 px-4 py-3 text-sm font-bold"><input name="randomizeQuestionOrder" type="checkbox" checked={randomizeQuestionOrder} onChange={(event) => setRandomizeQuestionOrder(event.target.checked)} />{t({ en: "Random order", zh: "隨機題序" })}</label>
		            <label className="soft-panel flex items-center gap-2 px-4 py-3 text-sm font-bold"><input name="showAnswersImmediately" type="checkbox" checked={showAnswersImmediately} onChange={(event) => setShowAnswersImmediately(event.target.checked)} />{t({ en: "Show answers immediately", zh: "即時顯示答案" })}</label>
	          </div>
	          </div> : null}
	          {activeStep === 1 ? <div className="glass-panel grid gap-4 p-5 sm:p-6">
	            <div className="grid gap-3 lg:grid-cols-5">
	              <label className="grid gap-2 lg:col-span-2"><span className="text-sm font-black">{t({ en: "Keyword", zh: "關鍵字" })}</span><input value={keyword} onChange={(event) => { setKeyword(event.target.value); setPage(1); }} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></label>
	              <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Topic", zh: "課題" })}</span><select value={topicFilter} onChange={(event) => { setTopicFilter(event.target.value); setPage(1); }} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]"><option value="all">{t({ en: "All", zh: "全部" })}</option>{topicOptions.map((topic) => <option key={topic.id} value={topic.id}>{text(topic.title)}</option>)}</select></label>
	              <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Difficulty", zh: "難度" })}</span><select value={difficultyFilter} onChange={(event) => { setDifficultyFilter(event.target.value as Difficulty | "all"); setPage(1); }} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]"><option value="all">{t({ en: "All", zh: "全部" })}</option>{difficulties.map((difficulty) => <option key={difficulty} value={difficulty}>{formatDifficultyLabel(difficulty, language)}</option>)}</select></label>
	              <label className="grid gap-2"><span className="text-sm font-black">{t({ en: "Type", zh: "題型" })}</span><select value={questionTypeFilter} onChange={(event) => { setQuestionTypeFilter(event.target.value as QuestionType | "all"); setPage(1); }} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]"><option value="all">{t({ en: "All", zh: "全部" })}</option>{questionTypes.map((type) => <option key={type} value={type}>{text(questionTypeLabels[type])}</option>)}</select></label>
	            </div>
	            <div className="flex flex-wrap gap-2">
	              <button type="button" onClick={() => { setSourceFilter("question-bank"); setPage(1); }} className={`focus-ring rounded-full px-4 py-2 text-sm font-black ${sourceFilter === "question-bank" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "soft-panel"}`}>{t({ en: "Question bank", zh: "題庫" })}</button>
	              <button type="button" onClick={() => { setSourceFilter("mistakes"); setPage(1); }} className={`focus-ring rounded-full px-4 py-2 text-sm font-black ${sourceFilter === "mistakes" ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "soft-panel"}`}>{t({ en: "Class mistakes", zh: "班級錯題" })}</button>
	            </div>
	            <div className="grid gap-3">
	              {isQuerying ? <p className="soft-panel p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "Loading questions...", zh: "正在載入題目..." })}</p> : null}
	              {queryResult.questions.map((question) => (
	                <div key={question.id} className="soft-panel grid gap-3 p-4">
	                  <div className="flex flex-wrap items-start justify-between gap-3">
	                    ...
	                    <button type="button" onClick={() => addQuestion(question)} className="focus-ring rounded-full bg-slate-950 px-4 py-2 text-sm font-black text-white dark:bg-white dark:text-slate-950">{t({ en: "Add", zh: "加入" })}</button>
	                  </div>
	                </div>
	              ))}
	              {!isQuerying && !queryResult.questions.length ? <p className="soft-panel p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No matching questions.", zh: "沒有符合條件的題目。" })}</p> : null}
	            </div>
	            <div className="flex items-center justify-between">
	              <button type="button" disabled={page <= 1} onClick={() => setPage((current) => Math.max(1, current - 1))} className="focus-ring rounded-full border border-slate-200/80 px-4 py-2 text-sm font-black disabled:opacity-40 dark:border-white/10">{t({ en: "Previous", zh: "上一頁" })}</button>
	              ...
	              <button type="button" disabled={page >= Math.ceil(queryResult.total / queryResult.pageSize)} onClick={() => setPage((current) => current + 1)} className="focus-ring rounded-full border border-slate-200/80 px-4 py-2 text-sm font-black disabled:opacity-40 dark:border-white/10">{t({ en: "Next", zh: "下一頁" })}</button>
	            </div>
	          </div> : null}
	          {activeStep === 2 ? <div className="grid gap-5">
	            <div className="glass-panel grid gap-4 p-5 sm:p-6">
	              <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Manual question", zh: "手動題目" })}</h2>
	              <textarea value={manualPrompt} onChange={(event) => setManualPrompt(event.target.value)} rows={3} placeholder={t({ en: "Question prompt", zh: "題目內容" })} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" />
	              <div className="grid gap-3 sm:grid-cols-[1fr_140px]"><input value={manualAnswer} onChange={(event) => setManualAnswer(event.target.value)} placeholder={t({ en: "Answer", zh: "答案" })} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /><input value={manualPoints} onChange={(event) => setManualPoints(Number(event.target.value) || 10)} type="number" min="1" className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" /></div>
	              <button type="button" onClick={addManualQuestion} className="focus-ring w-fit rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950">{t({ en: "Add manual question", zh: "加入手動題" })}</button>
	              <textarea value={manualText} onChange={(event) => setManualText(event.target.value)} rows={5} placeholder={t({ en: "Batch import: prompt | answer | points", zh: "批量匯入：題目 | 答案 | 分數" })} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" />
	              <button type="button" onClick={importManualQuestions} className="focus-ring w-fit rounded-full border border-slate-200/80 px-5 py-3 text-sm font-black dark:border-white/10">{t({ en: "Import batch", zh: "批量匯入" })}</button>
	            </div>
	            <div className="glass-panel grid gap-4 p-5 sm:p-6">
	              <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "AI generation", zh: "AI 生成題目" })}</h2>
	              <textarea value={aiPrompt} onChange={(event) => setAiPrompt(event.target.value)} rows={4} placeholder={t({ en: "Describe topic, level, and constraints.", zh: "描述課題、程度與限制。" })} className="focus-ring rounded-2xl border border-slate-200/80 bg-white/80 px-4 py-3 dark:border-white/10 dark:bg-white/[0.06]" />
	              <button type="button" disabled={isGenerating} onClick={generateQuestions} className="focus-ring w-fit rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950">{isGenerating ? t({ en: "Generating...", zh: "生成中..." }) : t({ en: "Generate", zh: "生成" })}</button>
	              <div className="grid gap-3">
	                {generatedQuestions.map((question) => <div key={question.apiId} className="soft-panel p-4"><MathText text={question.question} className="text-sm font-bold text-slate-950 dark:text-white" /><p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">{question.answer}</p><button type="button" onClick={() => addGeneratedQuestion(question)} className="focus-ring mt-3 rounded-full border border-slate-200/80 px-4 py-2 text-sm font-black dark:border-white/10">{t({ en: "Add generated question", zh: "加入生成題" })}</button></div>)}
	              </div>
	            </div>
	          </div> : null}
	          {activeStep === 3 ? <div className="glass-panel grid gap-5 p-5 sm:p-6">
	            <div className="flex flex-wrap items-center justify-between gap-3">
	              <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Paper preview", zh: "試卷預覽" })}</h2>
	              <button type="button" onClick={() => window.print()} className="focus-ring rounded-full border border-slate-200/80 px-4 py-2 text-sm font-black dark:border-white/10">{t({ en: "Print preview", zh: "列印預覽" })}</button>
	            </div>
	            {sections.map((section) => <div key={section.id} className="grid gap-3"><h3 className="text-lg font-black text-slate-950 dark:text-white">{text(section.title)}</h3>{section.items.map((item, index) => <div key={item.id} className="soft-panel p-4"><p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300">{index + 1} · {item.points} {t({ en: "points", zh: "分" })}</p><MathText text={itemPrompt(item)} className="mt-2 text-sm font-bold text-slate-950 dark:text-white" />{item.embeddedQuestion?.diagram ? <QuestionFigure diagram={item.embeddedQuestion.diagram} variant="day" compact language={language} /> : null}{item.embeddedQuestion ? <p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">{t({ en: "Answer", zh: "答案" })}: {item.embeddedQuestion.answer}</p> : null}</div>)}</div>)}
	          </div> : null}
	          {error ? <p role="alert" className="text-sm font-bold text-rose-700 dark:text-rose-200">{error}</p> : null}
	          <div className="flex flex-wrap gap-3">
	            <button value="draft" className="focus-ring rounded-full border border-slate-200/80 px-6 py-3 text-sm font-black dark:border-white/10" type="submit">{t({ en: "Save draft", zh: "儲存草稿" })}</button>
	            <button value="publish" className="focus-ring rounded-full bg-slate-950 px-6 py-3 text-sm font-black text-white dark:bg-white dark:text-slate-950" type="submit">{t({ en: "Publish assessment", zh: "發布測驗" })}</button>
	          </div>
	          </div>
	          <aside className="glass-panel h-fit p-5 sm:p-6 xl:sticky xl:top-24">
	            <div className="flex items-center justify-between gap-3">
	              <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Paper basket", zh: "組卷籃" })}</h2>
	              <button type="button" onClick={addSection} className="focus-ring rounded-full border border-slate-200/80 px-3 py-2 text-xs font-black dark:border-white/10">{t({ en: "Section", zh: "分段" })}</button>
	            </div>
	                  ...
	                  {section.items.map((item, index) => (
	                    <div key={item.id} className="soft-panel grid gap-2 p-3">
	                      ...
	                      <div className="flex flex-wrap items-center gap-2">
	                        ...
	                        <button type="button" onClick={() => moveItem(section.id, item.id, -1)} className="focus-ring rounded-full border border-slate-200/80 px-3 py-1 text-xs font-black dark:border-white/10">{t({ en: "Up", zh: "上移" })}</button>
	                        <button type="button" onClick={() => moveItem(section.id, item.id, 1)} className="focus-ring rounded-full border border-slate-200/80 px-3 py-1 text-xs font-black dark:border-white/10">{t({ en: "Down", zh: "下移" })}</button>
	                        <button type="button" onClick={() => removeItem(section.id, item.id)} className="focus-ring rounded-full border border-rose-200/80 px-3 py-1 text-xs font-black text-rose-700 dark:border-rose-300/40 dark:text-rose-200">{t({ en: "Remove", zh: "移除" })}</button>
	                      </div>
	                    </div>
	                  ))}
	                </div>
	              ))}
	              {!currentItemCount ? <p className="soft-panel p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "Selected questions appear here.", zh: "已選題目會顯示在此。" })}</p> : null}
	            </div>
	          </aside>
	        </form>
	      )}
      ...
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">
            {t({ en: "Analysis settings ready", zh: "分析口徑已就緒" })}
          </p>
          ...
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 dark:text-emerald-100">
            {text(assessment.examGroupName)} · {t({ en: "Pass", zh: "及格" })} {assessment.analysisSettings.passThreshold}% · {t({ en: "Borderline", zh: "臨界" })} ±{assessment.analysisSettings.borderlineRange}
          </p>
        </div>
        <div className="flex min-w-0 flex-wrap gap-2">
...
function AssessmentSettingsPanel({ detail }: { detail: TeacherAssessmentDetailData }) {
  ...
  return (
    <form id="assessment-settings" onSubmit={saveSettings} className="glass-panel grid gap-4 p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Analysis settings", zh: "分析口徑" })}</h2>
        ...
        <button type="submit" disabled={status === "saving"} className="focus-ring rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950">
          {status === "saving" ? t({ en: "Saving", zh: "儲存中" }) : t({ en: "Save settings", zh: "儲存口徑" })}
        </button>
      </div>
      <div className="grid gap-4 lg:grid-cols-6">
        <label className="grid gap-2 lg:col-span-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Exam group ID", zh: "考試組 ID" })}</span>
        ...
        <label className="grid gap-2 lg:col-span-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Exam group name", zh: "考試組名稱" })}</span>
        ...
        <label className="grid gap-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Pass", zh: "及格" })}</span>
        ...
        <label className="grid gap-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Excellent", zh: "優秀" })}</span>
        ...
        <label className="grid gap-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Low score", zh: "低分" })}</span>
        ...
        <label className="grid gap-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Borderline", zh: "臨界區間" })}</span>
        ...
        <label className="grid gap-2 lg:col-span-2">
          <span className="text-xs font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400">{t({ en: "Score bands", zh: "分數段" })}</span>
      ...
      {status === "saved" ? <p className="text-sm font-bold text-emerald-700 dark:text-emerald-200">{t({ en: "Settings saved.", zh: "口徑已儲存。" })}</p> : null}
      {status === "error" ? <p className="text-sm font-bold text-rose-700 dark:text-rose-200">{t({ en: "Could not save settings.", zh: "未能儲存口徑。" })}</p> : null}
...
function MarkingPanel({ detail, submission, onClose }: { detail: TeacherAssessmentDetailData; submission: AssessmentSubmission; onClose: () => void }) {
  ...
  const [rows, setRows] = useState(() => detail.questionAnalytics.map((question) => {
    ...
    return {
      ...
      teacherFeedback: text(answer?.teacherFeedback ?? { en: "", zh: "" })
    };
  }));
  const [status, setStatus] = useState<"idle" | "saving" | "saved" | "error">("idle");
  ...
  return (
    <aside className="glass-panel grid gap-4 p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Mark paper", zh: "閱卷" })}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={onClose} className="focus-ring rounded-full border border-slate-200/80 px-4 py-2 text-sm font-black dark:border-white/10">{t({ en: "Close", zh: "關閉" })}</button>
          <button type="button" disabled={status === "saving"} onClick={saveMarking} className="focus-ring rounded-full bg-slate-950 px-5 py-2 text-sm font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950">
            {status === "saving" ? t({ en: "Saving", zh: "儲存中" }) : t({ en: "Save marks", zh: "儲存分數" })}
          </button>
        </div>
      </div>
      <div className="grid gap-3">
        {detail.questionAnalytics.map((question, index) => {
          ...
          return (
            <div key={question.questionId} className="soft-panel grid gap-3 p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-black uppercase tracking-[0.14em] text-cyan-700 dark:text-cyan-200">Q{index + 1} · {question.maxPoints} {t({ en: "points", zh: "分" })}</p>
                  <MathText text={text(question.prompt)} className="mt-2 text-sm font-bold text-slate-950 dark:text-white" />
                  <p className="mt-2 break-words text-xs font-bold text-slate-500 dark:text-slate-400">{t({ en: "Student answer", zh: "學生答案" })}: {answer?.answer || "--"}</p>
                </div>
                <div className="grid w-full gap-2 sm:w-[260px] sm:grid-cols-[100px_1fr]">
                  <label className="grid gap-1">
                    <span className="text-[0.65rem] font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Score", zh: "得分" })}</span>
                  ...
                  <label className="grid gap-1">
                    <span className="text-[0.65rem] font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Judgement", zh: "判定" })}</span>
                    <select value={row?.isCorrect ? "correct" : "wrong"} onChange={(event) => updateRow(question.questionId, { isCorrect: event.target.value === "correct" })} className="focus-ring rounded-xl border border-slate-200/80 bg-white/80 px-3 py-2 text-sm font-black dark:border-white/10 dark:bg-white/[0.06]">
                      <option value="correct">{t({ en: "Correct", zh: "正確" })}</option>
                      <option value="wrong">{t({ en: "Wrong", zh: "錯誤" })}</option>
                    </select>
                  </label>
                </div>
              </div>
              <label className="grid gap-1">
                <span className="text-[0.65rem] font-black uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">{t({ en: "Teacher note", zh: "教師批註" })}</span>
      ...
      {status === "saved" ? <p className="text-sm font-bold text-emerald-700 dark:text-emerald-200">{t({ en: "Marks saved.", zh: "分數已儲存。" })}</p> : null}
      {status === "error" ? <p className="text-sm font-bold text-rose-700 dark:text-rose-200">{t({ en: "Could not save marks.", zh: "未能儲存分數。" })}</p> : null}
...
export function TeacherAssessmentDetailView({ detail }: { detail: TeacherAssessmentDetailData }) {
  ...
  const summaryCards = [
    { label: t({ en: "Highest", zh: "最高分" }), value: percentValue(detail.analysis.summary.highestScore), detail: t({ en: "Submitted papers", zh: "已提交試卷" }) },
    { label: t({ en: "Lowest", zh: "最低分" }), value: percentValue(detail.analysis.summary.lowestScore), detail: t({ en: "Submitted papers", zh: "已提交試卷" }) },
    { label: t({ en: "Average", zh: "平均分" }), value: percentValue(detail.analysis.summary.averageScore), detail: t({ en: "Class mean", zh: "班級均分" }) },
    { label: t({ en: "Std dev", zh: "標準差" }), value: detail.analysis.summary.standardDeviation === null ? "--" : String(detail.analysis.summary.standardDeviation), detail: t({ en: "Population", zh: "總體標準差" }) },
    { label: t({ en: "Pass rate", zh: "及格率" }), value: percentValue(detail.analysis.summary.passRate), detail: `${detail.analysis.settings.passThreshold}%` },
    { label: t({ en: "Excellent", zh: "優秀率" }), value: percentValue(detail.analysis.summary.excellentRate), detail: `${detail.analysis.settings.excellentThreshold}%` },
    { label: t({ en: "Low score", zh: "低分率" }), value: percentValue(detail.analysis.summary.lowScoreRate), detail: `${detail.analysis.settings.lowScoreThreshold}%` },
    { label: t({ en: "Completion", zh: "完成率" }), value: `${submissionRate}%`, detail: `${detail.submittedCount}/${detail.assessment.submissionCount}` }
  ];
  const tabLabels = [
    { id: "overview" as const, label: t({ en: "Overview", zh: "總覽" }) },
    { id: "rankings" as const, label: t({ en: "Rankings", zh: "班級排名" }) },
    { id: "questions" as const, label: t({ en: "Items", zh: "小題分析" }) },
    { id: "knowledge" as const, label: t({ en: "Knowledge", zh: "知識點" }) },
    { id: "grade" as const, label: t({ en: "Grade", zh: "年級對比" }) },
    { id: "borderline" as const, label: t({ en: "Borderline", zh: "臨界生" }) }
  ];
  const assessmentTitle = text(detail.assessment.title);
  ...
  return (
    <div className="grid gap-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {detail.assessment.status === "draft" || detail.submittedCount === 0 ? (
              <Link href={`/teacher/assessments/${detail.assessment.id}/edit`} className="focus-ring rounded-full border border-slate-200/80 px-5 py-3 text-sm font-black dark:border-white/10">{t({ en: "Edit", zh: "編輯" })}</Link>
            ) : null}
            ...
            <button type="button" disabled={isCloning} onClick={cloneAssessment} className="focus-ring rounded-full border border-slate-200/80 px-5 py-3 text-sm font-black disabled:opacity-50 dark:border-white/10">
              {isCloning ? t({ en: "Copying", zh: "複製中" }) : t({ en: "Copy", zh: "複製" })}
            </button>
            ...
            <button type="button" onClick={() => window.print()} className="focus-ring rounded-full border border-slate-200/80 px-5 py-3 text-sm font-black dark:border-white/10">{t({ en: "Print", zh: "列印" })}</button>
      ...
      <section className="glass-panel p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Exam analysis", zh: "試卷分析" })}</h2>
          <p className="text-sm font-black text-slate-500 dark:text-slate-400">{totalPoints(detail.assessment.paperSections)} {t({ en: "points", zh: "分" })}</p>
        </div>
        ...
        {activeTab === "overview" ? (
          <div className="mt-5 grid gap-6 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)]">
            <div className="soft-panel p-4">
              <h3 className="text-lg font-black text-slate-950 dark:text-white">{t({ en: "Score distribution", zh: "分數分佈" })}</h3>
            ...
            <div className="soft-panel p-4">
              <h3 className="text-lg font-black text-slate-950 dark:text-white">{t({ en: "Common wrong questions", zh: "常錯題" })}</h3>
              <div className="mt-4 grid gap-3">
                ...
                {!detail.commonWrongQuestions.length ? <p className="text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No submitted answers yet.", zh: "尚未有提交答案。" })}</p> : null}
              </div>
            </div>
          </div>
        ) : null}
        ...
        {activeTab === "questions" ? (
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[1120px] text-left text-sm">
              <thead className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400"><tr><th className="py-3 pr-4">{t({ en: "Question", zh: "題目" })}</th><th className="py-3 pr-4">{t({ en: "Topic", zh: "知識點" })}</th><th className="py-3 pr-4">{t({ en: "Score rate", zh: "得分率" })}</th><th className="py-3 pr-4">{t({ en: "Correct rate", zh: "正確率" })}</th><th className="py-3 pr-4">{t({ en: "Difficulty", zh: "難度" })}</th><th className="py-3 pr-4">{t({ en: "Discrimination", zh: "區分度" })}</th><th className="py-3">{t({ en: "Common wrong", zh: "常見錯答" })}</th></tr></thead>
              <tbody className="divide-y divide-slate-200/80 dark:divide-white/10">
          ...
          <div className="mt-5 grid gap-3">
            {detail.analysis.knowledgeMastery.map((topic) => (
              <div key={topic.topicId} className="soft-panel p-4">
                ...
                <p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">{topic.earnedPoints}/{topic.maxPoints} · {topic.questionCount} {t({ en: "items", zh: "小題" })}</p>
              </div>
            ))}
          </div>
        ) : null}
        ...
        {activeTab === "grade" ? (
          <div className="mt-5 grid gap-4">
            <div className="grid gap-4 md:grid-cols-4">
              <StatCard label={t({ en: "Grade average", zh: "年級均分" })} value={percentValue(detail.analysis.gradeComparison.gradeAverageScore)} detail={text(detail.analysis.gradeComparison.scopeLabel)} />
              <StatCard label={t({ en: "Class rank", zh: "班級排名" })} value={detail.analysis.gradeComparison.currentClassRank === null ? "--" : String(detail.analysis.gradeComparison.currentClassRank)} detail={`${detail.analysis.gradeComparison.classCount} ${t({ en: "classes", zh: "班級" })}`} />
              <StatCard label={t({ en: "Papers", zh: "同卷" })} value={String(detail.analysis.gradeComparison.assessmentCount)} detail={text(detail.assessment.examGroupName)} />
              <StatCard label={t({ en: "Submissions", zh: "提交" })} value={String(detail.analysis.gradeComparison.submittedCount)} detail={text(detail.analysis.gradeComparison.message)} />
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-left text-sm">
                <thead className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400"><tr><th className="py-3 pr-4">{t({ en: "Class", zh: "班級" })}</th><th className="py-3 pr-4">{t({ en: "Average", zh: "均分" })}</th><th className="py-3 pr-4">{t({ en: "Submitted", zh: "提交" })}</th><th className="py-3">{t({ en: "Assessment", zh: "試卷" })}</th></tr></thead>
        ...
        {activeTab === "borderline" ? (
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[820px] text-left text-sm">
              <thead className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400"><tr><th className="py-3 pr-4">{t({ en: "Student", zh: "學生" })}</th><th className="py-3 pr-4">{t({ en: "Type", zh: "類型" })}</th><th className="py-3 pr-4">{t({ en: "Score", zh: "分數" })}</th><th className="py-3 pr-4">{t({ en: "Threshold", zh: "線" })}</th><th className="py-3">{t({ en: "Gap", zh: "差距" })}</th></tr></thead>
              <tbody className="divide-y divide-slate-200/80 dark:divide-white/10">
            ...
            {!detail.analysis.borderlineStudents.length ? <p className="soft-panel mt-4 p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No borderline students under the current settings.", zh: "目前口徑下沒有臨界生。" })}</p> : null}
          </div>
        ) : null}
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.78fr)]">
        <div className="glass-panel overflow-hidden p-5 sm:p-6">
          ...
          <div className="mt-5 overflow-x-auto">
            <table className="w-full min-w-[900px] text-left text-sm">
              <thead className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400"><tr><th className="py-3 pr-4">{t({ en: "Rank", zh: "名次" })}</th><th className="py-3 pr-4">{t({ en: "Student", zh: "學生" })}</th><th className="py-3 pr-4">{t({ en: "Status", zh: "狀態" })}</th><th className="py-3 pr-4">{t({ en: "Score", zh: "分數" })}</th><th className="py-3 pr-4">{t({ en: "Attempt", zh: "嘗試" })}</th><th className="py-3 pr-4">{t({ en: "Submitted", zh: "提交時間" })}</th><th className="py-3">{t({ en: "Action", zh: "操作" })}</th></tr></thead>
              <tbody className="divide-y divide-slate-200/80 dark:divide-white/10">
                {detail.submissions.map((submission) => {
                  ...
                  return (
                    <tr key={submission.id} id={`assessment-submission-${submission.id}`}>
                      ...
                      <td className="py-4"><button type="button" onClick={() => setSelectedSubmissionId(submission.id)} className="focus-ring rounded-full border border-slate-200/80 px-3 py-2 text-xs font-black dark:border-white/10">{t({ en: "Mark", zh: "閱卷" })}</button></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {selectedSubmission ? <MarkingPanel detail={detail} submission={selectedSubmission} onClose={() => setSelectedSubmissionId(null)} /> : (
          <aside className="glass-panel p-5 sm:p-6">
            <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Marking panel", zh: "閱卷面板" })}</h2>
            <p className="mt-3 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "Choose a submission from the table.", zh: "從提交表選擇一位學生。" })}</p>
          </aside>
        )}
      </section>
      ...
      <section className="glass-panel p-5 sm:p-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Paper and answer key", zh: "試卷與答案" })}</h2>
          <p className="text-sm font-black text-slate-500 dark:text-slate-400">{totalPoints(detail.assessment.paperSections)} {t({ en: "points", zh: "分" })}</p>
        </div>
              ...
              {section.items.map((item, index) => {
                ...
                return (
                  <div key={item.id} className="soft-panel p-4">
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-cyan-600 dark:text-cyan-300">{index + 1} · {item.points} {t({ en: "points", zh: "分" })}</p>
                    ...
                    <p className="mt-2 text-xs font-bold text-slate-500 dark:text-slate-400">{t({ en: "Answer", zh: "答案" })}: {analytics?.correctAnswer ?? item.embeddedQuestion?.answer ?? "--"}</p>
