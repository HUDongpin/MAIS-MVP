...
function riskTagLabel(tag: TeacherStudentRiskTag) {
  const labels: Record<TeacherStudentRiskTag, { en: string; zh: string; zhHans?: string }> = {
    ...
    "repeated-mistakes": { en: "Repeated mistakes", zh: "重複錯題", zhHans: "重复错题" },
...
function actionLabel(action: TeacherInterventionAction) {
  const labels: Record<TeacherInterventionAction, { en: string; zh: string; zhHans?: string }> = {
    "rebuild-foundation": { en: "Rebuild foundation", zh: "補基礎", zhHans: "补基础" },
    "redo-mistakes": { en: "Redo mistakes", zh: "重做錯題", zhHans: "重做错题" },
    "challenge-extension": { en: "Challenge", zh: "進階挑戰", zhHans: "进阶挑战" },
    "teacher-message": { en: "Message follow-up", zh: "私信跟進", zhHans: "私信跟进" }
  };
  ...
  return (
    <Link href={cell.href} className={`focus-ring block overflow-hidden rounded-2xl border p-4 transition hover:-translate-y-0.5 ${heatTone(cell.averageMastery)}`}>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-xs font-bold">
        <span>{t({ en: "Weak", zh: "薄弱", zhHans: "薄弱" })} {cell.weakStudentCount}/{cell.studentCount}</span>
        <span>{t({ en: "Avg", zh: "均時", zhHans: "均时" })} {seconds(cell.averageAnswerSeconds)}</span>
        <span>{t({ en: "Hints", zh: "提示", zhHans: "提示" })} {cell.hintRequests}</span>
      </div>
      {cell.reteachRecommended ? (
        <p className="mt-3 rounded-full bg-rose-500/15 px-3 py-1 text-xs font-black text-rose-800 dark:text-rose-100">
          {t({ en: "Reteach signal", zh: "建議重講", zhHans: "建议重讲" })}
        </p>
      ) : null}
    </Link>
  );
    ...
    <Link href={risk.href} className="focus-ring soft-panel block overflow-hidden p-4 transition hover:-translate-y-0.5">
      <div className="grid min-w-0 gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {risk.tags.length ? risk.tags.map((tag) => (
              ...
              <span className="rounded-full border border-emerald-300/55 bg-emerald-400/12 px-2.5 py-1 text-xs font-black text-emerald-800 dark:text-emerald-100">
                {t({ en: "Extension ready", zh: "可挑戰", zhHans: "可挑战" })}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="grid min-w-0 gap-3 text-sm font-bold text-slate-600 dark:text-slate-300 sm:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] md:col-span-2">
          <div className="min-w-0">
            <p>{t({ en: "Mastery", zh: "掌握", zhHans: "掌握" })} {percent(risk.averageMastery)}</p>
            <p>{t({ en: "Avg time", zh: "均時", zhHans: "均时" })} {seconds(risk.averageAnswerSeconds)}</p>
          </div>
          <div className="min-w-0">
  ...
  return (
    <section aria-live="polite" className="mt-4 rounded-2xl border border-emerald-300/55 bg-emerald-400/12 p-4">
      <div className="flex min-w-0 flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="break-words text-xs font-black uppercase tracking-[0.14em] text-emerald-800 dark:text-emerald-100">{t({ en: "Follow-up ready", zh: "跟進作業已就緒", zhHans: "跟进作业已就绪" })}</p>
          ...
          <p className="mt-1 break-words text-xs font-bold text-emerald-900 [overflow-wrap:anywhere] dark:text-emerald-100">
            {group.studentNames.length} {t({ en: "students", zh: "學生", zhHans: "学生" })} · {text(actionLabel(group.action))}
          </p>
        </div>
...
function InterventionCard({ group }: { group: TeacherAnalyticsInterventionGroup }) {
  ...
  return (
    <article id={`intervention-group-${group.id}`} className="soft-panel scroll-mt-24 p-4">
      ...
      >
        {status === "saving"
          ? t({ en: "Creating", zh: "建立中", zhHans: "建立中" })
          : status === "created"
            ? t({ en: "Added", zh: "已加入", zhHans: "已加入" })
            : t({ en: "Create follow-up", zh: "加入跟進作業", zhHans: "加入跟进作业" })}
      </button>
      {status === "error" ? <p className="mt-2 text-xs font-bold text-rose-700 dark:text-rose-200">{t({ en: "Could not create assignment.", zh: "未能建立作業。", zhHans: "未能建立作业。" })}</p> : null}
    ...
    <div className="grid gap-7">
      <section className="glass-panel p-6 sm:p-8">
        <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_260px] xl:items-end">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">{t({ en: "Learning analytics", zh: "學習分析", zhHans: "学习分析" })}</p>
            <h1 className="mt-3 text-4xl font-black tracking-tight text-slate-950 dark:text-white sm:text-5xl">
              {t({ en: "Class insight and intervention", zh: "班級洞察與干預建議", zhHans: "班级洞察与干预建议" })}
            </h1>
          </div>
          <label className="grid gap-2">
            <span className="text-xs font-black uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">{t({ en: "Class", zh: "班級", zhHans: "班级" })}</span>
            <select
      ...
      <section className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,360px)]">
        <div className="min-w-0">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Topic mastery heatmap", zh: "課題掌握熱力圖", zhHans: "课题掌握热力图" })}</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
            {analytics.topicMastery.map((cell) => <TopicCell key={cell.id} cell={cell} />)}
          </div>
        </div>
        <aside className="glass-panel min-w-0 p-5">
          <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Active trend", zh: "活躍趨勢", zhHans: "活跃趋势" })}</h2>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 text-sm font-bold">
            <div className="soft-panel p-3"><p className="text-2xl font-black gradient-text">{analytics.summary.activeStudents7d}</p><p>{t({ en: "Active 7d", zh: "7日活躍", zhHans: "7日活跃" })}</p></div>
            <div className="soft-panel p-3"><p className="text-2xl font-black gradient-text">{analytics.summary.activeStudents30d}</p><p>{t({ en: "Active 30d", zh: "30日活躍", zhHans: "30日活跃" })}</p></div>
          </div>
        </aside>
      </section>
      ...
      <section className="grid min-w-0 items-start gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(320px,400px)]">
        <div className="grid min-w-0 gap-4">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t({ en: "Student risk list", zh: "學生風險列表", zhHans: "学生风险列表" })}</h2>
          {analytics.studentRisks.length ? analytics.studentRisks.map((risk) => <RiskRow key={`${risk.classId}-${risk.studentId}`} risk={risk} />) : (
            <div className="soft-panel p-6 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No risk signals yet.", zh: "暫未有風險訊號。", zhHans: "暂未有风险讯号。" })}</div>
          )}

          <section className="glass-panel min-w-0 p-5">
            <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Intervention groups", zh: "干預建議小組", zhHans: "干预建议小组" })}</h2>
            <div className="mt-4 grid gap-3">
              {analytics.interventionGroups.length ? analytics.interventionGroups.map((group) => <InterventionCard key={group.id} group={group} />) : (
                <p className="soft-panel p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No intervention group generated yet.", zh: "暫未生成干預小組。", zhHans: "暂未生成干预小组。" })}</p>
              )}
            </div>
          </section>
        </div>
        <aside className="grid min-w-0 gap-6">
          <section className="glass-panel min-w-0 p-5">
            <h2 className="text-xl font-black text-slate-950 dark:text-white">{t({ en: "Frequent mistakes", zh: "高頻錯題", zhHans: "高频错题" })}</h2>
            <div className="mt-4 grid gap-3">
              {analytics.frequentMistakes.length ? analytics.frequentMistakes.map((mistake) => (
                <article key={mistake.id} className="soft-panel p-3">
                  ...
                  <p className="mt-2 text-xs font-black text-rose-700 dark:text-rose-200">{mistake.wrongAttempts} {t({ en: "wrong attempts", zh: "次錯誤", zhHans: "次错误" })} · {mistake.studentCount} {t({ en: "students", zh: "學生", zhHans: "学生" })}</p>
                </article>
              )) : <p className="soft-panel p-4 text-sm font-bold text-slate-500 dark:text-slate-400">{t({ en: "No repeated mistakes yet.", zh: "暫未有重複錯題。", zhHans: "暂未有重复错题。" })}</p>}
            </div>
          </section>
        </aside>
      </section>
