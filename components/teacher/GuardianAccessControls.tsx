...
export function guardianAccessErrorCopy(
  ...
  if (status === "network") {
    return {
      ...
      zh: "網絡請求未能完成，請檢查連線後重試。", zhHans: "网络请求未能完成，请检查连线后重试。"
    };
  }
  if (status === 400) return { en: "Check the request and try again.", zh: "請檢查資料後重試。", zhHans: "请检查资料后重试。" };
  if (status === 403) return { en: "You do not have permission for this student and class.", zh: "你沒有此學生及班級的操作權限。", zhHans: "你没有此学生及班级的操作权限。" };
  if (status === 404) return { en: "The student, class, or guardian link was not found.", zh: "找不到學生、班級或家長連結。", zhHans: "找不到学生、班级或家长连结。" };
  if (status === 409) return { en: "Guardian access changed before this request completed. Refresh and try again.", zh: "家長存取狀態已變更，請重新整理後重試。", zhHans: "家长存取状态已变更，请重新整理后重试。" };
  if (status === 410) return { en: "This invitation expired or was revoked. Issue a new one.", zh: "此邀請已過期或被撤銷，請重新發出。", zhHans: "此邀请已过期或被撤销，请重新发出。" };
  if (status === 429) {
    ...
    return {
      ...
      zh: `請求過於頻密，請於 ${seconds} 秒後重試。`, zhHans: `请求过于频密，请于 ${seconds} 秒后重试。`
    };
  }
  return {
    en: "Guardian access is temporarily unavailable. Try again later.",
    zh: "家長存取功能暫時無法使用，請稍後重試。", zhHans: "家长存取功能暂时无法使用，请稍后重试。"
  };
}
...
export function GuardianAccessControls({ classId, studentId, guardianLinks }: GuardianAccessControlsProps) {
  ...
  const issueInvitation = async () => {
    ...
    try {
      ...
      setStatusMessage({
        ...
        zh: "已發出一次性家長邀請。", zhHans: "已发出一次性家长邀请。"
      });
      router.refresh();
    } catch {
  ...
  const copyInvitation = async () => {
    ...
    try {
      ...
      setStatusMessage({ en: "Invitation code copied.", zh: "邀請碼已複製。", zhHans: "邀请码已复制。" });
    } catch {
      setErrorMessage({
        en: "The code could not be copied automatically. Select and copy it manually.",
        zh: "未能自動複製邀請碼，請手動選取並複製。", zhHans: "未能自动复制邀请码，请手动选取并复制。"
      });
    }
  };
  ...
  const revokeLink = async (link: GuardianLink) => {
    ...
    const confirmed = window.confirm(t({
      ...
      zh: `確定撤銷 ${link.parentName} 的家長存取權嗎？現有存取將立即失效。`, zhHans: `确定撤销 ${link.parentName} 的家长存取权吗？现有存取将立即失效。`
    }));
    if (!confirmed) return;
    ...
    try {
      ...
      setStatusMessage({
        ...
        zh: `已撤銷 ${link.parentName} 的家長存取權；請立即複製新的單次邀請碼。`, zhHans: `已撤销 ${link.parentName} 的家长存取权；请立即复制新的单次邀请码。`
      });
      router.refresh();
    } catch {
      setErrorMessage(guardianAccessErrorCopy("network"));
    ...
    <section className="glass-panel p-5" aria-labelledby="guardian-access-heading">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 id="guardian-access-heading" className="text-xl font-black text-slate-950 dark:text-white">
            {t({ en: "Parent access", zh: "家長端存取", zhHans: "家长端存取" })}
          </h2>
          <p className="mt-1 text-sm font-bold text-slate-500 dark:text-slate-400">
            {t({
              en: "Issue a 24-hour, one-time code or revoke an active guardian link.",
              zh: "發出 24 小時有效的一次性邀請碼，或撤銷現有家長連結。", zhHans: "发出 24 小时有效的一次性邀请码，或撤销现有家长连结。"
            })}
          </p>
        </div>
        <button
        ...
        >
          {busyOperation === "issue"
            ? t({ en: "Issuing…", zh: "發出中…", zhHans: "发出中…" })
            : t({ en: "Issue / rotate code", zh: "發出／輪換邀請碼", zhHans: "发出／轮换邀请码" })}
        </button>
      </div>
      ...
      {!classId ? (
        <p role="alert" className="mt-4 rounded-2xl border border-amber-300/55 bg-amber-400/10 p-3 text-sm font-bold text-amber-800 dark:text-amber-100">
          {t({ en: "Select an authorised class before managing parent access.", zh: "請先選擇獲授權的班級，才可管理家長存取。", zhHans: "请先选择获授权的班级，才可管理家长存取。" })}
        </p>
      ) : null}
      ...
      {revealedInvitation ? (
        <div className="mt-4 rounded-2xl border border-cyan-300/55 bg-cyan-400/10 p-4">
          <p className="text-sm font-black text-cyan-900 dark:text-cyan-100">
            {t({ en: "Copy this code now. It will not be shown again.", zh: "請立即複製此邀請碼；離開後不會再次顯示。", zhHans: "请立即复制此邀请码；离开后不会再次显示。" })}
          </p>
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <button type="button" onClick={copyInvitation} className="focus-ring rounded-full border border-cyan-400/50 px-3 py-2 text-xs font-black">
              {t({ en: "Copy code", zh: "複製邀請碼", zhHans: "复制邀请码" })}
            </button>
          </div>
          <p className="mt-2 text-xs font-bold text-slate-600 dark:text-slate-300">
            {t({ en: "Expires", zh: "到期時間", zhHans: "到期时间" })}: {expiryLabel(revealedInvitation.expiresAt, language)}
          </p>
        </div>
      ) : null}
      ...
      <div className="mt-5 grid gap-2">
        {activeLinks.map((link) => (
          <div key={link.id} className="soft-panel flex flex-wrap items-center justify-between gap-3 p-3 text-sm font-bold">
            ...
            >
              {busyOperation === link.id ? t({ en: "Revoking…", zh: "撤銷中…", zhHans: "撤销中…" }) : t({ en: "Revoke", zh: "撤銷", zhHans: "撤销" })}
            </button>
          </div>
        ))}
        {!activeLinks.length ? (
          <p className="text-sm font-bold text-slate-500 dark:text-slate-400">
            {t({ en: "No parent accounts linked yet.", zh: "尚未綁定家長帳戶。", zhHans: "尚未绑定家长账号。" })}
          </p>
        ) : null}
      </div>
    </section>
