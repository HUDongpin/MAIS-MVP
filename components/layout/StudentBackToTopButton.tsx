...
export function StudentBackToTopButton() {
  ...
  useEffect(() => {
    if (!shouldRender) {
      setScrollState({ progress: 0, visible: false });
      return;
    }
    ...
    pollingInterval = window.setInterval(updateScrollState, 250);
  ...
  return (
    <button
      ...
      onClick={scrollToTop}
      className={cn(
        "focus-ring group fixed bottom-[calc(5.25rem+env(safe-area-inset-bottom))] right-4 z-[65] h-14 w-14 rounded-full transition duration-300 sm:bottom-[calc(5.65rem+env(safe-area-inset-bottom))] sm:right-5",
        scrollState.visible
