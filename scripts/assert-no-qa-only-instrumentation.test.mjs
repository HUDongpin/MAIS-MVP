...
test("only the exact current 188-file control-plus-Canvas composition is locally authorized", async () => {
  ...
  assert.equal(result.signatureSentinelFiles, 188);
  assert.equal(result.canvasSentinelFiles, 188);
  ...
  assert.equal(CALIFORNIA_COMPOSED_BENCH_SOURCE_FILES.length, 188);
  assert.equal(CALIFORNIA_COMPOSED_NON_BENCH_JSX_FILES.length, 4);
});
...
test("composed authorization binds every family, exact JSX inventory, and final staged bytes", async () => {
  ...
  assert.throws(() => assertComposedQa(root), /only the exact 188 reviewed bench files/i);
...
async function loadNextProductionConfig(root, overrides = {}) {
  ...
  const program = [
    ...
    "console.log(`DIST_DIR=${config.distDir ?? ''}`);",
...
test("Next config normalizes an absolute in-root tsconfig path and rejects escape paths", async () => {
  const root = await cleanNextConfigFixture("next-config-explicit-tsconfig");
  const explicitName = "tsconfig.california-acceptance.generated.json";
  const explicitPath = path.join(root, explicitName);
  await writeFile(explicitPath, "{\"extends\":\"./tsconfig.json\"}\n");

  const loaded = await loadNextProductionConfig(root, {
    NEXT_TSCONFIG_PATH: explicitPath
  });
  assert.equal(loaded.status, 0, `${loaded.stdout}\n${loaded.stderr}`);
  assert.match(loaded.stdout, new RegExp(`TSCONFIG_PATH=${explicitName.replace(/[.]/g, "[.]")}`));

  const escaped = await loadNextProductionConfig(root, {
    NEXT_TSCONFIG_PATH: path.join(path.dirname(root), "tsconfig.outside.json")
  });
  assert.notEqual(escaped.status, 0);
  assert.match(`${escaped.stdout}\n${escaped.stderr}`, /must stay inside the project root/i);

  await mkdir(path.join(root, "nested"));
  await writeFile(path.join(root, "nested", "tsconfig.json"), "{}\n");
  const nested = await loadNextProductionConfig(root, {
    NEXT_TSCONFIG_PATH: "nested/tsconfig.json"
  });
  assert.notEqual(nested.status, 0);
  assert.match(`${nested.stdout}\n${nested.stderr}`, /one root tsconfig/i);
});

test("Next config normalizes an absolute in-root dist directory and rejects escape paths", async () => {
  const root = await cleanNextConfigFixture("next-config-explicit-dist");
  const absoluteDist = path.join(root, ".ca-acceptance", "next-dist");
  const loaded = await loadNextProductionConfig(root, {
    NEXT_DIST_DIR: absoluteDist
  });
  assert.equal(loaded.status, 0, `${loaded.stdout}\n${loaded.stderr}`);
  assert.match(loaded.stdout, /DIST_DIR=[.]ca-acceptance\/next-dist/);

  const escaped = await loadNextProductionConfig(root, {
    NEXT_DIST_DIR: path.join(path.dirname(root), "outside-next-dist")
  });
  assert.notEqual(escaped.status, 0);
  assert.match(`${escaped.stdout}\n${escaped.stderr}`, /must stay strictly inside the project root/i);

  const projectRoot = await loadNextProductionConfig(root, {
    NEXT_DIST_DIR: root
  });
  assert.notEqual(projectRoot.status, 0);
  assert.match(`${projectRoot.stdout}\n${projectRoot.stderr}`, /must stay strictly inside the project root/i);
});
