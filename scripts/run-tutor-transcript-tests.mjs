...
const outputDir = join(".tmp", `tutor-transcript-tests-${process.pid}-${Date.now()}`);
...
// The compiled tests import runtime modules through the "@/..." path alias. This
// runner only compiles the listed test files and their imports, so the emitted JS
// lives under the outDir tree; point node_modules/@ back at that compiled tree so
// "@/lib/..." resolves to the compiled output rather than the untranspiled source.
...
try {
  const compiled = run(tscBin, ["-p", "tsconfig.tutor-transcript.json", "--outDir", outputDir]);
  ...
  const passed = run("node", [
    "--test",
    join(outputDir, "lib/server/userStoreAiTutorTranscriptAccessPersistence.test.js"),
    join(outputDir, "lib/server/userStoreTeacherOpsStudentProfilePersistence.test.js")
  ]);
