...
const outputDir = join(".tmp", `content-safety-tests-${process.pid}-${Date.now()}`);
...
try {
  const compiled = run(tscBin, ["-p", "tsconfig.content-safety.json", "--outDir", outputDir]);
  ...
  const passed = run("node", [
    "--test",
    join(outputDir, "lib/server/contentSafety.test.js"),
    join(outputDir, "lib/server/userStore/contentSafetyPersistence.test.js")
  ]);
