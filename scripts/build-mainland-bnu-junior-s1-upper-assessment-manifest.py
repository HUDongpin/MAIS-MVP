#!/usr/bin/env python3
"""Build local-only metadata artifacts for Mainland BNU S1 upper assessments.
...
DEFAULT_OUT_DIR = Path(".local/rag/mainland-bnu-junior-s1-upper-assessments")
SAFETY_NOTE = (
...
UNIT_MARKERS = [
    ("丰富的图形世界", ["丰富的图形世界", "几何体", "展开与折叠", "从不同方向看"]),
    ("有理数及其运算", ["有理数及其运算", "有理数", "数轴", "绝对值", "正负数"]),
    ("整式及其加减", ["整式及其加减", "整式", "同类项", "代数式", "合并同类项"]),
    ("基本平面图形", ["基本平面图形", "平面图形", "线段", "射线", "角"]),
    ("一元一次方程", ["一元一次方程", "方程", "等式性质"]),
    ("数据的收集与整理", ["数据的收集与整理", "数据收集", "统计图", "调查"]),
]

SAFE_PATTERN_SLOTS = [
    {
        "draftId": "bnu-junior-s1-upper-assessment-spatial-figures-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["丰富的图形世界"],
        "conceptSignals": ["spatial-figures", "views-of-objects", "nets"],
        "difficultyBand": "foundation",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-rational-numbers-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["有理数及其运算"],
        "conceptSignals": ["rational-numbers", "number-line", "absolute-value", "signed-number-operations"],
        "difficultyBand": "core",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-algebraic-expressions-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["整式及其加减"],
        "conceptSignals": ["algebraic-expressions", "like-terms", "polynomial-addition-subtraction"],
        "difficultyBand": "core",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-plane-figures-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["基本平面图形"],
        "conceptSignals": ["basic-plane-figures", "line-ray-segment", "angles"],
        "difficultyBand": "foundation",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-linear-equations-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["一元一次方程"],
        "conceptSignals": ["linear-equations-one-variable", "equation-solving", "modeling-with-equations"],
        "difficultyBand": "exam",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-data-collection-unit",
        "sourceKind": "school-assessment-pattern",
        "assessmentFamilies": ["unit-test"],
        "materialKinds": ["unit-test", "topic-practice", "paper"],
        "unitTitles": ["数据的收集与整理"],
        "conceptSignals": ["data-collection", "data-organization", "statistical-charts"],
        "difficultyBand": "foundation",
    },
    {
        "draftId": "bnu-junior-s1-upper-assessment-midterm-final-integrated",
        "sourceKind": "review-pattern",
        "assessmentFamilies": ["midterm", "final", "mock", "comprehensive", "topic-review"],
...
def chapter_token_to_int(value: str) -> int | None:
    return CHAPTER_TOKEN_VALUES.get(value)


def unit_numbers_from_name(name: str) -> list[int]:
    numbers: list[int] = []
    for lower, upper in re.findall(r"第\s*([一二三四五六1-6])\s*[~～\-—至到]\s*([一二三四五六1-6])\s*章", name):
        start = chapter_token_to_int(lower)
        end = chapter_token_to_int(upper)
        if start is not None and end is not None and start <= end:
            numbers.extend(range(start, end + 1))
    for value in re.findall(r"第\s*([一二三四五六1-6])\s*章", name):
        number = chapter_token_to_int(value)
...
def infer_unit_titles(name: str) -> list[str]:
    units: list[str] = []
    numbers = unit_numbers_from_name(name)
    ...
    if "期中" in name and not units:
        units.extend([S1_UPPER_UNIT_BY_NUMBER[number] for number in (1, 2, 3)])
    if "第一次月考" in name and not units:
        units.extend([S1_UPPER_UNIT_BY_NUMBER[number] for number in (1, 2)])
...
def infer_assessment_families(name: str) -> list[str]:
    ...
    if "期中" in name or "月考" in name:
        families.append("midterm")
    if "期末" in name:
        families.append("final")
    if "模拟" in name:
        families.append("mock")
    if any(term in name for term in ["专题", "专项", "复习", "培优", "提升", "压轴"]):
        families.append("topic-review")
    if any(term in name for term in ["综合", "期中", "期末", "模拟", "月考"]):
        families.append("comprehensive")
    return unique_ordered(families) or ["topic-review"]
...
def infer_material_kinds(name: str, families: list[str]) -> list[str]:
    ...
    if any(family in families for family in ["midterm", "final", "mock"]):
        kinds.append("midterm-final")
    if "topic-review" in families or any(term in name for term in ["专题", "专项", "同步", "复习", "培优", "提升", "压轴"]):
        kinds.append("topic-practice")
    if any(term in name for term in ["复习", "模拟", "综合", "压轴", "月考"]):
        kinds.append("review")
    kinds.append("paper")
...
def infer_difficulty(name: str) -> str:
    if any(term in name for term in ["压轴", "培优", "提高", "提升", "能力", "难点", "B卷"]):
        return "challenge"
    if any(term in name for term in ["基础", "同步", "A卷", "知识"]):
        return "foundation"
    if any(term in name for term in ["期中", "期末", "模拟", "综合", "月考"]):
        return "exam"
    return "core"
...
def archive_record(path: Path) -> dict[str, object]:
    ...
    return {
        "archiveId": f"bnu-junior-s1-upper-assessment-archive-{digest[:12]}",
        ...
        with ZipFile(resolved) as archive:
            for info in archive.infolist():
                ...
                if extension not in {".doc", ".docx", ".pdf"}:
                    continue
                file_name = decoded_name.rsplit("/", 1)[-1]
                unit_titles = infer_unit_titles(file_name)
                assessment_families = infer_assessment_families(file_name)
                material_kinds = infer_material_kinds(file_name, assessment_families)
                digest = sha256_for_member(archive, info.filename)
                entries.append(
                    {
                        "entryId": f"bnu-junior-s1-upper-assessment-{digest[:12]}",
                        ...
                        "grade": "S1",
                        "semester": "upper",
                        "role": infer_role(file_name, extension),
                        "assessmentFamilies": assessment_families,
                        "materialKinds": material_kinds,
                        "unitTitles": unit_titles,
                        "difficultyBand": infer_difficulty(file_name),
                        "needsS18MappingReview": needs_mapping_review(file_name, unit_titles),
                        "ownerProvided": True,
...
def qa_report(manifest: dict[str, object], cards: list[dict[str, object]]) -> str:
    ...
    return "\n".join(
        [
            "# Mainland BNU Junior S1 Upper Assessment Metadata QA",
            "",
...
def run_self_test() -> None:
    with TemporaryDirectory() as tmp:
        ...
        with ZipFile(unit_zip, "w") as archive:
            archive.writestr("单元测试/第1章 丰富的图形世界 单元测试卷.docx", BODY_SENTINEL)
            archive.writestr("单元测试/第三章 整式及其加减 单元测试卷.doc", BODY_SENTINEL)
            archive.writestr("单元测试/第6章 数据的收集与整理 单元测试卷.pdf", BODY_SENTINEL)
            archive.writestr("__MACOSX/._ignored", BODY_SENTINEL)
        with ZipFile(period_zip, "w") as archive:
            archive.writestr("期中期末/七年级上册期中模拟卷 第1~3章.docx", BODY_SENTINEL)
            archive.writestr("期中期末/七年级上册期末综合卷 第1~6章.docx", BODY_SENTINEL)
        ...
        assert manifest["counts"]["assessmentFamilies"]["unit-test"] == 3  # type: ignore[index]
        assert manifest["counts"]["assessmentFamilies"]["midterm"] == 1  # type: ignore[index]
        assert manifest["counts"]["assessmentFamilies"]["final"] == 1  # type: ignore[index]
        assert len(cards) == 7
        ...
        assert any(entry["role"] == "assessment-resource" for entry in manifest["sources"])  # type: ignore[index]
    ...
    print("Self-test passed: metadata-only BNU junior S1 upper assessment manifest handles Chinese archives and safe draft cards.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build local-only Mainland BNU junior S1 upper assessment metadata artifacts.")
    parser.add_argument("zip_paths", nargs="*", help="Path(s) to owner-provided BNU junior S1 upper assessment archives.")
    ...
    print(f"Wrote {manifest['totals']['files']} metadata-only BNU junior S1 upper assessment entries to {out_dir}")
