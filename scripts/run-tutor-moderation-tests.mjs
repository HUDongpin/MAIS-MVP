...
const outputDir = join(".tmp", `tutor-moderation-tests-${process.pid}-${Date.now()}`);
...
try {
  const compiled = run(tscBin, ["-p", "tsconfig.tutor-moderation.json", "--outDir", outputDir]);
  ...
  const passed = run("node", [
    ...
    join(outputDir, "lib/server/tutorVoiceModeration.test.js")
  ]);
