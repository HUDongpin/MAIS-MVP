...
export const CALIFORNIA_LAYER_A_PRODUCT_SMOKE_TIMEOUT_MS = 60 * 60_000;
export const CALIFORNIA_MAC_CHROMIUM_TMP_ROOT =
  "/Volumes/Starship/.ca-viz-chrome";
export const CALIFORNIA_DISABLE_CRASHPAD_FOR_TESTING_ARGUMENT =
...
export type CaliforniaAcceptanceMode = "discovery" | "formal";

export type CaliforniaAcceptanceCliOptions = {
...
export type CaliforniaCrashpadContainmentBinding = {
  argument: string;
  disableArgument: typeof CALIFORNIA_DISABLE_CRASHPAD_FOR_TESTING_ARGUMENT;
...
export const CALIFORNIA_ACCEPTANCE_FORMAL_PHASE_SEQUENCE = Object.freeze([
  "preflight-catalog",
  "source-freeze-compose",
  "execution-ownership",
  "layer-a-build",
  "layer-a-product-smoke",
  "layer-a-seal",
  "layer-b-build",
  "layer-b-server-start",
  "broad-producer",
  "broad-finalizer",
  "exhaustive-producer",
  "exhaustive-finalizer",
  "terminal-inventory",
  "terminal-seal"
] as const);

export const CALIFORNIA_ACCEPTANCE_DISCOVERY_PHASE_SEQUENCE = Object.freeze(
  CALIFORNIA_ACCEPTANCE_FORMAL_PHASE_SEQUENCE.slice(0, 12)
);

export type CaliforniaAcceptancePhase =
  typeof CALIFORNIA_ACCEPTANCE_FORMAL_PHASE_SEQUENCE[number];

export type CaliforniaAcceptancePhaseLifecycle = {
  readonly completed: CaliforniaAcceptancePhase[];
  readonly mode: CaliforniaAcceptanceMode;
  readonly next: CaliforniaAcceptancePhase | null;
};

type MutableCaliforniaAcceptancePhaseLifecycle = CaliforniaAcceptancePhaseLifecycle & {
  cursor: number;
};

const californiaAcceptancePhaseLifecycleBrands =
  new WeakSet<MutableCaliforniaAcceptancePhaseLifecycle>();

function californiaAcceptanceExpectedPhases(mode: CaliforniaAcceptanceMode):
readonly CaliforniaAcceptancePhase[] {
  return mode === "formal"
    ? CALIFORNIA_ACCEPTANCE_FORMAL_PHASE_SEQUENCE
    : CALIFORNIA_ACCEPTANCE_DISCOVERY_PHASE_SEQUENCE;
}

export function createCaliforniaAcceptancePhaseLifecycle(
  mode: CaliforniaAcceptanceMode
): CaliforniaAcceptancePhaseLifecycle {
  assert.ok(mode === "formal" || mode === "discovery",
    "California acceptance phase lifecycle mode is invalid");
  const lifecycle: MutableCaliforniaAcceptancePhaseLifecycle = {
    completed: [],
    cursor: 0,
    mode,
    get next() {
      return californiaAcceptanceExpectedPhases(mode)[this.cursor] ?? null;
    }
  };
  californiaAcceptancePhaseLifecycleBrands.add(lifecycle);
  return lifecycle;
}

export async function runCaliforniaAcceptancePhase<T>(options: {
  lifecycle: CaliforniaAcceptancePhaseLifecycle;
  operation(): Promise<T>;
  phase: CaliforniaAcceptancePhase;
}): Promise<T> {
  const lifecycle = options.lifecycle as MutableCaliforniaAcceptancePhaseLifecycle;
  assert.ok(californiaAcceptancePhaseLifecycleBrands.has(lifecycle),
    "California acceptance phase lifecycle is not runner-owned");
  const expected = californiaAcceptanceExpectedPhases(lifecycle.mode)[lifecycle.cursor];
  assert.ok(expected,
    `California ${lifecycle.mode} acceptance has already completed every phase`);
  assert.equal(options.phase, expected,
    `California ${lifecycle.mode} acceptance phase order drifted`);
  assert.equal(typeof options.operation, "function",
    `California acceptance ${options.phase} operation is missing`);
  const result = await options.operation();
  (lifecycle.completed as CaliforniaAcceptancePhase[]).push(options.phase);
  lifecycle.cursor += 1;
  return result;
}

export function assertCaliforniaAcceptancePhaseLifecycleComplete(
  lifecycleValue: CaliforniaAcceptancePhaseLifecycle
): void {
  const lifecycle = lifecycleValue as MutableCaliforniaAcceptancePhaseLifecycle;
  assert.ok(californiaAcceptancePhaseLifecycleBrands.has(lifecycle),
    "California acceptance phase lifecycle is not runner-owned");
  const expected = californiaAcceptanceExpectedPhases(lifecycle.mode);
  assert.deepEqual(lifecycle.completed, expected,
    `California ${lifecycle.mode} acceptance phase sequence is incomplete`);
  assert.equal(lifecycle.cursor, expected.length,
    `California ${lifecycle.mode} acceptance phase cursor is incomplete`);
  assert.equal(lifecycle.next, null,
    `California ${lifecycle.mode} acceptance has an unexecuted terminal phase`);
}
...
export function parseCaliforniaAcceptanceCli(argv: readonly string[]): CaliforniaAcceptanceCliOptions {
  ...
  assert.ok(mode === "discovery" || mode === "formal",
    "--mode must be exactly discovery or formal");
...
export function buildCaliforniaAcceptanceEnvironment(options: {
  ...
  Object.assign(env, {
    ...
    LLM_API_KEY: "",
    MAC_CHROMIUM_TMPDIR: CALIFORNIA_MAC_CHROMIUM_TMP_ROOT,
...
export function assertCaliforniaWritableEnvironmentClosed(options: {
  ...
  assert.equal(
    options.env.MAC_CHROMIUM_TMPDIR,
    CALIFORNIA_MAC_CHROMIUM_TMP_ROOT,
    "macOS Chrome process-singleton temp must use the exact short Starship root"
  );
}
...
async function ensureWritableEnvironmentDirectories(env: NodeJS.ProcessEnv) {
  ...
  await mkdir(CALIFORNIA_MAC_CHROMIUM_TMP_ROOT, { recursive: true, mode: 0o700 });
  await assertDirectoryNoSymlink(
    CALIFORNIA_MAC_CHROMIUM_TMP_ROOT,
    "macOS Chrome process-singleton temp root"
  );
...
export async function verifyManifestTree(options: {
  ...
  for (const entry of options.entries) {
    ...
    if (options.verifyMode !== false) {
      assert.equal(actual.mode, entry.mode, `${entry.path}: durable mode drifted`);
    }
  }
}
...
export function buildCaliforniaCommandPlans(options: {
  ...
  return {
    ...
    ffmpegInstall: {
      argv: [node, options.entrypoints.playwrightCli, "install", "ffmpeg"],
      cwd: CALIFORNIA_ACCEPTANCE_WORKTREE_ROOT,
      label: "Starship-local Playwright FFmpeg preflight"
    },
    productSmoke: {
...
export async function verifyCaliforniaPlaywrightFfmpeg(options: {
  env: NodeJS.ProcessEnv;
  runRoot: string;
}) {
  const browsersRoot = path.resolve(options.env.PLAYWRIGHT_BROWSERS_PATH ?? "");
  assert.ok(options.env.PLAYWRIGHT_BROWSERS_PATH && path.isAbsolute(browsersRoot),
    "PLAYWRIGHT_BROWSERS_PATH must be one explicit absolute path");
  assert.ok(isInside(options.runRoot, browsersRoot),
    "Playwright browser binaries must stay inside the fresh Starship run root");
  assert.equal(await realpath(browsersRoot), browsersRoot,
    "Playwright browser binary root must not traverse a symlink");
  const ffmpegDirectories = (await readdir(browsersRoot, { withFileTypes: true }))
    .filter((entry) => entry.isDirectory() && /^ffmpeg-[0-9]+$/.test(entry.name));
  assert.equal(ffmpegDirectories.length, 1,
    "Playwright browser root must contain exactly one revisioned FFmpeg directory");
  const ffmpegRoot = path.join(browsersRoot, ffmpegDirectories[0]!.name);
  await assertNoSymlinkSegments(browsersRoot, ffmpegRoot);
  ...
  await assertNoSymlinkSegments(browsersRoot, executablePath);
  const identity = await lstat(executablePath);
  assert.ok(identity.isFile() && !identity.isSymbolicLink() && identity.nlink === 1,
    "Playwright FFmpeg must be one regular non-symlink, non-hardlinked file");
  if (process.platform !== "win32") {
    assert.notEqual(identity.mode & 0o111, 0,
      "Playwright FFmpeg must retain an executable mode");
  }
  ...
  assert.match(version, /^ffmpeg version /,
    "Playwright FFmpeg did not return its expected version banner");
  const bytes = await readFile(executablePath);
  return {
    executablePath,
    revisionDirectory: ffmpegDirectories[0]!.name,
    sha256: sha256(bytes),
    size: bytes.length,
...
function assertProductSmokeCrashpadContainment(
  ...
  exactKeys(rawContainment, ["argument", "disableArgument", "scheme", "verificationBoundary"],
  ...
  assert.equal(rawContainment.disableArgument, CALIFORNIA_DISABLE_CRASHPAD_FOR_TESTING_ARGUMENT,
    `${label}: Crashpad testing-disable argument drifted`);
...
export function buildCaliforniaProductSmokeManifest(options: {
  ...
  return {
    ...
    crashpadContainment: {
      ...
      disableArgument: canonical.crashpadContainment.disableArgument,
...
export function layerEnvironment(options: {
  ...
  if (options.layer === "a") {
    ...
    Object.assign(extras, {
      ...
      CA_SIGNATURE_FROZEN_SOURCE_ROOT: options.layout.frozenSourceRoot,
...
export type CaliforniaNextEnvSnapshot = {
  absolutePath: string;
  content: Buffer;
  handle: FileHandle;
...
function assertSingleRegularBuildFile(identity: BigIntStats, label: string) {
  assert.ok(identity.isFile() && !identity.isSymbolicLink() && identity.nlink === 1n,
    `${label}: expected one regular non-symlink, non-hardlinked file`);
}

function sameBuildFileObject(left: BigIntStats, right: BigIntStats) {
  return left.dev === right.dev && left.ino === right.ino && left.nlink === right.nlink &&
    left.isFile() === right.isFile();
}

export async function snapshotCaliforniaNextEnv(sourceRoot: string): Promise<CaliforniaNextEnvSnapshot> {
  const resolvedRoot = path.resolve(sourceRoot);
  assert.ok(isInside(CALIFORNIA_ACCEPTANCE_TMP_ROOT, resolvedRoot),
    "California next-env snapshot source must stay under the acceptance .tmp root");
  assert.equal(await realpath(resolvedRoot), resolvedRoot,
    "California next-env snapshot source must not traverse a symlink");
  const absolutePath = path.join(resolvedRoot, "next-env.d.ts");
  const before = await lstat(absolutePath, { bigint: true });
  assertSingleRegularBuildFile(before, "California next-env snapshot path");
  assert.equal(await realpath(absolutePath), absolutePath,
    "California next-env snapshot path must not traverse a symlink");
  const handle = await open(
    absolutePath,
  ...
  try {
    ...
    assertSingleRegularBuildFile(opened, "Open California next-env snapshot");
    assert.ok(sameBuildFileObject(before, opened),
      "California next-env changed before it could be snapshotted");
    const content = await handle.readFile();
    const afterRead = await handle.stat({ bigint: true });
    const afterPath = await lstat(absolutePath, { bigint: true });
    assert.ok(sameBuildFileObject(opened, afterRead) && sameBuildFileObject(afterRead, afterPath),
      "California next-env changed while it was being snapshotted");
    return {
      absolutePath,
      content,
      handle,
...
export async function restoreCaliforniaNextEnv(snapshot: CaliforniaNextEnvSnapshot) {
  try {
    const [opened, currentPath] = await Promise.all([
      snapshot.handle.stat({ bigint: true }),
      lstat(snapshot.absolutePath, { bigint: true })
    ]);
    assertSingleRegularBuildFile(opened, "Open California next-env restore file");
    assertSingleRegularBuildFile(currentPath, "California next-env restore path");
    assert.ok(
      sameBuildFileObject(snapshot.identity, opened) && sameBuildFileObject(opened, currentPath),
      `Refusing to restore California next-env because its pathname identity changed: ${snapshot.absolutePath}`
    );
    await snapshot.handle.truncate(0);
    if (snapshot.content.length > 0) {
      const { bytesWritten } = await snapshot.handle.write(
        snapshot.content,
        0,
        snapshot.content.length,
        0
      );
      assert.equal(bytesWritten, snapshot.content.length,
        `Could not restore every California next-env byte: ${snapshot.absolutePath}`);
    }
    await snapshot.handle.chmod(snapshot.originalMode);
    ...
    assert.ok(
      sameBuildFileObject(snapshot.identity, restored) && sameBuildFileObject(restored, restoredPath),
      `California next-env identity changed during restoration: ${snapshot.absolutePath}`
    );
    assert.equal(restored.size, BigInt(snapshot.content.length),
      `California next-env size changed during restoration: ${snapshot.absolutePath}`);
    const reread = Buffer.alloc(snapshot.content.length);
    let offset = 0;
    while (offset < reread.length) {
      const { bytesRead } = await snapshot.handle.read(
        reread,
        offset,
    ...
    assert.equal(offset, snapshot.content.length,
      `California next-env reread ended early: ${snapshot.absolutePath}`);
    assert.ok(reread.equals(snapshot.content),
      `California next-env bytes changed during restoration: ${snapshot.absolutePath}`);
    const finalIdentity = await snapshot.handle.stat({ bigint: true });
    assert.ok(sameBuildFileObject(restored, finalIdentity),
      `California next-env identity changed during final reread: ${snapshot.absolutePath}`);
  } finally {
...
async function runBuild(options: {
  ...
  const nextEnvSnapshot = await snapshotCaliforniaNextEnv(options.sourceRoot);
  ...
  try {
    ...
    await options.operations.runCommand({
      ...options.plan,
      env: options.env,
      ...paths,
      timeoutMs: 30 * 60_000
    ...
    try {
      await restoreCaliforniaNextEnv(nextEnvSnapshot);
    } catch (error) {
      if (cleanupError) throw new AggregateError(
        [cleanupError, error],
        "California build cleanup and next-env restoration both failed"
      );
      throw error;
    }
...
export async function runCaliforniaVisualizationAcceptance(
  ...
  const phaseLifecycle = createCaliforniaAcceptancePhaseLifecycle(cli.mode);
  ...
  await runCaliforniaAcceptancePhase({
    ...
    operation: async () => {
      ...
      await operations.runCommand({
        ...
        env: preflightEnv,
        expectedExitCodes: [0],
        stderrPath: path.join(runRoot, "preflight", "logs", "ffmpeg-install.stderr.log"),
        stdoutPath: path.join(runRoot, "preflight", "logs", "ffmpeg-install.stdout.log"),
        timeoutMs: 120_000
      });
      await verifyCaliforniaPlaywrightFfmpeg({ env: preflightEnv, runRoot });
    }
  });
  ...
  const actualBuildIdA = await runCaliforniaAcceptancePhase({
    lifecycle: phaseLifecycle,
    phase: "layer-a-build",
    operation: () => runBuild({
      env: envA,
  ...
  const layerASeal = await runCaliforniaAcceptancePhase({
    lifecycle: phaseLifecycle,
    phase: "layer-a-seal",
    operation: () => sealLayerA(
      layout.layerAReceiptPath,
      layout.layerASealPath,
      layerAReceipt
    )
  });
  ...
  const serverB = await runCaliforniaAcceptancePhase({
    lifecycle: phaseLifecycle,
    phase: "layer-b-server-start",
    operation: async () => {
      await operations.assertPortFree(cli.layerBPort);
      return operations.startServer({
        ...plans.startB,
  ...
  try {
    await runCaliforniaAcceptancePhase({
      lifecycle: phaseLifecycle,
      phase: "broad-producer",
      operation: () => operations.runCommand({
        ...plans.broad,
        env: withPlaywrightPort(envB, cli.layerBPort),
    ...
    await runCaliforniaAcceptancePhase({
      lifecycle: phaseLifecycle,
      phase: "exhaustive-producer",
      operation: () => runCaliforniaSignatureExhaustiveProducerUnderCapacityHold({
    ...
    const ledgerResult = await runCaliforniaAcceptancePhase({
      lifecycle: phaseLifecycle,
      phase: "exhaustive-finalizer",
      operation: () => operations.runCommand({
        ...
        expectedExitCodes: cli.mode === "discovery"
    ...
    if (cli.mode === "discovery") {
      ...
      assertCaliforniaAcceptancePhaseLifecycleComplete(phaseLifecycle);
  ...
  const inventory = await runCaliforniaAcceptancePhase({
    lifecycle: phaseLifecycle,
    phase: "terminal-inventory",
    operation: () => finalInventory(layout, sourceManifest)
  });
  ...
  const { finalReread, finalSeal } = await runCaliforniaAcceptancePhase({
    lifecycle: phaseLifecycle,
    phase: "terminal-seal",
    operation: async () => {
      const published = await publishFinalSeal({
        layout,
      ...
      return { finalReread: reread, finalSeal: published };
    }
  });
