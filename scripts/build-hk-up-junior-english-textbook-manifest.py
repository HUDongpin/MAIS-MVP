#!/usr/bin/env python3
"""Build local-only metadata artifacts for Hong Kong UP junior English textbook PDFs.

The script records file metadata, checksums, volume/grade/semester signals,
page-count signals, and safe chapter abstractions only. It does not extract or
persist PDF body text, OCR text, exercises, worked responses, figures, tables,
page images, page locators, source excerpts, or embeddings. Default outputs
live under `.local/`, which is ignored by the repository.
"""
...
DEFAULT_OUT_DIR = Path(".local/rag/hk-up-junior-en")
...
SAFETY_NOTE = (
    "Local metadata-only artifact for Pearson/UP junior English S1-S3 textbooks. "
    "Do not commit source PDFs, extracted PDF body text, OCR text, exercises, "
    "worked responses, figures, tables, page images, page locators, source "
...
def build_entry(path: Path) -> dict[str, object]:
    ...
    if volume == "2A":
        review_flags.append("large-page-count-review-needed")
    ...
    return {
        ...
        "seriesAlias": "Pearson/UP junior English; Mathematics and Life third edition",
...
def draft_cards(entries: list[dict[str, object]]) -> list[dict[str, object]]:
    ...
    for entry in entries:
        ...
        for index, chapter in enumerate(entry["chapterSignals"], start=1):  # type: ignore[index]
            cards.append({
                "draftId": f"hk-up-junior-en-{volume.lower()}-{index:02d}",
                "curriculumTrack": "HK",
                "publisher": "HK_UNITED_PRIME_MIA",
                "stage": "junior-secondary",
                "sourceLanguage": "en",
                ...
                "safeSummaryDraft": "Use this card as abstract UP junior English textbook sequencing and item-design guidance only.",
...
def build_manifest(paths: list[Path]) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    manifest = {
        ...
        "sourceFamily": "Hong Kong UP/Pearson junior English S1-S3 textbook PDFs",
        "totals": {
            "files": len(entries),
...
def qa_report(manifest: dict[str, object], cards: list[dict[str, object]]) -> str:
    ...
    two_a_flags = [
        ...
        if entry["volume"] == "2A"  # type: ignore[index]
    ]
    return "\n".join([
        "# HK UP Junior English Textbook Local Manifest QA",
        "",
        ...
        f"- 2A review flags: {json.dumps(two_a_flags, ensure_ascii=False)}",
        ...
        "- Passed: no PDF body text, OCR text, exercises, worked responses, figures, tables, page images, page locators, source excerpts, or embeddings are extracted or persisted.",
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for volume in EXPECTED_VOLUMES:
            ...
            path = root / f"Pearson UP junior English {volume}.pdf"
    ...
    assert manifest["totals"]["pageCount"] == 2889, manifest["totals"]
    ...
    two_a = [entry for entry in manifest["entries"] if entry["volume"] == "2A"]  # type: ignore[index]
    assert two_a and "large-page-count-review-needed" in two_a[0]["reviewFlags"], two_a
    serialized = json.dumps(manifest, ensure_ascii=False)
    for forbidden in ["body text sample", "sourcePage", "page image bytes", "embeddingPayload", "workedResponsePayload"]:
        assert forbidden not in serialized
    print("Self-test passed: HK UP junior English manifest builder recognized 6 volumes, 2889 pages, 18 safe-card drafts, and the 2A review flag.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build local-only Hong Kong UP junior English textbook metadata artifacts.")
    parser.add_argument("pdf_paths", nargs="*", help="Paths to the six local UP/Pearson junior English textbook PDFs.")
    parser.add_argument("--out-dir", default=str(DEFAULT_OUT_DIR), help="Output directory. Defaults to ignored .local/rag/hk-up-junior-en/.")
    ...
    print(
        "Wrote local HK UP junior English manifest with "
