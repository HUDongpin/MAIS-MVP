#!/usr/bin/env python3
"""Build a local-only metadata manifest for Mainland PEP junior math PDFs.

The script records coarse file metadata only. It does not extract, persist, or
print PDF body text, OCR text, exercises, worked examples, answers, tables,
illustrations, page images, or page-level descriptions. Default outputs live
...
DEFAULT_OUT_DIR = Path(".local/rag/mainland-pep-junior")
SAFETY_NOTE = (
    ...
    "page images, source locators, or page-level descriptions."
)
...
def infer_semester(path: Path) -> str:
    name = path.name
    if any(marker in name for marker in ["上册", "上冊", "上学期", "上學期", "七上", "八上", "九上", "upper"]):
        return "upper"
    if any(marker in name for marker in ["下册", "下冊", "下学期", "下學期", "七下", "八下", "九下", "lower"]):
        return "lower"
    return "unknown"
...
def manifest_entry(path: Path) -> dict[str, object]:
    ...
    return {
        "id": f"pep-junior-{grade.lower()}-{semester}-{hashlib.sha1(str(resolved).encode('utf-8')).hexdigest()[:10]}",
...
def coverage(entries: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        ...
        "completeS1ToS3UpperLower": len(missing) == 0 and len(entries) == 6,
        "missing": missing,
...
def qa_report(manifest: dict[str, object]) -> str:
    ...
    return "\n".join([
        "# Mainland PEP Junior PDF Manifest QA",
        "",
        ...
        "- Passed: no PDF body text, OCR text, exercises, answers, tables, figure descriptions, page images, source locators, or page-level descriptions are extracted or persisted.",
...
def run_self_test() -> None:
    with TemporaryDirectory() as tmp:
        tmp_dir = Path(tmp)
        paths = []
        chinese_grades = ["七年级", "八年级", "九年级"]
        for index, grade_name in enumerate(chinese_grades, start=1):
            for semester_name in ["上册", "下册"]:
                path = tmp_dir / f"人教版初中数学{grade_name}{semester_name}2024秋.pdf"
                path.write_bytes(fake_pdf_bytes(page_count=index + 7))
                paths.append(path)
    ...
    print("Self-test passed: metadata-only manifest covers 6 S1-S3 upper/lower PDFs.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Build a local-only Mainland PEP junior PDF metadata manifest.")
    parser.add_argument("pdf_paths", nargs="*", help="Paths to the 6 private S1-S3 upper/lower PEP junior math PDFs.")
