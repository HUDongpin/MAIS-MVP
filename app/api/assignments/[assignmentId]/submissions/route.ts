...
async function mediaPolicyViolationResponse(imageDataUrl: unknown, userId: string) {
  ...
  await recordAiGovernanceEvent({
    ...
    console.error("Assignment media governance event recording failed", error instanceof Error ? error.name : typeof error);
  });
...
async function validateAssignmentImageObject(
  ...
  if (!decision.allowed) {
    await recordAiGovernanceEvent({
      ...
      console.error("Assignment object governance event recording failed", error instanceof Error ? error.name : typeof error);
    });
    return {
  ...
  if (stored.status !== "ok") {
    await recordAiGovernanceEvent({
      ...
      console.error("Assignment object read governance event recording failed", error instanceof Error ? error.name : typeof error);
    });
    return {
      response: NextResponse.json(
...
export async function POST(request: Request, { params }: { params: Promise<{ assignmentId: string }> }) {
  ...
  if (!body) return NextResponse.json({ error: "Invalid submission payload." }, { status: 400 });
  ...
  if (result.status !== "submitted") {
    const status = result.status === "not-found" ? 404 : result.status === "closed" ? 409 : 400;
