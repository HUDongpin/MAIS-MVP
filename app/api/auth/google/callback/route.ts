...
import { sessionCookieOptions, sessionSecretMissingResponse, setSessionCookie as defaultSetSessionCookie } from "@/lib/server/sessionCookie";
...
export type GoogleCallbackDependencies = {
  ...
  exchangeGoogleAuthorizationCode: (input: { code: string; config: GoogleOAuthConfig }) => Promise<{ status: "ok"; idToken: string } | { status: "invalid" }>;
...
export async function handleGoogleOAuthCallback(request: Request, dependencies: GoogleCallbackDependencies = defaultDependencies) {
  ...
  if (authenticated.status === "teacher-invite-required") return redirectToLogin(request, "teacher_invite_required");
