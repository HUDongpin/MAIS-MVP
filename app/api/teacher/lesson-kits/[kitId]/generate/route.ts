...
import { generateTeacherLessonKitWithAI } from "@/lib/server/userStore";
...
export async function POST(request: Request, { params }: { params: Promise<{ kitId: string }> }) {
  ...
  const result = await generateTeacherLessonKitWithAI({
  ...
  if (result.status !== "generated") {
    const status =
      result.status === "missing-config" ? 503 :
      result.status === "forbidden" ? 403 :
      result.status === "not-found" ? 404 :
      result.status === "provider-error" ? 502 :
      400;
