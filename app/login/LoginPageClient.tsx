...
import { resolveGoogleSignInUi, type GoogleSignInErrorCode } from "@/lib/googleSignInUi";
import { formatGradeLabelForCurriculum, formatLearnerName } from "@/lib/i18n";
import type { CurriculumProfile, CurriculumRegion, Grade, GradeId, LocalizedText, TextbookPublisher } from "@/types";
...
const authLinkCopy = {
  ...
  googleUnavailable: {
    en: "Google sign-in is not available here. Use your email and password to log in.",
    zh: "此處尚未提供 Google 登入。請使用電郵和密碼登入。",
    zhHans: "此处尚未提供 Google 登录。请使用邮箱和密码登录。"
  },
  googleErrors: {
...
function googleSignInErrorMessage(
  errorCode: GoogleSignInErrorCode | null,
  translate: (copy: LocalizedText) => string
) {
  switch (errorCode) {
    case "teacher_invite_required":
      return translate(authLinkCopy.googleErrors.teacherInviteRequired);
    case "generic":
      return translate(authLinkCopy.googleErrors.generic);
    case null:
      return "";
    default: {
...
export default function LoginPageClient({
  ...
  const googleSignInUi = resolveGoogleSignInUi({
    available: googleSignInAvailable,
    googleError
  });
  const googleErrorMessage = googleSignInErrorMessage(googleSignInUi.errorCode, t);
      ...
      <div className="mx-auto grid max-w-3xl gap-6">
        <section className="glass-panel p-6 sm:p-8">
          ...
          <form method="post" action="/api/auth/login" onSubmit={handleSubmit} className="mt-8 grid gap-3">
            ...
            {googleSignInUi.showUnavailableNotice ? (
              <p role="status" className="rounded-2xl border border-slate-200/80 bg-slate-100/80 px-4 py-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:bg-white/[0.06] dark:text-slate-200">
                {t(authLinkCopy.googleUnavailable)}
              </p>
            ) : null}
