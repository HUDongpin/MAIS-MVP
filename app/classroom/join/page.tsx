...
export default function JoinClassPage() {
  ...
  async function joinClass(event: FormEvent<HTMLFormElement>) {
    ...
    setIsSaving(false);
    if (!response.ok) {
      setMessage(t({ en: "No class was found for this invite code.", zh: "找不到此邀請碼對應的班級。" }));
      return;
    }
    setMessage(
      payload?.status === "duplicate"
        ? t({ en: "You are already in this class.", zh: "你已加入此班級。" })
        : t({ en: `Joined ${payload?.class?.name ?? "class"}.`, zh: `已加入 ${payload?.class?.name ?? "班級"}。` })
    );
  }
  ...
  return (
    <div className="page-container py-10 sm:py-12">
      <section className="glass-panel mx-auto max-w-3xl p-6 sm:p-8">
        <p className="text-sm font-black uppercase tracking-[0.24em] text-cyan-600 dark:text-cyan-300">{t({ en: "Class invite", zh: "班級邀請" })}</p>
        <h1 className="mt-3 text-4xl font-black tracking-tight text-slate-950 dark:text-white">{t({ en: "Join a teacher class", zh: "加入教師班級" })}</h1>
        <form onSubmit={joinClass} className="mt-7 grid gap-4 sm:grid-cols-[1fr_auto]">
          <input
            value={inviteCode}
            onChange={(event) => setInviteCode(event.target.value.toUpperCase())}
            required
            placeholder={t({ en: "Invite code", zh: "邀請碼" })}
          ...
          <button disabled={isSaving} className="focus-ring rounded-full bg-slate-950 px-6 py-3 text-sm font-black text-white disabled:opacity-50 dark:bg-white dark:text-slate-950">
            {isSaving ? t({ en: "Joining", zh: "加入中" }) : t({ en: "Join", zh: "加入" })}
          </button>
        </form>
