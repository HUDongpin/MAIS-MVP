...
import {
  ...
  rebaseLearningAnalyticsClearFence,
  ...
  assert.equal(
    isDurableLearningAnalyticsDeliveryResponse(
      409,
      {
        ...
        currentClearRequestId: "clear-request-4",
        currentGeneration: 4,
        dispositions: [
...
test("clear and generation-mismatch receipts are strict, exact-user, and canonical", () => {
  const clearReceipt = {
    ...
    generation: 4,
    ok: true,
    requestId: "clear-request-4"
  };
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, clearReceipt, "student/A+B", 3, "clear-request-4"),
    true
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, null, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(202, clearReceipt, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, clearedAt: null }, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, clearedAt: "2026-05-07T12:00:00Z" }, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, acknowledgedUserId: "student/A B" }, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, generation: 3 }, "student/A+B", 3, "clear-request-4"),
    false
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, generation: 5 }, "student/A+B", 3, "clear-request-4"),
    false,
    "an exact clear request can advance only its one server generation"
  );
  assert.equal(
    isLearningAnalyticsClearAcknowledgement(200, { ...clearReceipt, requestId: "other" }, "student/A+B", 3, "clear-request-4"),
    false
  );
...
test("a durable exact-user clear fence blocks every writer and survives a lost DELETE ACK", () => {
  ...
  assert.equal(
    resolveLearningAnalyticsClearHandshake(
      ...
      3,
      "2026-05-07T12:00:00.200Z",
      "different-clear-request"
    ),
    "rebase-delete"
  );
  const rebasedFence = rebaseLearningAnalyticsClearFence(
    storage,
    restartedFence.value,
    3
  );
  assert.equal(rebasedFence.baseGeneration, 3);
  assert.equal(rebasedFence.requestId, restartedFence.value.requestId);
  assert.equal(rebasedFence.deleteAttemptedAt, null);
  assert.deepEqual(rebasedFence.clearedStorageKeys, restartedFence.value.clearedStorageKeys);
  assert.equal(
...
test("a distinct prior clear receipt rebases and preserves the pending local clear intent", () => {
  const storage = memoryStorage();
  const userId = "student-clear-a-event-b";
  replaceLearningAnalyticsGeneration(storage, userId, 0);
  const fenceB = beginLearningAnalyticsClearFence(
    storage,
    userId,
    0,
    "2099-05-07T12:00:00.000Z",
    "clear-request-b"
  );
  const postClearAEvent = event({
    id: "event-created-after-clear-a",
    timestamp: "1999-05-07T12:00:00.000Z"
  });
  assert.equal(
    persistLearningAnalyticsEventsForCurrentProtocol(
      storage,
  ...
  const retryFence = prepareLearningAnalyticsClearDeleteAfterMismatch(
    ...
    fenceB,
    {
      ...
      currentGeneration: 1,
      clearedAt: "2026-05-07T12:00:00.000Z",
      currentClearRequestId: "clear-request-a"
    }
  );
  assert.ok(retryFence);
  assert.equal(retryFence.baseGeneration, 1);
  assert.equal(retryFence.requestId, fenceB.requestId);
  assert.equal(retryFence.requestedAt, fenceB.requestedAt);
  assert.equal(retryFence.deleteAttemptedAt, null);
  assert.equal(
    readLearningAnalyticsClearFence(storage, userId).status,
    "valid",
    "the local B intent must remain fenced until its rebased DELETE is durably ACKed"
  );
  assert.deepEqual(
    readUnconfirmedLearningAnalyticsOutbox(storage, userId).map(({ id }) => id),
    [postClearAEvent.id],
    "client/server timestamps are never used to treat a distinct clear as satisfying B"
  );
});
...
test("AppProviders clear fences identity, local queues, and the exact server generation", async () => {
  ...
  assert.match(
    clearSource,
    /method: "DELETE",\s*headers: \{\s*"X-MAIS-Analytics-User-Id": encodeURIComponent\(clearUserId\),\s*"X-MAIS-Analytics-Generation": String\(clearFence\.baseGeneration\),\s*"X-MAIS-Analytics-Clear-Request-Id": clearFence\.requestId/
  );
  assert.match(clearSource, /isLearningAnalyticsClearAcknowledgement\([\s\S]*clearUserId,[\s\S]*clearFence\.baseGeneration,[\s\S]*clearFence\.requestId/);
...
test("AppProviders installs remote causal boundaries before finalization and keeps session retries wakeable", async () => {
  ...
  const freezeIndex = adoptSource.indexOf("freezeLearningAnalyticsIdentity();");
  ...
  assert.match(
    sessionSource,
    /const flushIdentity = analyticsIdentityRef\.current;[\s\S]*flushIdentity\.userId !== flushUserId[\s\S]*const sessionDeliveryIsCurrent = \(\) =>[\s\S]*currentUserRef\.current\.id === flushUserId[\s\S]*isCurrentLearningAnalyticsIdentity\([\s\S]*flushIdentity/
  );
  assert.match(
    sessionSource,
