...
test("session tokens require an explicit production secret", async () => {
  ...
  try {
    ...
    await assert.rejects(() => createSessionToken("student-peter", "credential-tag"));
...
test("session tokens verify with a configured secret", async () => {
  ...
  try {
    ...
    const token = await createSessionToken("student-peter", "credential-tag", Date.UTC(2026, 4, 7));
