import { isCcssId, normalizeCcssId, normalizeCcssIds } from "./ccssId";
 ...
 *
 * Without this resolver, a crosswalk row's only correctness control in the
 ...
 * exact AND the asset itself carries every CCSS id that row maps to. That
...
export function resolveStandardRefWithBinding(
  ...
  if (binding.kind === "ccss-identity") {
    ...
    return isCcssId(stateStandardId) && assetIds.has(normalizeCcssId(stateStandardId))
  ...
  const mappedIds = normalizeCcssIds(row.ccss.filter(isCcssId));
  if (mappedIds.length !== row.ccss.length) {
    return { display: "hidden", reason: "asset-does-not-carry-standard" };
  }
  // An exact row that maps multiple CCSS statements may only be claimed when
  // the asset carries all of them. A partial relation needs at least one.
  const overlaps = mappedIds.length > 0 && (row.relation === "exact"
    ? mappedIds.every((id) => assetIds.has(id))
    : mappedIds.some((id) => assetIds.has(id)));
