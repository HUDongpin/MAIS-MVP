...
test("read isolates users and sorts by queuedAt, moduleId, topicId, then source", () => {
  const storage = memoryStorage();
...
test("read uses source as the final deterministic order for equal queue scopes", () => {
  const storage = memoryStorage();
  ...
  queueVisualizationSessionOutbox(storage, visualizationLab);
...
test("queue keeps different sources as distinct immutable revisions for one module and topic", () => {
  const storage = memoryStorage();
  const first = record({ source: "geometry", queuedAt: 100 });
  const secondSource = record({ source: "visualization-lab", queuedAt: 200 });

  const firstResult = queueVisualizationSessionOutbox(storage, first);
  const secondResult = queueVisualizationSessionOutbox(storage, secondSource);

  assert.deepEqual(firstResult, first);
  assert.deepEqual(secondResult, secondSource);
  assert.deepEqual(
    readVisualizationSessionOutbox(storage, first.userId),
    [first, secondSource]
  );
  assert.notEqual(
    visualizationSessionOutboxRecordStorageKey(first),
    visualizationSessionOutboxRecordStorageKey(secondSource)
  );
  assert.equal(storage.length, 2);
});

test("queue preserves the first queue age for retries of one exact source identity", () => {
  const storage = memoryStorage();
  const first = record({ source: "geometry", queuedAt: 100 });
  const retry = record({ source: "geometry", queuedAt: 200 });

  assert.deepEqual(queueVisualizationSessionOutbox(storage, first), first);
  assert.deepEqual(queueVisualizationSessionOutbox(storage, retry), first);
  assert.deepEqual(readVisualizationSessionOutbox(storage, first.userId), [first]);
  assert.equal(storage.length, 1);
});

test("ACK removes only its exact source revision and leaves a same-scope source peer", () => {
  const storage = memoryStorage();
  const geometry = record({ source: "geometry", queuedAt: 100 });
  const visualizationLab = record({ source: "visualization-lab", queuedAt: 200 });
  queueVisualizationSessionOutbox(storage, geometry);
  queueVisualizationSessionOutbox(storage, visualizationLab);

  assert.equal(acknowledgeVisualizationSessionOutbox(storage, geometry), true);
  assert.deepEqual(
    readVisualizationSessionOutbox(storage, geometry.userId),
    [visualizationLab]
  );
});

test("quarantine removes only its exact source revision and leaves a same-scope source peer", () => {
  const storage = memoryStorage();
  const rejected = record({ source: "geometry", queuedAt: 100 });
  const peer = record({ source: "visualization-lab", queuedAt: 200 });
  queueVisualizationSessionOutbox(storage, rejected);
  queueVisualizationSessionOutbox(storage, peer);

  assert.equal(
    quarantineVisualizationSessionOutboxRecord(storage, rejected, "server-rejected"),
    true
  );
  assert.deepEqual(readVisualizationSessionOutbox(storage, rejected.userId), [peer]);
});
