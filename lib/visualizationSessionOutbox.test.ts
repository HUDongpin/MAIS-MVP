...
test("queue is idempotent and preserves the first revision for one exact tuple", () => {
  const storage = memoryStorage();
  const first = record({ source: "geometry", queuedAt: 100 });
  const conflictingRetry = record({ source: "geometry", queuedAt: 200 });
...
test("queue preserves distinct source revisions for one module and topic", () => {
  const storage = memoryStorage();
  const geometry = record({ source: "geometry", queuedAt: 20 });
  const coordinatePlane = record({ source: "coordinate-plane", queuedAt: 20 });

  queueVisualizationSessionOutbox(storage, geometry);
  queueVisualizationSessionOutbox(storage, coordinatePlane);

  assert.deepEqual(
    readVisualizationSessionOutbox(storage, geometry.userId).map((entry) => entry.source),
    ["coordinate-plane", "geometry"],
    "source is part of the exact durable identity and deterministic order"
  );
  assert.equal(acknowledgeVisualizationSessionOutbox(storage, geometry), true);
  assert.deepEqual(readVisualizationSessionOutbox(storage, geometry.userId), [coordinatePlane]);
});
...
test("quarantine fails closed when a peer rewrites the exact immutable key during removal", () => {
  const backing = memoryStorage();
  const rejected = record({ queuedAt: 40, source: "geometry" });
  queueVisualizationSessionOutbox(backing, rejected);
  const rejectedKey = visualizationSessionOutboxRecordStorageKey(rejected);
  let rewritten = false;
  const storage = {
    ...
    removeItem(key: string) {
      backing.removeItem(key);
      if (key === rejectedKey && !rewritten) {
        rewritten = true;
        backing.setItem(key, JSON.stringify(rejected));
      }
    },
    setItem(key: string, value: string) {
      backing.setItem(key, value);
  ...
  assert.equal(backing.getItem(rejectedKey), JSON.stringify(rejected));
  assert.deepEqual(readVisualizationSessionOutbox(backing, rejected.userId), [rejected]);
});

test("quarantine verifies its exact artifact before deleting the source", () => {
  for (const setBehavior of ["no-op", "mutate"] as const) {
    const backing = memoryStorage();
    const rejected = record({
      queuedAt: setBehavior === "no-op" ? 50 : 51,
      source: "geometry",
    });
    queueVisualizationSessionOutbox(backing, rejected);
    const rejectedKey = visualizationSessionOutboxRecordStorageKey(rejected);
    ...
    const storage = {
      ...
      removeItem(key: string) {
        if (key === rejectedKey) sourceRemoveCalls += 1;
        backing.removeItem(key);
      },
      setItem(key: string, value: string) {
        if (key !== quarantineKey) {
          backing.setItem(key, value);
        } else if (setBehavior === "mutate") {
          backing.setItem(key, `${value} `);
        }
      },
    };
    ...
    assert.equal(sourceRemoveCalls, 0, setBehavior);
    assert.equal(backing.getItem(rejectedKey), JSON.stringify(rejected), setBehavior);
    assert.deepEqual(
      readVisualizationSessionOutbox(backing, rejected.userId),
      [rejected],
      setBehavior,
    );
  }
...
test("session ACK requires an exact durable identity, module, topic, and source match", () => {
  ...
  assert.equal(
    ...
    false,
    "a response with an unmodeled top-level field is not the strict ACK"
  );
  assert.equal(
    isVisualizationSessionOutboxAcknowledgement(200, {
      ...acknowledged,
      session: { ...acknowledged.session, staleAccepted: true }
    }, pending),
    false,
    "a response with an unmodeled session field is not the strict ACK"
  );
  for (const timestampMutation of [
    { completedAt: null },
    ...
    assert.equal(
      isVisualizationSessionOutboxAcknowledgement(200, {
        ...acknowledged,
        session: { ...acknowledged.session, ...timestampMutation }
      }, pending),
      false,
      `the strict ACK must reject ${JSON.stringify(timestampMutation)}`
    );
  }
  const { updatedAt: _updatedAt, ...missingUpdatedAt } = acknowledged.session;
  assert.equal(
    isVisualizationSessionOutboxAcknowledgement(200, {
      ...acknowledged,
      session: missingUpdatedAt
    }, pending),
    false,
    "the strict ACK must carry the complete serialized session"
  );
});
