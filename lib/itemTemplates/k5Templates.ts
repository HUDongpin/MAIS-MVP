...
export const k5ItemTemplates: readonly K5ItemTemplate[] = [
  ...
  },
  {
    ...
    generate: (seed) => {
      const r = rng(seed);
      const n = r(2, 99);
      return item("k5_count_before", `What number plus 1 equals ${n}?`, n - 1, { n });
    },
    solve: (p) => parse(p, /^What number plus 1 equals (\d+)\?$/, ([n]) => n - 1)
  },
  {
    id: "k5_skip_count",
    grades: ["P2"],
  ...
  },
  {
    ...
    solve: (p) => {
      const match = p.match(/^A pattern starts at (\d+) and adds (\d+) each time: (\d+), (\d+), (\d+), (\d+)\. What is the next number\?$/);
      if (!match) return null;
      const [start, step, ...terms] = match.slice(1).map(Number);
      if (!terms.every((term, index) => term === start + index * step)) return null;
      return terms[3] + step;
    }
  },
  ...
  // --- Measurement and data -----------------------------------------------
  {
    ...
    generate: (seed) => {
      const r = rng(seed);
      const meters = r(2, 40);
      return item("k5_length_conversion", `How many centimeters are in ${meters} meters?`, meters * 100, { meters });
    },
    solve: (p) => parse(p, /^How many centimeters are in (\d+) meters\?$/, ([m]) => m * 100)
  },
  {
