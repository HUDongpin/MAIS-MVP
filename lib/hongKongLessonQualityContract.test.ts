...
function topicText(topicId: FinalHkTopicId) {
  const topic = topics.find((candidate) => candidate.curriculumTrack === "HK" && candidate.id === topicId);
  assert.ok(topic, `${topicId}: missing HK topic`);
  return `${topic.title.en} ${topic.title.zh} ${topic.description.en} ${topic.description.zh}`;
}
...
test("the ten manual HK content repairs remain locked in learner-facing lesson and question copy", () => {
...
test("dedicated HK visualization promises preserve the repaired mathematical invariants", () => {
  const contracts: Array<{
    ...
    },
    {
      ...
      required: [
        /non-negative amount remaining.*including exactly zero.*positive overspend|非負餘款.*包括恰好為零.*正數超支/i
      ],
      forbidden: [/positive amount remaining|正數餘款/i]
    },
    {
      topicId: "integers",
...
test("all 51 HK lesson and topic summaries preserve the final copy-to-model boundaries", () => {
  const p1Measurement = lessonText("p1-measurement-time");
  assert.match(p1Measurement, /hour hand sits halfway to the next hour|時針位於本小時和下一小時兩個鐘點數字的正中間/i);
  assert.doesNotMatch(p1Measurement, /時針位於兩個鐘點數字之間/);

  const p2PlaceValue = visualizationText("p2-place-value");
  assert.match(p2PlaceValue, /Build one number from 0 to 1000|建立一個 0 至 1000 的數/i);
  assert.doesNotMatch(p2PlaceValue, /compare two values place by place|逐位比較兩個數/i);

  const p3Measurement = visualizationText("p3-measurement");
  assert.match(p3Measurement, /separate bar-chart mode|獨立的棒形圖模式/i);
  assert.match(p3Measurement, /shared numerical scale that starts at zero|同一條由零開始的數值刻度/i);

  const p3Geometry = lessonText("p3-geometry-patterns");
  assert.match(p3Geometry, /both pairs of opposite sides are parallel.*parallelogram.*quadrilateral|兩組對邊都互相平行.*平行四邊形.*四邊形/i);
  assert.doesNotMatch(p3Geometry, /It belongs to the quadrilateral family|它屬於四邊形/);

  for (const content of [lessonText("p5-volume"), topicText("p5-volume")]) {
    assert.match(content, /unit cubes.*volume of cuboids|單位立方體.*長方體的體積/i);
    assert.doesNotMatch(content, /calculate cuboids|由小立方體理解體積，並計算長方體體積/i);
  }
  const p5Volume = visualizationText("p5-volume");
  assert.match(p5Volume, /V = length × width × height|V＝長×闊×高/i);

  const p6Percentages = visualizationText("p6-percentages");
  assert.match(p6Percentages, /given base and given percentage.*change = base × p\/100.*final = base ± change|已知基準量和已知百分數.*改變量＝基準量×p\/100.*最後數量＝基準量±改變量/i);

  for (const content of [lessonText("p6-ratio-proportion"), topicText("p6-ratio-proportion")]) {
    assert.match(content, /one or two ordered.*series on a single broken-line graph|同一幅折線圖中閱讀一條或兩條.*數列/i);
    assert.doesNotMatch(content, /one or two broken-line graphs|一條或兩條折線圖/i);
  }
  const p6RatioVisualization = visualizationText("p6-ratio-proportion");
  assert.match(p6RatioVisualization, /Series B offset is zero.*two series.*mean readouts coincide exactly|數列 B 偏移量為 0.*兩條數列.*平均數讀數完全重合/i);

  const p6ProblemSolving = visualizationText("p6-pre-secondary-problem-solving");
  assert.match(p6ProblemSolving, /four stages|四步/i);
  assert.doesNotMatch(p6ProblemSolving, /four checks/i);

  const identities = visualizationText("identities-square-patterns");
  assert.match(identities, /\(a-b\)\^2.*a\^2-2ab\+b\^2|\(a-b\).*−2ab/i);
  assert.match(identities, /signed-area models|帶符號面積模型/i);

  const seniorTrigonometry = topicText("trigonometry-s5");
  assert.match(seniorTrigonometry, /sine and cosine graphs|正弦與餘弦圖像/i);
  assert.doesNotMatch(seniorTrigonometry, /Transform identities|轉化恆等式/i);
});
...
test("S6 z-score extension compares unusualness by absolute distance from the mean", () => {
  const statisticsLesson = productionLessonByTopicId.get("statistics-s6");
  assert.ok(statisticsLesson, "S6 statistics lesson must exist");
  const extension = statisticsLesson.blocks.find((block) => block.type === "extension");
  const extensionText = (extension?.items ?? [])
    .flatMap((item) => [item.en, item.zh])
    .join(" ");

  assert.match(extensionText, /absolute values?.*larger.*\|z\|.*farther from the mean.*more unusual/i);
  assert.match(extensionText, /絕對值.*\|z\|.*較大.*離平均數較遠.*較不尋常/);
});
