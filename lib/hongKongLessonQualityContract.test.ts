...
test("each HK lesson displays exactly five fully bound, bilingual, grade-correct questions", () => {
  ...
  const displayedQuestions: (typeof questions)[number][] = [];
  ...
  const sortedQuestions = [...new Map(displayedQuestions.map((question) => [question.id, question])).values()]
    .sort((left, right) => left.id.localeCompare(right.id));
  const sha256 = (value: unknown) => createHash("sha256").update(JSON.stringify(value)).digest("hex");
  assert.equal(sortedQuestions.filter((question) => /^hk-ease-/.test(question.id)).length, 0);
  assert.equal(sortedQuestions.filter((question) => Boolean(question.diagram)).length, 24);
  assert.equal(
    sha256(sortedQuestions.map((question) => question.id)),
    "4d6987630b491d94bd9bdc0f6f7b3fc02234864becc6cecbce011c0b68203a6f",
    "the exact promoted 255-question lesson selection drifted"
  );
  assert.equal(
    sha256(sortedQuestions),
...
test("the seventeen manual HK content repairs remain locked in learner-facing lesson and question copy", () => {
  ...
  assert.ok(coordinatesQuestions.some((question) => question.id === "supp-coordinates-first-step-v3"));
...
test("displayed trigonometry strategy keeps the key fact concise and misconception alternatives distinct", () => {
  ...
  assert.equal(firstStep.id, "supp-trigonometry-basics-first-step-v3");
  assert.match(firstStep.explanation.en, /\\cos\\theta.*\\text\{adjacent\}.*\\text\{hypotenuse\}/);
  assert.match(firstStep.explanation.zh, /\\cos\\theta.*\\text\{鄰邊\}.*\\text\{斜邊\}/);
  assert.doesNotMatch(firstStep.explanation.en, /The alternatives instead encode/i);
  assert.doesNotMatch(firstStep.explanation.zh, /其餘選項分別包含/);

  const distractors = firstStep.options?.filter((option) => option.en !== firstStep.answer) ?? [];
  assert.equal(distractors.length, 3);
  assert.equal(new Set(distractors.map((option) => `${option.en}|${option.zh}`)).size, 3);
  for (const distractor of distractors) {
    assert.equal(firstStep.explanation.en.includes(distractor.en), false);
    assert.equal(firstStep.explanation.zh.includes(distractor.zh), false);
  }
});
