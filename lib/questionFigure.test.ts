...
test("normalizeQuestionDiagram accepts the three data-display forms and rejects un-drawable ones", () => {
  const pictureGraph = normalizeQuestionDiagram({
    ...
    scale: 2,
    categories: [
      { label: { en: "Dogs", zh: "狗", zhHans: "狗" }, value: 6 },
  ...
  const barGraph = normalizeQuestionDiagram({
    ...
    display: "bar-graph",
    categories: [
      { label: { en: "Apples", zh: "蘋果", zhHans: "苹果" }, value: 6 },
      { label: { en: "Bananas", zh: "香蕉", zhHans: "香蕉" }, value: 9 }
    ]
  });
  ...
  const categoryFields = {
    kind: "data-display",
    display: "picture-graph",
    categories: [{ label: { en: "Dogs", zh: "狗", zhHans: "狗" }, value: 4 }]
  };
