...
export function isLearningAnalyticsClearAcknowledgement(
  ...
  return (
    ...
    record?.ok === true &&
    record.acknowledgedUserId === expectedUserId &&
    isCanonicalLearningAnalyticsClearRequestId(expectedRequestId) &&
    record.requestId === expectedRequestId &&
    record.ignored !== true &&
    record.durablyPersisted === true &&
    typeof record.generation === "number" &&
    Number.isSafeInteger(record.generation) &&
    record.generation === baseGeneration + 1 &&
    isCanonicalIsoTimestamp(record.clearedAt)
  );
}

export function isCanonicalLearningAnalyticsClearRequestId(
  value: unknown
): value is string {
...
export function readLearningAnalyticsGenerationMismatchReceipt(
  ...
  if (record.currentGeneration > sentGeneration) {
    if (
      !isCanonicalIsoTimestamp(record.clearedAt) ||
      !isCanonicalLearningAnalyticsClearRequestId(record.currentClearRequestId)
    ) return null;
    return {
      clearedAt: record.clearedAt,
      currentClearRequestId: record.currentClearRequestId,
      currentGeneration: record.currentGeneration,
      direction: "forward"
    };
  }
  if (record.clearedAt !== null || record.currentClearRequestId !== null) return null;
  return {
    clearedAt: null,
    currentClearRequestId: null,
    currentGeneration: record.currentGeneration,
...
export function rebaseLearningAnalyticsClearFence(
  storage: Pick<Storage, "getItem" | "setItem">,
  expected: LearningAnalyticsClearFence,
  baseGeneration: number
) {
  if (
    !isNonNegativeSafeInteger(baseGeneration) ||
    baseGeneration <= expected.baseGeneration
  ) throw new Error("A clear fence can only rebase to a newer server generation.");
  const current = readLearningAnalyticsClearFence(storage, expected.userId);
  if (
    current.status !== "valid" ||
    current.value.requestId !== expected.requestId ||
    current.value.baseGeneration !== expected.baseGeneration
  ) throw new Error("Cannot rebase an absent, corrupt, or superseded clear fence.");
  const next: LearningAnalyticsClearFence = {
    ...current.value,
    baseGeneration,
    deleteAttemptedAt: null
  };
  storage.setItem(
    learningAnalyticsClearFenceStorageKey(expected.userId),
    JSON.stringify(next)
  );
  return next;
}
...
export function resolveLearningAnalyticsClearHandshake(
  ...
  currentClearRequestId: string | null = null
): "retry-delete" | "rebase-delete" | "reject" {
  if (!isNonNegativeSafeInteger(serverGeneration)) return "reject";
  if (serverGeneration === fence.baseGeneration && clearedAt === null) return "retry-delete";
  if (
    serverGeneration > fence.baseGeneration &&
    isCanonicalIsoTimestamp(clearedAt) &&
    isCanonicalLearningAnalyticsClearRequestId(currentClearRequestId)
  ) {
    return currentClearRequestId === fence.requestId
      ? "retry-delete"
      : "rebase-delete";
  }
  return "reject";
...
export function prepareLearningAnalyticsClearDeleteAfterMismatch(
  storage: Pick<Storage, "getItem" | "setItem">,
  fence: LearningAnalyticsClearFence,
  mismatch: LearningAnalyticsGenerationMismatchReceipt
) {
  if (mismatch.direction !== "forward") return null;
  const resolution = resolveLearningAnalyticsClearHandshake(
    fence,
    mismatch.currentGeneration,
    mismatch.clearedAt,
    mismatch.currentClearRequestId
  );
  if (resolution === "reject") return null;
  return resolution === "retry-delete"
    ? fence
...
export function unconfirmedLearningAnalyticsOutboxRecordStorageKey(
  ...
  const boundarySuffix = boundaryToken
    ...
    : "";
  return `${baseKey}${generationSuffix}${boundarySuffix}`;
}
...
export function mergeDurableLearningAnalyticsEvents(
  ...
  const chronologicallyOrdered = [...byId.values()].sort((left, right) => {
    const timestampDelta = new Date(left.timestamp).getTime() - new Date(right.timestamp).getTime();
    return timestampDelta || left.id.localeCompare(right.id);
  });
...
export function persistLearningAnalyticsEventsForCurrentProtocol(
  ...
  const deliveryEvents = events.map(learningAnalyticsEventForDelivery);
