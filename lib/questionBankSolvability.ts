...
export function hkIndependentAnswersById() {
  ...
  // result forward through the checked-in version graph. Any question whose
  // mathematical target changed must override that result explicitly below.
  for (const [historicalId, activeId] of activeHongKongQuestionIdByHistoricalId) {
    ...
    if (independentlySolvedAnswer && !answers.has(activeId)) {
      answers.set(activeId, independentlySolvedAnswer);
    }
  }
...
const historicalHongKongQuestionIdByActiveId = new Map(
  [...activeHongKongQuestionIdByHistoricalId].map(([historicalId, activeId]) => [activeId, historicalId] as const)
);
...
function independentAnswerFor(question: Question, hkAnswers: Map<string, string>): SolverResult {
  if (question.curriculumTrack === "HK") {
    ...
    if (easeMetadata) {
      return { answer: easeMetadata.independentAnswer };
    }
