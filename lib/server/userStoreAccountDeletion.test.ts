...
function seedDatabase() {
  return {
    ...
    guardian_invitations: [
      { id: "invite-1", student_id: studentId, consumed_by_parent_id: parentId, created_by: teacherId, token_digest: "secret-digest" },
      { id: "invite-2", student_id: otherStudentId, consumed_by_parent_id: null, created_by: teacherId, token_digest: "other-digest" }
    ],
...
test("deleting a student removes their own records", () => {
  ...
  assert.deepEqual((database.guardian_invitations as Array<{ id: string }>).map((row) => row.id), ["invite-2"]);
...
test("data export returns the subject's own rows without credentials", () => {
  ...
  const invitations = exported.tables.guardian_invitations as Record<string, unknown>[];
  assert.equal(invitations.length, 1);
  assert.ok(!("token_digest" in invitations[0]), "guardian invitation tokens are redacted");
