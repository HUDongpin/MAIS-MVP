...
test("the lazy lookup resolves retired IDs only and returns persisted QuestionRecord-compatible fields", async () => {
  ...
  const nonRetiredDiagram = historicalDiagramQuestions.find((question) => !isRetiredHongKongQuestionId(question.id));
  assert.ok(nonRetiredDiagram, "expected the unchanged number-line question to remain active");
  assert.equal(await resolveRetiredHongKongHistoricalQuestionRecord(nonRetiredDiagram.id), null);
...
test("every retired HK manifest ID resolves to one exact locked HK projection", async () => {
  assert.equal(retiredHongKongQuestionIds.size, 224);
