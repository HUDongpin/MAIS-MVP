...
const eligibleEntryByIdentityKey = new Map(
...
function entryMatchesLearnerCurriculum(
  ...
  if (entry.curriculumTrack === "CAPSTONE") return region === "MAINLAND";
...
export function isEligibleVisualizationSession(identity: VisualizationSessionIdentity) {
  return eligibleEntryByIdentityKey.has(visualizationSessionIdentityKey(identity));
}
...
export function isVisualizationSessionEligibleForLearner(
  ...
  const entry = eligibleEntryByIdentityKey.get(visualizationSessionIdentityKey(identity));
