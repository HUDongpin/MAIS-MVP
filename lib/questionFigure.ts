...
function dataDisplayAltText(diagram: DataDisplayQuestionDiagram): LocalizedText {
  ...
  if (diagram.display === "line-plot") {
    ...
    const unit = diagram.unit;
    const suffixEn = unit ? ` ${unit.en}` : "";
    const suffixZh = unit ? ` ${unit.zh}` : "";
    const suffixZhHans = unit ? ` ${unit.zhHans ?? unit.zh}` : "";
    return {
      en: `${headEn} with values ${valueText}${suffixEn}.`,
      zh: `${headZh}，數值為 ${valueText}${suffixZh}。`,
      zhHans: `${headZhHans}，数值为 ${valueText}${suffixZhHans}。`
    };
  }
  ...
  const barUnitEn = unit && diagram.display === "bar-graph" ? ` ${unit.en}` : "";
  const barUnitZh = unit && diagram.display === "bar-graph" ? ` ${unit.zh}` : "";
  const barUnitZhHans = unit && diagram.display === "bar-graph" ? ` ${unit.zhHans ?? unit.zh}` : "";
  return {
    en: `${headEn}: ${listEn}${barUnitEn}.${scaleSuffixEn}`,
    zh: `${headZh}：${listZh}${barUnitZh}。${scaleSuffixZh}`,
    zhHans: `${headZhHans}：${listZhHans}${barUnitZhHans}。${scaleSuffixZhHans}`
  };
}
