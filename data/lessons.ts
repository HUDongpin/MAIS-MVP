...
const hongKongFallbackVisualizationByTopicId: Partial<Record<string, NonNullable<LessonDraft["visualization"]>>> = {
  ...
  "p3-measurement": {
    title: { en: "Convert measures and read a bar chart", zh: "換算度量並閱讀棒形圖" },
    content: {
      en: "Preserve one physical quantity while converting exactly between appropriate metric units. In a separate bar-chart mode, read categorical bar heights from one shared numerical scale that starts at zero.",
      zh: "在合適的公制單位之間精確換算時保持同一物理量；在獨立的棒形圖模式中，按同一條由零開始的數值刻度閱讀各類棒的高度。"
    },
    moduleId: "configured-visualization-lab",
    source: "geometry"
  },
  ...
  "p6-ratio-proportion": {
    ...
    content: {
      en: "Edit one ordered series or add a second series on the same graph, and keep each series' total, count, mean, and values synchronized with the value table, plotted points, complete axes, scale, and units. Join only consecutive time or continuous-data points; if the Series B offset is zero, the two series and their mean readouts coincide exactly. Do not treat extrapolation as observed fact.",
      zh: "編輯一條有序數列，或在同一幅圖加入第二條數列，並把每條數列的總和、數據個數、平均數和數值與數值表、標繪點、完整坐標軸、刻度及單位同步。只連接相鄰的時間或連續數據點；當數列 B 偏移量為 0 時，兩條數列及其平均數讀數完全重合。不把延伸線段當作已觀察事實。"
    },
    moduleId: "configured-visualization-lab",
    source: "function-model"
  },
  "p6-pre-secondary-problem-solving": {
    title: { en: "Carry one problem through four stages", zh: "以四步完成並檢查同一題" },
    content: {
...
const hongKongProductionLessonSeeds: ProductionLessonSeed[] = [
  ...
  lesson({
    ...
    concept: {
      ...
      content: {
        ...
        zh: "公平比較長度時，要把大小相同的單位首尾相接，中間不能留空。整點時分針指向 12；半小時時分針指向 6，時針位於本小時和下一小時兩個鐘點數字的正中間。"
      }
    },
    workedExample: {
      content: {
  ...
  lesson({
    ...
    visualization: {
      ...
      content: {
        en: "Build one number from 0 to 1000 with hundreds, tens, and ones, and make the regrouping from ten hundreds to one thousand visible.",
        zh: "用百、十和個建立一個 0 至 1000 的數，並清楚顯示 10 個百重組為 1 個千。"
      },
      moduleId: "configured-visualization-lab",
      source: "coordinate-plane"
    },
  ...
  lesson({
    ...
    workedExample: {
      content: {
        en: "A closed figure has four straight sides. Its opposite sides carry two matching pairs of arrow marks, so both pairs of opposite sides are parallel. It is a parallelogram, and therefore also a quadrilateral. A triangle with three matching side marks is an equilateral triangle.",
        zh: "一個封閉圖形有四條直邊，而兩組對邊分別有相同箭嘴記號，所以兩組對邊都互相平行；它是平行四邊形，因此也屬於四邊形。若三角形的三條邊都有相同短線記號，便是等邊三角形。"
      }
    },
    checklist: {
  ...
  lesson({
    ...
    description: {
      en: "Build volume from unit cubes and calculate the volume of cuboids.",
      zh: "由單位立方體理解體積，並計算長方體的體積。"
    },
    estimatedMinutes: 32,
    concept: {
    ...
    visualization: {
      ...
      content: {
        en: "Build a cuboid, within the model's limits, from layers of unit cubes and keep V = length × width × height visible and true.",
        zh: "在模型容許範圍內用單位立方體逐層建立長方體，並保持「V＝長×闊×高」清楚可見且成立。"
      },
      moduleId: "configured-visualization-lab",
      source: "geometry"
    },
  ...
  lesson({
    ...
    visualization: {
      ...
      content: {
        en: "Change p and keep p%, p/100, its decimal form, and exactly p shaded cells in one fixed 100-cell grid synchronized. In increase or decrease mode, use the given base and given percentage to show change = base × p/100 and final = base ± change.",
        zh: "改變 p，並同步顯示 p%、p/100、相應小數，以及固定百格圖中正好 p 個已塗色方格。在增加或減少模式中，使用已知基準量和已知百分數顯示「改變量＝基準量×p/100」及「最後數量＝基準量±改變量」。"
      },
      moduleId: "configured-visualization-lab",
      source: "geometry"
    },
  ...
  lesson({
    ...
    description: {
      en: "Interpret the mean as a fair share and read one or two ordered time or continuous-data series on a single broken-line graph.",
      zh: "把平均數理解為平均分配，並在同一幅折線圖中閱讀一條或兩條按時間或連續量排列的數列。"
    },
    estimatedMinutes: 36,
    concept: {
  ...
  lesson({
    ...
    visualization: {
      ...
      content: {
        en: `Switch among the ${math("(a+b)^2 \\equiv a^2+2ab+b^2")}, ${math("(a-b)^2 \\equiv a^2-2ab+b^2")}, and ${math("a^2-b^2 \\equiv (a-b)(a+b)")} signed-area models. For the displayed area states use the allowed ${math("a>b\\ge0")} values, and use the exact pieces and signs to verify each case. The algebraic identities themselves hold for all real ${math("a")} and ${math("b")}.`,
        zh: `在 ${math("(a+b)^2 \\equiv a^2+2ab+b^2")}、${math("(a-b)^2 \\equiv a^2-2ab+b^2")} 和 ${math("a^2-b^2 \\equiv (a-b)(a+b)")} 的帶符號面積模型之間切換。顯示面積狀態時使用容許的 ${math("a>b\\ge0")}，並用精確面積塊及正負號驗證每種情況；代數恆等式本身對所有實數 ${math("a")}、${math("b")} 都成立。`
      },
      moduleId: "configured-visualization-lab",
      source: "function-model"
    },
  ...
  lesson({
    ...
    extension: {
      items: [
        {
          en: "Compare the absolute values of two z-scores; the larger |z| is farther from the mean and is more unusual in the normal model.",
          zh: "比較兩個 z 分數的絕對值；|z| 較大的數值離平均數較遠，在正態模型中較不尋常。"
        },
