...
const paymentFixture: DynamicFixture = {
  ...
  modeId: "money",
  controls: [
    { id: "price", min: 1, max: 99, step: 1, initial: 32 },
    { id: "payment", min: 1, max: 100, step: 1, initial: 50 },
  ],
};
...
const numberBondFixture: DynamicFixture = {
  ...
  controls: [
    { id: "total", min: 0, max: 20, step: 1, initial: 12 },
    { id: "knownPart", min: 0, max: 12, step: 1, initial: 7 },
  ],
};
  ...
  test("keeps undeclared clamps RED and executes 27 chained fraction-bar numerator transitions", async ({
    ...
    const fractionResult =
      await exerciseHkVisualizationDynamicRangeMicrofixture({
        ...
        controlSelectors: [
          '#dynamic-model [data-viz-parameter="value"]',
          '#dynamic-model [data-viz-parameter="comparison"]',
        ],
        expectedStateCount: 27,
        labId: "p3-fractions-intro",
        modeIds: ["fraction", "equivalent", "compare"],
        modeSelectors: [
          '#dynamic-model [data-viz-mode-button][data-viz-mode="fraction"]',
          '#dynamic-model [data-viz-mode-button][data-viz-mode="equivalent"]',
          '#dynamic-model [data-viz-mode-button][data-viz-mode="compare"]',
        ],
        rangeDomainId: "fraction-bar-numerator-v1",
        modelSelector: "#dynamic-model",
        resetSelector: "#dynamic-model [data-viz-reset-model]",
...
function expectFailure(
  ...
  >,
  code: string,
  _legacyRawMessagePattern: RegExp,
) {
  const matching = result.failures.filter((failure) => failure.code === code);
  expect(
    matching.length,
    `Expected failure code ${code}; observed ${JSON.stringify(
      result.failures.map(({ code: observedCode, phase }) => ({
        code: observedCode,
        phase,
      })),
    )}`,
...
async function mountDynamicFixture(
  ...
  const siblingControl =
    defect === "sibling-control"
      ...
      : "";
  const secondary = fixture.labId === "identities-square-patterns";
  const grade = fixture.labId === "p1-counting-number-bonds"
    ? "P1"
    : fixture.labId === "p2-money-time"
      ? "P2"
      : secondary
        ? "S3"
        : "S6";
  const analyticsSource = fixture.labId === "p1-counting-number-bonds"
    ? "coordinate-plane"
    : fixture.labId === "p2-money-time"
      ? "probability"
      : "geometry";
  const ownerLabel = fixture.labId === "p1-counting-number-bonds"
  ...
  const surfaceIdentity = secondary
    ? "identity-square-patterns"
    : fixture.labId;
  const surface = secondary
    ? `<div data-viz-compact-frame><div data-viz-secondary-surface><p data-viz-pan-hint>Pan the exact model.</p><div data-viz-pan-container><svg data-viz-surface="" data-hk-viz-surface="${surfaceIdentity}" data-viz-interactive="false" role="img" aria-label="${ownerLabel}" viewBox="0 0 640 400" class="aspect-[8/5] h-auto w-full min-w-[640px] overflow-visible"></svg></div></div></div>`
    : `<div data-viz-primary-surface-column><div data-viz-primary-svg-frame><p data-viz-pan-hint>Pan the exact model.</p><div data-viz-scroll-container><svg data-viz-surface="" role="img" aria-label="${ownerLabel}" viewBox="0 0 640 360" preserveAspectRatio="xMidYMid meet" class="block h-auto min-h-[360px] w-full min-w-[640px] overflow-visible"></svg></div></div></div>`;
  const summaryAndFormula = `<p data-viz-state-summary>Loading dynamic state</p><p data-hk-viz-formula>Loading formula</p>`;
  const aside = `<aside>${mode}${secondary ? "" : summaryAndFormula}<div data-viz-controls>${controls}</div>${defect === "missing-reset" ? "" : `<button type="button" data-viz-reset-model data-viz-reset-module-id="configured-visualization-lab" data-viz-reset-topic-id="${fixture.labId}">Reset model</button>`}</aside>`;
  const modelTag = secondary ? "section" : "div";
  const model = `<${modelTag} id="dynamic-model" data-hk-viz-model="${secondary ? "secondary-dedicated-v1" : "primary-dedicated-v1"}" data-hk-viz-topic="${fixture.labId}" data-viz-range-domain-id="${rootDomain}">${decoy}<div data-viz-model-grid>${secondary ? `<div data-viz-secondary-surface-column>${surface}${summaryAndFormula}</div>` : surface}${aside}</div></${modelTag}>`;
  const directLabHref = `/visualization-lab?grade=${grade}&track=all&lab=${fixture.labId}`;
  const youngLearner = grade === "P1" || grade === "P2";
  const fixtureHtml = `<!doctype html>
    <html lang="en-HK"><head><style>
      body { background:#ffffff; color:#111827; font:16px/1.4 system-ui,sans-serif; margin:0; }
      body { overflow-x:hidden; overflow-y:auto; font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
      body > div { display:flex; }
      body > div > main > div { overflow:hidden; }
      [data-viz-card] { overflow:hidden; }
      [data-viz-model-grid] { align-items:start; display:grid; gap:24px; grid-template-columns:640px 320px; width:984px; }
      [data-viz-model-grid] > aside { min-width:0; width:320px; }
      [data-viz-scroll-container], [data-viz-pan-container] { overflow:auto; width:640px; }
      svg[data-viz-surface] { display:block; height:360px; overflow:visible; width:640px; }
      svg[data-viz-surface] text { x:attr(x px); y:attr(y px); }
      svg[data-hk-viz-surface] { height:400px; }
      #dynamic-model { background:#ffffff; }
      ...
      label { align-items:center; display:flex; gap:10px; min-height:44px; margin:8px 0; width:300px; }
      ...
      <div class="relative z-10 flex min-h-screen flex-col">
        <main class="flex-1 pb-20 sm:pb-0">
          <div class="min-h-full overflow-hidden bg-transparent text-slate-950">
            <div class="relative mx-auto w-full max-w-[1500px] px-4 py-6 sm:py-8">
              <section aria-label="Visualization Lab workspace" class="scroll-mt-24" data-viz-active-grade="${grade}" data-viz-active-lab-id="${fixture.labId}" data-viz-direct-lab-href="${directLabHref}" data-viz-lab-runtime-status="lab" data-viz-link-status="ok" data-viz-panel-mode="lab" data-viz-requested-lab-id="${fixture.labId}" data-viz-young-learner-mode="${youngLearner}" id="lab-example-${fixture.labId}">
                <section class="rounded-[1.35rem] bg-white/95 p-4 shadow-2xl shadow-cyan-900/15 ring-1 ring-cyan-100 sm:p-7" data-lab-id="${fixture.labId}" data-viz-copy-controls-ready="true" data-viz-current-grade="${grade}" data-viz-current-track="HK" data-viz-direct-lab-href="${directLabHref}">
                  <div class="min-w-0">
                    <section class="overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-900/5 dark:border-white/10 dark:bg-slate-900 dark:shadow-none sm:p-6" data-viz-card="true" data-viz-explore-gate="engaged" data-viz-module-id="${analyticsSource}:${fixture.labId}:${fixture.labId}" data-viz-save-state="saved" data-viz-topic-id="${fixture.labId}">
                      <div data-viz-card-body="true">
                        <div data-viz-lab-runtime-ready-probe="${fixture.labId}" data-viz-lab-runtime-root="true">
                          <div class="min-w-0" data-viz-lesson-session-owner="external-card" data-viz-module-id="configured-visualization-lab" data-viz-production-renderer="hk-dedicated" data-viz-topic-id="${fixture.labId}">
                            <div class="min-w-0" data-hk-viz-dispatcher="v1" data-hk-viz-dispatcher-lab-id="${fixture.labId}" data-hk-viz-dispatcher-target="${secondary ? "secondary" : "primary"}">${model}</div>
                          </div>
                        </div>
                      </div>
                    </section>
  ...
  const fixtureUrl = `http://127.0.0.1/__hk-viz-dynamic-range-microfixture__/${encodeURIComponent(fixture.labId)}/${encodeURIComponent(defect ?? "valid")}`;
  ...
  await page.locator("#dynamic-model").evaluate(
    (root, options) => {
      ...
      const surface = root.querySelector<SVGSVGElement>("svg[data-viz-surface]")!;
      ...
      const projectTriangle = (
        ...
        const controlIds = ["ax", "ay", "bx", "by", "cx", "cy"];
        const companions = [
          companionId,
          ...controlIds.filter(
        ...
        const considerCandidate = (candidate: Record<string, number>) => {
          const order = candidateOrder++;
          if (candidate[id] !== requested || !triangleValid(candidate)) return;
          const squaredDisplacement = controlIds.reduce(
            (total, controlId) =>
              total + (candidate[controlId] - next[controlId]) ** 2,
            0,
          );
        ...
        const otherVertices = ["a", "b", "c"].filter(
          (vertex) => vertex !== changedVertex,
        );
        const mirroredBoundaryCandidates = [-1, 1].flatMap((mirror) =>
          [false, true].map((swap) => {
        ...
        for (const candidate of mirroredBoundaryCandidates)
          considerCandidate(candidate);

        if (nearestCandidate.value !== null)
          return nearestCandidate.value.candidate;
        if (triangleValid(current)) return current;
        throw new Error(
          "Unable to project a finite non-degenerate triangle state.",
        );
      };
      const apply = (id: string, requested: number) => {
        ...
        if (domainId === "number-bond-v1") {
          if (id === "total") {
            next.total = clamp(requested, 0, 20);
      ...
      const render = () => {
        controls.innerHTML = inputs
          ...
          .join("");
        const publicState = domainId === "payment-at-least-price-v1"
          ? {
              mode: "money",
              price: state.price,
              payment: defect === "stale-visible-state"
                ? state.payment + 1
                : state.payment,
              hour: 9,
              halfHour: true,
          ...
          : domainId === "number-bond-v1"
            ...
            : domainId === "identity-positive-a-gt-b-v1"
              ? { a: state.a, b: state.b, activeMode: "square-sum" }
              : defect === "noop-evidence"
                ? { ...state, noOpEvidenceRevision }
                : { ...state };
        const serialized = defect === "malformed-state"
          ? "{"
        ...
        if (domainId === "number-bond-v1") {
          ...
          const counters = Array.from({ length: 20 }, (_, index) => {
            ...
            const fill = !filled
              ...
              : part === "known"
                ? "#22d3ee"
                : "#f59e0b";
            const stroke = filled ? "#0f172a" : "rgba(15,23,42,0.16)";
            return `<circle data-viz-mark data-viz-name="counter" data-viz-part="${filled ? part : "empty"}" cx="${(index % 10) * 48}" cy="${Math.floor(index / 10) * 48}" r="16" fill="${fill}" stroke="${stroke}" stroke-width="3"${filled ? "" : ' stroke-dasharray="5 4"'} />`;
          }).join("");
          const partNode = (
            name: string,
            value: number,
            ...
            const overlap = name === "whole-node"
              ? {
                  ok: "label-inside-own-whole-node",
                  reason: "The whole value label is intentionally centered inside its own part-whole node.",
                }
              : name === "known-part-node"
                ? {
                    ok: "label-inside-own-known-part-node",
                    reason: "The known-part value label is intentionally centered inside its own part node.",
                  }
                : {
                    ok: "label-inside-own-missing-part-node",
                    reason: "The missing-part value label is intentionally centered inside its own part node.",
                  };
            return `<g data-viz-mark data-viz-name="${name}"><g data-viz-overlap-ok="${overlap.ok}" data-viz-overlap-reason="${overlap.reason}"><circle data-viz-mark data-viz-overlap-member="mark" cx="${x}" cy="${y}" r="${radius}" fill="${fill}" stroke="#0f172a" stroke-width="3" /><text data-viz-overlap-member="label" x="${x}" y="${y + (y === 220 ? 12 : 11)}" text-anchor="middle" fill="${textFill}" font-size="${fontSize}" font-weight="900">${value}</text></g></g>`;
          };
          surface.innerHTML = `<g data-viz-mark data-viz-name="counter-set" data-viz-total="${total}" transform="translate(76 66)">${counters}</g><path data-viz-mark data-viz-name="known-connector" d="M260 238 L214 276" fill="none" stroke="#22d3ee" stroke-width="7" stroke-linecap="round" /><path data-viz-mark data-viz-name="missing-connector" d="M380 238 L426 276" fill="none" stroke="#f59e0b" stroke-width="7" stroke-linecap="round" />${partNode("whole-node", total, 320, 220, 42, "#34d399", "#052e16", 32)}${partNode("known-part-node", known, 170, 300, 38, "#22d3ee", "#082f49", 28)}${partNode("missing-part-node", missing, 470, 300, 38, "#f59e0b", "#451a03", 28)}`;
        } else if (domainId === "payment-at-least-price-v1") {
          const price = state.price;
          const payment = state.payment;
          const change = payment - price;
          const priceWidth = 488 * price / payment;
          const changeWidth = 488 * change / payment;
          const segment = (
            name: string,
            attribute: string,
            value: number,
            ...
            const showsLabel = width >= threshold;
            const overlap = name === "price-segment"
              ? {
                  ok: "label-inside-own-price-segment",
                  reason: "The price label is intentionally centered inside its own payment segment.",
                }
              : {
                  ok: "label-inside-own-change-segment",
                  reason: "The change label is intentionally centered inside its own payment segment.",
                };
            return `<g${showsLabel ? ` data-viz-overlap-ok="${overlap.ok}" data-viz-overlap-reason="${overlap.reason}"` : ""}><rect data-viz-mark${showsLabel ? ' data-viz-overlap-member="mark"' : ""} data-viz-name="${name}" ${attribute}="${value}" x="${x}" y="228" width="${width}" height="70" rx="16" fill="${fill}" />${showsLabel ? `<text data-viz-overlap-member="label" x="${x + width / 2}" y="270" text-anchor="middle" fill="${textFill}" font-size="17" font-weight="900">${label}</text>` : ""}</g>`;
          };
          surface.innerHTML = `<rect data-viz-name="payment-bar" data-viz-payment="${payment}" x="76" y="228" width="488" height="70" rx="16" fill="rgba(15,23,42,0.07)" stroke="#334155" stroke-width="3" />${segment("price-segment", "data-viz-price", price, 76, priceWidth, "#22d3ee", "#082f49", `HK$${price}`, 72)}${change > 0 ? segment("change-segment", "data-viz-change", change, 76 + priceWidth, changeWidth, "#f59e0b", "#451a03", `+HK$${change}`, 82) : ""}`;
        } else if (domainId === "identity-positive-a-gt-b-v1") {
          const a = state.a;
          const b = state.b;
          const scale = 270 / (a + b);
          ...
          surface.innerHTML = `<g data-viz-mark data-viz-name="identity-square-whole" data-viz-a="${a}" data-viz-b="${b}" data-viz-area="${(a + b) ** 2}"><rect x="${left}" y="${top}" width="${aSize}" height="${aSize}" fill="#22d3ee" opacity="0.76" stroke="#0f172a" stroke-width="3" data-viz-name="identity-a2" data-viz-area="${a ** 2}" /><g data-viz-name="identity-cross-parts" data-viz-area="${2 * a * b}"><rect x="${left + aSize}" y="${top}" width="${bSize}" height="${aSize}" fill="#facc15" opacity="0.76" stroke="#0f172a" stroke-width="3" /><rect x="${left}" y="${top + aSize}" width="${aSize}" height="${bSize}" fill="#c084fc" opacity="0.72" stroke="#0f172a" stroke-width="3" /></g><rect x="${left + aSize}" y="${top + aSize}" width="${bSize}" height="${bSize}" fill="#34d399" opacity="0.78" stroke="#0f172a" stroke-width="3" data-viz-name="identity-b2" data-viz-area="${b ** 2}" /><rect x="${left}" y="${top}" width="${wholeSize}" height="${wholeSize}" fill="none" stroke="#334155" stroke-width="7" /></g>`;
        } else {
          surface.innerHTML = `<rect data-viz-mark x="12" y="22" width="${60 + (Math.abs(Object.values(state)[0] ?? 0) % 10) * 18 + noOpEvidenceRevision}" height="42" rx="4" fill="#2563eb" data-viz-state='${JSON.stringify(state)}' />`;
        }
        controls
  ...
  const workspace = page.locator(`#lab-example-${fixture.labId}`);
