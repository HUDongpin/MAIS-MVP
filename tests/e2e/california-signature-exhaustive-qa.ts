...
/**
 * Owner-reviewed terminal key snapshot, deterministically projected from the
 * frozen source oracle and reviewed bench-to-route ownership.
 */
export const CALIFORNIA_SIGNATURE_REVIEWED_RUNTIME_SNAPSHOT = {
  blueprintSha256: CALIFORNIA_SIGNATURE_CONTROL_BLUEPRINT_SHA256,
  keyCount: 1_127_995,
  keysSha256: "7101284483134b9c1893089d6e8073dd6b250202fb52878a4b4d28b1cf0d23eb",
...
type CaliforniaSignatureEvidenceKeyIdentity = Pick<
  CaliforniaSignatureExactEvidenceRecord,
  "axisId" | "benchId" | "branchPath" | "combinationActivations" | "endpoint" |
  "instanceKey" | "labId" | "numericMidpoint" | "phase" | "sourceEndpoint" |
  "sourceSiteKey" | "stepKey"
>;

function evidenceKey(record: CaliforniaSignatureEvidenceKeyIdentity) {
...
async function assertNoBrowserDiagnostics(diagnostics: CaliforniaBrowserDiagnostics, label: string) {
  const drained = diagnostics.drain().diagnostics;
  if (drained.length > 0) {
 ...
 * every source receipt ready, `prepareFinalCompositor` authorizes exactly one
 * page-level screenshot crop per Canvas, then browser finalize rechecks all
 * source, URL, root, layout, and paint-mutation fences before issuing the ACK.
 * Any screenshot outside that prepare -> crop -> finalize authorization, or
 * any unrelated I/O/control mutation within it, invalidates the one-shot gate.
 */
export async function runCaliforniaCanvasGraphicsOneShotGate(options: {
  ...
  if (contrast.findings.length > 0) {
    throw new Error(
      ...
      contrast.findings.map((item) => item.detail).join(" | ")
    );
  }
...
/**
 * Derive the terminal runtime-key snapshot entirely from frozen source IR and
 * the independently reviewed bench-to-route ownership. No browser discovery,
 * runtime record, or production ledger participates in this projection.
 */
export function deriveCaliforniaSignatureRuntimeSnapshotFromSource(options: {
  manifest: CaliforniaSignatureSourceManifest;
  oracle: CaliforniaSignatureSourceExpectedEvidenceOracle;
}) {
  assert.equal(options.oracle.blueprintSha256, options.manifest.blueprintSha256,
    "California signature reviewed snapshot source-oracle blueprint drifted");
  assert.equal(options.oracle.componentSourceSha256,
    options.manifest.componentSourceSha256,
  "California signature reviewed snapshot source-oracle component drifted");
  const visitByBench = new Map<string, CaliforniaBenchVisit>(
    buildCaliforniaSignatureExhaustivePackages(options.manifest, 24)
      .flatMap((workPackage) => workPackage.benches)
      .map(({ bench, visit }) => [bench.benchId, visit] as const)
  );
  assert.equal(visitByBench.size, options.manifest.benches.length,
    "California signature reviewed snapshot route ownership is incomplete");
  ...
  assert.equal(rowByKey.size, options.oracle.counts.evidenceRows,
    "California signature reviewed snapshot source rows are incomplete");

  const orderedRuntimeKeyHash = createHash("sha256");
  let keyCount = 0;
  for (const expectation of iterateCaliforniaSignatureExpandedSourceEvidenceOracle(options)) {
    const row = rowByKey.get(expectation.oracleRowKey);
    assert.ok(row,
      `${expectation.benchId}: reviewed snapshot lost source row ${expectation.oracleRowKey}`);
    const visit = visitByBench.get(row.benchId);
    assert.ok(visit,
      `${expectation.benchId}: reviewed snapshot lost route ownership`);
    const runtimeKey = evidenceKey({
      axisId: expectation.axisId,
      benchId: row.benchId,
      branchPath: [...row.branchPath],
      combinationActivations: row.combinationActivations,
    ...
    orderedRuntimeKeyHash.update(runtimeKey).update("\n");
    keyCount += 1;
    assert.ok(Number.isSafeInteger(keyCount),
      "California signature reviewed snapshot key count exceeded a safe integer");
  }
  assert.ok(keyCount > 0,
    "California signature reviewed snapshot source projection is empty");
  return {
    blueprintSha256: options.manifest.blueprintSha256,
    keyCount,
    keysSha256: orderedRuntimeKeyHash.digest("hex"),
    schemaVersion: CALIFORNIA_SIGNATURE_EXHAUSTIVE_SCHEMA_VERSION
  } as const;
}
