...
// Diagnostic capacity partitions can describe exact group ownership, but they
// never authorize browser execution. This module fails before declaring any
// Playwright test until a separate, reviewed measured-authorization protocol
// exists and is integrated explicitly.
assertCaliforniaSignatureFormalExecutionAuthorizationUnavailable(executionPlan);
const frozenSourceRoot = resolveCaliforniaSignatureFrozenSourceRoot();
const manifest = buildCaliforniaSignatureSourceManifest(frozenSourceRoot);
...
for (const workPackage of packages) {
  test(`${workPackage.packageId} exhausts source-attributed lesson/control states`, async ({ page, baseURL }, testInfo) => {
    ...
    try {
      ...
      await missingCaliforniaSignatureDurabilityProvider.assertReady();
      ...
      for (const group of sourceExecutionGroups) {
        ...
        const groupEvidence = group.phase === "structural"
          ...
          : await runCaliforniaSignatureEndpointBench({
              ...
              contrastProvider: californiaSignatureContrastProvider,
              diagnostics,
              durabilityProvider: missingCaliforniaSignatureDurabilityProvider,
              page,
              phase: group.phase,
              sourceEvidenceOracle,
              sourceExpectedProvider: californiaSignatureSourceExpectedProvider,
