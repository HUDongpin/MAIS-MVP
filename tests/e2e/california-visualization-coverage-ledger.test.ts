...
test("California signature reset pixels come from the intrinsic visible learner Canvas", { timeout: 40_000 }, async (t) => {
  ...
  const installResetFixture = async (mode: "changed" | "dynamic" | "same") => {
    await page.setContent(`
      ...
      <div id="switcher" data-viz-active-signature-bench="${benchId}"></div>
...
test("California control/reset and premium playback evidence reject redraw and lifecycle races", { timeout: 30_000 }, async (t) => {
  const helpers = await import("./california-visualization-qa-helpers") as unknown as Record<string, unknown>;
  assert.equal(
  ...
  await page.setContent(`
    <div id="switcher" data-viz-active-signature-bench="algebra-tiles"></div>
  ...
  const changedDefault = await captureDefault(page.locator("#changed"), 2_000);
  assert.equal(
    (await smokeControl(page.locator("#changed"), 2_000, {
  ...
  const stableDefault = await captureDefault(page.locator("#stable-to-dynamic"), 2_000);
  await assert.rejects(
    () => resetBench(
      page,
