...
import { isExpectedCanceledCaliforniaProductRscPrefetch } from "./california-product-smoke-browser-diagnostics";
...
export const CALIFORNIA_ACTIVE_SIGNATURE_BENCH_ATTRIBUTE =
...
export async function enforceCaliforniaFormalReducedMotion(page: Page): Promise<void> {
  ...
  assert.equal(
    reducedMotion,
    true,
    "California formal coverage requires prefers-reduced-motion: reduce before application navigation"
  );
}
...
export async function openCaliforniaDirectoryLab(
  ...
  await expect(switcher).toHaveAttribute(CALIFORNIA_ACTIVE_SIGNATURE_BENCH_ATTRIBUTE, lab.assignment.primary, {
...
export async function openSignatureBench(
  ...
  const activeBenchId = await switcher.getAttribute(CALIFORNIA_ACTIVE_SIGNATURE_BENCH_ATTRIBUTE);
  ...
  await expect(switcher).toHaveAttribute(CALIFORNIA_ACTIVE_SIGNATURE_BENCH_ATTRIBUTE, benchId, {
    timeout: stepTimeout(timeout)
  });
...
async function modelResponseState(root: Locator) {
  return root.evaluate((element) => {
    ...
    const semanticSelector = [
      ...
      ".feedback",
      "[data-viz-surface] [role=img][aria-label]",
      "[data-viz-surface] canvas[aria-label]",
      "[data-viz-math-state]",
...
async function rawSignatureBenchState(
  ...
  const surfacePng = Buffer.from(Uint8Array.from(await canvas.evaluate(async (element) => {
    const drawingCanvas = element as HTMLCanvasElement;
    const blob = await new Promise<Blob>((resolve, reject) => {
      drawingCanvas.toBlob((value) => {
        if (value) resolve(value);
        else reject(new Error("Signature Canvas could not encode its intrinsic PNG."));
      }, "image/png");
    });
    return Array.from(new Uint8Array(await blob.arrayBuffer()));
  }, undefined, { timeout: stepTimeout(timeout) })));
...
export async function captureSignatureBenchDefaultState(
  ...
  // Intrinsic PNG encoding is intentionally independent of page scroll/raster
  // phase, but a real Chrome toBlob sample can take around 0.5s under the
  // Canvas instrumentation. Preserve the three-identical-sample requirement
  // and give it enough time to obtain those samples.
  const localBudget = Math.min(stepTimeout(timeout), 4_000);
  let previous = await rawSignatureBenchState(root, timeout);
  const controlVariants = new Set([previous.controlFingerprint]);
  const semanticVariants = new Set([previous.semanticFingerprint]);
  const surfaceVariants = new Set([previous.surfaceFingerprint]);
  let sampleCount = 1;
  ...
  while (Date.now() - startedAt <= localBudget && timeoutRemaining(timeout) > 0) {
    ...
    sampleCount += 1;
    controlVariants.add(current.controlFingerprint);
    semanticVariants.add(current.semanticFingerprint);
    surfaceVariants.add(current.surfaceFingerprint);
  ...
  throw new Error(
    "Signature bench did not reach stable controls and mathematical semantics; " +
    "draw revision alone is not model-response evidence. " +
    `samples=${sampleCount}; controlVariants=${controlVariants.size}; ` +
    `semanticVariants=${semanticVariants.size}; surfaceVariants=${surfaceVariants.size}.`
  );
}
...
export const CALIFORNIA_TOUCH_AUDIT_ALGORITHM_SHA256 = sha256Text(
  "california-touch-audit:v4:semantic-inventory:partition:device-pixel-fallback:deep-shadow:" +
  "text-glyph-ranges:analytics-lifecycle-settle:structured-viewport-recheck:" +
  "composed-environment-quiescent-journal"
);
...
async function prepareCaliforniaViewportRecheck(
  ...
  try {
    ...
    const canvasStates = elements.filter((element): element is HTMLCanvasElement =>
      ...
      const graphicsRuntimeInstalled = Boolean((globalThis as typeof globalThis & {
        __californiaCanvasGraphicsRuntime?: { version?: number };
      }).__californiaCanvasGraphicsRuntime?.version);
      if (!graphicsRuntimeInstalled) {
        try {
    ...
    const markGraphicsMutation = () => {
      if (journal.internalMutationDepth === 0) journal.graphicsMutationEpoch += 1;
    };
    ...
    const patchSetter = (
      ...
      mutationKind: "environment" | "graphics" = "environment"
    ) => {
      if (!prototype) return;
      ...
      Object.defineProperty(prototype, property, {
        ...
        set(this: unknown, value: unknown) {
          if (mutationKind === "graphics") markGraphicsMutation();
          else markNonDomMutation();
    ...
    const installRealmInstrumentation = (realm: Window & typeof globalThis): NativeSchedulers => {
      ...
      for (const property of Object.getOwnPropertyNames(twoDPrototype ?? {})) {
        ...
        if (descriptor?.set) patchSetter(twoDPrototype, property, "graphics");
      }
      for (const method of [
      ...
      ]) patchMethod(twoDPrototype, method, "graphics");
      for (const constructorName of ["WebGLRenderingContext", "WebGL2RenderingContext"] ) {
        ...
        for (const property of Object.getOwnPropertyNames(prototype ?? {})) {
          if (typeof Object.getOwnPropertyDescriptor(prototype ?? {}, property)?.value === "function" &&
            ...
            patchMethod(prototype, property, "graphics");
          }
        }
      }
      return {
    ...
    const internalAuditMutation = (record: MutationRecord) => {
      if (record.type === "attributes" && record.target instanceof Element &&
          record.target.localName === "canvas" &&
          (record.attributeName === "width" || record.attributeName === "height") &&
          record.oldValue === record.target.getAttribute(record.attributeName)) {
        // A responsive Canvas effect may assign its already-current bitmap
        // dimensions after scroll/analytics re-rendering. Chromium still emits
        // an attribute record, although geometry is unchanged. Persistent
        // bitmap drift is independently rejected by snapshotMatches().
        return true;
      }
      if (record.type === "attributes" && record.target instanceof HTMLInputElement &&
        ...
        const targetRecords = [...journal.auditRecords, ...journal.externalRestoreRecords]
          .filter((candidate) => candidate.type === "attributes" && candidate.target === record.target &&
            (candidate.attributeName === "name" || candidate.attributeName === "type"));
        const isExactNetZeroControlledRangeNormalization =
          record.target.getAttribute("name") === null &&
          record.target.getAttribute("type") === "range" &&
          targetRecords.length >= 3 &&
          targetRecords.every((candidate) =>
            candidate.attributeName === "name"
              ? candidate.oldValue === null || candidate.oldValue === ""
              : candidate.oldValue === "range"
          ) &&
          targetRecords.some((candidate) =>
            candidate.attributeName === "name" && candidate.oldValue === null
          ) &&
          targetRecords.some((candidate) =>
            candidate.attributeName === "name" && candidate.oldValue === ""
          ) &&
          targetRecords.some((candidate) =>
            candidate.attributeName === "type" && candidate.oldValue === "range"
          );
        if (isExactNetZeroControlledRangeNormalization) {
          // React's controlled-input synchronization temporarily sets an
          // absent name to "", writes the already-current range type, then
          // removes the name again. This exact, complete round trip cannot
          // affect final form semantics or layout. Persistent DOM, geometry,
          // and pixel drift remain independently rejected by snapshotMatches.
          return true;
        }
      }
    ...
    candidate.activePreparation = {
      ...
      restore: async () => {
        ...
        // A timer, promise, or pixel-equivalent Canvas/WebGL redraw that runs
        // while scrolling is not itself a layout mutation. DOM/style/geometry
        // and non-graphics environment changes remain hard failures; Canvas
        // fingerprints and the quiescent fence still reject persistent visual
        // drift, pending work, or unstable geometry before restoration passes.
        let layoutMutated = auditMutationObserved || preRestoreSnapshotChanged ||
          journal.nonDomMutationEpoch > 0 || journal.mediaChangeEpoch > 0 ||
        ...
        for (let attempt = 0; attempt < 4; attempt += 1) {
          ...
          drainToCurrentPhase();
          if (journal.externalRestoreRecords.slice(handledExternalRecords)
              .some((record) => !internalAuditMutation(record))) {
            layoutMutated = true;
          }
        ...
        if (!environmentRestored) {
          ...
          if (finalExternalRecords.some((record) => !internalAuditMutation(record))) {
            layoutMutated = true;
...
export async function collectVisualizationUiAuditEvidence(root: Locator, axis: CaliforniaQaAxis) {
  const productLifecycle = await root.evaluate(async (element) => {
    const rootElement = element as HTMLElement;
    const deadline = performance.now() + 6_000;
    let quietSamples = 0;
    let previousState = "";
    const capture = () => Array.from(rootElement.querySelectorAll<HTMLElement>("[data-viz-card]"))
      .map((card) => ({
        exploreGate: card.getAttribute("data-viz-explore-gate") ?? "missing",
        moduleId: card.getAttribute("data-viz-module-id") ?? "unknown",
        saveState: card.getAttribute("data-viz-save-state") ?? "missing"
      }));
    const isPending = (states: ReturnType<typeof capture>) => states.some((state) =>
      state.exploreGate === "dwell" ||
      state.saveState === "saving" ||
      (state.exploreGate === "engaged" && state.saveState === "idle")
    );
    let states = capture();
    while (performance.now() < deadline) {
      const state = JSON.stringify(states);
      if (!isPending(states) && state === previousState) quietSamples += 1;
      else quietSamples = 0;
      if (quietSamples >= 2) {
        return { settled: true, states, waitedMs: Math.round(6_000 - (deadline - performance.now())) };
      }
      previousState = state;
    ...
    return { settled: false, states, waitedMs: 6_000 };
  });
  if (!productLifecycle.settled) {
    throw new Error(
      "Visualization product lifecycle did not settle before viewport audit: " +
      JSON.stringify(productLifecycle.states)
    );
  }
  ...
  const evaluateCurrentGeometry = (viewportFocusIndex: number | null) => root.evaluate(async (element, options) => {
    ...
    const pushCandidateFinding = (
      ...
      if (category === "viewport-text-clip" && candidateIndices.length === 0) return;
    ...
    const excludedTextAncestor = (target: Element) => closestComposed(target,
      'script,style,template,noscript,select,option,textarea,.sr-only,[class~="sr-only"]'
    );
...
export class CaliforniaBrowserDiagnostics {
  ...
  private readonly pendingDiagnosticCollections = new Set<Promise<void>>();
  private readonly responseMetadata = new WeakMap<Request, {
  ...
  async drainAfterPending() {
    while (this.pendingDiagnosticCollections.size > 0) {
      await Promise.all([...this.pendingDiagnosticCollections]);
    }
    return this.drain();
  }
  ...
  async awaitTerminalQuiescence(options: {
    ...
    while (Date.now() < absoluteDeadline && timeoutRemaining(timeout) > 0) {
      ...
      if (
        ...
        this.pendingDiagnosticCollections.size === 0 &&
    ...
    const pendingRequests = Array.from(pendingCounts)
      ...
      .sort((left, right) => left.localeCompare(right));
    const drained = await this.drainAfterPending();
    return {
  ...
  private readonly onRequestFailed = (request: Request) => {
    ...
    const response = this.responseMetadata.get(request);
    const pending = this.collectFailedRequestDiagnostic({
      pageUrl,
      request,
      responseContentType: response?.contentType ?? null,
      responseStatus: response?.status ?? null
    });
    this.pendingDiagnosticCollections.add(pending);
    void pending.finally(() => {
      this.pendingDiagnosticCollections.delete(pending);
      this.markNetworkActivity();
    });
  };

  private async collectFailedRequestDiagnostic(options: {
    pageUrl: string;
    request: Request;
    ...
    try {
      const [requestHeaders, studentLessonHref, visualizationDirectoryHref] = await Promise.all([
        options.request.allHeaders(),
        this.page.locator('[data-tour="student-lesson"]').first()
          .getAttribute("href", { timeout: 2_000 })
          .catch(() => null),
        this.page.locator('a[href="/student/tools/visualizations"]').first()
          .getAttribute("href", { timeout: 2_000 })
          .catch(() => null)
      ]);
      if (isExpectedCanceledCaliforniaProductRscPrefetch({
        failureText: options.request.failure()?.errorText ?? null,
        isNavigationRequest: options.request.isNavigationRequest(),
      ...
      const failure = options.request.failure()?.errorText ?? "unknown failure";
      let pathname = options.request.url();
      try { pathname = new URL(options.request.url()).pathname; } catch { /* keep raw URL */ }
      const normalizedHeaders = Object.fromEntries(
        Object.entries(requestHeaders).map(([name, value]) => [name.toLowerCase(), value])
      );
      this.diagnostics.push({
        kind: "request-failed",
        detail: `${options.request.resourceType()} ${pathname}: ${failure} ${JSON.stringify({
          isNavigationRequest: options.request.isNavigationRequest(),
          method: options.request.method(),
          nextUrl: normalizedHeaders["next-url"] ?? null,
          responseContentType: options.responseContentType,
          responseStatus: options.responseStatus,
          rsc: normalizedHeaders.rsc ?? null,
          secFetchDest: normalizedHeaders["sec-fetch-dest"] ?? null,
          secFetchMode: normalizedHeaders["sec-fetch-mode"] ?? null,
          studentLessonHref,
          visualizationDirectoryHref
        })}`
      });
      ...
      this.diagnostics.push({
        kind: "request-failed",
        detail: `diagnostic collection failed for ${options.request.method()} ${options.request.url()}: ${errorDetail(error)}`
      });
    }
  }
  ...
  private readonly onResponse = (response: {
    ...
    this.responseMetadata.set(response.request(), {
...
export async function appendCaliforniaDiagnosticFindings(options: {
  context: CaliforniaQaContext;
  diagnostics: CaliforniaBrowserDiagnostics;
  issues: string[];
}): Promise<CaliforniaDiagnosticDrainResult> {
  const drained = await options.diagnostics.drainAfterPending();
...
export async function runCaliforniaQaAction(options: {
  ...
  if (destructiveNavigation) {
    ...
    const preActionDrain = await appendCaliforniaDiagnosticFindings({
  ...
  const postActionDrain = await appendCaliforniaDiagnosticFindings({
