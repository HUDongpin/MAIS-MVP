...
test("machine acceptance persists the raw package result before any derived receipt can fail", () => {
  const source = readFileSync(
    "tests/e2e/hk-visualization-machine-acceptance.spec.ts",
    "utf8",
  );
  const runIndex = source.indexOf("const result = await runHkVisualizationPackage");
  const attachIndex = source.indexOf(
    "await testInfo.attach(`${sanitizeAttachmentName(regressionPackage.id)}.json`",
  );
  const scrollAggregateIndex = source.indexOf(
    "aggregateHkVisualizationBrowserScrollObservationReceipts(",
  );

  assert.ok(runIndex >= 0, "machine acceptance must execute the raw package");
  assert.ok(attachIndex > runIndex, "raw result attachment must follow package execution");
  assert.ok(
    attachIndex < scrollAggregateIndex,
    "raw result attachment must precede every derived browser receipt",
  );
  assert.equal(
    source.match(/await testInfo\.attach\(`\$\{sanitizeAttachmentName\(regressionPackage\.id\)\}\.json`/gu)?.length,
    1,
  );
});

test("active-lab navigation and copy controls expose the exact 44px minimum touch target", () => {
  const source = readFileSync(
    "components/visualizations/VisualizationLabPage.tsx",
    "utf8",
  );
  for (const selectorAttribute of [
    "data-viz-back-to-control-panel-link",
    "data-viz-copy-lab-link",
    "data-viz-copy-lab-snapshot",
  ]) {
    const controlStart = source.indexOf(selectorAttribute, source.indexOf("data-viz-copy-controls-ready"));
    assert.ok(controlStart >= 0, `Missing ${selectorAttribute}.`);
    const classStart = source.indexOf('className="', controlStart);
    const classEnd = source.indexOf('"', classStart + 'className="'.length);
    assert.ok(classStart > controlStart && classEnd > classStart);
    const className = source.slice(classStart, classEnd);
    assert.match(className, /(?:^|\s)min-h-11(?:\s|$)/u, selectorAttribute);
  }
});

test("learner visualization surfaces use deterministic solid backdrops without inset paint", () => {
  ...
  const cardStart = cardSource.indexOf("data-viz-card");
  const cardEnd = cardSource.indexOf(">", cardStart);
  assert.ok(cardStart >= 0 && cardEnd > cardStart);
  const cardOpeningTag = cardSource.slice(cardStart, cardEnd);
  assert.doesNotMatch(cardOpeningTag, /glass-panel|backdrop-blur/iu);
  assert.match(cardOpeningTag, /\bbg-white\b/u);
  assert.match(cardOpeningTag, /\bdark:bg-slate-900\b/u);

  const nightThemeStart = themeSource.indexOf("const nightVisualizationTheme");
  const nightThemeEnd = themeSource.indexOf("export function visualizationThemeForTheme");
  assert.ok(nightThemeStart >= 0 && nightThemeEnd > nightThemeStart);
  const nightTheme = themeSource.slice(nightThemeStart, nightThemeEnd);
  assert.doesNotMatch(nightTheme, /\bshadow-inner\b/u);
});
