...
import { usesAuditedTwoDimensionalValueRenderer } from "../../components/visualizations/three/configuredThreeDRenderPlan";
import {
  isExpectedCanceledCaliforniaProductRscPrefetch,
  isExpectedPinnedReactThreeFiberClockDeprecation
} from "./california-product-smoke-browser-diagnostics";
...
const VISUALIZATION_DIRECTORY_HREF = "/student/tools/visualizations";
const DISABLE_CRASHPAD_FOR_TESTING_ARGUMENT = "--disable-crashpad-for-testing";
const STARSHIP_E2E_ROOT =
  "/Volumes/Starship/MAIS-MVP/.worktrees/a06-ca-viz-resume-20260819/.tmp";
...
type BrowserDiagnostics = {
  activeProductDeliveryRequests: Set<Request>;
  entries: BrowserDiagnostic[];
  expectedStudentLessonHrefByPage: WeakMap<Page, string>;
  expectedVisualizationDirectoryHrefByPage: WeakMap<Page, string>;
  pending: Set<Promise<void>>;
};

function isProductDeliveryRequest(request: Request) {
  try {
    return request.method() === "POST" && new URL(request.url()).pathname === "/api/learning-events";
  } catch {
    return false;
  }
}
...
function assertFormalLayerAEnvironment(testInfo: TestInfo) {
  ...
  assert.deepEqual(launchArgs, [
    DISABLE_CRASHPAD_FOR_TESTING_ARGUMENT,
    `--breakpad-dump-location=${crashpadDir}`
  ], "Layer-A must launch Chrome with the exact Crashpad disable-and-contain arguments.");
...
function readProductRuntimeDependencies() {
  return (
...
function collectBrowserDiagnostics(
  ...
  const productRuntimeDependencies = readProductRuntimeDependencies();
  const responseMetadata = new WeakMap<object, { contentType: string | null; status: number }>();
  page.on("request", (request) => {
    if (isProductDeliveryRequest(request)) diagnostics.activeProductDeliveryRequests.add(request);
  });
  page.on("requestfinished", (request) => {
    diagnostics.activeProductDeliveryRequests.delete(request);
  });
  page.on("console", (message) => {
    if (message.type() === "error" || message.type() === "warning" || message.type() === "assert") {
      if (isExpectedPinnedReactThreeFiberClockDeprecation({
        locationUrl: message.location().url,
        messageType: message.type(),
        pageUrl: page.url(),
        reactThreeFiberVersion: productRuntimeDependencies["@react-three/fiber"] ?? null,
        text: message.text(),
        threeVersion: productRuntimeDependencies.three ?? null
      })) {
        return;
      }
      diagnostics.entries.push({ detail: diagnosticDetail(message), kind: "console" });
    }
  });
  page.on("pageerror", (error) => {
    diagnostics.entries.push({ detail: error.message, kind: "page" });
  });
  page.on("requestfailed", (request) => {
    diagnostics.activeProductDeliveryRequests.delete(request);
    const pageUrl = page.url();
    const response = responseMetadata.get(request);
    const pending = (async () => {
      try {
        const [requestHeaders, liveStudentLessonHref, liveVisualizationDirectoryHref] = await Promise.all([
          request.allHeaders(),
          diagnostics.expectedStudentLessonHrefByPage.has(page)
            ? Promise.resolve(null)
            : page.locator('[data-tour="student-lesson"]').first().getAttribute("href", { timeout: 2_000 })
                .catch(() => null),
          diagnostics.expectedVisualizationDirectoryHrefByPage.has(page)
            ? Promise.resolve(null)
            : page.locator(`a[href="${VISUALIZATION_DIRECTORY_HREF}"]`).first()
                .getAttribute("href", { timeout: 2_000 })
                .catch(() => null)
        ]);
        const studentLessonHref =
          diagnostics.expectedStudentLessonHrefByPage.get(page) ?? liveStudentLessonHref;
        const visualizationDirectoryHref =
          diagnostics.expectedVisualizationDirectoryHrefByPage.get(page) ?? liveVisualizationDirectoryHref;
        if (studentLessonHref?.startsWith("/student/lessons/us-ca-math-")) {
          diagnostics.expectedStudentLessonHrefByPage.set(page, studentLessonHref);
        }
        if (visualizationDirectoryHref === VISUALIZATION_DIRECTORY_HREF) {
          diagnostics.expectedVisualizationDirectoryHrefByPage.set(page, visualizationDirectoryHref);
        }
        if (isExpectedCanceledCaliforniaProductRscPrefetch({
          ...
          studentLessonHref: studentLessonHref ?? null,
          visualizationDirectoryHref
        })) {
          return;
        }
        const safeContext = {
          isNavigationRequest: request.isNavigationRequest(),
          nextUrl: requestHeaders["next-url"] ?? null,
          pageUrl,
          resourceType: request.resourceType(),
          responseContentType: response?.contentType ?? null,
          responseStatus: response?.status ?? null,
          rsc: requestHeaders.rsc ?? null,
          secFetchDest: requestHeaders["sec-fetch-dest"] ?? null,
          secFetchMode: requestHeaders["sec-fetch-mode"] ?? null,
          studentLessonHref: studentLessonHref ?? null,
          visualizationDirectoryHref
        };
        diagnostics.entries.push({
          detail: `${request.method()} ${request.url()} ${request.failure()?.errorText ?? "unknown failure"} ${JSON.stringify(safeContext)}`,
          kind: "request"
        });
      } catch (error) {
        diagnostics.entries.push({
          detail: `diagnostic collection failed for ${request.method()} ${request.url()}: ${error instanceof Error ? error.message : String(error)}`,
          kind: "request"
        });
      }
    })();
    diagnostics.pending.add(pending);
    void pending.finally(() => diagnostics.pending.delete(pending));
  });
  page.on("response", (response) => {
    responseMetadata.set(response.request(), {
      contentType: response.headers()["content-type"] ?? null,
      status: response.status()
    });
    if (response.status() >= 400) {
      diagnostics.entries.push({ detail: `${response.status()} ${response.request().method()} ${response.url()}`, kind: "http" });
    }
  });
  return diagnostics;
}
...
async function expectCurrentCaliforniaStudent(
  ...
  expect(body).toMatchObject({
    lessonEntryTarget: { grade: student.grade },
  ...
  assert.ok(body && typeof body === "object", "Authenticated California student response must be an object.");
  const lessonEntryTarget = (body as { lessonEntryTarget?: unknown }).lessonEntryTarget;
  assert.ok(lessonEntryTarget && typeof lessonEntryTarget === "object",
    `California ${student.grade} student must expose one authenticated lesson entry target.`);
  const href = (lessonEntryTarget as { href?: unknown }).href;
  assert.ok(typeof href === "string", `California ${student.grade} lesson entry href must be a string.`);
  assert.match(href, /^\/student\/lessons\/us-ca-math-/,
    `California ${student.grade} lesson entry href must remain on the signed-in California lesson surface.`);
  return href;
}
...
async function loginCaliforniaStudentForGrade(
  ...
  return expectCurrentCaliforniaStudent(request, student);
}
...
async function expectProductDeliveryOutboxesDrained(
  page: Page,
  diagnostics: BrowserDiagnostics
) {
  ...
  await expect.poll(async () => ({
    activeProductDeliveryRequests: diagnostics.activeProductDeliveryRequests.size,
    closeWindowElapsed: Date.now() - settleStartedAt >= PRODUCT_DELIVERY_CLOSE_SETTLE_MS,
    storageKeys: await page.evaluate((prefixes) =>
      ([
    ...
    message: "California Layer-A must durably settle visualization sessions and learning-event delivery before changing learner scope.",
    timeout: 30_000
  }).toEqual({ activeProductDeliveryRequests: 0, closeWindowElapsed: true, storageKeys: [] });
}
...
async function retireActiveCaliforniaStudent(
  ...
  await expectProductDeliveryOutboxesDrained(page, diagnostics);
...
function expectExactRoute(page: Page, route: ProductSmokeRoute, direct: boolean) {
  ...
  expect([...actual.searchParams.entries()]).toEqual(
    direct
      ...
      : [["grade", route.grade], ["lab", route.labId]]
  );
}
...
async function expectReadyVisualizationSurface(surface: Locator, labId: string) {
  ...
  const dimensions = await surface.evaluate((element) => {
    ...
    const drawable = [
      ...(element.matches("canvas,svg") ? [element as HTMLCanvasElement | SVGSVGElement] : []),
      ...element.querySelectorAll<HTMLCanvasElement | SVGSVGElement>("canvas,svg")
    ];
...
async function rememberExpectedStudentLessonHref(
  page: Page,
  diagnostics: BrowserDiagnostics,
  labId: string
) {
  const lessonLink = page.locator('[data-tour="student-lesson"]').first();
  await expect(lessonLink, `${labId} must expose the signed-in learner lesson entry.`).toHaveAttribute(
    "href",
    /^\/student\/lessons\/us-ca-math-/,
    { timeout: ROUTE_READY_TIMEOUT_MS }
  );
  const href = await lessonLink.getAttribute("href");
  assert.ok(href, `${labId} signed-in learner lesson entry lost its href.`);
  diagnostics.expectedStudentLessonHrefByPage.set(page, href);
}

async function expectDirectoryLabProductReady(
  page: Page,
  route: ProductSmokeRoute,
  ...
  if ((assignment.related?.length ?? 0) > 0) {
    await expect(switcher).toHaveAttribute("data-viz-active-signature-bench-id", assignment.primary, {
      timeout: ROUTE_READY_TIMEOUT_MS
    });
  } else {
    await expect(switcher, `${route.labId} has one bench and must not expose an empty switcher.`).toHaveCount(0);
  }
  ...
  await expectReadyVisualizationSurface(surface, route.labId);
  expectExactRoute(page, route, false);
  await rememberExpectedStudentLessonHref(page, diagnostics, route.labId);
...
async function expectPremiumLabProductReady(
  page: Page,
  route: ProductSmokeRoute,
  loadedScripts: Set<string>,
  diagnostics: BrowserDiagnostics
  ...
  const premiumFamilyId = route.lab.threeD?.familyId;
  assert.ok(premiumFamilyId, `${route.labId} premium route must declare its reviewed 3D family.`);
  const usesAuditedTwoDimensionalSurface = usesAuditedTwoDimensionalValueRenderer({
    familyId: premiumFamilyId,
    labId: route.labId,
    templateId: route.lab.templateId
  });
  if (usesAuditedTwoDimensionalSurface) {
    const surface = panel.locator('svg[data-viz-surface][role="img"]').first();
    await expectReadyVisualizationSurface(surface, route.labId);
    await expect(
      panel.locator("[data-viz-three-progressive-surface]"),
      `${route.labId} must remain on its reviewed 2D renderer.`
    ).toHaveCount(0);
    await expect(
      panel.locator("[data-viz-manim-control-dock]"),
      `${route.labId} audited 2D renderer must not expose a detached 3D control dock.`
    ).toHaveCount(0);

    const alternateMode = panel.locator('button[data-viz-mode-button][data-viz-mode-index="1"]').first();
    await expect(alternateMode, `${route.labId} audited 2D model switch must be visible.`).toBeVisible({
      timeout: ROUTE_READY_TIMEOUT_MS
    });
    await alternateMode.click({ timeout: ROUTE_READY_TIMEOUT_MS });
    await expect(alternateMode).toHaveAttribute("data-viz-mode-active", "true");
    await expect(surface).toHaveAttribute("data-viz-active-mode", "1");

    const reset = panel.locator("button[data-viz-reset-model]").first();
    await expect(reset, `${route.labId} audited 2D reset must be visible.`).toBeVisible({
      timeout: ROUTE_READY_TIMEOUT_MS
    });
    await expect(reset, `${route.labId} audited 2D reset must be enabled.`).toBeEnabled();
    await reset.click({ timeout: ROUTE_READY_TIMEOUT_MS });
    await expect(surface).toHaveAttribute("data-viz-active-mode", "0");
    await expect(
      panel.locator('button[data-viz-mode-button][data-viz-mode-index="0"]').first()
...
async function expectNoBrowserDiagnostics(diagnostics: BrowserDiagnostics) {
  while (diagnostics.pending.size > 0) {
    await Promise.all([...diagnostics.pending]);
  }
  expect(
    diagnostics.entries,
    diagnostics.entries.map((diagnostic) => `${diagnostic.kind}: ${diagnostic.detail}`).join("\n")
  ).toEqual([]);
}
...
test.describe("California Visualization Layer-A uninstrumented product smoke", () => {
  ...
  test(CALIFORNIA_PRODUCT_SMOKE_TEST_TITLE, async ({ context, page }, testInfo) => {
    ...
    const diagnostics: BrowserDiagnostics = {
      activeProductDeliveryRequests: new Set(),
      entries: [],
      expectedStudentLessonHrefByPage: new WeakMap(),
      expectedVisualizationDirectoryHrefByPage: new WeakMap(),
      pending: new Set()
    };
    ...
    const selectStudentForRoute = async (route: ProductSmokeRoute) => {
      ...
      if (activePage !== null) {
        await retireActiveCaliforniaStudent(context, activePage, diagnostics);
        activePage = null;
        activeGrade = null;
      }
      ...
      const expectedStudentLessonHref = await loginCaliforniaStudentForGrade(context.request, student);
    ...
    for (const route of californiaVisualizationProductSmokeManifest.directoryRoutes) {
      ...
      await expectDirectoryLabProductReady(routePage, route, loadedScripts, diagnostics);
      visitedDirectoryLabIds.push(route.labId);
      await routePage.waitForTimeout(250);
      await expectProductDeliveryOutboxesDrained(routePage, diagnostics);
      await expectNoBrowserDiagnostics(diagnostics);
    }
    for (const route of californiaVisualizationProductSmokeManifest.premiumRoutes) {
      const routePage = await selectStudentForRoute(route);
      await expectPremiumLabProductReady(routePage, route, loadedScripts, diagnostics);
    ...
    await expectNoBrowserDiagnostics(diagnostics);
    await retireActiveCaliforniaStudent(context, finalPage, diagnostics);
    activePage = null;
    activeGrade = null;
    await expectNoBrowserDiagnostics(diagnostics);
