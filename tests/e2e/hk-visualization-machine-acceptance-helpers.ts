...
export async function exerciseHkVisualizationInteractionMicrofixture(args: {
  ...
  const result = {
    cellId: `microfixture:interaction:${args.language}`,
    collisions: [],
    contract: { renderer: "svg" },
    controls: [],
    failures: [],
...
export async function exerciseHkVisualizationDynamicRangeMicrofixture(args: {
  ...
  const result = {
    cellId: `microfixture:${args.labId}:${args.language ?? "en"}`,
    collisions: [],
    contract: { renderer: "svg" },
    controls: [],
    failures: [],
...
function dynamicMicrofixtureCellResult(
  ...
  return {
    cellId: `fresh-microfixture:${labId}:${language}`,
    collisions: [],
    contract: { renderer: "svg" },
    controls: [],
    failures: [],
...
async function preflightFreshPageDynamicRangePlan(
  ...
  for (const range of ranges) {
    const slider = dynamicDescendantLocator(rangeRoot, range.selector);
    await assertLocalizedAccessibleName(
      slider,
      language,
      "control",
...
function dynamicDescendantLocator(modelRoot: Locator, selector: string) {
  ...
  return modelRoot.locator(selector);
}
...
async function assertActiveRangeDomainEdgeMetadata(
  ...
  for (const range of ranges) {
    const control = dynamicDescendantLocator(model, range.selector);
    ...
    try {
      for (const range of ranges) {
        ...
        const slider = activeDomain
          ? dynamicDescendantLocator(rangeRoot, range.selector)
...
async function resetAndPrepareHkDependentTransitionMode(
  ...
  const reset = dynamicDescendantLocator(model, contract.selectors.reset);
  if (
    (await reset.count()) !== 1 ||
...
async function visibleDeclaredRanges(
  ...
  for (let index = 0; index < contract.selectors.controls.length; index += 1) {
    ...
    const control = dynamicModelScoped
      ? dynamicDescendantLocator(workspace, selector)
...
async function visibleDeclaredModeGroups(
  ...
  for (let index = 0; index < contract.selectors.modes.length; index += 1) {
    const mode = dynamicModelScoped
      ? dynamicDescendantLocator(workspace, contract.selectors.modes[index])
...
function assertFinalVisualizationRoute(
  ...
  const expected = new URL(buildVisualizationLabHref(lab, "HK"), baseURL);
  ...
  if (
    actual.searchParams.get("track") !== "HK" ||
    actual.searchParams.get("grade") !== lab.grade
  ) {
    throw new Error(
      `Final ${routeKind} route did not preserve track=HK and grade=${lab.grade}.`,
    );
  }
  if (routeKind === "query" && actual.searchParams.get("lab") !== lab.labId) {
    throw new Error(`Final query route did not preserve lab=${lab.labId}.`);
  }
  if (routeKind === "premium-direct" && actual.searchParams.has("lab")) {
    throw new Error(
