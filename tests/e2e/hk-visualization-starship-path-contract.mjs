...
function captureVolumeIdentity({
  artifactWorkspace,
  executionWorkspace,
  sourceWorkspace,
}) {
  const volumeIdentity = Object.freeze({
    artifactWorkspace: captureHkVisualizationPhysicalPathIdentity(
      "HK Visualization artifact workspace identity",
      artifactWorkspace,
    ),
    executionWorkspace: captureHkVisualizationPhysicalPathIdentity(
      "HK Visualization execution workspace identity",
      executionWorkspace,
    ),
    sourceWorkspace: captureHkVisualizationPhysicalPathIdentity(
      "HK Visualization source workspace identity",
      sourceWorkspace,
    ),
  ...
  for (const role of [
    ...
    if (volumeIdentity.starshipRoot.device !== volumeIdentity[role].device) {
      throw new Error(
        `HK Visualization ${role} must be on the same physical device as /Volumes/Starship.`,
      );
    }
  }
  if (
...
function validateVolumeIdentity(volumeIdentity, paths) {
  const issues = [];
  if (!exactObjectKeys(volumeIdentity, VOLUME_IDENTITY_KEYS)) {
    return ["volumeIdentity key set drift"];
  }
  const expectedPaths = {
    artifactWorkspace: paths.artifactWorkspace,
    executionWorkspace: paths.executionWorkspace,
    sourceWorkspace: paths.sourceWorkspace,
    systemRoot: "/",
    starshipRoot: HK_VISUALIZATION_STARSHIP_ROOT,
    workspace: paths.executionWorkspace,
  };
  ...
  for (const role of [
    ...
    if (volumeIdentity.starshipRoot?.device !== volumeIdentity[role]?.device) {
      issues.push(`${role} device differs from Starship root device`);
    }
  }
  if (
    canonicalJson(volumeIdentity.workspace) !==
    canonicalJson(volumeIdentity.executionWorkspace)
  ) {
    issues.push("workspace identity differs from execution workspace identity");
  }
  if (
    volumeIdentity.systemRoot?.device === volumeIdentity.starshipRoot?.device
  ) {
  ...
  try {
    const liveIdentity = captureVolumeIdentity(paths);
...
function buildPaths(
  { artifactWorkspace, executionWorkspace, sourceWorkspace },
  runId,
) {
  const isolated =
    sourceWorkspace !== executionWorkspace ||
    artifactWorkspace !== executionWorkspace;
  const artifactRoot = isolated
    ? join(artifactWorkspace, runId)
    : join(executionWorkspace, ".tmp", runId);
  ...
  return Object.freeze({
    ...
    nextDistDir: isolated
      ? join(executionWorkspace, ".next")
      : join(artifactRoot, "next-dist"),
...
export function buildHkVisualizationE2eTsconfig({
  workspace,
  nextDistDir,
  tsconfigPath = process.env.PLAYWRIGHT_NEXT_TSCONFIG_PATH,
  exclude = [],
}) {
  ...
  const absoluteTsconfigPath = tsconfigPath
    ? assertHkVisualizationStarshipPath(
        "HK Visualization external e2e tsconfig",
        tsconfigPath,
      )
    : null;
  const configDirectory = absoluteTsconfigPath
    ? dirname(absoluteTsconfigPath)
    : absoluteWorkspace;
  const fromConfig = (absolutePath) => {
    const value = relative(configDirectory, absolutePath).replaceAll("\\", "/");
    if (!value) return ".";
    return value.startsWith(".") ? value : `./${value}`;
  };
  const workspacePrefix = fromConfig(absoluteWorkspace);
  const externalConfig = configDirectory !== absoluteWorkspace;
  const scopedExclude = (value) => {
    if (!externalConfig || isAbsolute(value)) return value;
    const normalized = value.replaceAll("\\", "/");
    const negated = normalized.startsWith("!");
    const body = negated ? normalized.slice(1) : normalized;
    const prefix = workspacePrefix === "." ? "" : `${workspacePrefix}/`;
    return `${negated ? "!" : ""}${prefix}${body}`;
  };
  return Object.freeze({
    extends: externalConfig
      ? fromConfig(join(absoluteWorkspace, "tsconfig.json"))
      : "./tsconfig.json",
    include: Object.freeze([
      externalConfig
        ? fromConfig(join(absoluteWorkspace, "next-env.d.ts"))
        : "next-env.d.ts",
      externalConfig ? `${workspacePrefix}/**/*.ts` : "**/*.ts",
      externalConfig ? `${workspacePrefix}/**/*.tsx` : "**/*.tsx",
      externalConfig
        ? `${workspacePrefix}/.next/types/**/*.ts`
        : ".next/types/**/*.ts",
      externalConfig
        ? `${fromConfig(absoluteNextDistDir)}/types/**/*.ts`
        : `${relativeNextDistDir}/types/**/*.ts`,
    ]),
    exclude: Object.freeze(exclude.map(scopedExclude)),
  });
}
...
export function buildHkVisualizationManagedWebServerCommand({
  ...
  const isolated =
    paths.sourceWorkspace !== paths.executionWorkspace ||
    paths.artifactWorkspace !== paths.executionWorkspace;
  ...
  if (
    ...
    !/^tsconfig\.playwright-[A-Za-z0-9._-]+\.tmp\.json$/.test(
      basename(nextTsconfigPathForBuild),
    ) ||
    (isolated
      ? !pathIsInside(paths.nextTsconfigPath, paths.artifactRoot)
      : nextTsconfigPathForBuild.startsWith("..") ||
        dirname(nextTsconfigPathForBuild) !== "."
    )
  ) {
    throw new Error(
  ...
  return [
    ...
    `/bin/rm -f ${shellQuote(paths.nextTsconfigPath)}`,
...
export function buildHkVisualizationStarshipPathManifest({
  artifactWorkspace = process.env.PLAYWRIGHT_ARTIFACT_WORKSPACE,
  runId,
  sourceWorkspace = process.env.PLAYWRIGHT_SOURCE_WORKSPACE,
  workspace,
}) {
  const absoluteExecutionWorkspace = assertHkVisualizationStarshipPath(
    "HK Visualization E2E workspace",
    workspace,
  );
  const absoluteSourceWorkspace = assertHkVisualizationStarshipPath(
    "HK Visualization source workspace",
    sourceWorkspace ?? absoluteExecutionWorkspace,
  );
  const absoluteArtifactWorkspace = assertHkVisualizationStarshipPath(
    "HK Visualization artifact workspace",
    artifactWorkspace ?? absoluteExecutionWorkspace,
  );
  const workspaceRoles = [
    absoluteSourceWorkspace,
    absoluteExecutionWorkspace,
    absoluteArtifactWorkspace,
  ];
  const isolated = new Set(workspaceRoles).size !== 1;
  if (isolated) {
    if (new Set(workspaceRoles).size !== workspaceRoles.length) {
      throw new Error(
        "HK Visualization isolated source, execution, and artifact workspaces must be distinct without overlap.",
      );
    }
    for (let left = 0; left < workspaceRoles.length; left += 1) {
      for (let right = left + 1; right < workspaceRoles.length; right += 1) {
        if (
          pathIsInside(workspaceRoles[left], workspaceRoles[right]) ||
          pathIsInside(workspaceRoles[right], workspaceRoles[left])
        ) {
          throw new Error(
            "HK Visualization isolated source, execution, and artifact workspaces cannot nest or overlap.",
          );
        }
      }
    }
  ...
  const workspacePaths = Object.freeze({
    artifactWorkspace: absoluteArtifactWorkspace,
    executionWorkspace: absoluteExecutionWorkspace,
    sourceWorkspace: absoluteSourceWorkspace,
  });
  const paths = buildPaths(workspacePaths, runId);
  for (const key of PATH_KEYS)
    assertHkVisualizationStarshipPath(`HK Visualization ${key}`, paths[key]);
  const volumeIdentity = captureVolumeIdentity(workspacePaths);
  const base = {
...
export function validateHkVisualizationStarshipPathManifest(manifest) {
  ...
  try {
    ...
    const sourceWorkspace = assertHkVisualizationStarshipPath(
      "manifest source workspace",
      manifest.paths.sourceWorkspace,
    );
    const executionWorkspace = assertHkVisualizationStarshipPath(
      "manifest execution workspace",
      manifest.paths.executionWorkspace,
    );
    const artifactWorkspace = assertHkVisualizationStarshipPath(
      "manifest artifact workspace",
      manifest.paths.artifactWorkspace,
    );
    if (workspace !== executionWorkspace) {
      issues.push("workspace differs from executionWorkspace");
    }
    const workspaceRoles = [
      sourceWorkspace,
      executionWorkspace,
    ...
    if (new Set(workspaceRoles).size !== 1) {
      if (new Set(workspaceRoles).size !== workspaceRoles.length) {
        issues.push("isolated workspace roles are not distinct");
      }
      for (let left = 0; left < workspaceRoles.length; left += 1) {
        for (let right = left + 1; right < workspaceRoles.length; right += 1) {
          if (
            ...
            issues.push("isolated workspace roles overlap or nest");
          }
        }
      }
    }
    expectedPaths = buildPaths(
      { artifactWorkspace, executionWorkspace, sourceWorkspace },
      manifest.runId,
    );
  } catch (error) {
  ...
  if (expectedPaths) {
    issues.push(
      ...validateVolumeIdentity(manifest.volumeIdentity, expectedPaths),
    );
  }
