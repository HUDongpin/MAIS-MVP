...
const executionFixtureRoots = [];

test.after(() => {
  for (const root of executionFixtureRoots.splice(0)) {
    removeReadonlyFixtureTree(root);
  }
});
...
function removeReadonlyFixtureTree(root) {
  const openDirectories = (entry) => {
    ...
    for (const name of readdirSync(entry)) openDirectories(join(entry, name));
  };
  ...
  for (const spec of HK_VISUALIZATION_RELEASE_SPECS) {
    ...
    writeFileSync(
      ...
      frozenDeadlineSource
        ? readFileSync(join("/Volumes/Starship/MAIS-hk-viz-labs-wt", spec))
        : "fixture",
    );
  }
...
const fixtureReleaseEvidenceSeams = Object.freeze({
  allocateExecutionRoots({ sourceWorkspace }) {
    const executionWorkspace = freshFixtureWorkspace("hk-viz-stage-run-");
    const artifactWorkspace = freshFixtureWorkspace("hk-viz-stage-artifacts-");
    executionFixtureRoots.push(executionWorkspace, artifactWorkspace);
    return Object.freeze({
      artifactWorkspace,
      executionWorkspace,
      runtimeRoot: testArtifactRoot,
      sourceWorkspace,
    });
  },
...
test("execution staging materializes exact source and dependency bytes on new read-only inodes", () => {
  const sourceWorkspace = freshFixtureWorkspace("hk-viz-stage-source-");
  const executionWorkspace = freshFixtureWorkspace("hk-viz-stage-execution-");
  const sourceRows = Object.freeze([
    Object.freeze({
      mode: "0644",
      path: "alpha.txt",
      sha256: "e6c601299f708a4a4ebd0fc071e68c73f3ece3aa9e86296629a8cf475697c3ba",
      size: 20,
      type: "file",
    }),
    Object.freeze({
      mode: "0644",
      path: "nested/tool.js",
      sha256: "a9d066efbc52b23be4ccf6aa4a414141a0e84eed76a7c6efbee88e673f0e1187",
      size: 31,
      type: "file",
    }),
  ]);
  const dependencyRows = Object.freeze([
    Object.freeze({
      ...
      mode: "755",
      size: 27,
      target: "../stage-dependency/tool.js",
      resolvedRelative: "stage-dependency/tool.js",
      insideRoot: true,
      broken: false,
      sha256: "c82912db460a395eebadfbfa1d23ade1553ab45be19c50eb695d664dcf0d0992",
    }),
    Object.freeze({
      path: "stage-dependency/package.json",
      type: "file",
      mode: "644",
      size: 46,
      sha256: "71ad793897a9d9f2e7364dccf8d9ab017207b8e3825d7c89d59bb6ad3f10e2e5",
    }),
    Object.freeze({
      path: "stage-dependency/tool.js",
      type: "file",
      mode: "644",
      size: 20,
      sha256: "a59c47872b71f12589942892464e764c0db350c20b72228645615cc36e0a0725",
    }),
  ]);
  try {
    writeFileSync(join(sourceWorkspace, "alpha.txt"), "alpha-stage-fixture\n");
    mkdirSync(join(sourceWorkspace, "nested"));
    writeFileSync(
      join(sourceWorkspace, "nested", "tool.js"),
      "export const stageFixture = 7;\n",
    );
    mkdirSync(join(sourceWorkspace, "node_modules", ".bin"), {
      recursive: true,
    });
    mkdirSync(join(sourceWorkspace, "node_modules", "stage-dependency"), {
      recursive: true,
    });
    writeFileSync(
      join(sourceWorkspace, "node_modules", "stage-dependency", "package.json"),
      '{"name":"stage-dependency","version":"1.0.0"}\n',
    );
    writeFileSync(
      join(sourceWorkspace, "node_modules", "stage-dependency", "tool.js"),
      "#!/usr/bin/env node\n",
    );
    symlinkSync(
      "../stage-dependency/tool.js",
      join(sourceWorkspace, "node_modules", ".bin", "stage-tool"),
    );
    const sourceSnapshot = captureHkVisualizationReleaseSourceSnapshot({
      workspace: sourceWorkspace,
    });
    assert.deepEqual(
      sourceSnapshot.entries.filter(({ type }) => type === "file"),
      sourceRows,
      "the fixture source authority is an independent exact literal table",
    );
    const dependencyManifest = Object.freeze({
      aggregateSha256: sha256(JSON.stringify(dependencyRows)),
      entries: dependencyRows,
      entryCount: 3,
      fileCount: 2,
      symlinkCount: 1,
    });
    ...
    const materialize =
      hkVisualizationReleaseGateModule
        .materializeHkVisualizationExecutionStage;
    ...
    assert.equal(receipt.sourceAggregateSha256, sourceSnapshot.sourceAggregateSha256);
    assert.equal(receipt.dependencyAggregateSha256, dependencyManifest.aggregateSha256);
    assert.equal(receipt.sourceFileCount, 2);
    assert.equal(receipt.dependencyFileCount, 2);
    assert.equal(receipt.dependencySymlinkCount, 1);
    assert.equal(receipt.materializedRegularFileCount, 4);
    assert.equal(receipt.materializedBytes, 117);
    assert.deepEqual(receipt.sourceRows, sourceSnapshot.entries);
    assert.deepEqual(receipt.dependencyRows, dependencyRows);
    assert.equal(
      readFileSync(join(executionWorkspace, "alpha.txt"), "utf8"),
      "alpha-stage-fixture\n",
    );
    assert.equal(
      readlinkSync(join(executionWorkspace, "node_modules", ".bin", "stage-tool")),
      "../stage-dependency/tool.js",
    );
    assert.notEqual(
      lstatSync(join(sourceWorkspace, "alpha.txt"), { bigint: true }).ino,
      lstatSync(join(executionWorkspace, "alpha.txt"), { bigint: true }).ino,
    );
    assert.equal(lstatSync(executionWorkspace).mode & 0o222, 0);
    assert.equal(
      lstatSync(join(executionWorkspace, "alpha.txt")).mode & 0o222,
      0,
    );
    assert.equal(
      lstatSync(join(executionWorkspace, "node_modules")).mode & 0o222,
      0,
    );
    assert.notEqual(
      lstatSync(join(executionWorkspace, ".next")).mode & 0o200,
      0,
    );
    assert.throws(
      () => writeFileSync(join(executionWorkspace, "late-source.js"), "drift"),
      /EACCES|permission denied|read-only/i,
    );
  } finally {
    removeReadonlyFixtureTree(executionWorkspace);
    removeReadonlyFixtureTree(sourceWorkspace);
  }
});
...
function writePassingRunnerArtifacts(workspace, options) {
  ...
  const report = passingReport(pathManifest.workspace, pathManifest);
  writeFileSync(
...
function paintedGeometryValidatedSourceReceiptFixture(
  workspaceInput = "/Volumes/Starship/MAIS-hk-viz-labs-wt",
) {
...
function passingReport(
  workspace = "/Volumes/Starship/MAIS-hk-viz-labs-wt",
...
test("report validator accepts the exact nine-spec desktop evidence with zero skips, flakes, and failures", () => {
  ...
  assert.equal(result.actualPaths.workspace, "/Volumes/Starship/MAIS-hk-viz-labs-wt");
...
test("native painted-geometry pair runner exposes the separate v1 authority and freezes its source chain", () => {
  ...
  assert.equal(HK_VISUALIZATION_RELEASE_WORKLOAD_TIMEOUT_MS, 175_460_000);
});
...
test("deadline/source TDD exact6 static authority covers 370 of 370 release tests and splits the setup reserve", () => {
  const validate = hkVisualizationReleaseGateModule
  ...
  assert.equal(HK_VISUALIZATION_RELEASE_EXPECTED_TEST_COUNT, 370);
  assert.equal(HK_VISUALIZATION_RELEASE_WORKLOAD_TIMEOUT_MS, 175_460_000);
});
...
test("canonical workload deadline is a finite exact-count upper bound rather than an unbounded spawn", () => {
  ...
  assert.equal(remainingTestCount, 124);
  ...
  assert.deepEqual(HK_VISUALIZATION_NON_PACKAGE_TEST_TIMEOUT_TOPOLOGY, {
    fileCount: 7,
    testCount: 124,
    timeoutSumMs: 50_900_000,
  });
  assert.equal(
    HK_VISUALIZATION_NON_PACKAGE_TEST_TIMEOUT_SUM_MS,
    113 * 300_000 + 10 * 500_000 + 12_000_000,
  );
  assert.equal(
    hkVisualizationReleaseGateModule.HK_VISUALIZATION_STATIC_TEST_TIMEOUT_SUM_MS,
    720_000,
  ...
  assert.equal(
    HK_VISUALIZATION_RELEASE_WORKLOAD_TIMEOUT_MS,
    113_670_000 + 9_090_000 + 50_900_000 + 720_000 + 1_080_000,
  );
  assert.equal(HK_VISUALIZATION_RELEASE_WORKLOAD_TIMEOUT_MS, 175_460_000);
...
test("runner executes only the fixed plan from sealed source with external artifacts", () => {
  ...
  try {
    ...
    const result = runHkVisualizationReleaseGate({
      ...
      spawn(command, args, options) {
        ...
        assert.notEqual(
          options.cwd,
          workspace,
          "the workload must execute from an immutable staging workspace",
        );
        assert.equal(
          args[0],
          join(options.cwd, "node_modules", "@playwright", "test", "cli.js"),
        );
        assert.notEqual(args[0], playwright);
        const stagedPlaywrightBytes = readFileSync(args[0]);
        const originalPlaywrightBytes = readFileSync(playwright);
        writeFileSync(
          playwright,
          Buffer.concat([originalPlaywrightBytes, Buffer.from("// transient\n")]),
        );
        assert.deepEqual(
          readFileSync(args[0]),
          stagedPlaywrightBytes,
          "a transient original dependency mutation must not alter staged execution bytes",
        );
        writeFileSync(playwright, originalPlaywrightBytes);
        assert.deepEqual(
        ...
        const stagedTsconfig = JSON.parse(
          readFileSync(options.env.PLAYWRIGHT_NEXT_TSCONFIG_PATH, "utf8"),
        );
        assert.equal(
          resolve(
            dirname(options.env.PLAYWRIGHT_NEXT_TSCONFIG_PATH),
            stagedTsconfig.extends,
          ),
          join(options.cwd, "tsconfig.json"),
        );
        assert.equal(
        ...
        assert.equal(pathManifest.sourceWorkspace, workspace);
        assert.equal(pathManifest.executionWorkspace, options.cwd);
        assert.equal(pathManifest.workspace, options.cwd);
        assert.notEqual(pathManifest.artifactWorkspace, workspace);
        assert.notEqual(pathManifest.artifactWorkspace, options.cwd);
        assert.equal(
          pathManifest.nextDistDir,
          join(options.cwd, ".next"),
        );
        assert.equal(
          pathManifest.nextTsconfigPath.startsWith(`${options.cwd}/`),
          false,
          "the disposable tsconfig must remain outside sealed source",
        );
        assert.equal(
          pathManifest.artifactRoot.startsWith(`${options.cwd}/`),
          false,
          "reports and artifacts must remain outside sealed source",
        );
        assert.equal(
          readFileSync(join(options.cwd, "next-env.d.ts"), "utf8"),
          canonicalNextEnv,
        );
        assert.throws(
          () => rmSync(join(options.cwd, "next-env.d.ts")),
          /EACCES|permission denied|read-only/i,
        );
        const report = passingReport(options.cwd, pathManifest);
        writeFileSync(
          options.env.PLAYWRIGHT_JSON_OUTPUT_FILE,
          JSON.stringify(report),
        );
    ...
    const canonicalNextEnvSha256 = createHash("sha256")
      ...
      .digest("hex");
    assert.equal(result.nextEnvPath, join(result.plan.cwd, "next-env.d.ts"));
    ...
    assert.equal(
      releaseSourceReceipt.executionStage.contractVersion,
      "hk-viz-immutable-execution-stage-v1",
    );
    assert.equal(releaseSourceReceipt.executionStage.status, "sealed");
    assert.equal(
      releaseSourceReceipt.executionStage.stageIdentity.realpath,
      result.plan.cwd,
    );
    assert.deepEqual(
      releaseSourceReceipt.executionStage.policy,
      independentExecutionStagePolicy,
    );
    assert.equal(
      releaseSourceReceipt.executionStage.policyHash,
      hashCanonicalFixture(independentExecutionStagePolicy),
    );
    assert.deepEqual(
      releaseSourceReceipt.executionStage.sourceRows,
      releaseSourceReceipt.before.entries,
    );
    assert.equal(
      releaseSourceReceipt.executionStage.sourceAggregateSha256,
      releaseSourceReceipt.before.sourceAggregateSha256,
    );
    assert.equal(
      releaseSourceReceipt.executionStage.dependencyAggregateSha256,
      releaseSourceReceipt.dependencyBefore.aggregateSha256,
    );
    const executionStagePayload = structuredClone(
      releaseSourceReceipt.executionStage,
    );
    delete executionStagePayload.manifestHash;
    assert.equal(
      releaseSourceReceipt.executionStage.manifestHash,
      hashCanonicalFixture(executionStagePayload),
    );
    assert.deepEqual(
...
test("runner rejects sealed execution-stage source drift before accepting evidence", () => {
  const workspace = freshFixtureWorkspace("hk-viz-stage-drift-runner-");
  const monitorHarness = buildRunnerMonitorHarness();
  let spawnCount = 0;
  try {
    prepareRunnableFixtureWorkspace(workspace);
    assert.throws(
      () =>
        runHkVisualizationReleaseGate({
          ...
          cwd: workspace,
          environment: {},
          now: new Date("2026-08-09T12:34:57.000Z"),
          sampleActiveProfiles: () => completeProfileProcessSample(),
          ...fixtureReleaseEvidenceSeams,
          ...monitorHarness,
          materializeExecutionStage(options) {
            const receipt =
              hkVisualizationReleaseGateModule
                .materializeHkVisualizationExecutionStage(options);
            const packagePath = join(options.executionWorkspace, "package.json");
            chmodSync(packagePath, 0o644);
            writeFileSync(packagePath, '{"stage":"mutated"}\n');
      ...
      /execution-stage.*(drift|differs)|stage.*(drift|differs)/i,
    );
...
test("runner rejects source mutation during materialization before spawn", () => {
  const workspace = freshFixtureWorkspace("hk-viz-pre-stage-drift-runner-");
  const monitorHarness = buildRunnerMonitorHarness();
  let spawnCount = 0;
  try {
    prepareRunnableFixtureWorkspace(workspace);
    assert.throws(
      () =>
        runHkVisualizationReleaseGate({
          ...
          cwd: workspace,
          environment: {},
          now: new Date("2026-08-09T12:34:58.000Z"),
          sampleActiveProfiles: () => completeProfileProcessSample(),
          ...fixtureReleaseEvidenceSeams,
          ...monitorHarness,
          materializeExecutionStage(options) {
            writeFileSync(
              join(options.sourceWorkspace, "package.json"),
      ...
      /execution stage source size\/SHA drifted|source execution monitor|stable source file/i,
    );
    assert.equal(spawnCount, 0);
  } finally {
    rmSync(workspace, { force: true, recursive: true });
