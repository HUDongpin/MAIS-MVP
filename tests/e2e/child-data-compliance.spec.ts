...
test.describe("public legal documents", () => {
  ...
  test("the privacy policy renders in English, Traditional and Simplified Chinese", async ({ page }) => {
    ...
    await expect(page.getByRole("note")).toContainText("Reviewed by legal professionals.");
    ...
    await expect(page.getByRole("note")).toContainText("經法律專業人員審定。");
    ...
    await expect(page.getByRole("note")).toContainText("经过法律专业人员审定。");
  });
...
test.describe("parental consent gate", () => {
  ...
  test("a public registrant cannot self-declare school permission", async ({ request }) => {
    const response = await request.post("/api/auth/register", {
      data: {
        role: "student",
        name: "School Claim Child",
        username: uniqueUsername("e2e-school-claim"),
        password: "e2ePassword123",
        grade: "P4",
        curriculumTrack: "US_CA_MATH",
        parentalConsent: { ...consent, relationship: "school" }
      }
    });
    expect(response.status()).toBe(400);
    expect((await response.json()).reason).toBe("relationship-invalid");
  });
});
