...
test("current source derives its counts and hashes without using them as planner inputs", async (t) => {
  ...
  assert.equal(summary.receiptCount, 1_033_884);
  assert.equal(summary.cropCount, 1_052_136);
  assert.equal(summary.groupCount, 4_512);
  assert.equal(reviewed.executionGroups.length, 4_700);
  assert.equal(reviewed.sourceIdentity.evidenceRecordCount, 1_131_863);
  assert.equal(reviewed.sourceIdentity.groupCount, 4_700);
  ...
  assert.deepEqual(reviewed.sourceIdentity.projects, [
    {
      cropCount: 526_068,
      evidenceRecordCount: 572_933,
      groupCount: 2_444,
      projectName: "desktop-chrome",
      receiptCount: 516_942
    },
    {
      cropCount: 526_068,
      evidenceRecordCount: 558_930,
      groupCount: 2_256,
      projectName: "mobile-chrome",
      receiptCount: 516_942
    }
