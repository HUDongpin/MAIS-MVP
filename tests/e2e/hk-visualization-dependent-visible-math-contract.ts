...
function expectedComputedGeometry(
  ...
  return canonicalComputedGeometry(
    ...
    keys.map((name) => {
      ...
      if (name === "d" || value === "auto") return [name, value];
      const numericValue = canonicalNumber(
        `expected ${tagName} computed geometry.${name}`,
        value,
      );
      // Chromium serializes SVG CSSOM geometry lengths to six significant
      // digits even when the presentation attribute retains more precision.
      // Keep the attribute projection exact at 1e-9, but model the separately
      // observable computed-style value at the browser's serialization
      // boundary so the two independent evidence layers are not conflated.
      return [name, Number(numericValue.toPrecision(6)).toString()];
    }),
    tagName,
  );
}
...
const CHROME_BLINK_MAC_SYSTEM_FONT_ALIAS =
...
function canonicalComputedRender(label: string, value: unknown) {
  ...
  const fontFamilyText = text("fontFamily").replace(/\s*,\s*/gu, ",");
  return Object.freeze({
    ...
    fontFamily: fontFamilyText === CHROME_BLINK_MAC_SYSTEM_FONT_ALIAS
      ? EXPECTED_VISIBLE_MATH_FONT_FAMILY
      : fontFamilyText,
...
function canonicalWrapperTopology(label: string, value: unknown) {
  ...
  return Object.freeze(value.map((candidate, index) => {
    ...
    const transitionDuration = canonicalZeroTimeList(
      `${entryLabel}.transitionDuration`,
      candidate.transitionDuration,
    );
    ...
    if (transitionDuration !== "0s") {
      throw new Error(`${entryLabel}.transitionDuration must be one exact no-motion duration.`);
    }
    return Object.freeze({
...
function canonicalZeroTimeList(label: string, value: unknown) {
  ...
  if (
    !text ||
    entries.some((entry) => entry !== "0s" && entry !== "1e-06s")
  ) {
    throw new Error(`${label} must remain an exact zero-duration list.`);
  }
  // Chrome serializes the app's prefers-reduced-motion 0.001ms rule as
  // 1e-06s. Treat that exact accessibility sentinel as no motion while still
  // rejecting every longer duration and preserving list cardinality.
  return entries.map(() => "0s").join(", ");
}
...
function canonicalOwnerAttributeContract(
  ...
  if (primary) {
    if (
      attributes.get("class") !== "block h-auto min-h-[360px] w-full min-w-[640px] overflow-visible" ||
    ...
    if (
      attributes.get("class") !== "aspect-[8/5] h-auto w-full min-w-[640px] overflow-visible" ||
...
function canonicalDocumentAncestorAudit(label: string, value: unknown) {
  ...
  value.forEach((candidate, index) => {
    ...
    const knownVisualizationCardClip =
      ...
      tagName === "section" &&
      structuralClass ===
        "overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-xl shadow-slate-900/5 dark:border-white/10 dark:bg-slate-900 dark:shadow-none sm:p-6";
...
export function projectHkVisualizationDependentVisibleMathRawProjection(
  ...
  const scaleHash = createHash("sha256").update(JSON.stringify({
    ...
    ].map(([layout, rendered], index) =>
      Math.abs(layout - rendered) <= 0.5
        ? 1
        : canonicalNumber(
            `normalized layout agreement[${index}]`,
            layout / rendered,
          )
    ),
    preserveAspectRatio,
  ...
  const expectedAncestors = canonicalDocumentAncestorAudit(
    ...
    [
      ...
      },
      {
        ...safeEffects,
        attributes: [
          ["data-viz-lab-runtime-ready-probe", topicId],
...
function expectedNumberBondProjections(
  ...
  return Object.freeze([
    ...
    connector("known-connector", "M260 238 L214 276"),
    connector("missing-connector", "M380 238 L426 276"),
  ]);
}
