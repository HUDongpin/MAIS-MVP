...
test("capacity execution-group ownership splits heavy benches and binds the exact 102-artifact source union", async () => {
  ...
  assert.equal(ownership.groupCount, 4_650);
  assert.equal(ownership.evidenceRecordCount, 1_127_995);
  assert.equal(ownership.receiptCount, 1_031_016);
  assert.equal(ownership.cropCount, 1_049_268);
  ...
  assert.equal(allSlices.reduce((sum, slice) => sum + slice.groupKeys.length, 0), 4_650);
  assert.equal(new Set(allSlices.flatMap((slice) => slice.groupKeys)).size, 4_650);
  assert.equal(allSlices.reduce((sum, slice) => sum + slice.expectedRecordCount, 0), 1_127_995);
  assert.equal(allSlices.reduce((sum, slice) => sum + slice.receiptCount, 0), 1_031_016);
  assert.equal(allSlices.reduce((sum, slice) => sum + slice.cropCount, 0), 1_049_268);
