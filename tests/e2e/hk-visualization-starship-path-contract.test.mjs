...
test("builds one exact absolute Starship-only runtime path manifest", () => {
  ...
  assert.deepEqual(
    manifest.volumeIdentity.sourceWorkspace,
    manifest.volumeIdentity.workspace,
  );
  assert.deepEqual(
    manifest.volumeIdentity.executionWorkspace,
    manifest.volumeIdentity.workspace,
  );
  assert.deepEqual(
    manifest.volumeIdentity.artifactWorkspace,
    manifest.volumeIdentity.workspace,
  );
  assert.equal(
...
test("builds an isolated execution manifest with external artifacts and canonical .next output", () => {
  const fixtureRoot = mkdtempSync(
    "/Volumes/Starship/MAIS-MVP/.tmp/hk-viz-isolated-paths-",
  );
  const executionWorkspace = join(fixtureRoot, "execution");
  const artifactWorkspace = join(fixtureRoot, "artifacts");
  mkdirSync(executionWorkspace);
  mkdirSync(artifactWorkspace);
  try {
    const manifest = buildHkVisualizationStarshipPathManifest({
      artifactWorkspace,
      runId: "hk-viz-isolated-path-contract",
      sourceWorkspace: workspace,
      workspace: executionWorkspace,
    });

    assert.deepEqual(validateHkVisualizationStarshipPathManifest(manifest), []);
    assert.equal(manifest.sourceWorkspace, workspace);
    assert.equal(manifest.executionWorkspace, executionWorkspace);
    assert.equal(manifest.workspace, executionWorkspace);
    assert.equal(manifest.artifactWorkspace, artifactWorkspace);
    assert.equal(manifest.nextDistDir, join(executionWorkspace, ".next"));
    assert.ok(manifest.artifactRoot.startsWith(`${artifactWorkspace}/`));
    assert.ok(manifest.nextTsconfigPath.startsWith(`${manifest.artifactRoot}/`));
    assert.equal(
      manifest.nextTsconfigPath.startsWith(`${executionWorkspace}/`),
      false,
    );
    for (const artifactPath of [
      ...
      assert.equal(artifactPath.startsWith(`${executionWorkspace}/`), false);
    }
    assert.deepEqual(Object.keys(manifest.volumeIdentity).sort(), [
      "artifactWorkspace",
      "executionWorkspace",
    ...
    const config = buildHkVisualizationE2eTsconfig({
      ...
      tsconfigPath: manifest.nextTsconfigPath,
      workspace: executionWorkspace,
    });
    assert.equal(
      config.extends,
      relative(dirname(manifest.nextTsconfigPath), join(executionWorkspace, "tsconfig.json")),
    );
    assert.ok(
      config.include.includes(
        relative(dirname(manifest.nextTsconfigPath), join(executionWorkspace, "next-env.d.ts")),
      ),
    );
    assert.ok(
      config.include.includes(
        `${relative(dirname(manifest.nextTsconfigPath), executionWorkspace)}/**/*.ts`,
      ),
    );
  } finally {
...
test("isolated execution manifests reject overlap, nesting, relative, foreign, and symlink workspaces", async (t) => {
  const fixtureRoot = mkdtempSync(
    "/Volumes/Starship/MAIS-MVP/.tmp/hk-viz-isolated-path-negatives-",
  );
  const executionWorkspace = join(fixtureRoot, "execution");
  const artifactWorkspace = join(fixtureRoot, "artifacts");
  const nestedArtifactWorkspace = join(executionWorkspace, "artifacts");
  const physicalAliasTarget = join(fixtureRoot, "alias-target");
  const alias = join(fixtureRoot, "alias");
  for (const directory of [
    executionWorkspace,
    artifactWorkspace,
    nestedArtifactWorkspace,
    physicalAliasTarget,
  ]) {
    mkdirSync(directory, { recursive: true });
  }
  symlinkSync(physicalAliasTarget, alias);
  const cases = [
    ["equal", executionWorkspace, executionWorkspace],
    ["nested", executionWorkspace, nestedArtifactWorkspace],
    ["relative", "relative-execution", artifactWorkspace],
    ["foreign", "/tmp/hk-viz-execution", artifactWorkspace],
    ["symlink", alias, artifactWorkspace],
  ];
  try {
    for (const [name, candidateExecution, candidateArtifact] of cases) {
      await t.test(name, () => {
        assert.throws(
          () =>
            buildHkVisualizationStarshipPathManifest({
              artifactWorkspace: candidateArtifact,
              runId: `hk-viz-isolated-negative-${name}`,
              sourceWorkspace: workspace,
              workspace: candidateExecution,
            }),
          /overlap|nest|absolute|Starship|symlink|distinct/i,
        );
      });
    }
  } finally {
