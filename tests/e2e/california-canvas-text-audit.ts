...
function californiaCanvasTextAuditInit() {
  ...
  function nativeImageData(
    ...
    // The graphics runtime is installed after this text recorder and treats
    // product-facing getImageData calls as unsupported authored behavior.
    // Text contrast needs bounded QA-only readback, so retain and invoke the
    // pre-runtime native method instead of contaminating product evidence.
    return Reflect.apply(nativeGetImageData, context, [sx, sy, sw, sh]) as ImageData;
  }
  ...
  function maskFor(record: TextRecord): MaskResult {
    ...
    function replay(options: { applyClips: boolean; composite: GlobalCompositeOperation }) {
      ...
      const data = nativeImageData(context, 0, 0, width, height).data;
      let maximumAlpha = 0;
  ...
  function capturePrePaintContrast(
    ...
    try {
      imageData = nativeImageData(context, minX, minY, width, height);
    } catch (error) {
      return {
        bounds: null,
        destinationPixels: [],
  ...
  function terminalTextContrastInput(
    ...
    if (bounds) {
      try {
        finalImageData = nativeImageData(
          state.context,
          bounds.x,
          bounds.y,
          bounds.width,
