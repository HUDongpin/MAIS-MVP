...
export const CALIFORNIA_CANVAS_GRAPHICS_CONTRAST_ACK_VERSION = 4;
export const CALIFORNIA_CANVAS_GRAPHICS_FINAL_COMPOSITOR_VERSION = 3;
...
export function verifyCaliforniaCanvasGraphicsFinalCompositorProof(
  ...
  if (!integralPositiveSize(proof.backingSize) || !integralPositiveSize(proof.cssSize) ||
      !integralRect(proof.clip) || !integralRect(proof.canvasPageRect) ||
      proof.clip.x !== proof.canvasPageRect.x || proof.clip.y !== proof.canvasPageRect.y ||
      proof.clip.width !== proof.canvasPageRect.width ||
      proof.clip.height !== proof.canvasPageRect.height ||
      proof.clip.width !== proof.cssSize.width || proof.clip.height !== proof.cssSize.height ||
      proof.clip.width * proof.clip.height >
        CALIFORNIA_CANVAS_GRAPHICS_FINAL_COMPOSITOR_MAX_PIXELS ||
      !Number.isFinite(proof.deviceScaleFactor) || proof.deviceScaleFactor <= 0) {
    issues.push("final-compositor-integral-geometry-invalid");
  }
...
export function verifyCaliforniaCanvasGraphicsStateEvidence(
  ...
  for (const canvas of evidence.canvases) {
    ...
    if (paperLayers.length < 2 || paperSurfaces.length !== 1 ||
        ...
        canvas.paperProof.compositingLayers.some((layer) =>
...
function californiaCanvasGraphicsRuntimeInit(config: CaliforniaCanvasGraphicsRuntimeConfig) {
  ...
  function advancePaintEnvironment(kind: PaintEnvironmentKind) {
  ...
  function trackPaintEnvironmentEvent(
    ...
    target?.addEventListener(eventName, () => advancePaintEnvironment(kind), {
      capture: true,
      passive: true
    });
  }
  ...
  function patchContextMethod(
    ...
    Object.defineProperty(contextPrototype, name, {
      ...
      value: function(this: CanvasRenderingContext2D, ...args: unknown[]) {
        if (this.canvas.isConnected) advancePaintEnvironment("state");
  ...
  function paperProof(canvas: HTMLCanvasElement) {
    ...
    while (currentLayer) {
      ...
      if (layer.opacity !== 1 || layer.filter !== "none" || layer.backdropFilter !== "none" ||
  ...
  async function capturePaintEnvironmentSnapshot(
    ...
    if (fontPayload.faces.length > 0) {
      throwPaintEnvironmentDrift(
        expectedEpoch,
        kindCounts,
        `${stage}-custom-font-state-not-atomically-capturable`,
        "font"
      );
    }
  ...
  function exactIntegralCompositorGeometry(
    registration: ContrastCanvasRegistration
  ) {
    const { canvas, canvasEvidence } = registration;
    const rect = canvas.getBoundingClientRect();
    const pageX = rect.x + scrollX;
    const pageY = rect.y + scrollY;
    const values = [pageX, pageY, rect.width, rect.height];
    if (values.some((value) => !Number.isFinite(value) ||
        Math.abs(value - Math.round(value)) > 1e-7) || pageX < 0 || pageY < 0 ||
        rect.width < 1 || rect.height < 1 || rect.width * rect.height > 4_194_304) {
      throw new Error("california-canvas-final-compositor-integral-geometry-required");
    }
    ...
    if (width !== canvasEvidence.cssWidth || height !== canvasEvidence.cssHeight) {
    ...
    const documentHeight = Math.max(document.documentElement.clientHeight,
      document.documentElement.scrollHeight);
    if (pageX + width > documentWidth || pageY + height > documentHeight) {
  ...
  async function prepareReferenceRaster(
    registration: ContrastCanvasRegistration,
    clip: CaliforniaCanvasGraphicsFinalCompositorRect
  ) {
    const source = document.createElement("canvas");
    source.width = registration.terminalPixels.backingWidth;
    source.height = registration.terminalPixels.backingHeight;
    const sourceContext = source.getContext("2d");
    if (!sourceContext) throw new Error("california-canvas-final-compositor-source-context-missing");
    sourceContext.putImageData(new ImageData(
      new Uint8ClampedArray(registration.terminalPixels.pixels),
      source.width,
      source.height
    ), 0, 0);
    const reference = document.createElement("canvas");
    reference.width = clip.width;
    reference.height = clip.height;
    const context = reference.getContext("2d", { alpha: false });
    if (!context) throw new Error("california-canvas-final-compositor-reference-context-missing");
    ...
    const referencePixels = new Uint8ClampedArray(
      context.getImageData(0, 0, reference.width, reference.height).data
    );
    const pngBytes = await canvasPngBytes(reference);
    if (pngBytes.length < 1 || pngBytes.length > 16 * 1024 * 1024) {
      throw new Error("california-canvas-final-compositor-reference-png-byte-cap");
    }
    return {
      pngBase64: browserBytesToBase64(pngBytes),
      raster: {
        height: reference.height,
        pngByteLength: pngBytes.length,
        pngSha256: await browserSha256(pngBytes),
        rgbaSha256: await browserSha256(new Uint8Array(
          referencePixels.buffer,
          referencePixels.byteOffset,
          referencePixels.byteLength
        )),
    ...
    async prepareFinalCompositor(root, request) {
      ...
      try {
        ...
        const prepared = await Promise.all(ordered.map(async (registration) => {
          const clip = exactIntegralCompositorGeometry(registration);
          ...
          const reference = await prepareReferenceRaster(registration, clip);
          const postIssues = contrastConsumptionCurrentIssues(consumption, true);
          const layoutFenceAfterReference = compositorLayoutFence(root, registration.canvas);
          const paintFenceAfterReference = await compositorPaintFence(root, registration.canvas);
          if (postIssues.length > 0 || layoutFenceAfterReference !== layoutFence ||
              paintFenceAfterReference !== paintFence) {
            throw new Error(
              `california-canvas-graphics-final-compositor-reference-fence-drift:${postIssues.join(",")}`
            );
          }
          const plan: CaliforniaCanvasGraphicsFinalCompositorCapturePlan["canvases"][number] = {
            ...
            referencePngBase64: reference.pngBase64,
          ...
          return {
            ...
            paintFence,
            plan,
            referencePixels: reference.referencePixels,
            registration
          };
        }));
        ...
        if (finalIssues.length > 0 || evidence.capturedUrl !== location.href ||
          ...
          throw new Error(
            ...
            `${finalIssues.join(",")}:mutations=${mutations.length}:` +
    ...
    async takeContrastConsumeAck(root, request) {
      ...
      try {
        ...
        const finalProofs = await Promise.all(prepared.canvases.map(async (entry) => {
          ...
          if (decoded.width !== entry.plan.clip.width || decoded.height !== entry.plan.clip.height) {
          ...
          const comparison = browserExactComparison(
            entry.referencePixels,
            decoded.pixels,
            decoded.width,
            decoded.height
          );
          if (browserStableSerialize(compositor) !== browserStableSerialize(result.compositor) ||
          ...
          const {
            referencePngBase64: _referencePngBase64,
            ...baseProof
          } = entry.plan;
        ...
        if (postCaptureIssues.length > 0 || evidence.capturedUrl !== location.href ||
          ...
          throw new Error(
            `california-canvas-final-compositor-post-capture-fence-drift:${postCaptureIssues.join(",")}`
          );
        }
 ...
 * page-level crops. The browser retains the terminal reference only until the
 * matching screenshots are independently decoded/compared in Node and then
 * re-decoded/re-compared by the browser before it seals the ACK.
 */
export async function consumeCaliforniaCanvasGraphicsContrastAckedStateEvidence(options: {
  ...
  for (const [canvasIndex, canvasPlan] of capturePlan.canvases.entries()) {
    if (canvasPlan.referencePngBase64.length >
        Math.ceil(CALIFORNIA_CANVAS_GRAPHICS_FINAL_COMPOSITOR_MAX_PNG_BYTES * 4 / 3) + 8) {
      throw new Error("california-canvas-final-compositor-reference-base64-byte-cap");
    }
    const referencePng = Buffer.from(canvasPlan.referencePngBase64, "base64");
    if (referencePng.length !== canvasPlan.reference.pngByteLength ||
        sha256(referencePng) !== canvasPlan.reference.pngSha256) {
      throw new Error("california-canvas-final-compositor-reference-png-forged");
    }
    const referenceDecoded = await sharp(referencePng).ensureAlpha().raw().toBuffer({
      resolveWithObject: true
    });
    if (referenceDecoded.info.channels !== 4 ||
        referenceDecoded.info.width !== canvasPlan.reference.width ||
        referenceDecoded.info.height !== canvasPlan.reference.height ||
        sha256(referenceDecoded.data) !== canvasPlan.reference.rgbaSha256) {
      throw new Error("california-canvas-final-compositor-reference-rgba-forged");
    }
    await options.root.evaluate((element, request) => {
      ...
      receiptId: options.evidence.receiptId,
      stage: `before-screenshot-${canvasIndex}`
    });
    const compositorPng = await options.root.page().screenshot({
    ...
    await options.root.evaluate((element, request) => {
      ...
      captureToken: capturePlan.captureToken,
      receiptId: options.evidence.receiptId,
      stage: `after-screenshot-${canvasIndex}`
    });
    if (compositorPng.length < 1 || compositorPng.length >
        CALIFORNIA_CANVAS_GRAPHICS_FINAL_COMPOSITOR_MAX_PNG_BYTES) {
      throw new Error("california-canvas-final-compositor-png-byte-cap");
    }
    ...
    if (compositorDecoded.info.channels !== 4 ||
        compositorDecoded.info.width !== canvasPlan.clip.width ||
        compositorDecoded.info.height !== canvasPlan.clip.height) {
      throw new Error("california-canvas-final-compositor-screenshot-size-mismatch");
    }
    const comparison = compareCaliforniaCanvasGraphicsFinalCompositorRgba({
