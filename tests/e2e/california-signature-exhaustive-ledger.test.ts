...
import {
  ...
  missingCaliforniaSignatureDurabilityProvider,
...
test("browser discovery refuses to start without the analytics durability provider", async () => {
  await assert.rejects(() => missingCaliforniaSignatureDurabilityProvider.assertReady(),
    /durability provider not installed/);
});
...
test("synthetic source-declared inactive empty select stays present and disabled", async () => {
  const selectSite = {
    ...site,
    endpointExpressions: [],
    endpointTargets: [],
    expectedMultiplicity: 0,
    kind: "select" as const,
    lessonChoiceMultiplicity: [],
    multiplicityIsExact: true,
    numericMidpoint: null,
    siteKey: "select:synthetic-inactive-canary",
    sourceConditionKeys: ["synthetic-inactive-select"],
    sourceText: "<select disabled />"
  };
  const selectBench = {
    ...bench,
    controlSites: [selectSite],
...
test("acceptance is fail-closed until the discovery digest is reviewed and pinned", () => {
  assert.throws(() => assertReviewedCaliforniaSignatureExhaustiveSnapshot([record()]),
    /runtime snapshot is UNREVIEWED.*discovery candidate=/);
});
...
test("runtime ledger aggregation remains red when a real evidence directory is requested before pinning", async () => {
  ...
  if (!configured) {
    assert.throws(() => assertReviewedCaliforniaSignatureExhaustiveSnapshot([record()]), /UNREVIEWED/);
    return;
  }
