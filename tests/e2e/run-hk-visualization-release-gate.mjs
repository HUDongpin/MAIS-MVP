...
export const HK_VISUALIZATION_RELEASE_EXPECTED_TEST_COUNTS = Object.freeze({
  ...
  "tests/e2e/hk-visualization-interaction-microfixtures.spec.ts": 17,
...
export const HK_VISUALIZATION_RELEASE_SOURCE_RECEIPT_SCHEMA =
  "hk-viz-release-source-receipt-v1";
...
if (
  ...
  throw new Error(
    "HK Visualization deadline topology does not cover the exact 371 release titles.",
  );
}
...
export function validateHkVisualizationReleaseSourceSnapshotWorkspace(
  ...
  const workspaceInput = expectedWorkspace ?? expectedPathManifest?.workspace;
  ...
  if (
    ...
    resolve(String(expectedPathManifest.workspace ?? "")) !== normalizedWorkspace
  ) {
    throw new Error(
...
export function validateHkVisualizationStaticDeadlineSourceReceipt(
  ...
  const boundWorkspace = expectedWorkspace ?? expectedPathManifest?.workspace ??
...
function validateReleaseSourceReceipt(
  ...
  try {
    ...
    validateHkVisualizationReleaseSourceSnapshotWorkspace(receipt.before, {
      ...
      expectedWorkspace: manifest.workspace,
    });
...
export function buildHkVisualizationReleasePlan({
  ...
  const childEnvironment = {
    ...
    HK_VISUALIZATION_CANONICAL_RELEASE: "1",
  ...
  return {
    ...
    toolchainInputs: buildHkVisualizationReleaseToolchainInputs(workspace),
...
export function runHkVisualizationReleaseGate({
  ...
  const plan = buildHkVisualizationReleasePlan({ cwd, environment, now });
  if (parsedArgs.dryRun) return { dryRun: true, plan: publicPlan(plan) };
  ...
  if (!existsSync(plan.command)) {
    throw new Error(
      `Playwright executable is missing at ${plan.command}. Run npm install in the worktree first.`,
    );
  }
  ...
  const nextTsconfigBytes = buildHkVisualizationCanonicalE2eTsconfigBytes({
    workspace: plan.cwd,
  ...
  try {
    ...
    try {
      const restoredNextEnv = restoreRegularFileAtomically(
        nextEnvSnapshot,
        "HK Visualization canonical next-env.d.ts",
      );
      nextEnvAfterSha256 = sha256Text(restoredNextEnv.bytes);
    } catch (error) {
      executionError = combineReleaseGateErrors(
        executionError,
        error,
