...
const options = {
  budget: readNumberArg(args.budget, 350),
  dryRun: Boolean(args["dry-run"]),
  intervalMs: readNumberArg(args["interval-ms"], 1400),
  label: String(args.label ?? `${dateStamp}-simpletex-rec-mode-fractions-ab`),
  maxCalls: readNumberArg(args["max-calls"], 300),
  rawPath: String(args.raw ?? `/tmp/mais-simpletex-rec-mode-fractions-${dateStamp}.json`),
  reportPath: String(args.report ?? path.join(cwd, "coordination", "reports", `${dateStamp}-simpletex-rec-mode-fractions-ab.md`)),
  routeSmokeRaw: args["route-smoke-raw"] ? String(args["route-smoke-raw"]) : "",
  saveImages: args["save-images"] !== "false",
  timeoutMs: readNumberArg(args["timeout-ms"], 45000)
};

const modes = ["formula", "auto", "document"];
const imageRoot = path.join("/tmp", `mais-simpletex-rec-mode-fractions-images-${dateStamp}`);
...
try {
  ...
  const routeSmoke = readRouteSmokeSummary(options.routeSmokeRaw);
  const preflight = {
    env: envPreflight,
    routeSmoke,
    matrixPlan: {
  ...
  await finishRun({
    ...
    preflight: {
      ...
      routeSmoke: readRouteSmokeSummary(options.routeSmokeRaw),
      matrixPlan: {}
    },
    results: [],
    stoppedEarly: true,
...
function readEnvPreflight() {
  ...
  const apiUrl = parsed.SIMPLETEX_API_URL?.trim() || "https://server.simpletex.cn/api/simpletex_ocr";
...
function readRouteSmokeSummary(rawPath) {
  if (!rawPath || !fs.existsSync(rawPath)) {
    return { provided: Boolean(rawPath), available: false };
  }
  try {
    ...
    return {
      ...
      http200: counts.http200 ?? null,
      simpletexRouteProvider: counts.simpletexRouteProvider ?? null,
      stoppedEarly: Boolean(parsed.stoppedEarly),
      stopReason: parsed.stopReason || ""
    };
  } catch {
    return { provided: true, available: false, rawPath, error: "unreadable route smoke raw JSON" };
  }
}
...
function buildDirectResult({ elapsedMs, image, json, response, testCase }) {
  ...
  return {
    ...
    failureClass,
    responseCode: typeof json?.code === "string" ? json.code : typeof json?.err_code === "string" ? json.err_code : "",
    responseMsg: typeof json?.msg === "string" ? json.msg.slice(0, 160) : typeof json?.message === "string" ? json.message.slice(0, 160) : "",
    evidenceImage: "",
    render: image.render
  };
}
...
function classifyResult({ acceptedLike, candidate, currentMatch, httpStatus, json, proposedMatch, testCase }) {
  ...
  if (httpStatus === 402) return "billing/resource";
...
function renderOptions(testCase) {
  const seed = hashSeed(`${testCase.id}:${testCase.variant}`);
...
function buildMatrix() {
  ...
  const directCases = baseCases.flatMap((testCase) => modes.map((recMode) => ({
    ...testCase,
    recMode
  })));
...
function buildCounts(results) {
  const modeStats = Object.fromEntries(modes.map((mode) => {
    const modeRows = results.filter((result) => result.recMode === mode);
    const existingRows = modeRows.filter((result) => result.source === "existing-saturation");
    const visualRows = modeRows.filter((result) => result.source === "visual-supplement");
    return [mode, buildStatsForRows(modeRows, { existingRows, visualRows })];
  }));
...
function classifyConclusion(counts, stoppedEarly, stopReason) {
  ...
  const formula = counts.modeStats.formula;
  const auto = counts.modeStats.auto;
  const document = counts.modeStats.document;
  const formulaRate = ratio(formula.currentActionable, formula.calls);
  const autoRate = ratio(auto.currentActionable, auto.calls);
  const documentRate = ratio(document.currentActionable, document.calls);
  const bestAlternative = Math.max(autoRate, documentRate);
  const bestAlternativeRisk = autoRate >= documentRate ? auto.wrongAcceptedRiskCurrent : document.wrongAcceptedRiskCurrent;
  if (bestAlternative - formulaRate >= 0.1 && bestAlternativeRisk <= formula.wrongAcceptedRiskCurrent) {
    return "Mode switch candidate";
  }
  if (formula.proposedActionable >= 0.85 * formula.calls || auto.proposedActionable >= 0.85 * auto.calls || document.proposedActionable >= 0.85 * document.calls) {
    return "Normalization/policy candidate";
  }
  return "Needs deeper OCR investigation";
}
...
async function writeReport({ conclusion, counts, preflight, results, stoppedEarly, stopReason }) {
  ...
  const lines = [
    `# SimpleTex Rec Mode Fractions/Percent A/B - ${dateStamp}`,
    "",
    `- Date: ${dateStamp}`,
    "- Session: S11",
    ...
    `| Route smoke raw | ${preflight.routeSmoke?.available ? `available: \`${preflight.routeSmoke.rawPath}\`` : "not available in this report"} |`,
    `| Route smoke simpletex provider | ${preflight.routeSmoke?.available ? `${preflight.routeSmoke.simpletexRouteProvider}/${preflight.routeSmoke.total}` : "n/a"} |`,
    "",
    "## Matrix",
    "",
    "| Area | Count |",
    ...
    `| Modes | ${(preflight.matrixPlan?.recModes ?? modes).join(", ")} |`,
    `| Planned direct calls | ${preflight.matrixPlan?.plannedDirectCalls ?? 0} |`,
    `| Executed direct calls | ${counts.total} |`,
    "",
    "## Mode Accuracy",
    "",
    "| Mode | Calls | HTTP 200 | Raw actionable | Current MAIS actionable | Proposed-normalized actionable | Wrong risk current | Wrong risk proposed | `%` escape | Star artifact | Spacing artifact | P95 latency |",
    "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ...modes.map((mode) => modeStatsRow(mode, counts.modeStats[mode])),
    "",
    "## Existing vs Visual Supplement",
    "",
    "| Mode | Existing current | Existing proposed | Visual current | Visual proposed |",
    "| --- | ---: | ---: | ---: | ---: |",
    ...modes.map((mode) => existingVisualRow(mode, counts.modeStats[mode])),
    "",
    "## Failure Classes",
    "",
    "| Mode | Failure counts |",
    "| --- | --- |",
    ...modes.map((mode) => `| ${mode} | ${formatFailureCounts(counts.modeStats[mode].failureCounts)} |`),
    "",
    "## Worst Cases",
    "",
    "| # | Mode | Input | Variant | Class | Confidence | Raw | Current normalized | Proposed normalized | Evidence |",
    "| ---: | --- | --- | --- | --- | ---: | --- | --- | --- | --- |",
    ...worstCases(results).map((result) => `| ${result.callIndex} | ${result.recMode} | \`${escapeMarkdownCell(result.input)}\` | ${result.variant} | ${result.failureClass || "none"} | ${formatNullable(result.confidence)} | \`${escapeMarkdownCell(result.raw)}\` | \`${escapeMarkdownCell(result.currentMatch.normalizedActual)}\` | \`${escapeMarkdownCell(result.proposedMatch.normalizedActual)}\` | \`${result.evidenceImage || ""}\` |`),
    "",
    "## Decision",
    "",
...
function chineseSummary(counts, conclusion, stoppedEarly, stopReason) {
  if (stoppedEarly) {
    return `本次 A/B 运行提前停止：${stopReason || conclusion}。已执行 ${counts.total} 次直连 SimpleTex 调用，未继续消耗额度。`;
  }
  const formula = counts.modeStats.formula;
  const auto = counts.modeStats.auto;
  const document = counts.modeStats.document;
  return `本次完成 ${counts.total} 次直连 SimpleTex 调用。当前 MAIS 归一化口径下，formula/auto/document actionable 分别为 ${formatRatio(formula.currentActionable, formula.calls)}、${formatRatio(auto.currentActionable, auto.calls)}、${formatRatio(document.currentActionable, document.calls)}。按 proposed normalization 模拟后分别为 ${formatRatio(formula.proposedActionable, formula.calls)}、${formatRatio(auto.proposedActionable, auto.calls)}、${formatRatio(document.proposedActionable, document.calls)}。结论：${conclusion}。`;
}

function englishSummary(counts, conclusion, stoppedEarly, stopReason) {
  if (stoppedEarly) {
    return `The A/B run stopped early: ${stopReason || conclusion}. It executed ${counts.total} direct SimpleTex calls and did not continue spending quota.`;
  }
  const formula = counts.modeStats.formula;
  const auto = counts.modeStats.auto;
  const document = counts.modeStats.document;
  return `Executed ${counts.total} direct SimpleTex calls. Under the current MAIS normalization, formula/auto/document actionable rates were ${formatRatio(formula.currentActionable, formula.calls)}, ${formatRatio(auto.currentActionable, auto.calls)}, and ${formatRatio(document.currentActionable, document.calls)}. With proposed normalization replay, they were ${formatRatio(formula.proposedActionable, formula.calls)}, ${formatRatio(auto.proposedActionable, auto.calls)}, and ${formatRatio(document.proposedActionable, document.calls)}. Conclusion: ${conclusion}.`;
}

function modeStatsRow(mode, stats) {
  return [
    `| ${mode}`,
    stats.calls,
    stats.http200,
...
function existingVisualRow(mode, stats) {
  return `| ${mode} | ${formatRatio(stats.existingCurrentActionable, 80)} | ${formatRatio(stats.existingProposedActionable, 80)} | ${formatRatio(stats.visualCurrentActionable, 12)} | ${formatRatio(stats.visualProposedActionable, 12)} |`;
}
...
function decisionLines(counts) {
  ...
  const autoRate = ratio(auto.currentActionable, auto.calls);
  const documentRate = ratio(document.currentActionable, document.calls);
  const bestAlternativeMode = autoRate >= documentRate ? "auto" : "document";
  const bestAlternativeRate = Math.max(autoRate, documentRate);
  const bestAlternative = counts.modeStats[bestAlternativeMode];

  if (bestAlternativeRate - formulaRate >= 0.1 && bestAlternative.wrongAcceptedRiskCurrent <= formula.wrongAcceptedRiskCurrent) {
    return [
      `- Recommend S12 investigate route-level mode strategy because \`${bestAlternativeMode}\` beats \`formula\` by at least 10 percentage points without higher current wrong-accepted risk.`,
      "- Still keep normalization work in scope because percent/star artifacts may remain across modes."
    ];
  }

  return [
    "- Do not switch MAIS route mode solely based on this run unless product owners accept the tradeoffs below.",
    "- Prioritize MAIS normalization/policy if `%` escape, star multiplication, or spacing artifacts dominate the remaining failures.",
    "- Consider mode splitting only if document/auto clearly improves mixed text while formula remains better for pure equations."
  ];
}
