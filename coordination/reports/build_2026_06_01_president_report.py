...
OUT = ROOT / "coordination/reports/2026-06-01-president-report.docx"
...
metadata_rows = [
    ...
    ["收件人", "Dr. Peter Hu"],
    ["报告日期", "2026-06-01"],
    ["报告窗口", "2026-05-31 08:00 至 2026-06-01 08:00 Asia/Hong_Kong"],
    ["报告会话", "S10 / Automation mais-mvp-9-am-president-report"],
    ["生成时间", NOW_HKT],
    ["证据范围", "AGENTS.md、automation memory、session logs、blockers、decisions、mtime scan、git status/log、fresh type-check/test:analytics"],
]

session_rows = [
    ["会话", "状态", "本窗口活动", "关注点 / 下一步"],
    ["S01", "No fresh log", "无 fresh Home/layout/navigation handoff。", "沿用上一报告 carryover；release regression 覆盖即可。"],
    ["S02", "No fresh log", "无 fresh Dashboard/progress log。", "Adaptive dashboard UI carryover 仍需 S11/S15 review。"],
    ...
    ["S10", "Fresh report-only", "2026-05-31 08:06-08:07 HKT 生成上一份 president report、builder 和 S10 handoff；本次 automation 生成本报告。", "未改 feature code；只写 coordination/reporting artifact 和 automation memory。"],
    ["S11", "No fresh log", "无 fresh QA/release log。", "仍建议作为下一优先级建立 release-readiness matrix。"],
    ["S12", "No fresh log", "无 fresh backend/API log。", "API regression 仍按 release gate 覆盖。"],
...
completed_rows = [
    ["领域", "本窗口完成内容", "影响"],
    ["Reporting", "S10 于 2026-05-31 08:06-08:07 HKT 生成 `2026-05-31-president-report.docx`、builder 和 S10 handoff。", "上一日状态已正式归档；本窗口没有新功能交付。"],
    ["Fresh report-owner checks", "本次 2026-06-01 automation 执行 `npm run type-check` 与 `npm run test:analytics`，均通过；analytics/adaptive tests 21/21 passed。", "说明当前 carryover 工作树仍可通过 TypeScript 与核心学习分析/自适应单元测试。"],
    ["Evidence scan", "mtime scan 仅发现 2026-05-31 S10 report artifacts；无 fresh blocker、decision、feature-session log 或 commit。", "支持“本窗口无 fresh assigned feature work”的结论。"],
]
...
risk_rows = [
    ...
    ["Generated/local outputs", "`.next/` 与 `.local/` 属生成/本地 artifact；不应进入 Git。", "保持 generated/local outputs out of source control。"],
...
test_rows = [
    ["Check", "Result", "Notes"],
    ["Fresh `npm run type-check`", "Pass", "2026-06-01 report-owner check；TypeScript strict check 通过。"],
    ["Fresh `npm run test:analytics`", "Pass 21/21", "Learning analytics + adaptive learning tests passed。"],
    ["Current-window S10 `npm run type-check`", "Pass", "记录于 2026-05-31 S10 log。"],
    ["Current-window S10 `npm run build`", "Pass", "记录于 2026-05-31 S10 log；Next.js generated 88 static pages。"],
    ["Current-window feature tests", "Not run", "本窗口无 fresh feature code；未重跑 question-bank/RAG/E2E。"],
    ...
    ["DOCX render-to-PNG visual QA", "Blocked", "`render_docx.py` 缺少 `pdf2image` dependency，且本机未找到 `soffice`/LibreOffice；本报告改用 zip、OOXML/text extraction、textutil 验证。"],
]

changed_rows = [
    ["Changed file / artifact", "mtime in reporting window", "Notes"],
    ["coordination/reports/build_2026_05_31_president_report.py", "2026-05-31 08:06 HKT", "上一份 report builder；S10-owned reporting artifact。"],
    ["coordination/reports/2026-05-31-president-report.docx", "2026-05-31 08:06 HKT", "上一份 president report deliverable。"],
    ["coordination/session-logs/2026-05-31-S10.md", "2026-05-31 08:07 HKT", "上一份 S10 handoff log。"],
]
...
def build_document_xml() -> str:
    ...
    parts.append(para("No assigned work in this reporting window. 本窗口从 2026-05-31 08:00 到 2026-06-01 08:00 HKT，除上一份 S10 president report artifact、builder 和 S10 handoff log 外，未发现 fresh assigned feature work、fresh blocker record、fresh decision record 或 commit。"))
    parts.append(para("项目健康度保持稳定：本次 automation 重新执行 `npm run type-check` 与 `npm run test:analytics` 均通过。主要工作重点不应继续扩大功能面，而应转向 release control：owner-scoped handoff/commit、S11 regression matrix、S18 内容 QA blockers、S19 credential hygiene 和部署包体积。"))

    parts.append(heading("English Executive Summary", 1))
    parts.append(para("No assigned work in this reporting window. From 2026-05-31 08:00 to 2026-06-01 08:00 HKT, the only current-window project artifacts were the prior S10 president report, its builder, and the S10 handoff log; no fresh feature-session handoff, blocker, decision record, or commit was found."))
    parts.append(para("Current health remains stable: fresh `npm run type-check` and `npm run test:analytics` both passed, with 21/21 analytics/adaptive tests passing. The next useful work is release control, not feature expansion: ownership-based handoffs or commits for the large carryover working tree, S11 regression coverage, S18 content-QA blockers, S19 credential hygiene, and deployment-size planning."))

    parts.append(heading("报告窗口摘要", 1))
    parts.append(para("报告窗口：2026-05-31 08:00 至 2026-06-01 08:00 Asia/Hong_Kong。读取范围包括 AGENTS.md、automation memory、coordination/session-logs、coordination/blockers、coordination/decisions、mtime changed-file scan、git status/log、package scripts，以及本次 fresh checks。"))
    ...
    parts.append(para("本窗口没有新的产品功能交付。最新可用项目状态仍沿用 2026-05-31 president report：上一报告判断项目没有退化，并把工作重心放在 release-control、S11 回归矩阵、S18 BNU/EASE 内容 QA blockers、S19 credential hygiene 和 Vercel/static asset size risk。"))
    parts.append(para("本次 fresh type-check 与 analytics/adaptive unit tests 继续通过，说明 carryover worktree 仍可编译，并且核心学习分析/自适应逻辑没有在当前环境暴露回归。下一阶段应先把各 owner session 的大 diff 拆分为可审查 handoff/commit，再用 S11 regression gate 决定是否进入 public release 或 deployment packaging。"))
    ...
    parts.append(para("本 DOCX 使用 standard_business_brief preset：US Letter、1 inch margins、Calibri / Microsoft YaHei、fixed DXA tables、simple business formatting。`render_docx.py` 因缺少 `pdf2image` dependency 无法启动，且本机未找到 `soffice`/LibreOffice；因此无法执行 render-to-PNG visual QA。已执行 zip、OOXML/text extraction、textutil extraction 验证。"))
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:dcmitype="http://purl.org/dc/dcmitype/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>MAIS-MVP 2026-06-01 President Report</dc:title>
