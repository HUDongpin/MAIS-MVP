# SimpleTex UAT Local Strict Smoke Test - Run 2

- Date: 2026-05-21
- Session: S19
- Runtime: local MAIS-MVP dev server at `http://127.0.0.1:3100`
- Scope: second 10-call real OCR smoke through MAIS-MVP `/api/handwriting-recognition`
- Result: **Functional / Quality Amber**

## Executive Summary

Run 2 reproduced the Run 1 result exactly at the summary-metric level. The local MAIS-MVP route reached SimpleTex through UAT authentication, and all 10 app-route requests returned HTTP 200 with `provider: "simpletex"`.

The test still did **not** meet the Strict Pass threshold because only 6/10 responses were auto-accepted above the app confidence threshold. The 4 remaining responses were low-confidence but actionable SimpleTex suggestions. This confirms local UAT/provider functionality is stable across two runs, while OCR confidence quality remains amber.
...
| --- | ---: | ---: | --- |
| HTTP 200 | 10/10 | 10/10 | No change |
| `provider: "simpletex"` | 10/10 | 10/10 | No change |
| `accepted: true` | 6/10 | 6/10 | No change |
| Accepted or actionable SimpleTex suggestion | 10/10 | 10/10 | No change |
| `provider: "none"` | 0 | 0 | No change |
| Auth failures | 0 | 0 | No change |
| Rate-limit failures | 0 | 0 | No change |
| Timeout failures | 0 | 0 | No change |
| Low-confidence results | 4 | 4 | No change |
| Overall classification | Functional / Quality Amber | Functional / Quality Amber | No change |
...
Raw non-secret result JSON: `/tmp/mais-simpletex-uat-strict-smoke-results-run-2.json`.
...
The repeated local evidence shows UAT authentication and the MAIS-MVP OCR route are functional. The remaining issue is confidence/quality tuning for some expressions, not local SimpleTex reachability.
