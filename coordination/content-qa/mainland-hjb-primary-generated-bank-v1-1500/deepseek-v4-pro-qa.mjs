  ...
  return [
    {
      ...
      content: [
        ...
        "Review Shanghai Education Press / HuJiaoBan primary P1-P6 candidate questions for possible future public-bank promotion.",
        "Check every item independently for mathematical correctness, answer/explanation alignment, multiple-choice ambiguity, missing conditions, grade/topic suitability for 沪教版小学数学, unsafe visual/source artifacts, wording clarity, accepted-answer coverage, and public-readiness risk.",
    ...
    {
      role: "user",
      content: `请用 json 审核以下 ${batch.length} 道沪教版小学候选题：\n${JSON.stringify(batch.map(compactQuestion))}`
    }
  ];
}
...
function writeSummary({ records, questions, config, args }) {
  ...
  const resultJson = {
    ...
    scope: "mainland-hjb-primary-v1-1500-candidate",
  ...
  const markdown = `# DeepSeek V4 Pro QA - Mainland HJB Primary V1 Candidate
