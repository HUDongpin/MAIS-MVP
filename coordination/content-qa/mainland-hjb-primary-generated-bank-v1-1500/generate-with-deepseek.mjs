...
const batchSize = Number(process.env.HJB_PRIMARY_GENERATION_BATCH_SIZE ?? 5);
...
function buildScopeCountsByCardId(curriculumCards) {
  ...
  for (const grade of grades) {
    ...
    if (!gradeCards.length) throw new Error(`Missing HJB primary curriculum safe cards for ${grade}`);
...
function buildPlan({ curriculumCards, assessmentPatternCards }) {
  ...
  for (const [cardId, count] of Object.entries(scopeCountsByCardId)) {
    ...
    if (!curriculumCard) throw new Error(`Missing HJB primary curriculum safe card ${cardId}`);
    ...
    if (!supportingAssessmentIds.length) {
      throw new Error(`Missing HJB primary assessment pattern evidence for ${cardId}`);
    }

    for (let localIndex = 0; localIndex < count; localIndex += 1) {
      const grade = curriculumCard.grade;
      const gradeIndex = gradeIndexes.get(grade) ?? 0;
      const id = `hjb-primary-ds-v1-${grade.toLowerCase()}-${String(gradeIndex + 1).padStart(3, "0")}`;
      rows.push({
        id,
        ...
        reviewNotes: "Generation target built from committed HJB primary safe-RAG metadata only."
      });
...
function userPrompt({ targets, curriculumCards, assessmentPatternCards }) {
  return JSON.stringify({
    task: `Generate exactly ${targets.length} original Shanghai Education Press primary math questions.`,
    outputContract: {
      root: "questions",
    ...
    safeHjbPrimaryAssessmentPatternEvidence: assessmentPatternCards.map(compactPatternCard)
  });
}
...
function normalizeQuestion(target, raw) {
  ...
  const normalized = {
    ...
    reviewNotes: "Generated offline with DeepSeek V4 Pro from MAIS HJB primary safe-RAG metadata only; candidate QA package only, not approved for public integration."
  };
...
function buildQaReport({ questions, plan, model, apiUrl, startedAt, finishedAt, curriculumCards, assessmentPatternCards }) {
  ...
  return `# Mainland HJB Primary Generated Bank V1 Candidate QA Report
...
- Generated ${questions.length} original Simplified Chinese questions for Mainland Shanghai Education Press / HuJiaoBan primary mathematics.
...
- RAG evidence source: ${curriculumCards.length} committed HJB primary textbook safe cards and ${assessmentPatternCards.length} committed HJB primary assessment-pattern cards.
...
- Required next step for candidate QA: run \`node coordination/content-qa/mainland-hjb-primary-generated-bank-v1-1500/audit-solvability.mjs\`.
`;
}
...
async function main() {
  ...
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 20) throw new Error(`Invalid HJB_PRIMARY_GENERATION_BATCH_SIZE: ${batchSize}`);

  const curriculumCards = loadTsExport(path.join(rootDir, "data/rag/mainlandHjbPrimary.ts"), "mainlandHjbPrimaryRagCards");
  const assessmentPatternCards = loadTsExport(path.join(rootDir, "data/rag/mainlandHjbPrimaryAssessmentPatterns.ts"), "mainlandHjbPrimaryAssessmentPatternCards");
  ...
  for (let index = 0; index < batches.length; index += 1) {
    ...
    const batchQuestions = await generateBatch({
      ...
      apiKey,
      model,
      existingPromptKeys: seenPromptKeys
    });
    questions.push(...batchQuestions);
    for (const question of batchQuestions) seenPromptKeys.add(normalizeIdentity(question.promptZhHans));
  }
