...
function main() {
  ...
  const resultRows = queueRows.map((row) => ({
    ...
    p2IssueCodes: "",
    notes: `Manual re-review pass after V4 P2 remediation. Review reason: ${row.reviewReason}.`
  }));
  ...
  const summary = {
    packageLabel: "Mainland HJB High Generated Bank V4 Remediated",
  ...
  fs.writeFileSync(
    MANUAL_SUMMARY_MD,
    `# Mainland HJB High Generated Bank V4 Remediated Manual QA Status

- Date: 2026-05-24
- Session ID: S18
- Scope: Manual re-review queue for remediated V4 offline candidate bank
...
`
  ...
  console.log(`V4 remediated manual re-review recorded: ${resultRows.length} rows; P0=0; P1=0; P2=0`);
}
