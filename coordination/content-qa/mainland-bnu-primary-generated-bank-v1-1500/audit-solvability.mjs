...
function auditRow(row, sets) {
  ...
  if (row.batch !== "bnu-primary-v1") issues.push({ code: "bad-batch", severity: "P1", detail: row.batch });
...
function candidateRows(rows) {
  return rows.map((row) => ({
    ...
    reviewNotes: "S18 BNU primary V1 automated source-distance/structure gate passed with 300-row grade sample queue generated; candidate QA package only, not approved for public integration."
  }));
}
...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues, approved }) {
  ...
  return [
    "# S18 Mainland BNU Primary Generated Bank V1 Candidate Solvability Audit",
    "",
    "- Date: 2026-05-26",
    "- Session ID: S18",
...
function writeReleaseDecision({ approved, manualQueue, inventoryIssues, auditRows }) {
  const issueRows = auditRows.filter((row) => row.qaStatus !== "pass");
  const text = approved
    ? `# S18 Promotability Decision - Mainland BNU Primary Generated Bank V1 Candidate
...
- Required next step: rewrite failing rows or regenerate affected batches, then rerun \`node coordination/content-qa/mainland-bnu-primary-generated-bank-v1-1500/audit-solvability.mjs\`.
`;
