...
const packageLabel = "Mainland HJB High Generated Bank V2";
const priorBankJsonl = path.join(__dirname, "../mainland-hjb-high-generated-bank-v1/questions.jsonl");
  ...
  for (const grade of grades) {
    ...
    for (const scope of scopes) {
      for (let localIndex = 0; localIndex < scope.count; localIndex += 1) {
        ...
        const id = `hjb-high-ds-v2-${grade.toLowerCase()}-${String(gradeIndex + 1).padStart(3, "0")}`;
        rows.push({
          id,
          grade,
          ...
          reviewNotes: "V2 generation target built from committed HJB safe-RAG metadata only."
        });
        gradeIndex += 1;
      }
    }
...
function systemPrompt() {
  return [
    ...
    "这是第二轮 V2 题包；避免复用第一轮题包的题面风格、数值组合、叙述顺序和模板语感。",
  ...
  return JSON.stringify({
    ...
    outputContract: {
      ...
      rules: [
        ...
        "This is V2; make prompts visibly different from an earlier V1 package while preserving the same curriculum target.",
...
function loadPriorPromptSet() {
  if (!fs.existsSync(priorBankJsonl)) return { checked: false, total: 0, prompts: new Set() };
...
function generateLocalQuestion(target, index) {
  ...
  return {
    ...target,
    promptZhHans: `二轮变式${String(index + 1).padStart(4, "0")}：${core.prompt}`,
    ...
    mathQaStatus: "pending-manual",
    terminologyQaStatus: "pending-manual",
    reviewNotes: "Generated as V2 offline candidate by deterministic MAIS safe-template fallback from committed HJB safe-RAG metadata after live DeepSeek transport failed; manual S18 sampling required before app integration."
  };
}
...
- No API key, request header, raw prompt transcript, source file path, OCR text, page image, page number, or source locator is written into these artifacts.
- Batch cache files contain parsed generated questions only.`
      : `- Live DeepSeek transport failed before any batch cache was written, so generated rows were produced by the local deterministic fallback.
- No API key, request header, raw prompt transcript, source file path, OCR text, page image, page number, or source locator is written into these artifacts.
- The DeepSeek generator remains in this package for a later provider-stable rerun.`;
  const crossBankLine = crossBankStats.checked
    ? `- Cross-v1 exact prompt duplicate check: checked ${crossBankStats.priorRows} prior rows; duplicates ${crossBankStats.duplicateCount}.`
    : "- Cross-v1 exact prompt duplicate check: not run because prior v1 questions.jsonl was not found in this workspace.";
...
- Package version: V2 additional offline candidate package; intended to sit alongside the previously generated V1 package.
