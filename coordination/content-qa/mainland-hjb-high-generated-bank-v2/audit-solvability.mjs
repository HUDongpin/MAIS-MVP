...
function buildInventoryIssues(rows, auditRows, crossBankStats) {
  ...
  if (crossBankStats.checked && crossBankStats.duplicateCount > 0) {
    issues.push(`Cross-v1 exact prompt duplicates: ${crossBankStats.duplicateCount}.`);
  }
...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues, crossBankStats }) {
  ...
  const recommendation =
    inventoryIssues.length === 0
      ? "Green candidate package: automated structure, source-distance wording, answer-shape, duplicate, and quota checks passed; S18 manual sampling remains required before app integration."
      : "Red: offline package is blocked until inventory or row-level issues are fixed.";

  const crossBankLine = crossBankStats.checked
    ? `Checked ${crossBankStats.priorRows} v1 rows; duplicate exact prompts ${crossBankStats.duplicateCount}.`
    : "Not run because prior v1 questions.jsonl was not found in this workspace.";
...
- Physical source directory: \`${path.basename(__dirname)}\`; business, QA, app, and batch metadata label this package V2
...
| Cross-v1 duplicate check | ${crossBankLine} |
...
- After any edit to \`questions.jsonl\`, rerun \`node coordination/content-qa/mainland-hjb-high-generated-bank-v2/audit-solvability.mjs\`.
...
function main() {
  ...
  const summary = {
    ...
    releaseRecommendation:
      inventoryIssues.length === 0
        ? "candidate-auto-qa-green-pending-s18-manual-sampling"
...
- Reason: ${inventoryIssues.length ? inventoryIssues.join(" ") : "Automated offline package checks passed. Manual S18 sampling remains required before app integration."}
- App integration status: Not approved in this phase. Coordinate with S04/S08/S18 after manual sampling and owner approval.
`
  );
