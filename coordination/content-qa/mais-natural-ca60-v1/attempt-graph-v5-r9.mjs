...
import {
  reconstructAttemptGraphCoreV5R9,
  roleOutputByEvidenceV5R9,
} from "./attempt-graph-core-v5-r9.mjs";
import {
...
export function reconstructAttemptGraphV5R9(input) {
  let base;
  const wrapperErrors = [];
  try {
    base = reconstructAttemptGraphCoreV5R9(input);
  } catch (error) {
...
export { roleOutputByEvidenceV5R9 };

export function validateAttemptGraphReconstructionReceiptV5R9({ attemptGraphReceipt, ...input }) {
  const errors = [];
  errors.push(...validateClosedSelfHashedArtifactV5R9(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV2"));
  try {
    const rebuilt = reconstructAttemptGraphV5R9({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R9 attempt graph differs from exact bidirectional evidence reconstruction");
    }
  } catch (error) {
...
export const ATTEMPT_GRAPH_V5_R9_CONSTANTS = Object.freeze({
