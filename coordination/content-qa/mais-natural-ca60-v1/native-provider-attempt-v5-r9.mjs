...
import {
  ...
  NATIVE_PROVIDER_ADAPTER_V5_R9_CONSTANTS,
...
function authorityErrors(input) {
  return validateDispatchAuthorityV5R9(input);
}

export function createNativeProviderTransportV5R9({ fetchImplementation, credentialReader,
  repoRoot, protectedRoot, clock = () => new Date() } = {}) {
  const adapter = createNativeProviderAdapterV5R9({ fetchImplementation, credentialReader,
    repoRoot, protectedRoot, clock });
  return Object.freeze({
    kind: "V5_R9_NATIVE_EXACT_TRANSPORT",
...
function buildCompatibilityPermit(input, reservation) {
  ...
  assertClosedSelfHashedArtifactV5R9(receipt, "ProviderDispatchPermitV3");
  return receipt;
}
...
function buildEvent({ input, reservation, compatibilityPermit, transport, parsed, attemptStatus,
  ...
  assertClosedSelfHashedArtifactV5R9(receipt, "ProviderEventReceiptV4");
  return receipt;
}
...
function buildRoleOutput(input, event, parsed) {
  ...
  assertClosedSelfHashedArtifactV5R9(receipt, "ProviderRoleOutputV1");
  return receipt;
}

export class NativeProviderAttemptFailureV5R9 extends Error {
  constructor(message, activity, cause) {
    super(message, { cause });
    this.name = "NativeProviderAttemptFailureV5R9";
    this.activity = activity;
  }
}

export async function runNativeProviderAttemptV5R9(input) {
  ...
  try {
    const errors = [
      ...validateProviderActivationV5R9(input),
    ...
    for (const name of ["dispatchAuthorityStore", "dispatchPermitStore",
      ...
      requireCondition(typeof input?.[name]?.append === "function", `R9 ${name} is unavailable`);
    }
    requireCondition(typeof input?.ledger?.reserve === "function"
      ...
      && input?.transport?.kind === "V5_R9_NATIVE_EXACT_TRANSPORT",
    "R9 ledger/raw/native-transport dependencies are unavailable");
    ...
    requireCondition(authorityPersisted?.contentHash === input.dispatchAuthority.selfHash,
      "R9 dispatch authority was not durable before reservation");
    const reservation = await input.ledger.reserve({
      requestArtifact: input.requestArtifact.compatibilityRequestArtifact,
    });
    const permit = buildProviderDispatchPermitV5R9({ input, reservation });
    ...
    requireCondition(permitPersisted?.contentHash === permit.selfHash,
      "R9 dispatch permit was not durable before credential read");
    ...
    requireCondition(compatibilityPermitPersisted?.contentHash === compatibilityPermit.selfHash,
      "R9 compatibility permit was not durable before attempt intent");
    ...
    requireCondition(bindingPersisted?.persistedBeforeCompletion === true,
      "R9 raw binding was not durable before attempt intent");
    const committed = await commitProviderAttemptWithDurableIntentV5R9({
      activeRunnerRegistrationHash: input.activeRegistration.selfHash,
      authorizationHash: input.authorization.selfHash,
    ...
    throw new NativeProviderAttemptFailureV5R9(
...
export const NATIVE_PROVIDER_ATTEMPT_V5_R9_CONSTANTS = Object.freeze({
  requestTimeoutMs: NATIVE_PROVIDER_ADAPTER_V5_R9_CONSTANTS.requestTimeoutMs,
  maximumResponseBytes: NATIVE_PROVIDER_ADAPTER_V5_R9_CONSTANTS.maximumResponseBytes,
