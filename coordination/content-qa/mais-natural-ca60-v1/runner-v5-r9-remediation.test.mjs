...
import {
  createNativeProviderAdapterV5R9,
} from "./native-provider-adapter-v5-r9.mjs";
import {
  createRunnerRuntimeV5R9,
  persistFinalCommandJournalV5R9,
  V5_R9_RUNTIME_PATHS,
} from "./runner-v5-r9-runtime.mjs";
import {
  reloadCommandJournalSidecarsV5R9,
...
const H = (value) => sha256V5R3(`v5-r9:${value}`);
...
test("R9 accepts the same local findingId on different items using the frozen composite key", () => {
  ...
  const built = buildFrozenStatisticalBundleV5R9(statisticalInput(rows));
...
test("R9 rejects an orphan DISPATCH_COMPLETED ledger row before any resume", () => {
  ...
  assert.ok(validateResumeCustodyV5R9({ ledgerEntries: [completion], preparedCompletions: [],
    attemptCommitIntents: [], resolvedAttemptReceipts: [] })
    .some((error) => /orphan ledger completion/iu.test(error)));
});

test("R9 custody requires retained prepared-completion bytes for a durable intent", () => {
  ...
  assert.throws(() => inspectInterruptedAttemptCustodyV5R9({ ledgerEntries: [reservation],
...
test("R9 decision evidence ignores caller-authored favorable or adverse integrity flags", () => {
  const base = emptyDecisionInput();
  const favorable = reconstructDecisionEvidenceV5R9({ ...base, receiptChainValid: true,
  ...
  const adverse = reconstructDecisionEvidenceV5R9({ ...base, receiptChainValid: false,
...
test("R9 self-hashed but schema-invalid authorizations cannot manufacture provider or cap validity", () => {
  const base = emptyDecisionInput();
  ...
  const receipt = reconstructDecisionEvidenceV5R9({ ...base, authorizations });
...
test("R9 unknown command-journal accounting forces terminal fail-closed decision evidence", () => {
  const base = emptyDecisionInput();
  ...
  const receipt = reconstructDecisionEvidenceV5R9({ ...base, commandJournals: [journal] });
  assert.equal(receipt.receiptChainValid, false);
  assert.equal(receipt.capsValid, false);
...
test("R9 failed or successful final command journals share the same required workflow-index custody path", async () => {
  ...
  const calls = [];
  const transition = await persistFinalCommandJournalV5R9({
    context,
    journal,
...
test("R9 reloads a valid protected command-journal sidecar that was not yet indexed", async (t) => {
  ...
  await atomicWriteProtectedJsonV5R4({ trustedRoot,
    relativePath: path.join("runtime-custody-v5-r9", "transition-journals",
      `${journal.selfHash}.json`),
    value: journal });
  const reloaded = await reloadCommandJournalSidecarsV5R9({ trustedRoot, activeRegistration });
  assert.equal(reloaded.length, 1);
  assert.deepEqual(reloaded[0], journal);
});

test("R9 route validation rejects a signed-summary-shaped envelope without raw source and probe custody", () => {
  const activeRegistration = { selfHash: H("route-registration"), trustedProviderEvidenceAnchors: [] };
  const errors = validateTrustedProviderEvidenceEnvelopeV5R9({ envelope: null,
...
test("R9 final adapter reloads immutable Git custody before credential access", async () => {
  let credentialReads = 0;
  let fetches = 0;
  const adapter = createNativeProviderAdapterV5R9({
  ...
  await assert.rejects(() => adapter.send({ activeRegistration: { selfHash: H("fake") },
    registrationEvidence: { selfHash: H("fake-evidence") } }), /V5-R9 registration path|Git|registration/iu);
  assert.equal(credentialReads, 0);
  assert.equal(fetches, 0);
});

test("R9 default runtime installs the native workflow and blocks absent authority at exact zero activity", async () => {
  const runtime = createRunnerRuntimeV5R9({ clock: () => "2026-08-26T15:00:00.000Z" });
  assert.equal(V5_R9_RUNTIME_PATHS.defaultNativePlannerExecutorInstalled, true);
  assert.equal(V5_R9_RUNTIME_PATHS.defaultRecoveryLoaderInstalled, true);
...
test("R9 runtime has no caller-authored scoring or all-missing terminal fallback", async () => {
