...
function validateGraphAndBlindness(input) {
  ...
  `R11 reference seal requires a complete independently rebuilt OpenAI attempt graph: ${graphErrors.join("; ")}`);
  ...
  "R11 reference graph contains missing successes, non-reparsed success, leaked input, or extra-role evidence");
  ...
  requireCondition(exactSet(successfulOutputHashes,
    ...
    "R11 reference graph evidence does not equal the complete successful compatibility output set");
}

export function buildRawAuthoritativeMachineReferenceSealV5R11(input) {
  ...
  "R11 reference seal requires the exact active registration and fresh A11 concurrence");
  const authErrors = validateClosedSelfHashedArtifactV5R11(input?.authorization, "ProviderAuthorizationV5");
  ...
  `R11 OpenAI reference authorization is invalid: ${authErrors.join("; ")}`);
  ...
  "R11 compatibility reference seal differs from the raw-authoritative attempt graph");
  const seal = sealV5R3Artifact({
    schemaVersion: "MachineReferenceSealV7",
    ...
    sealReconstructionStatus: "FULL_RAW_RESPONSE_REPARSE_AND_ATTEMPT_GRAPH_RECONSTRUCTION_BOUND_TO_R11",
    sealedAt: input.sealedAt,
  });
...
export function validateRawAuthoritativeMachineReferenceSealV5R11({ seal, compatibilityReferenceSeal,
  ...input }) {
  ...
  try {
    const rebuilt = buildRawAuthoritativeMachineReferenceSealV5R11({
      ...input,
    ...
    if (canonicalJsonV5R3(rebuilt.seal) !== canonicalJsonV5R3(seal)
      ...
      errors.push("R11 reference seal differs from raw-authoritative graph and label reconstruction");
    }
  } catch (error) {
...
export function buildRawAuthoritativeReferenceValidationV5R11({ seal, compatibilityReferenceSeal,
  validatedAt, ...input }) {
  const errors = validateRawAuthoritativeMachineReferenceSealV5R11({
    ...input,
    seal,
    compatibilityReferenceSeal,
  });
  requireCondition(errors.length === 0 && Date.parse(validatedAt) > Date.parse(seal.sealedAt),
    `R11 reference validation failed or does not postdate sealing: ${errors.join("; ")}`);
  const graph = input.attemptGraphReceipt;
  const receipt = sealV5R3Artifact({
    schemaVersion: "ReferenceSealValidationReceiptV5",
    designId: "MAIS-NATURAL-CA60-V5",
    activeRunnerRegistrationHash: input.activeRegistration.selfHash,
    freshRunnerReviewHash: input.freshReview.selfHash,
    ...
    labelBlindnessVerified: true,
    sameModelCorrelatedErrorRisk: true,
    validationStatus: "RAW_AUTHORITATIVE_ATTEMPT_GRAPH_REBUILT_AND_BOUND_TO_R11",
    validatedAt,
  });
...
export function validateRawAuthoritativeReferenceValidationV5R11({ validationReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(validationReceipt, VALIDATION_SCHEMA)];
  try {
    const rebuilt = buildRawAuthoritativeReferenceValidationV5R11({
      ...input,
      validatedAt: validationReceipt?.validatedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(validationReceipt)) {
      errors.push("R11 reference validation receipt differs from exact reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const RAW_AUTHORITATIVE_REFERENCE_V5_R11_CONSTANTS = Object.freeze({
