...
function commandReceipt(command, status, context, output = {}) {
  ...
  return assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
    schemaVersion: "NaturalCaRunnerCommandReceiptV8",
    runnerVersion: "V5-R9",
...
export function parseExactArgumentsV5R9(argv) {
...
export async function runCliV5R9(argv = [], deps = createRunnerRuntimeV5R9()) {
  const parsed = parseExactArgumentsV5R9(argv);
  ...
  let engine;
  let args = [context];
  if (parsed.command === "register") engine = "adoptR9WorkflowIndex";
...
async function main() {
  const result = await runCliV5R9(process.argv.slice(2));
...
export const V5_R9_CLI_COMMANDS = Object.freeze([...COMMANDS]);
