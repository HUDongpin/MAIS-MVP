...
function validateCustodyInputs(input) {
  const errors = [
    ...validateClosedSelfHashedArtifactV5R9(input?.requestArtifact, "ProviderRequestArtifactV5"),
    ...validateClosedSelfHashedArtifactV5R9(input?.dispatchAudit,
      input?.dispatchAudit?.schemaVersion ?? "StateBoundDispatchAuditReceiptV2"),
    ...validateClosedSelfHashedArtifactV5R9(input?.reservation, "ProviderDispatchReservationV2"),
    ...validateClosedSelfHashedArtifactV5R9(input?.dispatchPermit,
      input?.dispatchPermit?.schemaVersion ?? "ProviderDispatchPermitV4"),
    ...validateClosedSelfHashedArtifactV5R9(input?.compatibilityDispatchPermit, "ProviderDispatchPermitV3"),
    ...
    ...validateClosedSelfHashedArtifactV5R9(input?.providerEventReceipt, "ProviderEventReceiptV4"),
    ...validateClosedSelfHashedArtifactV5R9(input?.preparedCompletion, "ProviderDispatchCompletionV2"),
  ];
  if (input?.roleOutput !== null) {
    errors.push(...validateClosedSelfHashedArtifactV5R9(input?.roleOutput, "ProviderRoleOutputV1"));
  }
  return [...new Set(errors)];
}

export function buildProviderAttemptCommitIntentV5R9(input) {
...
export function validateProviderAttemptCommitIntentV5R9({ intent, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(intent, INTENT_SCHEMA)];
  try {
    const rebuilt = buildProviderAttemptCommitIntentV5R9({ ...input, preparedAt: intent?.preparedAt });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(intent)) {
      errors.push("R9 attempt commit intent differs from exact pre-completion custody reconstruction");
    }
  } catch (error) {
...
export function buildResolvedProviderAttemptReceiptV5R9({ intent, committedCompletion, ...input }) {
  const intentErrors = validateProviderAttemptCommitIntentV5R9({ intent, ...input,
    preparedCompletion: input.preparedCompletion });
  requireCondition(intentErrors.length === 0, intentErrors.join("; "));
  requireCondition(validateClosedSelfHashedArtifactV5R9(committedCompletion,
...
export function validateResolvedProviderAttemptReceiptV5R9({ resolvedAttemptReceipt, intent,
  committedCompletion, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(resolvedAttemptReceipt, RESOLVED_SCHEMA)];
  try {
    const rebuilt = buildResolvedProviderAttemptReceiptV5R9({ ...input, intent, committedCompletion });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(resolvedAttemptReceipt)) {
      errors.push("R9 resolved attempt receipt differs from complete two-phase custody reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export async function commitProviderAttemptWithDurableIntentV5R9(input) {
  ...
  "R9 two-phase attempt commit requires append-only intent/final stores and an atomic intent-aware ledger");
  let intent = null;
  const committed = await input.ledger.completeWithDurableIntent({
    ...
    persistIntent: async (preparedCompletion) => {
      intent = buildProviderAttemptCommitIntentV5R9({ ...input, preparedCompletion });
      ...
      requireCondition(completionPersisted?.contentHash === preparedCompletion.selfHash,
        "R9 exact prepared completion bytes were not durable before commit intent");
  ...
  const resolvedAttemptReceipt = buildResolvedProviderAttemptReceiptV5R9({
    ...input,
...
export const ATTEMPT_TRANSACTION_V5_R9_CONSTANTS = Object.freeze({
  commitProtocol:
