...
function exactOne(values, predicate, label) {
  ...
  requireCondition(matches.length <= 1, `R10 custody contains duplicate ${label}`);
  return matches[0] ?? null;
}

export function inspectInterruptedAttemptCustodyV5R10(input) {
  const reservation = reservationFor(input);
  requireCondition(reservation && validateSelfHashV5R3(reservation),
    "R10 custody inspection requires the exact sealed reservation");
...
function validateAuthorization(input, custody) {
  ...
  "R10 reconciliation requires a sealed owner authorization");
  ...
  "R10 reconciliation authorization does not bind the exact reservation and zero-HTTP scope");
  ...
  requireCondition([at, issued, expires].every(Number.isFinite)
    && issued <= at && at < expires, "R10 reconciliation authorization is expired or not yet valid");
}
...
function exactRecoveredCompletion(input, custody) {
  ...
  "R10 recovered completion differs from the durable intent");
  return completion;
}
...
function exactRecoveredResolved(input, custody, completion) {
  ...
  requireCondition(resolved && validateSelfHashV5R3(resolved)
    ...
    && resolved.compatibilityCompletionHash === completion.selfHash,
  "R10 recovered resolved receipt differs from the exact intent/completion lineage");
  return resolved;
}

export function reconcileInterruptedAttemptV5R10(input) {
  ...
  const custody = inspectInterruptedAttemptCustodyV5R10(input);
  ...
  if (custody.state === "RESERVATION_WITHOUT_DURABLE_INTENT") {
    ...
    throw new TypeError("R10 custody state cannot be reconciled");
  }
...
export async function applyInterruptedAttemptReconciliationV5R10(input) {
  requireCondition(input?.transport === undefined && input?.credentialReader === undefined,
    "R10 reconciliation application forbids transport and credential dependencies");
  const planned = reconcileInterruptedAttemptV5R10(input);
  const custody = inspectInterruptedAttemptCustodyV5R10(input);
  const persisted = [];
  if (planned.plannedActions.includes("APPEND_EXACT_INTENT_BOUND_COMPLETION")) {
    requireCondition(typeof input?.ledger?.recoverIntentBoundCompletion === "function",
      "R10 reconciliation cannot append an intent-bound completion without the atomic ledger recovery API");
    ...
    "R10 atomic-ledger recovery authorization does not bind the same owner decision and exact intent");
    ...
    "R10 ledger recovery did not append the exact completion with zero provider activity");
  ...
  if (planned.plannedActions.includes("APPEND_EXACT_RESOLVED_ATTEMPT_RECEIPT")) {
    requireCondition(typeof input?.resolvedAttemptReceiptStore?.append === "function",
      "R10 reconciliation cannot append the resolved receipt without an append-only store");
    ...
    requireCondition(result?.contentHash === input.recoveredResolvedAttemptReceipt.selfHash,
      "R10 resolved attempt receipt was not durably appended by exact hash");
  ...
  requireCondition(typeof input?.reconciliationReceiptStore?.append === "function",
    "R10 reconciliation receipt requires an append-only public custody store");
  ...
  requireCondition(receiptPersisted?.contentHash === planned.receipt.selfHash,
    "R10 reconciliation receipt was not durably appended by exact hash");
...
export function validateResumeCustodyV5R10(input) {
  const errors = [];
...
export function buildReconciliationAuthorizationV5R10(input) {
...
export function reconciliationEvidenceRootV5R10(receipts) {
  return sha256V5R3(canonicalJsonV5R3((receipts ?? []).map(({ selfHash }) => selfHash).sort()));
}

export const ATTEMPT_RECOVERY_V5_R10_CONSTANTS = Object.freeze({
  zeroHttpOnly: true,
