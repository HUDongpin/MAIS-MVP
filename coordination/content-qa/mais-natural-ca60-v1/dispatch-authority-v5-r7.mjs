...
export function buildReferenceDispatchAuthorityV5R7(input) {
  ...
  `R7 reference authority upstream validation failed: ${errors.join("; ")}`);
  ...
  requireCondition(plan.planStatus === "NEXT_ACTION", `R7 reference planner is ${plan.planStatus}`);
  const item = input.inventory.items[plan.manifestOrdinal - 1];
  requireCondition(item?.itemHash === plan.itemHash, "R7 reference plan is outside the frozen inventory");
  ...
  const receipt = sealV5R3Artifact({
    ...
    attemptOrdinal,
    attemptId: `V5R7O:${input.activeRegistration.selfHash}:${plan.selfHash}:${attemptOrdinal}`,
  ...
  assertClosedSelfHashedAgainstV5R5(receipt, REFERENCE_AUTHORITY_SCHEMA, receipt.schemaVersion);
  return receipt;
}

export function validateReferenceDispatchAuthorityV5R7({ referenceDispatchAuthority, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(referenceDispatchAuthority,
    REFERENCE_AUTHORITY_SCHEMA)];
  try {
    const rebuilt = buildReferenceDispatchAuthorityV5R7({
      ...input,
      issuedAt: referenceDispatchAuthority?.issuedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(referenceDispatchAuthority)) {
      errors.push("R7 reference authority differs from exact planner reconstruction");
    }
  } catch (error) {
...
export function validateDispatchAuthorityV5R7(input) {
...
export function buildProviderDispatchPermitV5R7({ input, reservation }) {
  const authority = input.dispatchAuthority;
  const errors = validateDispatchAuthorityV5R7(input);
  ...
  `R7 dispatch authority/request binding is invalid: ${errors.join("; ")}`);
  const receipt = sealV5R3Artifact({
  ...
  assertClosedSelfHashedAgainstV5R5(receipt, PERMIT_SCHEMA, receipt.schemaVersion);
  return receipt;
}

export function validateProviderDispatchPermitV5R7({ permit, input, reservation }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(permit, PERMIT_SCHEMA)];
  try {
    const rebuilt = buildProviderDispatchPermitV5R7({
      input: { ...input, at: permit?.issuedAt },
      reservation,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(permit)) {
      errors.push("R7 dispatch permit differs from exact authority/request/reservation reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
