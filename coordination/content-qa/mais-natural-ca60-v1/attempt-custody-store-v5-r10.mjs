...
function baseRelativePath(provider, authorizationHash) {
  ...
  "R10 attempt custody store requires a provider and authorization hash");
  return path.join("runtime-custody-v5-r10", "attempts", provider.toLowerCase(), authorizationHash);
}
...
function artifactRelativePath(provider, authorizationHash, reservationHash, kind) {
  requireCondition(HASH.test(reservationHash ?? "") && FILES[kind],
    "R10 attempt custody path requires an exact reservation and artifact kind");
...
async function appendExact(trustedRoot, relativePath, value) {
  requireCondition(validateSelfHashV5R3(value), "R10 attempt custody artifact is not self-hashed");
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R10 append-only attempt custody path already contains different canonical bytes");
  } catch (error) {
  ...
  requireCondition(canonicalJsonV5R3(reloaded) === canonicalJsonV5R3(value),
    "R10 attempt custody artifact did not reload exactly after append");
...
export async function createAttemptCustodyStoreV5R10({ trustedRoot, provider, authorizationHash }) {
  ...
  return Object.freeze({
    ...
    async enumerateReservationDirectories() {
      ...
      requireCondition(names.every((name) => HASH.test(name)),
        "R10 attempt custody root contains an unexpected reservation directory");
      return Object.freeze([...names].sort());
    },
  });
}

export async function reloadAttemptCustodyStoreV5R10({ trustedRoot, provider,
  authorizationHash, ledgerEntries }) {
  const store = await createAttemptCustodyStoreV5R10({ trustedRoot, provider, authorizationHash });
...
export const ATTEMPT_CUSTODY_STORE_V5_R10_CONSTANTS = Object.freeze({
