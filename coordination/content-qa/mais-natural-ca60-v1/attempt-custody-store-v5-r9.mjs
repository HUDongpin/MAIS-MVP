...
function baseRelativePath(provider, authorizationHash) {
  ...
  "R9 attempt custody store requires a provider and authorization hash");
  return path.join("runtime-custody-v5-r9", "attempts", provider.toLowerCase(), authorizationHash);
}
...
function artifactRelativePath(provider, authorizationHash, reservationHash, kind) {
  requireCondition(HASH.test(reservationHash ?? "") && FILES[kind],
    "R9 attempt custody path requires an exact reservation and artifact kind");
...
async function appendExact(trustedRoot, relativePath, value) {
  requireCondition(validateSelfHashV5R3(value), "R9 attempt custody artifact is not self-hashed");
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R9 append-only attempt custody path already contains different canonical bytes");
  } catch (error) {
  ...
  requireCondition(canonicalJsonV5R3(reloaded) === canonicalJsonV5R3(value),
    "R9 attempt custody artifact did not reload exactly after append");
...
export async function createAttemptCustodyStoreV5R9({ trustedRoot, provider, authorizationHash }) {
  ...
  return Object.freeze({
    ...
    async enumerateReservationDirectories() {
      ...
      requireCondition(names.every((name) => HASH.test(name)),
        "R9 attempt custody root contains an unexpected reservation directory");
      return Object.freeze([...names].sort());
    },
  });
}

export async function reloadAttemptCustodyStoreV5R9({ trustedRoot, provider,
  authorizationHash, ledgerEntries }) {
  const store = await createAttemptCustodyStoreV5R9({ trustedRoot, provider, authorizationHash });
...
export const ATTEMPT_CUSTODY_STORE_V5_R9_CONSTANTS = Object.freeze({
