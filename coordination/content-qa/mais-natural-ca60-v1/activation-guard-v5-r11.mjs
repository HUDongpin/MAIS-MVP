...
function validateCostPreview(input, errors) {
  ...
  errors.push(...validateClosedSelfHashedArtifactV5R11(preview, "ProviderCostPreviewReceiptV1"));
  ...
  "R11 cost preview is not an exact signed-route price reconstruction with a 20 percent buffer");
}

export function validateProviderActivationV5R11(input) {
  const errors = [
    ...validateExactRunnerRegistrationEvidenceV5R11(input?.registrationEvidence),
    ...validateActiveRunnerRegistrationV5R11(input?.activeRegistration),
    ...validateFreshRunnerReviewV5R11(input ?? {}),
    ...validateResolvedProviderAuthorizationV5R6(input?.authorization),
    ...validateClosedSelfHashedArtifactV5R11(input?.ownerActivationGrant,
      "OwnerRunnerActivationGrantV1"),
    ...validateClosedSelfHashedArtifactV5R11(input?.priceSnapshot, "ProviderPriceSnapshotV2"),
    ...validateResumeCustodyV5R11(input?.attemptCustody ?? {}),
  ];
  try {
    errors.push(...validateTrustedProviderEvidenceEnvelopeV5R11({
  ...
  add(errors, evidence?.activeRegistration?.selfHash === registration?.selfHash,
    "R11 exact registration evidence does not bind the supplied active registration");
  add(errors, envelope && authorization?.model === envelope.model
    && authorization?.endpoint === envelope.endpoint,
  "R11 authorization provider, model, or endpoint differs from the frozen direct route");
  ...
  "R11 authorization does not bind the exact registration, review, signed route, and compatibility base");
  for (const field of ["provider", "model", "endpoint", "projectResidency", "dataRegion"]) {
    add(errors, authorization?.[field] === route?.[field],
    `R11 authorization ${field} differs from signed route evidence`);
  }
  add(errors, authorization?.projectIdentityHash === route?.subjectIdentityHash,
    "R11 authorization subject differs from signed route evidence");
  ...
  "R11 OpenAI authorization is not bound to US_STORAGE_PROCESSING and US data region");
  add(errors, !/UNKNOWN|PENDING|UNRESOLVED/iu.test(
    `${authorization?.projectResidency}:${authorization?.dataRegion}`),
  "R11 provider residency and data region remain unresolved");
  ...
  "R11 provider authorization differs from the frozen attempt, token, USD, or concurrency envelope");
  add(errors, allLiveAuthorityFlags(authorization) && allLiveAuthorityFlags(grant),
    "R11 provider authorization or owner activation grant lacks explicit live authority flags");
  ...
  "R11 owner grant does not bind the exact registration, review, route, cost, authorization, and inventory");
  ...
  "R11 owner grant tuple differs from the signed resolved authorization");
  ...
  "R11 owner grant caps, currency, or non-authorizations differ from the frozen envelope");
  ...
  "R11 compatibility ledger authorization, signed price, or inventory binding is invalid");
  ...
  "R11 authorization or grant chronology is invalid or expired");

  if (authorization?.provider === "DEEPSEEK_DIRECT") {
    errors.push(...validateRawAuthoritativeMachineReferenceSealV5R11({
      ...(input?.referenceSealContext ?? {}), seal: input?.referenceSeal,
      compatibilityReferenceSeal: input?.compatibilityReferenceSeal,
    }));
    errors.push(...validateRawAuthoritativeReferenceValidationV5R11({
    ...
    errors.push(...validateClosedSelfHashedArtifactV5R11(input?.executionRegistration,
    ...
    "R11 DeepSeek execution registration is not bound to the sealed reference and exact live tuple");
  }
  return Object.freeze([...new Set(errors)]);
}

export function assertProviderActivationV5R11(input) {
  const errors = validateProviderActivationV5R11(input);
  if (errors.length > 0) throw new TypeError(errors.join("; "));
  return input.authorization;
}

export const ACTIVATION_GUARD_V5_R11_CONSTANTS = Object.freeze({
