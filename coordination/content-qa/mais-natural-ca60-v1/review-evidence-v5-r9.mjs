...
function processEvidenceErrors(review, processArtifacts) {
  ...
  add(errors, validateClosedSelfHashedArtifactV5R9(staticGraph,
  ...
  add(errors, validateClosedSelfHashedArtifactV5R9(forbiddenScan,
  ...
  add(errors, validateClosedSelfHashedArtifactV5R9(runtime,
  ...
  add(errors, validateClosedSelfHashedArtifactV5R9(enumeration,
...
export function validatePublicReviewIdentityAnchorV5R9(anchor) {
  const errors = [];
  ...
  add(errors, anchor?.keyId === "a11-v5-r9-fresh-review-ed25519-4b46d8a969506553"
    && anchor?.algorithm === "Ed25519"
    && anchor?.purpose === "V5_R9_FRESH_A11_REVIEW_ONLY",
  ...
  try {
    ...
    add(errors, sha256V5R3(der) === anchor?.publicKeyFingerprintSha256
      && anchor.publicKeyFingerprintSha256
        === "4b46d8a969506553c2bd4ea366c204bcc5c3fcb375975fb8908e0dd76cfdbf07",
...
export function buildIndependentRunnerReviewSignaturePayloadV5R9(input) {
  return Object.freeze({
    schemaVersion: "IndependentExecutionRunnerReviewSignaturePayloadV1",
...
export function buildIndependentRunnerReviewReceiptV5R9(input) {
  const process = input.processEvidence;
  if (!process || typeof process !== "object") throw new TypeError("R9 A11 process evidence is required");
  const anchorErrors = validatePublicReviewIdentityAnchorV5R9(input.reviewerIdentityAnchor);
  if (anchorErrors.length > 0) throw new TypeError(anchorErrors.join("; "));
  const payload = buildIndependentRunnerReviewSignaturePayloadV5R9(input);
  ...
  if (!verify(null, Buffer.from(canonicalJsonV5R3(payload), "utf8"), publicKey, signatureBytes)) {
    throw new TypeError("R9 A11 review signature is invalid for the pinned reviewer identity");
  }
  const body = {
    schemaVersion: "IndependentExecutionRunnerReviewReceiptV7",
    designId: "MAIS-NATURAL-CA60-V5",
    runnerVersion: "V5-R9",
    reviewedAt: input.reviewedAt,
    decision: input.decision,
  ...
  return assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact(body),
    "IndependentExecutionRunnerReviewReceiptV7");
}

export function validateFreshRunnerReviewV5R9({ activeRegistration, registrationEvidence,
  freshReview, processArtifacts, reviewCustody, reviewerIdentityAnchor }) {
  const errors = [...validatePublicReviewIdentityAnchorV5R9(reviewerIdentityAnchor)];
  const schemaErrors = validateClosedSelfHashedArtifactV5R9(freshReview,
    "IndependentExecutionRunnerReviewReceiptV7");
  add(errors, schemaErrors.length === 0,
    `fresh A11 review fails its closed R9 schema or self-hash: ${schemaErrors.join("; ")}`);
  add(errors, freshReview?.decision === "CONCURRED" && freshReview?.findingCount === 0,
    "fresh A11 V5-R9 review must be CONCURRED with zero findings");
  ...
  add(errors, freshReview?.reviewerLane === "A11" && freshReview?.independentImplementation === true,
    "fresh A11 V5-R9 review identity or independence declaration is invalid");
  ...
  "fresh A11 V5-R9 review does not bind the exact immutable registration and roots");
  ...
  add(errors, exactArray(freshReview?.reviewedRemediatedFindingIds, FINDING_IDS),
    "fresh A11 V5-R9 review does not cover all eight R8 discrepancies exactly once");
  ...
  try {
    const expectedPayload = buildIndependentRunnerReviewSignaturePayloadV5R9({
  ...
  "fresh A11 V5-R9 review must strictly postdate registration");
  for (const field of ["credentialReadCount", "naturalQuestionReadCount", "providerCallCount",
    "naturalQuestionEgressCount", "tokenCount", "attemptCount", "usdSpent"]) {
...
export const REVIEW_EVIDENCE_V5_R9_CONSTANTS = Object.freeze({
