...
function exactZeroOutput(extra = {}) {
  return {
...
function commandReceipt(command, status, context, output = {}) {
  const journal = output.transitionJournal ?? null;
  const accounting = output.activityAccountingStatus ?? journal?.activityAccountingStatus ?? "EXACT";
  ...
  const providerCommandInvoked = activityPossible
    || (PROVIDER_EXECUTION_COMMANDS.has(command) && accounting !== "EXACT");
  const transition = output.transition ?? null;
  const observedDurableTransitionCount = journal?.observedDurableTransitionCount
    ?? (transition?.nextIndex?.selfHash ? 1 : 0);
  const lastObservedDurableWorkflowIndexHash = journal?.lastObservedDurableWorkflowIndexHash
    ?? transition?.nextIndex?.selfHash ?? null;
  const receipt = sealV5R3Artifact({
    schemaVersion: "NaturalCaRunnerCommandReceiptV6",
    runnerVersion: "V5-R7",
    designId: "MAIS-NATURAL-CA60-V5",
    command,
    status,
    activeRunnerRegistrationHash: context?.activeRegistration?.selfHash ?? null,
    providerCommandInvoked,
    providerEventCount,
    httpRequestCount,
    credentialReadCount,
    ...
    transitionJournalHash: journal?.selfHash ?? null,
    observedDurableTransitionCount,
    lastObservedDurableWorkflowIndexHash,
  ...
  assertClosedSelfHashedArtifactV5R7(receipt, "NaturalCaRunnerCommandReceiptV6");
  return receipt;
}
...
function outcome(command, exitCode, status, context = null, output = null) {
  return Object.freeze({
    exitCode,
    receipt: commandReceipt(command, status, context, output ?? exactZeroOutput()),
  });
}

export function parseExactArgumentsV5R7(argv) {
  const [command, ...rest] = argv;
  if (!COMMANDS.has(command)) return { error: "USAGE_ERROR_UNKNOWN_COMMAND", command: command ?? "UNKNOWN" };
  const valued = new Set(["--context", "--provider", "--canary", "--review"]);
  ...
  if (command === "register") {
    if (values.size !== 2 || typeof values.get("--review") !== "string") {
      return { error: "USAGE_ERROR_REGISTER_REQUIRES_EXACT_REVIEW", command };
    }
  }
  if (["register-route-evidence", "authorize-check"].includes(command)
    && (!provider || values.size !== 2)) return { error: "USAGE_ERROR_PROVIDER_REQUIRED", command };
  const noExtra = new Set([
    "freeze-frame", "audit-clusters", "freeze-sample", "label-qwen", "seal-reference-labels",
    "dry-run", "freeze-deepseek-registration", "score", "verify", "export-aggregate-report",
  ]);
  if (noExtra.has(command) && values.size !== 1) {
    return { error: "USAGE_ERROR_COMMAND_ARGUMENTS", command };
  }
  ...
  return {
    ...
    reviewPath: values.get("--review") ?? null,
    provider,
...
export async function runCliV5R7(argv = [], deps = createRunnerRuntimeV5R7()) {
  const parsed = parseExactArgumentsV5R7(argv);
  ...
  let context;
  try {
    context = await deps.loadWorkflowContext(parsed.contextPath);
  } catch {
    return outcome(parsed.command, 3, "WORKFLOW_CONTEXT_BLOCKED");
  }
  let engine;
  let args = [context];
  if (parsed.command === "register") {
    if (typeof deps.loadFreshReview !== "function") {
      return outcome(parsed.command, 3, "FRESH_REVIEW_LOADER_NOT_BOUND", context);
    }
    let review;
    try { review = await deps.loadFreshReview(parsed.reviewPath); }
    catch { return outcome(parsed.command, 3, "FRESH_REVIEW_BLOCKED", context); }
    engine = "adoptR7WorkflowIndex";
    args.push(review);
    ...
    engine = "dryRun";
  } else if (parsed.command === "authorize-check") {
    engine = "authorizeCheck";
    args.push(parsed.provider);
  } else if (parsed.command === "register-route-evidence") {
    engine = "registerAuthenticatedRouteEvidence";
    args.push(parsed.provider);
  } else if (parsed.command === "label-reference") {
    engine = "executeOpenAIResumeStep";
  } else if (parsed.command === "label-qwen") {
    engine = "rejectLegacyQwenCommand";
    ...
    engine = "verify";
  } else if (parsed.command === "export-aggregate-report") {
    engine = "exportAggregateReport";
  } else {
    engine = parsed.command === "seal-reference-labels"
  ...
  try {
    ...
    output = PROVIDER_EXECUTION_COMMANDS.has(parsed.command)
      ...
      : { ok: false, status: "ENGINE_FAIL_CLOSED", ...exactZeroOutput() };
  }
  if (!output || typeof output.status !== "string") {
    return outcome(parsed.command, 4, "ENGINE_RESULT_INVALID", context,
      PROVIDER_EXECUTION_COMMANDS.has(parsed.command)
        ? { ...exactZeroOutput({ providerEventCount: null, httpRequestCount: null,
          credentialReadCount: null, naturalQuestionEgressCount: null,
          activityAccountingStatus: "UNKNOWN_FAIL_CLOSED" }) }
        : exactZeroOutput());
  }
...
async function main() {
  const result = await runCliV5R7(process.argv.slice(2));
...
export const V5_R7_CLI_COMMANDS = Object.freeze([...COMMANDS]);
