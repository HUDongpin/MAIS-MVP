...
export const BOOTSTRAP_GOLDEN_VECTORS_V5_R9 = Object.freeze(
...
export function bootstrapBoundedIndexV5R9({ key, metric, replicate, draw, bound }) {
  requireCondition(typeof key === "string" && key.length > 0 && !key.includes("|"),
    "R9 bootstrap key must be nonempty and pipe-free");
  requireCondition(typeof metric === "string" && metric.length > 0 && !metric.includes("|"),
    "R9 bootstrap metric must be nonempty and pipe-free");
  requireCondition([replicate, draw, bound].every(Number.isSafeInteger)
    && replicate >= 0 && draw >= 0 && bound > 0, "R9 bootstrap index inputs are invalid");
  const range = 1n << 64n;
...
function assertGoldenVectors() {
  requireCondition(BOOTSTRAP_GOLDEN_VECTORS_V5_R9.every((vector) =>
    bootstrapBoundedIndexV5R9(vector) === vector.expectedIndex),
  "R9 counter-SHA256 bootstrap golden vectors failed");
}

export function wilsonIntervalV5R9(numerator, denominator, z) {
  requireCondition(Number.isInteger(numerator) && Number.isInteger(denominator)
    && numerator >= 0 && denominator >= 0 && numerator <= denominator
    && Number.isFinite(z) && z > 0, "R9 Wilson inputs are invalid");
...
function normalizeFinding(finding, itemId, stream, globalKeys) {
  ...
  `R9 ${stream} finding is not a frozen taxonomy leaf for ${itemId}`);
  ...
  requireCondition(!globalKeys.has(frozenCompositeKey),
    `R9 ${stream} frozen composite finding key ${frozenCompositeKey} is duplicated`);
...
export function deterministicFindingMatchesV5R9(referenceFindings, machineFindings) {
  const itemIds = unique([...(referenceFindings ?? []), ...(machineFindings ?? [])]
    .map(({ itemId }) => itemId));
  requireCondition(itemIds.length === 1 && typeof itemIds[0] === "string",
    "R9 finding matching requires exactly one item pseudonym");
...
function validateItemResult(item, index, expected, globalReferenceKeys, globalMachineKeys) {
  const schemaErrors = validateClosedSelfHashedArtifactV5R9(item, "ItemEvaluationResultV2");
  ...
  `R9 item result ${index + 1} is not a closed sealed ItemEvaluationResultV2: ${schemaErrors.join("; ")}`);
  ...
  `R9 item result ${index + 1} identity is invalid`);
  ...
  })) requireCondition(item[field] === value, `R9 item result ${index + 1} ${field} binding differs`);
  requireCondition(["COMPLETE", "MISSING_RECEIPT"].includes(item.executionDisposition)
    ...
    && ["RESOLVED_POSITIVE", "RESOLVED_NEGATIVE", "UNRESOLVED_REFERENCE", "INVALID"]
      .includes(item.referenceDisposition), `R9 item result ${index + 1} disposition is invalid`);
  ...
  `R9 item result ${index + 1} reference surface and findings conflict`);
  requireCondition(item.referenceDisposition !== "RESOLVED_NEGATIVE" || referenceFindings.length === 0,
    `R9 item result ${index + 1} resolved-negative reference contains findings`);
  ...
  `R9 item result ${index + 1} machine resolution, surface, and findings conflict`);
...
function buildObservedLedger(input) {
  requireCondition(Array.isArray(input.itemResults) && input.itemResults.length === EXPECTED_ITEM_COUNT,
    "R9 statistical analysis requires exactly 60 sealed item results, including missing-receipt rows");
  ...
  "R9 item hashes, pseudonyms, and homology clusters must each be unique across all 60 rows");
  ...
  for (const record of validated) {
    ...
    if (analysisResolved && item.referenceDisposition === "RESOLVED_POSITIVE") {
      matching = deterministicFindingMatchesV5R9(referenceFindings, machineFindings);
    } else if (analysisResolved && item.referenceDisposition === "RESOLVED_NEGATIVE") {
  ...
  requireCondition(accounting.completeReceiptItemCount + accounting.missingReceiptItemCount
    === EXPECTED_ITEM_COUNT, "R9 complete and missing receipt accounting does not equal 60");
  ...
  "R9 complete item accounting is not a disjoint partition");
  ...
  return assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact(body),
...
function perItemContribution(row, metric) {
  ...
  throw new TypeError(`R9 unsupported bootstrap metric ${metric}`);
}
...
function bootstrapBounds(observed, metric, direction) {
  ...
  for (let replicate = 0; replicate < BOOTSTRAP_REPLICATES; replicate += 1) {
    ...
    for (let draw = 0; draw < contributions.length; draw += 1) {
      const selected = contributions[bootstrapBoundedIndexV5R9({
...
function exactSurfaceWorldBounds(counterfactual, metric) {
  ...
  const bounds = counterfactual.surfaceWorlds.map(({ confusionMatrix }) => {
    ...
    return denominator === 0 ? { lower: 0, upper: 1 }
      : wilsonIntervalV5R9(numerator, denominator, ONE_SIDED_Z95);
  });
...
function proportionMetric(metric, numerator, denominator, clusterCount, observed, counterfactual) {
  const rule = METRIC_RULES[metric];
  const oneSided = wilsonIntervalV5R9(numerator, denominator, ONE_SIDED_Z95);
  const twoSided = wilsonIntervalV5R9(numerator, denominator, TWO_SIDED_Z95);
  ...
  if (counterfactual.integrityLimitExceeded) {
    ...
    const adverse = wilsonIntervalV5R9(adverseNumerator, adverseDenominator, ONE_SIDED_Z95);
...
function recomputeMetrics(observed, counterfactual) {
  ...
  return Object.freeze([
    ...
    Object.freeze({
      ...
      oneSidedWilsonMissRateLcb95: wilsonIntervalV5R9(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, ONE_SIDED_Z95).lower,
      oneSidedWilsonMissRateUcb95: wilsonIntervalV5R9(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, ONE_SIDED_Z95).upper,
      twoSidedWilsonMissRateL95: wilsonIntervalV5R9(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, TWO_SIDED_Z95).lower,
      twoSidedWilsonMissRateU95: wilsonIntervalV5R9(counts.p0FalseNegativeItemCount,
...
function validateInputChronology(input) {
  ...
  "R9 statistical chronology requires explicit timestamps");
  for (const field of ["receiptChainValid", "providerTupleValid", "capsValid",
    ...
    requireCondition(typeof input[field] === "boolean", `R9 ${field} must be Boolean evidence`);
  }
}

export function buildFrozenStatisticalBundleV5R9(input) {
  assertGoldenVectors();
  ...
  const metricInputLedger = assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
  ...
  const finalReceipt = assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
    ...
    statisticalKernel: {
      methodVersion: "MAIS-NATURAL-CA60-V5-R9_NATIVE_FROZEN_STATISTICAL_KERNEL",
      ...
      bootstrapGoldenVectors: BOOTSTRAP_GOLDEN_VECTORS_V5_R9,
...
export function verifyFrozenStatisticalBundleV5R9(input, bundle) {
  const errors = [];
  try {
    const rebuilt = buildFrozenStatisticalBundleV5R9(input);
    for (const field of ["itemResults", "completedItemMarkers", "observedLedger",
      ...
      if (canonicalJsonV5R3(rebuilt[field]) !== canonicalJsonV5R3(bundle?.[field])) {
        errors.push(`R9 ${field} differs from independent frozen-method reconstruction`);
      }
    }
  } catch (error) {
...
function terminalReceiptBody(input) {
  ...
  "R9 terminal cause codes are required");
  ...
  requireCondition(integrity.status === "FAILED",
    "R9 terminal integrity receipt requires independently failed execution integrity");
  ...
  for (const field of ["attemptGraphReceiptHash", "ledgerEntryRootHash",
    ...
    requireCondition(HASH.test(input[field] ?? ""), `R9 terminal receipt ${field} is absent`);
  }
  const completedItemMarkers = input.itemResults.filter(({ executionDisposition }) =>
    executionDisposition === "COMPLETE").map(({ manifestOrdinal, itemHash, completedItemMarkerHash }) => ({
...
export function buildTerminalExecutionDecisionReceiptV5R9(input) {
  return assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact(terminalReceiptBody(input)),
    "TerminalExecutionDecisionReceiptV2");
}

export function verifyTerminalExecutionDecisionReceiptV5R9({ receipt, ...input }) {
  const errors = [];
  try {
    requireCondition(validateSelfHashV5R3(receipt), "R9 terminal receipt self-hash is invalid");
    const rebuilt = buildTerminalExecutionDecisionReceiptV5R9(input);
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(receipt)) {
      errors.push("R9 terminal decision receipt differs from immutable item and integrity evidence reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const STATISTICAL_KERNEL_V5_R9_CONSTANTS = Object.freeze({
