...
import {
  parseExactArgumentsV5R9,
  runCliV5R9,
  V5_R9_CLI_COMMANDS,
} from "./runner-v5-r9-cli.mjs";
import {
  sha256V5R3,
...
test("R9 public CLI exposes the complete registered workflow and V8 command receipts", async () => {
  assert.deepEqual([...V5_R9_CLI_COMMANDS], EXPECTED_COMMANDS);
  assert.deepEqual(parseExactArgumentsV5R9([
  ...
  let adopted = 0;
  const result = await runCliV5R9(["register", "--context", "/protected/index.json"], {
    loadWorkflowContext: async () => context(),
    adoptR9WorkflowIndex: async () => { adopted += 1; return { ok: true,
      status: "V5_R9_WORKFLOW_INDEX_ADOPTED_NO_PROVIDER_ACTIVITY",
  ...
  assert.deepEqual(validateClosedSelfHashedArtifactV5R9(result.receipt,
    "NaturalCaRunnerCommandReceiptV8"), []);
});

test("R9 CLI rejects ambiguous provider flags before loading protected state", async () => {
  let loads = 0;
  ...
  for (const argv of [
    ...
    const result = await runCliV5R9(argv, deps);
...
test("R9 public default recovery command reaches its exact loader and zero-HTTP engine", async () => {
  const current = context();
  const recovery = { fixture: true };
  let loads = 0;
  let reconciliations = 0;
  const result = await runCliV5R9([
