...
import {
  validateRawAuthoritativeMachineReferenceSealV5R10,
  validateRawAuthoritativeReferenceValidationV5R10,
...
function validateCostPreview(input, errors) {
  ...
  errors.push(...validateClosedSelfHashedArtifactV5R10(preview, "ProviderCostPreviewReceiptV1"));
  ...
  "R10 cost preview is not an exact signed-route price reconstruction with a 20 percent buffer");
}

export function validateProviderActivationV5R10(input) {
  const errors = [
    ...validateExactRunnerRegistrationEvidenceV5R10(input?.registrationEvidence),
    ...validateActiveRunnerRegistrationV5R10(input?.activeRegistration),
    ...validateFreshRunnerReviewV5R10(input ?? {}),
    ...validateResolvedProviderAuthorizationV5R6(input?.authorization),
    ...validateClosedSelfHashedArtifactV5R10(input?.ownerActivationGrant,
      "OwnerRunnerActivationGrantV1"),
    ...validateClosedSelfHashedArtifactV5R10(input?.priceSnapshot, "ProviderPriceSnapshotV2"),
    ...validateResumeCustodyV5R10(input?.attemptCustody ?? {}),
  ];
  try {
    errors.push(...validateTrustedProviderEvidenceEnvelopeV5R10({
  ...
  add(errors, evidence?.activeRegistration?.selfHash === registration?.selfHash,
    "R10 exact registration evidence does not bind the supplied active registration");
  add(errors, envelope && authorization?.model === envelope.model
    && authorization?.endpoint === envelope.endpoint,
  "R10 authorization provider, model, or endpoint differs from the frozen direct route");
  ...
  "R10 authorization does not bind the exact registration, review, signed route, and compatibility base");
  for (const field of ["provider", "model", "endpoint", "projectResidency", "dataRegion"]) {
    add(errors, authorization?.[field] === route?.[field],
    `R10 authorization ${field} differs from signed route evidence`);
  }
  add(errors, authorization?.projectIdentityHash === route?.subjectIdentityHash,
    "R10 authorization subject differs from signed route evidence");
  ...
  "R10 OpenAI authorization is not bound to US_STORAGE_PROCESSING and US data region");
  add(errors, !/UNKNOWN|PENDING|UNRESOLVED/iu.test(
    `${authorization?.projectResidency}:${authorization?.dataRegion}`),
  "R10 provider residency and data region remain unresolved");
  ...
  "R10 provider authorization differs from the frozen attempt, token, USD, or concurrency envelope");
  add(errors, allLiveAuthorityFlags(authorization) && allLiveAuthorityFlags(grant),
    "R10 provider authorization or owner activation grant lacks explicit live authority flags");
  ...
  "R10 owner grant does not bind the exact registration, review, route, cost, authorization, and inventory");
  ...
  "R10 owner grant tuple differs from the signed resolved authorization");
  ...
  "R10 owner grant caps, currency, or non-authorizations differ from the frozen envelope");
  ...
  "R10 compatibility ledger authorization, signed price, or inventory binding is invalid");
  ...
  "R10 authorization or grant chronology is invalid or expired");

  if (authorization?.provider === "DEEPSEEK_DIRECT") {
    errors.push(...validateRawAuthoritativeMachineReferenceSealV5R10({
      ...(input?.referenceSealContext ?? {}), seal: input?.referenceSeal,
      compatibilityReferenceSeal: input?.compatibilityReferenceSeal,
    }));
    errors.push(...validateRawAuthoritativeReferenceValidationV5R10({
    ...
    errors.push(...validateClosedSelfHashedArtifactV5R10(input?.executionRegistration,
    ...
    "R10 DeepSeek execution registration is not bound to the sealed reference and exact live tuple");
  }
  return Object.freeze([...new Set(errors)]);
}

export function assertProviderActivationV5R10(input) {
  const errors = validateProviderActivationV5R10(input);
  if (errors.length > 0) throw new TypeError(errors.join("; "));
  return input.authorization;
}

export const ACTIVATION_GUARD_V5_R10_CONSTANTS = Object.freeze({
