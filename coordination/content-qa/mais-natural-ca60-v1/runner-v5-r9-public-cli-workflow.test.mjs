  ...
  return {
    ...
    deps: {
      ...
      score: async () => {
        ...
        state.scoreBundle = buildFrozenStatisticalBundleV5R9(data.statisticalInput);
      ...
      verify: async () => {
        ...
        const rebuilt = buildFrozenStatisticalBundleV5R9(data.statisticalInput);
        const errors = verifyFrozenStatisticalBundleV5R9(data.statisticalInput, state.scoreBundle);
        if (errors.length > 0
...
async function command(harness, argv) {
  const result = await runCliV5R9([...argv.slice(0, 1), "--context", CONTEXT_PATH, ...argv.slice(1)],
    harness.deps);
...
test("R9 public CLI completes the entire offline fixture workflow through normal C0 and complete scoring", async () => {
