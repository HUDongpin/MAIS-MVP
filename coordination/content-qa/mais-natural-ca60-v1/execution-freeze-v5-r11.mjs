...
export function buildDeepSeekExecutionRegistrationV5R11(input) {
  ...
  "R11 DeepSeek authorization/validation does not bind the raw-authoritative reference seal");
  ...
  "R11 reference/authorization/execution-registration chronology is invalid");
  ...
  assertClosedSelfHashedArtifactV5R11(receipt, "DeepSeekExecutionRegistrationV4");
  return receipt;
}

export function validateDeepSeekExecutionRegistrationV5R11({ executionRegistration, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R11(executionRegistration,
    "DeepSeekExecutionRegistrationV4")];
  try {
    const rebuilt = buildDeepSeekExecutionRegistrationV5R11({
      ...input,
      registeredAt: executionRegistration?.registeredAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(executionRegistration)) {
      errors.push("R11 DeepSeek execution registration differs from raw-authoritative reconstruction");
    }
  } catch (error) {
