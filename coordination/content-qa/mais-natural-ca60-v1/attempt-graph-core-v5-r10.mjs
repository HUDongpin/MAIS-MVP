...
import {
  validateProviderAttemptCommitIntentV5R10,
  validateResolvedProviderAttemptReceiptV5R10,
...
import {
  buildCompatibilityCanaryC0DecisionSetV5R10,
...
export function reconstructAttemptGraphCoreV5R10(input) {
  const errors = [];
  ...
  for (const reservation of reservations) {
    ...
    const schemaErrors = [
      ...validateClosedSelfHashedArtifactV5R10(request, "ProviderRequestArtifactV5"),
      ...validateClosedSelfHashedArtifactV5R10(audit, audit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R10(permit, permit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R10(compatibilityPermit, "ProviderDispatchPermitV3"),
      ...validateRawResponseArtifactV5R6(raw),
      ...validateRawResponseBindingReceiptV5R6(binding),
      ...validateClosedSelfHashedArtifactV5R10(event, "ProviderEventReceiptV4"),
      ...validateClosedSelfHashedArtifactV5R10(completion, "ProviderDispatchCompletionV2"),
      ...validateClosedSelfHashedArtifactV5R10(intent, "ProviderAttemptCommitIntentV1"),
      ...validateClosedSelfHashedArtifactV5R10(resolved, "ResolvedProviderAttemptReceiptV2"),
    ];
    if (roleOutput) schemaErrors.push(...validateClosedSelfHashedArtifactV5R10(roleOutput, "ProviderRoleOutputV1"));
    ...
    const runtimeState = {
      ...
      c0ExecutionSet: audit.c0ExecutionSetHash ? input.c0ExecutionSet
        : audit.executionPlan?.mode === "DEEPSEEK_CANARY" && audit.canaryPredicateReceiptHash
          ? buildCompatibilityCanaryC0DecisionSetV5R10(input.canaryPredicateReceipt) : null,
    ...
    try {
      if (permit.schemaVersion === "ProviderDispatchPermitV5") {
        permitErrors.push(...validateProviderDispatchPermitV5R10({
          permit,
    ...
    if (input.authorization.provider === "DEEPSEEK_DIRECT") {
      ...
      if (!semanticAuthority) {
        ...
        semanticErrors.push(...validateSemanticDispatchAuthorityV5R10({
          ...runtimeState,
    ...
    const intentErrors = validateProviderAttemptCommitIntentV5R10({ ...common, intent });
    const resolvedErrors = validateResolvedProviderAttemptReceiptV5R10({ ...common,
...
export function validateAttemptGraphCoreReceiptV5R10({ attemptGraphReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV1")];
  try {
    const rebuilt = reconstructAttemptGraphCoreV5R10({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R10 attempt graph differs from exact ledger/request/raw/binding/intent/completion reconstruction");
    }
  } catch (error) {
...
export function roleOutputByEvidenceV5R10({ roleAttemptEvidenceReceipts, ledgerEntries }) {
  ...
  return roleAttemptEvidenceReceipts.map((receipt) => {
    const errors = validateClosedSelfHashedArtifactV5R10(receipt, "RoleAttemptEvidenceReceiptV1");
    ...
    if (errors.length > 0 || !output || output.attemptId !== receipt.attemptId
      ...
      throw new TypeError(`R10 role evidence cannot resolve its exact compatibility role output: ${errors.join("; ")}`);
    }
    return output;
  });
}

export const ATTEMPT_GRAPH_CORE_V5_R10_CONSTANTS = Object.freeze({
