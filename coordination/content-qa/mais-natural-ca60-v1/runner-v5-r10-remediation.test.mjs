...
import {
  createRunnerRuntimeV5R10,
  persistFinalCommandJournalV5R10,
  V5_R10_RUNTIME_PATHS,
} from "./runner-v5-r10-runtime.mjs";
import {
  reloadCommandJournalSidecarsV5R10,
...
const H = (value) => sha256V5R3(`v5-r10:${value}`);
...
test("R10 accepts the same local findingId on different items using the frozen composite key", () => {
  ...
  const built = buildFrozenStatisticalBundleV5R10(statisticalInput(rows));
...
test("R10 rejects an orphan DISPATCH_COMPLETED ledger row before any resume", () => {
  ...
  assert.ok(validateResumeCustodyV5R10({ ledgerEntries: [completion], preparedCompletions: [],
    attemptCommitIntents: [], resolvedAttemptReceipts: [] })
    .some((error) => /orphan ledger completion/iu.test(error)));
});

test("R10 custody requires retained prepared-completion bytes for a durable intent", () => {
  ...
  assert.throws(() => inspectInterruptedAttemptCustodyV5R10({ ledgerEntries: [reservation],
...
test("R10 decision evidence ignores caller-authored favorable or adverse integrity flags", () => {
  const base = emptyDecisionInput();
  const favorable = reconstructDecisionEvidenceV5R10({ ...base, receiptChainValid: true,
  ...
  const adverse = reconstructDecisionEvidenceV5R10({ ...base, receiptChainValid: false,
...
test("R10 self-hashed but schema-invalid authorizations cannot manufacture provider or cap validity", () => {
  const base = emptyDecisionInput();
  ...
  const receipt = reconstructDecisionEvidenceV5R10({ ...base, authorizations });
...
test("R10 unknown command-journal accounting forces terminal fail-closed decision evidence", () => {
  const base = emptyDecisionInput();
  ...
  const receipt = reconstructDecisionEvidenceV5R10({ ...base, commandJournals: [journal] });
  assert.equal(receipt.receiptChainValid, false);
  assert.equal(receipt.capsValid, false);
...
test("R10 failed or successful final command journals share the same required workflow-index custody path", async () => {
  ...
  const calls = [];
  const transition = await persistFinalCommandJournalV5R10({
    context,
    journal,
...
test("R10 reloads a valid protected command-journal sidecar that was not yet indexed", async (t) => {
  ...
  await atomicWriteProtectedJsonV5R4({ trustedRoot,
    relativePath: path.join("runtime-custody-v5-r10", "transition-journals",
      `${journal.selfHash}.json`),
    value: journal });
  const reloaded = await reloadCommandJournalSidecarsV5R10({ trustedRoot, activeRegistration });
  assert.equal(reloaded.length, 1);
  assert.deepEqual(reloaded[0], journal);
});

test("R10 route validation rejects a signed-summary-shaped envelope without raw source and probe custody", () => {
  const activeRegistration = { selfHash: H("route-registration"), trustedProviderEvidenceAnchors: [] };
  const errors = validateTrustedProviderEvidenceEnvelopeV5R10({ envelope: null,
...
test("R10 final adapter reloads immutable Git custody before credential access", async () => {
  let credentialReads = 0;
  let fetches = 0;
  const adapter = createNativeProviderAdapterV5R10({
  ...
  /V5-R10 registration path|Git|registration|A07 closeout|review/iu);
  assert.equal(credentialReads, 0);
  assert.equal(fetches, 0);
});

test("R10 default runtime installs the native workflow and blocks absent authority at exact zero activity", async () => {
  const runtime = createRunnerRuntimeV5R10({ clock: () => "2026-08-26T15:00:00.000Z" });
  assert.equal(V5_R10_RUNTIME_PATHS.defaultNativePlannerExecutorInstalled, true);
  assert.equal(V5_R10_RUNTIME_PATHS.defaultRecoveryLoaderInstalled, true);
...
test("R10 runtime has no caller-authored scoring or all-missing terminal fallback", async () => {
  const source = await readFile(new URL("./runner-v5-r10-runtime.mjs", import.meta.url), "utf8");
  const scorerSource = await readFile(new URL("./scorer-verifier-v5-r10.mjs", import.meta.url), "utf8");
  assert.doesNotMatch(source, /scoringInputBuilder|terminalEvidenceBuilder|buildTerminalMissingItemResultsV5R10/u);
  assert.doesNotMatch(source, /naturalQuestionEgressCount:\s*run\.providerEventCount/u);
  assert.doesNotMatch(scorerSource, /buildTerminalMissingItemResultsV5R10/u);
  assert.match(source, /buildTerminalExecutionBundleFromRawCustodyV5R10/u);
  assert.match(scorerSource, /buildTerminalItemEvaluationEvidenceV5R10/u);
