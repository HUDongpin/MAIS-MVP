...
function commandReceipt(command, status, context, output = {}) {
  ...
  return assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
    schemaVersion: "NaturalCaRunnerCommandReceiptV7",
    runnerVersion: "V5-R8",
...
export function parseExactArgumentsV5R8(argv) {
...
export async function runCliV5R8(argv = [], deps = createRunnerRuntimeV5R8()) {
  const parsed = parseExactArgumentsV5R8(argv);
  ...
  let engine;
  let args = [context];
  if (parsed.command === "register") engine = "adoptR8WorkflowIndex";
...
async function main() {
  const result = await runCliV5R8(process.argv.slice(2));
...
export const V5_R8_CLI_COMMANDS = Object.freeze([...COMMANDS]);
