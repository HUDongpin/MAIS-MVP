...
import {
  loadExactFreshRunnerReviewV5R10,
...
export const ACTIVE_REVIEW_KIND_V5_R10 = "FRESH_RUNNER_REVIEW_V5_R10";
export const ACTIVE_REGISTRATION_KIND_V5_R10 = "ACTIVE_RUNNER_REGISTRATION_V5_R10";
...
export function executionLedgerRelativePathV5R10(provider, compatibilityAuthorizationHash) {
  requireCondition(["OPENAI_DIRECT", "DEEPSEEK_DIRECT"].includes(provider)
    && /^[0-9a-f]{64}$/u.test(compatibilityAuthorizationHash ?? ""),
  "R10 execution-ledger path requires an exact provider and compatibility authorization hash");
  return path.join("execution-ledgers-v5-r10", provider === "OPENAI_DIRECT"
...
export async function reloadCommandJournalSidecarsV5R10(context) {
  requireCondition(context?.trustedRoot?.root && context?.activeRegistration?.selfHash,
    "R10 command-journal sidecar reload requires a trusted root and active registration");
  const relativeDirectory = path.join("runtime-custody-v5-r10", "transition-journals");
  ...
  try {
    ...
    requireCondition(!metadata.isSymbolicLink() && metadata.isDirectory(),
      "R10 command-journal sidecar root must be a real directory");
  ...
  requireCondition(names.every((name) => /^[0-9a-f]{64}\.json$/u.test(name)),
    "R10 command-journal sidecar root contains an unexpected entry");
  const journals = [];
  for (const name of [...names].sort()) {
    ...
    const errors = validateClosedSelfHashedArtifactV5R10(value,
    ...
    `R10 command-journal sidecar is invalid or registration-drifted: ${errors.join("; ")}`);
    journals.push(value);
  }
  requireCondition(new Set(journals.map(({ selfHash }) => selfHash)).size === journals.length,
    "R10 command-journal sidecar set contains duplicate hashes");
  return Object.freeze(journals);
}

export async function reconstructAttemptCustodyByProviderV5R10(context) {
  const result = {};
  ...
  for (const provider of ["OPENAI_DIRECT", "DEEPSEEK_DIRECT"]) {
    ...
    const authorization = context.artifacts.get(`${prefix}_AUTHORIZATION_V5_R10`);
    if (!authorization) {
      const orphans = [...allIntents, ...allResolved].filter((value) => value.provider === provider
        && value.activeRunnerRegistrationHash === context.activeRegistration.selfHash);
      requireCondition(orphans.length === 0,
        `R10 ${provider} attempt artifacts exist without the exact active authorization`);
    ...
    `R10 ${provider} active authorization is invalid or registration-drifted`);
    const priceSnapshot = context.artifacts.get(`${prefix}_PRICE_SNAPSHOT_V5_R10`);
    requireCondition(validateSelfHashV5R3(priceSnapshot) && validateSelfHashV5R3(inventory),
      `R10 ${provider} ledger cannot reload without sealed price and inventory artifacts`);
    const ledger = await createAtomicExecutionLedgerV5R7({
      trustedRoot: context.trustedRoot,
      ledgerRelativePath: executionLedgerRelativePathV5R10(provider,
    ...
    requireCondition(verified.errors.length === 0,
      `R10 ${provider} ledger reload failed: ${verified.errors.join("; ")}`);
    ...
    requireCondition(reloaded.orphanDirectoryHashes.length === 0,
      `R10 ${provider} attempt custody contains directories without ledger reservations`);
    const mergeExact = (left, right, label) => {
      const byHash = new Map();
      for (const value of [...left, ...right]) {
        requireCondition(validateSelfHashV5R3(value),
          `R10 ${provider} ${label} merge contains an invalid self-hashed artifact`);
        ...
        `R10 ${provider} ${label} merge contains different canonical bytes for one self-hash`);
    ...
    result[provider] = Object.freeze({ ...custody,
      custodyErrors: validateResumeCustodyV5R10(custody) });
  }
  return Object.freeze(result);
}

async function persistContentAddressedV5R10({ trustedRoot, family, value }) {
  requireCondition(/^[a-z0-9][a-z0-9-]*$/u.test(family) && validateSelfHashV5R3(value),
    "R10 protected artifact family or self-hash is invalid");
  const relativePath = path.join("derived-v5-r10", family, `${value.selfHash}.json`);
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R10 content-addressed artifact path contains different canonical bytes");
  } catch (error) {
...
export async function advanceProtectedWorkflowIndexV5R10({ context, command, appendedArtifacts,
  committedAt }) {
  ...
  "loaded R10 workflow context and at least one append-only artifact are required");
  const persisted = [];
  for (const entry of appendedArtifacts) {
    const relativePath = await persistContentAddressedV5R10({ trustedRoot: context.trustedRoot,
  ...
  const nextIndexRelativePath = path.join("workflow-indexes-v5-r10", `${nextIndex.selfHash}.json`);
  ...
  const transitionRelativePath = await persistContentAddressedV5R10({
...
export function resolveActiveRunnerReviewV5R10(artifactMap) {
  const review = artifactMap?.get?.(ACTIVE_REVIEW_KIND_V5_R10);
  requireCondition(review !== undefined,
    `protected workflow is missing ${ACTIVE_REVIEW_KIND_V5_R10}`);
  return review;
}

export async function loadProtectedWorkflowIndexV5R10(indexPath, { protectedRoot, repoRoot,
  registrationEvidenceLoader = () => loadExactTrackedRunnerRegistrationV5R10({ repoRoot }),
  freshReviewLoader = ({ registrationEvidence }) => loadExactFreshRunnerReviewV5R10({
  ...
  const evidenceErrors = validateExactRunnerRegistrationEvidenceV5R10(registrationEvidence);
  requireCondition(evidenceErrors.length === 0,
    `exact R10 runner registration evidence failed: ${evidenceErrors.join("; ")}`);
  ...
  if (indexBoundToR9) {
    requireCondition(compatibilityContext.artifacts.get(ACTIVE_REGISTRATION_KIND_V5_R10)?.selfHash
      === activeRegistration.selfHash
      && compatibilityContext.artifacts.get(ACTIVE_REVIEW_KIND_V5_R10)?.selfHash
        === exactReview.freshReview.selfHash,
    "active R10 workflow index lacks the exact append-only registration and review artifacts");
  }
  const context = {
  ...
  const sidecarJournals = await reloadCommandJournalSidecarsV5R10(context);
  for (const journal of sidecarJournals) {
    ...
    "R10 indexed command journal differs from its protected sidecar bytes");
    if (!indexed) context.artifacts.set(
      `RUNTIME_COMMAND_TRANSITION_JOURNAL_V5_R10:${journal.selfHash}`, journal);
  }
  context.attemptCustodyByProvider = await reconstructAttemptCustodyByProviderV5R10(context);
  return context;
}

export const WORKFLOW_INDEX_V5_R10_CONSTANTS = Object.freeze({
  activeReviewKind: ACTIVE_REVIEW_KIND_V5_R10,
  activeRegistrationKind: ACTIVE_REGISTRATION_KIND_V5_R10,
  ...
  indexRoot: "workflow-indexes-v5-r10",
