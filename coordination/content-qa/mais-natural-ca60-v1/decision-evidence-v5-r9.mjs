...
function authorizationByHash(authorizations) {
  ...
  for (const authorization of authorizations ?? []) {
    requireCondition(validateSelfHashV5R3(authorization),
      "R9 decision evidence contains an unsealed provider authorization");
    requireCondition(!map.has(authorization.selfHash),
      "R9 decision evidence contains duplicate provider authorization hashes");
...
function expectedDeepSeekRoleKeys(inventory, c0ExecutionSet) {
  requireCondition(Array.isArray(inventory?.items) && inventory.items.length === 60,
    "R9 decision reconstruction requires the exact 60-item inventory");
...
export function reconstructDecisionEvidenceV5R9(input) {
  ...
  "R9 decision reconstruction requires sealed registration, graph, inventory, and C0 roots");
  ...
  const authorizationSchemasValid = authorizations.every((value) =>
    validateClosedSelfHashedArtifactV5R9(value, "ProviderAuthorizationV5").length === 0
  ...
  const requestSchemasValid = requests.every((value) =>
    validateClosedSelfHashedArtifactV5R9(value, "ProviderRequestArtifactV5").length === 0);
  const resolvedSchemasValid = resolved.every((value) =>
    validateClosedSelfHashedArtifactV5R9(value, "ResolvedProviderAttemptReceiptV2").length === 0);
  const journalSchemasValid = journals.every((value) =>
    validateClosedSelfHashedArtifactV5R9(value, "CommandTransitionJournalReceiptV2").length === 0
      && value.activeRunnerRegistrationHash === input.activeRegistration.selfHash);
...
export function statisticalDecisionFlagsFromEvidenceV5R9(receipt) {
  requireCondition(validateSelfHashV5R3(receipt)
    && receipt.schemaVersion === "DecisionEvidenceReceiptV1",
  "R9 statistical decision facts require a sealed reconstructed decision-evidence receipt");
...
export const DECISION_EVIDENCE_V5_R9_CONSTANTS = Object.freeze({
