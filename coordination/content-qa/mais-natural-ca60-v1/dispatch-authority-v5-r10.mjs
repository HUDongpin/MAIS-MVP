...
export function buildReferenceDispatchAuthorityV5R10(input) {
  ...
  `R10 reference authority upstream validation failed: ${errors.join("; ")}`);
  ...
  requireCondition(plan.planStatus === "NEXT_ACTION", `R10 reference planner is ${plan.planStatus}`);
  const item = input.inventory.items[plan.manifestOrdinal - 1];
  requireCondition(item?.itemHash === plan.itemHash, "R10 reference plan is outside the frozen inventory");
  ...
  const receipt = sealV5R3Artifact({
    ...
    attemptOrdinal,
    attemptId: `V5R10O:${input.activeRegistration.selfHash}:${plan.selfHash}:${attemptOrdinal}`,
...
export function validateReferenceDispatchAuthorityV5R10({ referenceDispatchAuthority, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(referenceDispatchAuthority,
    "ReferenceDispatchAuthorityReceiptV1")];
  try {
    const rebuilt = buildReferenceDispatchAuthorityV5R10({
      ...input,
      issuedAt: referenceDispatchAuthority?.issuedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(referenceDispatchAuthority)) {
      errors.push("R10 reference authority differs from exact planner reconstruction");
    }
  } catch (error) {
...
export function validateDispatchAuthorityV5R10(input) {
...
export function buildProviderDispatchPermitV5R10({ input, reservation }) {
  const authority = input.dispatchAuthority;
  const errors = validateDispatchAuthorityV5R10(input);
  ...
  `R10 dispatch authority/request binding is invalid: ${errors.join("; ")}`);
  const receipt = sealV5R3Artifact({
...
export function validateProviderDispatchPermitV5R10({ permit, input, reservation }) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(permit, "ProviderDispatchPermitV5")];
  try {
    const rebuilt = buildProviderDispatchPermitV5R10({
      input: { ...input, at: permit?.issuedAt },
      reservation,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(permit)) {
      errors.push("R10 dispatch permit differs from exact authority/request/reservation reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
