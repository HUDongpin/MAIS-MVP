...
import {
  ...
  buildReconciliationAuthorizationV5R10,
  inspectInterruptedAttemptCustodyV5R10,
  reconcileInterruptedAttemptV5R10,
...
function custodyFixture(state) {
  ...
  const recoveryAuthorization = buildReconciliationAuthorizationV5R10({
    activeRunnerRegistrationHash: ACTIVE,
...
test("R10 classifies all four registered crash-injection custody states deterministically", () => {
  const cases = [
  ...
  for (const [injectionPoint, expected] of cases) {
    ...
    assert.equal(inspectInterruptedAttemptCustodyV5R10(input).state, expected);
    assert.equal(validateResumeCustodyV5R10(input).length, expected === "FULLY_RECONCILED" ? 0 : 1);
  }
});

test("R10 reservation-only recovery preserves worst-case accounting and cannot authorize resume", () => {
  const input = custodyFixture("AFTER_RESERVATION");
  const recovered = reconcileInterruptedAttemptV5R10(input);
...
test("R10 intent-only recovery appends the exact prepared completion and resolved receipt with zero HTTP", async () => {
  const input = custodyFixture("AFTER_DURABLE_INTENT");
  const calls = [];
  const result = await applyInterruptedAttemptReconciliationV5R10({
    ...input,
    ledger: {
...
test("R10 completion-only recovery appends no second ledger completion", async () => {
...
test("R10 fully reconciled recovery is idempotent apart from its append-only reconciliation receipt", async () => {
...
test("R10 recovery rejects transport or credential dependencies before any action", async () => {
  const input = custodyFixture("AFTER_DURABLE_INTENT");
  await assert.rejects(() => applyInterruptedAttemptReconciliationV5R10({
    ...input,
