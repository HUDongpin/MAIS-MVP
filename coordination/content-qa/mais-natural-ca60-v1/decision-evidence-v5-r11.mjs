...
function authorizationByHash(authorizations) {
  ...
  for (const authorization of authorizations ?? []) {
    requireCondition(validateSelfHashV5R3(authorization),
      "R11 decision evidence contains an unsealed provider authorization");
    requireCondition(!map.has(authorization.selfHash),
      "R11 decision evidence contains duplicate provider authorization hashes");
...
function expectedDeepSeekRoleKeys(inventory, c0ExecutionSet) {
  requireCondition(Array.isArray(inventory?.items) && inventory.items.length === 60,
    "R11 decision reconstruction requires the exact 60-item inventory");
...
export function reconstructDecisionEvidenceV5R11(input) {
  ...
  "R11 decision reconstruction requires sealed registration, graph, inventory, and C0 roots");
  ...
  const authorizationSchemasValid = authorizations.every((value) =>
    validateClosedSelfHashedArtifactV5R11(value, "ProviderAuthorizationV5").length === 0
  ...
  const requestSchemasValid = requests.every((value) =>
    validateClosedSelfHashedArtifactV5R11(value, "ProviderRequestArtifactV5").length === 0);
  const resolvedSchemasValid = resolved.every((value) =>
    validateClosedSelfHashedArtifactV5R11(value, "ResolvedProviderAttemptReceiptV2").length === 0);
  const journalSchemasValid = journals.every((value) =>
    validateClosedSelfHashedArtifactV5R11(value, "CommandTransitionJournalReceiptV2").length === 0
      && value.activeRunnerRegistrationHash === input.activeRegistration.selfHash);
...
export function statisticalDecisionFlagsFromEvidenceV5R11(receipt) {
  requireCondition(validateSelfHashV5R3(receipt)
    && receipt.schemaVersion === "DecisionEvidenceReceiptV1",
  "R11 statistical decision facts require a sealed reconstructed decision-evidence receipt");
...
export const DECISION_EVIDENCE_V5_R11_CONSTANTS = Object.freeze({
