...
const H = (value) => sha256V5R3(`r11-public-cli-handler-contract-fixture:${value}`);
...
function createOfflinePublicCliHarness() {
  ...
  return {
    ...
    deps: {
      ...
      adoptPredecessorWorkflowIndex: async () => {
        state.adopted = true;
        return ok("OFFLINE_FIXTURE_PREDECESSOR_INDEX_ADOPTED");
      },
      verifyFrozenUpstream: async () => state.adopted
        ? ok("OFFLINE_FIXTURE_FROZEN_UPSTREAM_VERIFIED")
        : blocked("OFFLINE_FIXTURE_PREDECESSOR_INDEX_NOT_ADOPTED"),
      ...
      score: async () => {
        ...
        state.scoreBundle = buildFrozenStatisticalBundleV5R11(data.statisticalInput);
      ...
      verify: async () => {
        ...
        const rebuilt = buildFrozenStatisticalBundleV5R11(data.statisticalInput);
        const errors = verifyFrozenStatisticalBundleV5R11(data.statisticalInput, state.scoreBundle);
        if (errors.length > 0
...
async function command(harness, argv) {
  const result = await runCliV5R11([...argv.slice(0, 1), "--context", CONTEXT_PATH, ...argv.slice(1)],
    harness.deps);
  assert.equal(result.exitCode, 0, `${argv.join(" ")}: ${result.receipt.status}`);
  assert.deepEqual(validateClosedSelfHashedArtifactV5R11(result.receipt,
...
test("R11 public CLI handler contract completes the offline orchestration fixture through scoring", async () => {
