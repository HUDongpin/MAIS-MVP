...
import {
  validateProviderAttemptCommitIntentV5R11,
  validateResolvedProviderAttemptReceiptV5R11,
...
import {
  buildCompatibilityCanaryC0DecisionSetV5R11,
  validateSemanticDispatchAuthorityV5R11,
} from "./semantic-dispatch-v5-r11.mjs";
import {
  buildProviderDispatchPermitV5R11,
  validateProviderDispatchPermitV5R11,
...
export function reconstructAttemptGraphCoreV5R11(input) {
  const errors = [];
  ...
  for (const reservation of reservations) {
    ...
    const schemaErrors = [
      ...validateClosedSelfHashedArtifactV5R11(request, "ProviderRequestArtifactV5"),
      ...validateClosedSelfHashedArtifactV5R11(audit, audit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R11(permit, permit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R11(compatibilityPermit, "ProviderDispatchPermitV3"),
      ...validateRawResponseArtifactV5R6(raw),
      ...validateRawResponseBindingReceiptV5R6(binding),
      ...validateClosedSelfHashedArtifactV5R11(event, "ProviderEventReceiptV4"),
      ...validateClosedSelfHashedArtifactV5R11(completion, "ProviderDispatchCompletionV2"),
      ...validateClosedSelfHashedArtifactV5R11(intent, "ProviderAttemptCommitIntentV1"),
      ...validateClosedSelfHashedArtifactV5R11(resolved, "ResolvedProviderAttemptReceiptV2"),
    ];
    if (roleOutput) schemaErrors.push(...validateClosedSelfHashedArtifactV5R11(roleOutput, "ProviderRoleOutputV1"));
    ...
    const runtimeState = {
      ...
      c0ExecutionSet: audit.c0ExecutionSetHash ? input.c0ExecutionSet
        : audit.executionPlan?.mode === "DEEPSEEK_CANARY" && audit.canaryPredicateReceiptHash
          ? buildCompatibilityCanaryC0DecisionSetV5R11(input.canaryPredicateReceipt) : null,
    ...
    try {
      if (permit.schemaVersion === "ProviderDispatchPermitV5") {
        permitErrors.push(...validateProviderDispatchPermitV5R11({
          permit,
    ...
    if (input.authorization.provider === "DEEPSEEK_DIRECT") {
      ...
      if (!semanticAuthority) {
        ...
        semanticErrors.push(...validateSemanticDispatchAuthorityV5R11({
          ...runtimeState,
    ...
    const intentErrors = validateProviderAttemptCommitIntentV5R11({ ...common, intent });
    const resolvedErrors = validateResolvedProviderAttemptReceiptV5R11({ ...common,
...
export function validateAttemptGraphCoreReceiptV5R11({ attemptGraphReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R11(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV1")];
  try {
    const rebuilt = reconstructAttemptGraphCoreV5R11({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R11 attempt graph differs from exact ledger/request/raw/binding/intent/completion reconstruction");
    }
  } catch (error) {
...
export function roleOutputByEvidenceV5R11({ roleAttemptEvidenceReceipts, ledgerEntries }) {
  ...
  return roleAttemptEvidenceReceipts.map((receipt) => {
    const errors = validateClosedSelfHashedArtifactV5R11(receipt, "RoleAttemptEvidenceReceiptV1");
    ...
    if (errors.length > 0 || !output || output.attemptId !== receipt.attemptId
      ...
      throw new TypeError(`R11 role evidence cannot resolve its exact compatibility role output: ${errors.join("; ")}`);
    }
    return output;
  });
}

export const ATTEMPT_GRAPH_CORE_V5_R11_CONSTANTS = Object.freeze({
