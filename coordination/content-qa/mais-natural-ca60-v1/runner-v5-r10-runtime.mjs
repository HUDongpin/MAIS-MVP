...
} from "./execution-evidence-v5-r10.mjs";
import {
...
import {
  validateRouteProbeArtifactSetV5R10,
  validateTrustedProviderEvidenceEnvelopeV5R10,
} from "./trusted-provider-evidence-v5-r10.mjs";
import {
  applyInterruptedAttemptReconciliationV5R10,
  inspectInterruptedAttemptCustodyV5R10,
  validateResumeCustodyV5R10,
} from "./attempt-recovery-v5-r10.mjs";
import {
  buildTerminalExecutionBundleFromRawCustodyV5R10,
  scoreNaturalCaV5R10,
  verifyNaturalCaScoreFromRawEvidenceV5R10,
} from "./scorer-verifier-v5-r10.mjs";
import {
  verifyTerminalExecutionDecisionReceiptV5R10,
} from "./statistical-kernel-v5-r10.mjs";
import {
  ACTIVE_REGISTRATION_KIND_V5_R10,
  ACTIVE_REVIEW_KIND_V5_R10,
  advanceProtectedWorkflowIndexV5R10,
  loadProtectedWorkflowIndexV5R10,
  executionLedgerRelativePathV5R10,
} from "./workflow-index-v5-r10.mjs";
import {
...
import {
  buildSemanticDispatchAuthorityV5R10,
  buildSemanticDispatchVerificationReceiptV5R10,
} from "./semantic-dispatch-v5-r10.mjs";
import {
  NativeProviderAttemptFailureV5R10,
  createNativeProviderTransportV5R10,
  runNativeProviderAttemptV5R10,
} from "./native-provider-attempt-v5-r10.mjs";
import {
...
import {
  createAttemptCustodyStoreV5R10,
  reloadAttemptCustodyStoreV5R10,
} from "./attempt-custody-store-v5-r10.mjs";
import {
  reconstructAttemptGraphV5R10,
...
import {
  buildRawAuthoritativeMachineReferenceSealV5R10,
  buildRawAuthoritativeReferenceValidationV5R10,
} from "./raw-authoritative-reference-v5-r10.mjs";
import {
  buildDeepSeekExecutionRegistrationV5R10,
} from "./execution-freeze-v5-r10.mjs";
import {
  atomicWriteProtectedJsonV5R4,
  readProtectedJsonV5R4,
} from "./protected-storage-v5-r4.mjs";
import {
  buildResolvedProviderAttemptReceiptV5R10,
...
function clockIso(clock) {
  ...
  requireCondition(Number.isFinite(date.getTime()), "R10 runner clock is invalid");
...
function artifact(context, kind, { required = true } = {}) {
  ...
  if (required && value === undefined) throw new Error(`protected R10 workflow artifact ${kind} is absent`);
  return value;
}
...
async function writeContentAddressedV5R10({ trustedRoot, family, value }) {
  requireCondition(validateSelfHashV5R3(value), "R10 runtime custody value is not self-hashed");
  const relativePath = path.join("runtime-custody-v5-r10", family, `${value.selfHash}.json`);
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R10 runtime custody path already contains different canonical bytes");
  } catch (error) {
...
function appendStore(context, family) {
  return Object.freeze({ append: (value) => writeContentAddressedV5R10({
...
function providerPrefix(provider) {
  requireCondition(PROVIDERS.has(provider), "R10 provider is invalid");
  return provider === "OPENAI_DIRECT" ? "OPENAI" : "DEEPSEEK";
}

function providerArtifact(context, provider, suffix, options) {
  return artifact(context, `${providerPrefix(provider)}_${suffix}_V5_R10`, options);
}
...
function upstreamErrors(context, { requireAdoption = true } = {}) {
  const errors = [...validateExactRunnerRegistrationEvidenceV5R10(context?.registrationEvidence),
    ...validateFreshRunnerReviewV5R10(reviewInput(context))];
  if (requireAdoption && context?.needsR9Adoption) {
    errors.push("protected workflow has not adopted the exact V5-R10 registration");
  }
  try {
...
function activationInput(context, provider, at) {
  ...
  return {
    ...
    referenceSeal: artifact(context, "REFERENCE_SEAL_V5_R10", { required: false }),
    compatibilityReferenceSeal: artifact(context, "REFERENCE_COMPATIBILITY_SEAL_V5_R10",
      { required: false }),
    referenceSealValidationReceipt: artifact(context, "REFERENCE_SEAL_VALIDATION_V5_R10",
      { required: false }),
    referenceSealContext: context.referenceSealContext,
    executionRegistration: artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R10",
      { required: false }),
    provider,
  };
}
...
async function persistResult(advanceWorkflow, context, command, entries, at) {
  requireCondition(Array.isArray(entries) && entries.length > 0,
    "R10 result persistence requires at least one sealed artifact");
  return advanceWorkflow({ context, command, appendedArtifacts: entries, committedAt: at });
}

export async function persistFinalCommandJournalV5R10({ advanceWorkflow, context, journal,
  command, committedAt }) {
  ...
  "R10 final command-journal persistence requires an active context, command, and chronology");
  const errors = validateClosedSelfHashedArtifactV5R10(journal,
  ...
  `R10 final command journal is invalid or registration-drifted: ${errors.join("; ")}`);
  return persistResult(advanceWorkflow, context, command, [{
    kind: `COMMAND_TRANSITION_JOURNAL_V5_R10:${journal.selfHash}`,
    family: "transition-journals-indexed",
    value: journal,
  }], committedAt);
}

export function createRunnerRuntimeV5R10({
  protectedRoot = DEFAULT_PROTECTED_ROOT,
  repoRoot = DEFAULT_REPO_ROOT,
  clock = () => new Date(),
  advanceWorkflow = advanceProtectedWorkflowIndexV5R10,
  fetchImplementation = globalThis.fetch,
  credentialReaders = Object.freeze({}),
  nativeAttemptRunner = runNativeProviderAttemptV5R10,
  transportFactory = createNativeProviderTransportV5R10,
  registrationEvidenceLoader,
  freshReviewLoader,
} = {}) {
  async function loadWorkflowContext(indexPath) {
    requireCondition(path.isAbsolute(indexPath ?? ""),
      "R10 workflow index path must be absolute");
    return loadProtectedWorkflowIndexV5R10(indexPath, {
      protectedRoot,
      repoRoot,
      registrationEvidenceLoader,
      freshReviewLoader,
  ...
  async function openLedger(context, evidence) {
    return createAtomicExecutionLedgerV5R7({
      ...
      ledgerRelativePath: executionLedgerRelativePathV5R10(evidence.authorization.provider,
  ...
  function roleOutputsForEvidence(evidenceReceipts, ledgerEntries) {
    return roleOutputByEvidenceV5R10({ roleAttemptEvidenceReceipts: evidenceReceipts, ledgerEntries });
  }

  function canaryPredicateState(context, evidence, ledgerEntries, extras = []) {
    const predicate = artifact(context, "CANARY_C0_PREDICATE_V5_R10", { required: false });
    ...
    requireCondition(graph, "R10 canary predicate source graph is absent");
  ...
  function normalC0State(context, evidence, ledgerEntries, extras = []) {
    const c0ExecutionSet = artifact(context, "DEEPSEEK_C0_EXECUTION_SET_V5_R10", { required: false });
    ...
    requireCondition(graph, "R10 full base-role source graph for the C0 set is absent");
    const baseEvidence = successfulEvidence(graph, BASE_ROLES);
    ...
    const errors = validateNormalC0ExecutionSetV5R7({
      ...
      executionRegistrationHash: artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R10").selfHash,
  ...
  function canaryGateState(context, evidence, ledgerEntries, extras = []) {
    const canaryGate = artifact(context, "CANARY_GATE_V5_R10", { required: false });
    ...
    requireCondition(graph && prefixEnd >= 0 && semantic,
      "R10 canary gate graph, ledger prefix, or semantic receipt is absent");
  ...
  function buildGraphContext(context, evidence, ledgerEntries, extras = [], overrides = {}) {
    ...
    return {
      ...
      executionRegistration: evidence.provider === "DEEPSEEK_DIRECT"
        ? artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R10") : null,
      canaryPredicateReceipt: predicateState.predicate,
      canaryPredicateInput: predicateState.input,
  ...
  function reconstructGraph(context, evidence, ledgerEntries, extras, derivedAt, overrides = {}) {
    ...
    const built = reconstructAttemptGraphV5R10({ ...graphContext, derivedAt });
  ...
  function semanticAuthorityContexts(context, evidence, ledgerEntries, graphContext, graph) {
    ...
    for (const roleEvidence of graph.roleAttemptEvidenceReceipts) {
      ...
      requireCondition(authority, "R10 semantic evidence lacks its exact authority");
      ...
      requireCondition(reservationIndex >= 0, "R10 semantic authority reservation is absent");
  ...
  function buildSemanticVerification(context, evidence, ledgerEntries, built, verifiedAt, mode,
    ...
    requireCondition(authorities.every(Boolean),
      "R10 graph evidence lacks ordered semantic authorities");
    return buildSemanticDispatchVerificationReceiptV5R10({
      activeRegistration: context.activeRegistration,
  ...
  async function referenceSealContextFromContext(context, { derivedAt = clockIso(clock) } = {}) {
    ...
    const seal = artifact(context, "REFERENCE_SEAL_V5_R10", { required: false });
    ...
    if (graph) requireCondition(canonicalJsonV5R3(graph) === canonicalJsonV5R3(built.receipt),
      "stored R10 reference graph differs from raw-attempt reconstruction");
  ...
  async function adoptR9WorkflowIndex(context) {
    try {
      const errors = upstreamErrors(context, { requireAdoption: false });
      requireCondition(errors.length === 0, errors.join("; "));
      if (!context.needsR9Adoption) {
        return success("V5_R10_WORKFLOW_INDEX_ALREADY_ACTIVE_NO_PROVIDER_ACTIVITY");
      }
      const committedAt = transitionTime(context, clockIso(clock));
      const process = context.processArtifacts;
      const transition = await persistResult(advanceWorkflow, context, "register-v5-r10", [
        { kind: ACTIVE_REGISTRATION_KIND_V5_R10, family: "runner-registrations",
          value: context.activeRegistration },
        { kind: ACTIVE_REVIEW_KIND_V5_R10, family: "runner-reviews", value: context.freshReview },
        { kind: `A11_STATIC_IMPORT_GRAPH_V5_R10:${process.staticImportGraphReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.staticImportGraphReceipt },
        { kind: `A11_FORBIDDEN_PATH_SCAN_V5_R10:${process.forbiddenPrimaryScorerPathScanReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.forbiddenPrimaryScorerPathScanReceipt },
        { kind: `A11_COMMAND_RUNTIME_V5_R10:${process.commandRuntimeReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.commandRuntimeReceipt },
        { kind: `A11_SOURCE_ENUMERATION_V5_R10:${process.independentSourceEnumerationReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.independentSourceEnumerationReceipt },
      ], committedAt);
      context.needsR9Adoption = false;
      return success("V5_R10_WORKFLOW_INDEX_ADOPTED_NO_PROVIDER_ACTIVITY", { transition });
    } catch (error) {
      return blocked("V5_R10_WORKFLOW_INDEX_ADOPTION_BLOCKED",
  ...
  async function verifyFrozenUpstream(context) {
    ...
    return errors.length > 0 ? blocked("FROZEN_UPSTREAM_VERIFICATION_BLOCKED", errors)
      : success("V5_R10_FROZEN_FRAME_SAMPLE_METHOD_AND_CLAIM_CEILING_VERIFIED_NO_PROVIDER_ACTIVITY");
  }

  async function dryRun(context) {
    const errors = upstreamErrors(context);
    if (errors.length > 0) return blocked("V5_R10_DRY_RUN_BLOCKED", errors);
    const noAnchor = context.activeRegistration.trustedProviderEvidenceAnchors.length === 0;
    return success(noAnchor
      ? "V5_R10_OFFLINE_DRY_RUN_READY_ROUTE_AUTHENTICITY_BLOCKED_NO_PROVIDER_ACTIVITY"
      : "V5_R10_OFFLINE_DRY_RUN_READY_NO_PROVIDER_ACTIVITY", {
  ...
  async function authorizeCheck(context, provider) {
    try {
      ...
      const errors = [...upstreamErrors(context), ...validateProviderActivationV5R10(input)];
  ...
  async function registerTrustedRouteEvidence(context, provider) {
    try {
      ...
      const setErrors = validateRouteProbeArtifactSetV5R10(routeProbeArtifactSet);
      requireCondition(setErrors.length === 0, setErrors.join("; "));
      const errors = validateTrustedProviderEvidenceEnvelopeV5R10({ envelope,
      ...
      const additions = routeAlreadyRegistered ? [] : [
        { kind: `${providerPrefix(provider)}_TRUSTED_ROUTE_ENVELOPE_V5_R10`,
          family: "trusted-route-evidence", value: envelope },
        { kind: `${providerPrefix(provider)}_AUTHENTICATED_ROUTE_EVIDENCE_V5_R10`,
          family: "authenticated-route-evidence", value: authenticatedRouteEvidence },
        { kind: `${providerPrefix(provider)}_ROUTE_PROBE_ARTIFACT_SET_V5_R10`,
          family: "route-probe-artifact-sets", value: routeProbeArtifactSet },
        { kind: `${providerPrefix(provider)}_PRICE_SNAPSHOT_V5_R10`,
      ...
      requireCondition(presentLiveCandidates.length === 0 || presentLiveCandidates.length === 3,
        "R10 authorization, owner-grant, and cost-preview candidates must be adopted all-or-none");
      requireCondition(presentLiveCandidates.length === 0
        || !providerArtifact(context, provider, "AUTHORIZATION", { required: false }),
      "R10 live authorization is already registered; replacement requires a new registration version");
      if (presentLiveCandidates.length === 3) {
        ...
        if (provider === "DEEPSEEK_DIRECT") {
          ...
          const referenceInputs = {
            ...
            compatibilityReferenceSeal: artifact(context,
              "REFERENCE_COMPATIBILITY_SEAL_V5_R10", { required: false }),
            referenceSealValidationReceipt: artifact(context,
              "REFERENCE_SEAL_VALIDATION_V5_R10", { required: false }),
            referenceSealContext,
          };
          const provisionalExecutionRegistration = buildDeepSeekExecutionRegistrationV5R10({
            ...activation,
            ...referenceInputs,
        ...
        const activationErrors = validateProviderActivationV5R10(activation);
        requireCondition(activationErrors.length === 0,
          `R10 candidate live bundle is invalid: ${activationErrors.join("; ")}`);
      }
      for (const { suffix, family, value } of presentLiveCandidates) additions.push({
        kind: `${providerPrefix(provider)}_${suffix}_V5_R10`, family, value,
      });
      requireCondition(additions.length > 0,
        "R10 route and authorization evidence are already registered or no complete candidate exists");
  ...
  async function makeJournal(context, command, startedAt) {
    ...
    return createCommandTransitionJournalV5R7({
      ...
      activeRunnerRegistrationHash: context.activeRegistration.selfHash,
      startedAt,
      persistIntent: (value) => writeContentAddressedV5R10({ trustedRoot: context.trustedRoot,
        family: "transition-intents", value }),
      persistSubreceipt: (value) => writeContentAddressedV5R10({ trustedRoot: context.trustedRoot,
        family: "transition-subreceipts", value }),
      persistFinalJournal: (value) => writeContentAddressedV5R10({ trustedRoot: context.trustedRoot,
  ...
  function attemptArtifactAdditions(run) {
    return [
      { kind: `DISPATCH_PERMIT_V5_R10:${run.permit.selfHash}`,
        family: "dispatch-permits", value: run.permit },
      { kind: `COMPATIBILITY_DISPATCH_PERMIT_V5_R10:${run.compatibilityPermit.selfHash}`,
        family: "compatibility-dispatch-permits", value: run.compatibilityPermit },
      { kind: `RAW_RESPONSE_ARTIFACT_V5_R10:${run.rawResponseArtifact.selfHash}`,
        family: "raw-response-artifacts", value: run.rawResponseArtifact },
      { kind: `RAW_RESPONSE_BINDING_V5_R10:${run.rawResponseBindingReceipt.selfHash}`,
        family: "raw-response-bindings", value: run.rawResponseBindingReceipt },
      { kind: `PROVIDER_EVENT_V5_R10:${run.providerEventReceipt.selfHash}`,
        family: "provider-events", value: run.providerEventReceipt },
      { kind: `ATTEMPT_COMMIT_INTENT_V5_R10:${run.attemptCommitIntent.selfHash}`,
        family: "attempt-commit-intents", value: run.attemptCommitIntent },
      { kind: `RESOLVED_PROVIDER_ATTEMPT_V5_R10:${run.resolvedAttemptReceipt.selfHash}`,
        family: "resolved-provider-attempts", value: run.resolvedAttemptReceipt },
      ...(run.roleOutput ? [{ kind: `ROLE_OUTPUT_V5_R10:${run.roleOutput.selfHash}`,
  ...
  function derivePostAttemptArtifacts(context, evidence, ledgerEntries, extras, at) {
    ...
    requireCondition(built.receipt.graphStatus === "COMPLETE_VALID",
      built.receipt.lineageErrors.join("; "));
    const additions = [{ kind: `ATTEMPT_GRAPH_V5_R10:${built.receipt.selfHash}`,
    ...
    if (!predicate && canaryBase.length === 2) {
      ...
      additions.push({ kind: "CANARY_C0_PREDICATE_V5_R10",
    ...
    if (!built.graphContext.canaryGate && predicate && canaryEvidence.length === requiredCanaryCount) {
      ...
      additions.push({ kind: `SEMANTIC_DISPATCH_VERIFICATION_V5_R10:${semantic.selfHash}`,
        family: "semantic-dispatch-verifications", value: semantic },
      { kind: "CANARY_GATE_V5_R10", family: "canary-gates", value: gate });
    }
    ...
    if (!c0ExecutionSet && new Set(baseEvidence
      ...
      additions.push({ kind: "DEEPSEEK_C0_EXECUTION_SET_V5_R10",
  ...
  async function executeProviderStep(context, provider, executionMode) {
    ...
    try {
      ...
      const errors = [...upstreamErrors(context), ...validateProviderActivationV5R10(activation)];
      requireCondition(errors.length === 0, errors.join("; "));
      const custodyErrors = validateResumeCustodyV5R10(defaultAttemptCustody(context, provider));
      ...
      const dispatchAuthority = provider === "OPENAI_DIRECT"
        ? buildReferenceDispatchAuthorityV5R10(authorityContext)
        : buildSemanticDispatchAuthorityV5R10(authorityContext);
      ...
      const custodyStore = await createAttemptCustodyStoreV5R10({ trustedRoot: context.trustedRoot,
      ...
      requireCondition(transport?.kind === "V5_R10_NATIVE_EXACT_TRANSPORT",
        "R10 native transport factory did not return the exact transport contract");
      ...
      const journalResult = await runJournaledTransitionSequenceV5R7({ journal, steps: [
        { name: "REQUEST_AND_DISPATCH_AUTHORITY", preparedAt: at,
          ...
          run: () => persistResult(advanceWorkflow, context, "request-and-dispatch-authority", [
            { kind: `PROVIDER_REQUEST_V5_R10:${requestArtifact.selfHash}`,
              family: "provider-requests", value: requestArtifact },
            { kind: `DISPATCH_AUTHORITY_V5_R10:${dispatchAuthority.selfHash}`,
        ...
        { name: "NATIVE_PROVIDER_ATTEMPT_AND_DERIVED_STATE", preparedAt: later(at, 2),
          ...
          run: async () => {
            ...
            try {
              ...
              const activity = error instanceof NativeProviderAttemptFailureV5R10
                ? error.activity : null;
      ...
      if (!journalResult.ok) {
        ...
        try {
          failedJournalTransition = await persistFinalCommandJournalV5R10({ advanceWorkflow,
      ...
      const journalTransition = await persistFinalCommandJournalV5R10({ advanceWorkflow,
        context, journal: journalResult.journal, command: "commit-provider-command-journal",
        committedAt: later(at, 21) });
      const after = await ledger.verify();
      const reloaded = await reloadAttemptCustodyStoreV5R10({ trustedRoot: context.trustedRoot,
        provider, authorizationHash: activation.authorization.selfHash,
      ...
      context.attemptCustodyByProvider[provider] = Object.freeze({ ledgerEntries: after.entries,
        ...
        custodyErrors: validateResumeCustodyV5R10({ ledgerEntries: after.entries,
          preparedCompletions: reloaded.preparedCompletions,
          attemptCommitIntents: reloaded.attemptCommitIntents,
          resolvedAttemptReceipts: reloaded.resolvedAttemptReceipts }) });
      return Object.freeze({ ...run, ok: run.status === "SUCCEEDED",
        status: run.status === "SUCCEEDED"
          ? "V5_R10_PROVIDER_ATTEMPT_SUCCEEDED_AND_RECONSTRUCTED"
          : `V5_R10_PROVIDER_ATTEMPT_RECORDED_${run.status}`,
      ...
      const activity = error instanceof NativeProviderAttemptFailureV5R10 ? error.activity : error?.activity;
  ...
  async function sealReferenceLabels(context) {
    try {
      const errors = upstreamErrors(context);
      requireCondition(errors.length === 0, errors.join("; "));
      requireCondition(!artifact(context, "REFERENCE_SEAL_V5_R10", { required: false }),
        "R10 reference labels are already sealed; replacement requires a new registration version");
      ...
      const referenceSealContext = await referenceSealContextFromContext(context,
        { derivedAt: sealedAt });
      const built = buildRawAuthoritativeMachineReferenceSealV5R10({
        ...referenceSealContext, sealedAt,
      });
      const validatedAt = later(sealedAt, 1);
      const validationReceipt = buildRawAuthoritativeReferenceValidationV5R10({
        ...referenceSealContext,
        seal: built.seal,
      ...
      const transition = await persistResult(advanceWorkflow, context,
        "seal-reference-labels", [
          { kind: `ATTEMPT_GRAPH_V5_R10:${referenceSealContext.attemptGraphReceipt.selfHash}`,
            family: "attempt-graphs", value: referenceSealContext.attemptGraphReceipt },
          { kind: "REFERENCE_COMPATIBILITY_SEAL_V5_R10",
            family: "reference-compatibility-seals", value: built.compatibilityReferenceSeal },
          { kind: "REFERENCE_SEAL_V5_R10", family: "reference-seals", value: built.seal },
          { kind: "REFERENCE_SEAL_VALIDATION_V5_R10",
            family: "reference-seal-validations", value: validationReceipt },
        ], validatedAt);
      return success("MACHINE_REFERENCE_LABELS_FROZEN_RAW_AUTHORITATIVE_V5_R10_NO_PROVIDER_ACTIVITY", {
  ...
  async function freezeDeepSeekExecutionRegistration(context) {
    try {
      const errors = upstreamErrors(context);
      requireCondition(errors.length === 0, errors.join("; "));
      requireCondition(!artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R10",
        { required: false }),
      "R10 DeepSeek execution registration is already frozen");
      ...
      const executionRegistration = buildDeepSeekExecutionRegistrationV5R10({
        ...evidence,
        referenceSeal: artifact(context, "REFERENCE_SEAL_V5_R10"),
        compatibilityReferenceSeal: artifact(context, "REFERENCE_COMPATIBILITY_SEAL_V5_R10"),
        referenceSealValidationReceipt: artifact(context, "REFERENCE_SEAL_VALIDATION_V5_R10"),
        referenceSealContext,
        registeredAt,
      ...
      const transition = await persistResult(advanceWorkflow, context,
        "freeze-deepseek-registration", [{ kind: "DEEPSEEK_EXECUTION_REGISTRATION_V5_R10",
          family: "deepseek-execution-registrations", value: executionRegistration }], registeredAt);
      return success("DEEPSEEK_EXECUTION_REGISTRATION_V5_R10_FROZEN_NO_PROVIDER_ACTIVITY", {
  ...
  async function auditAttemptCustody(context, provider) {
    const errors = validateResumeCustodyV5R10(defaultAttemptCustody(context, provider));
  ...
  function rebuildResolvedAttemptFromCustody(context, provider, authorization, custody) {
    ...
    requireCondition(preparedCompletion && custody.intent,
      "R10 recovery cannot rebuild a resolved receipt without exact intent/completion custody");
    ...
    "R10 recovery is missing exact request/audit/permit/raw/binding/event custody");
    return buildResolvedProviderAttemptReceiptV5R10({
      activeRunnerRegistrationHash: context.activeRegistration.selfHash,
  ...
  async function loadRecoveryInput(recoveryPath, provider, context) {
    requireCondition(path.isAbsolute(recoveryPath ?? "") && PROVIDERS.has(provider),
      "R10 recovery loader requires an absolute protected path and exact provider");
    ...
    "R10 recovery authorization path must remain inside the established protected root");
    const bundle = await readProtectedJsonV5R4({ trustedRoot: context.trustedRoot, relativePath });
    const schemaErrors = validateClosedSelfHashedArtifactV5R10(bundle,
    ...
    `R10 recovery authorization bundle is invalid: ${schemaErrors.join("; ")}`);
    ...
    requireCondition(bundle.providerAuthorizationHash === authorization.selfHash,
      "R10 recovery bundle provider authorization differs from the active custody");
    const providerCustody = defaultAttemptCustody(context, provider);
    const custody = inspectInterruptedAttemptCustodyV5R10({ ...providerCustody,
  ...
  async function reconcileInterruptedAttempt(context, recoveryInput) {
    try {
      const result = await applyInterruptedAttemptReconciliationV5R10(recoveryInput);
      const additions = [{ kind: `ATTEMPT_CUSTODY_RECONCILIATION_V5_R10:${result.receipt.selfHash}`,
      ...
      if (recoveryInput.recoveredResolvedAttemptReceipt) additions.push({
        kind: `RESOLVED_PROVIDER_ATTEMPT_V5_R10:${recoveryInput.recoveredResolvedAttemptReceipt.selfHash}`,
      ...
      context.attemptCustodyByProvider = await (async () => {
        ...
        requireCondition(PROVIDERS.has(provider), "R10 recovery input provider is absent");
        const authorization = providerArtifact(context, provider, "AUTHORIZATION");
        ...
        current[provider] = Object.freeze({ ...current[provider], ledgerEntries: verified.entries,
          ...
          custodyErrors: validateResumeCustodyV5R10({ ledgerEntries: verified.entries,
            preparedCompletions: reloaded.preparedCompletions,
            attemptCommitIntents: reloaded.attemptCommitIntents,
            resolvedAttemptReceipts: reloaded.resolvedAttemptReceipts }) });
        return current;
  ...
  async function nativeScoringEvidence(context, scoredAt, { terminal = false,
    ...
    "stored R10 DeepSeek graph differs from exact raw-custody reconstruction");
    ...
    requireCondition(c0ExecutionSet,
      "R10 complete scoring requires the frozen normal C0 execution set");
    ...
    if (!terminal) {
      ...
      requireCondition(successfulEvidence(built.receipt).length === expectedSuccessfulCount,
        "R10 execution is incomplete; complete frozen-metric scoring is forbidden");
      ...
      "stored R10 semantic verification differs from exact reconstruction");
    }
    const referenceSealContext = await referenceSealContextFromContext(context,
      { derivedAt: scoredAt });
  ...
  async function persistScoringBundle(context, built, terminalReceipt, committedAt) {
    const entries = [
      { kind: `ATTEMPT_GRAPH_V5_R10:${built.rawEvidence.deepSeekAttemptGraphReceipt.selfHash}`,
        family: "attempt-graphs", value: built.rawEvidence.deepSeekAttemptGraphReceipt },
      { kind: `SEMANTIC_DISPATCH_VERIFICATION_V5_R10:${built.rawEvidence.semanticDispatchVerificationReceipt.selfHash}`,
        family: "semantic-dispatch-verifications",
        value: built.rawEvidence.semanticDispatchVerificationReceipt },
      { kind: `DECISION_EVIDENCE_V5_R10:${built.decisionEvidenceReceipt.selfHash}`,
        family: "decision-evidence", value: built.decisionEvidenceReceipt },
      ...built.itemResults.map((value) => ({ kind: `ITEM_RESULT_V5_R10:${value.itemHash}`,
        family: "item-results", value })),
      ...built.completedItemMarkers.map((value) => ({
        kind: `COMPLETED_ITEM_MARKER_V5_R10:${value.itemHash}`,
        family: "completed-item-markers", value })),
      { kind: `OBSERVED_LEDGER_V5_R10:${built.observedLedger.selfHash}`,
        family: "observed-ledgers", value: built.observedLedger },
      { kind: `COUNTERFACTUAL_LEDGER_V5_R10:${built.counterfactualLedger.selfHash}`,
        family: "counterfactual-ledgers", value: built.counterfactualLedger },
      { kind: `METRIC_INPUT_LEDGER_V5_R10:${built.metricInputLedger.selfHash}`,
        family: "metric-input-ledgers", value: built.metricInputLedger },
      { kind: `FINAL_EVALUATION_V5_R10:${built.finalReceipt.selfHash}`,
        family: "final-evaluations", value: built.finalReceipt },
      { kind: `AGGREGATE_SCORE_V5_R10:${built.scoreReceipt.selfHash}`,
        family: "aggregate-scores", value: built.scoreReceipt },
    ];
    if (terminalReceipt) entries.push({ kind: `TERMINAL_DECISION_V5_R10:${terminalReceipt.selfHash}`,
      family: "terminal-decisions", value: terminalReceipt });
    return persistResult(advanceWorkflow, context, "score-v5-r10", entries, committedAt);
  }
  ...
  async function persistTerminalBundle(context, built, committedAt) {
    const entries = [
      { kind: `ATTEMPT_GRAPH_V5_R10:${built.rawEvidence.deepSeekAttemptGraphReceipt.selfHash}`,
        family: "attempt-graphs", value: built.rawEvidence.deepSeekAttemptGraphReceipt },
      { kind: `TERMINAL_C0_SCOPE_V5_R10:${built.rawEvidence.c0ExecutionSet.selfHash}`,
      ...
      ...built.completedItemMarkers.map((value) => ({
        kind: `COMPLETED_ITEM_MARKER_V5_R10:${value.itemHash}`,
        family: "completed-item-markers", value })),
      { kind: `TERMINAL_DECISION_V5_R10:${built.terminalReceipt.selfHash}`,
        family: "terminal-decisions", value: built.terminalReceipt },
    ];
    return persistResult(advanceWorkflow, context, "score-terminal-v5-r10", entries, committedAt);
  }
  ...
  async function score(context) {
    ...
    try {
      ...
      const built = scoreNaturalCaV5R10({ ...input, scoredAt: later(scoredAt, 1) });
      if (built.finalReceipt.overallDecision === "EXECUTION_INTEGRITY_FAILED") {
        const terminal = buildTerminalExecutionBundleFromRawCustodyV5R10({
    ...
    try {
      ...
      if (scoreReceipt) {
        ...
        requireCondition(graphReceipt && semanticReceipt,
          "R10 score-bound graph or semantic receipt is absent");
        ...
        const verifyErrors = verifyNaturalCaScoreFromRawEvidenceV5R10({ ...input, scoreReceipt });
        return verifyErrors.length > 0
          ? blocked("EXECUTION_RESULT_VERIFICATION_BLOCKED", verifyErrors)
          : success("V5_R10_RAW_EVIDENCE_SCORE_RECOMPUTED_NO_PASS_CLAIM", { scoreReceipt,
      ...
      requireCondition(terminalReceipt, "no R10 aggregate or terminal execution receipt exists");
      const graphReceipt = findByHash(context, terminalReceipt.attemptGraphReceiptHash);
      requireCondition(graphReceipt, "R10 terminal graph receipt is absent");
      const input = await nativeScoringEvidence(context, terminalReceipt.derivedAt,
        { terminal: true, expectedGraph: graphReceipt });
      const rebuilt = buildTerminalExecutionBundleFromRawCustodyV5R10({
        ...input, scoredAt: terminalReceipt.derivedAt,
      });
      const verifyErrors = [...verifyTerminalExecutionDecisionReceiptV5R10({
      ...
      if (canonicalJsonV5R3(rebuilt.terminalReceipt) !== canonicalJsonV5R3(terminalReceipt)) {
        verifyErrors.push("R10 terminal receipt differs from raw-custody reconstruction");
      }
      return verifyErrors.length > 0
        ? blocked("TERMINAL_EXECUTION_RESULT_VERIFICATION_BLOCKED", verifyErrors)
        : success("V5_R10_TERMINAL_DECISION_RECOMPUTED_NO_METRIC_INFERENCE_NO_PASS_CLAIM",
          { terminalReceipt });
    } catch (error) {
...
export const V5_R10_RUNTIME_PATHS = Object.freeze({
  repoRoot: DEFAULT_REPO_ROOT,
  protectedRoot: DEFAULT_PROTECTED_ROOT,
  workflowIndexPattern: "workflow-indexes-v5-r10/<selfHash>.json",
  derivedArtifactRoot: "derived-v5-r10",
