...
function validateGraphAndBlindness(input) {
  ...
  `R9 reference seal requires a complete independently rebuilt OpenAI attempt graph: ${graphErrors.join("; ")}`);
  ...
  "R9 reference graph contains missing successes, non-reparsed success, leaked input, or extra-role evidence");
  ...
  requireCondition(exactSet(successfulOutputHashes,
    ...
    "R9 reference graph evidence does not equal the complete successful compatibility output set");
}

export function buildRawAuthoritativeMachineReferenceSealV5R9(input) {
  ...
  "R9 reference seal requires the exact active registration and fresh A11 concurrence");
  const authErrors = validateClosedSelfHashedArtifactV5R9(input?.authorization, "ProviderAuthorizationV5");
  ...
  `R9 OpenAI reference authorization is invalid: ${authErrors.join("; ")}`);
  ...
  "R9 compatibility reference seal differs from the raw-authoritative attempt graph");
  const seal = sealV5R3Artifact({
    ...
    sealReconstructionStatus: "FULL_RAW_RESPONSE_REPARSE_AND_ATTEMPT_GRAPH_RECONSTRUCTION_BOUND_TO_R9",
    sealedAt: input.sealedAt,
  });
...
export function validateRawAuthoritativeMachineReferenceSealV5R9({ seal, compatibilityReferenceSeal,
  ...input }) {
  ...
  try {
    const rebuilt = buildRawAuthoritativeMachineReferenceSealV5R9({
      ...input,
    ...
    if (canonicalJsonV5R3(rebuilt.seal) !== canonicalJsonV5R3(seal)
      ...
      errors.push("R9 reference seal differs from raw-authoritative graph and label reconstruction");
    }
  } catch (error) {
...
export function buildRawAuthoritativeReferenceValidationV5R9({ seal, compatibilityReferenceSeal,
  validatedAt, ...input }) {
  const errors = validateRawAuthoritativeMachineReferenceSealV5R9({
    ...input,
    seal,
    compatibilityReferenceSeal,
  });
  requireCondition(errors.length === 0 && Date.parse(validatedAt) > Date.parse(seal.sealedAt),
    `R9 reference validation failed or does not postdate sealing: ${errors.join("; ")}`);
  const graph = input.attemptGraphReceipt;
  const receipt = sealV5R3Artifact({
    ...
    labelBlindnessVerified: true,
    sameModelCorrelatedErrorRisk: true,
    validationStatus: "RAW_AUTHORITATIVE_ATTEMPT_GRAPH_REBUILT_AND_BOUND_TO_R9",
    validatedAt,
  });
...
export function validateRawAuthoritativeReferenceValidationV5R9({ validationReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(validationReceipt, VALIDATION_SCHEMA)];
  try {
    const rebuilt = buildRawAuthoritativeReferenceValidationV5R9({
      ...input,
      validatedAt: validationReceipt?.validatedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(validationReceipt)) {
      errors.push("R9 reference validation receipt differs from exact reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const RAW_AUTHORITATIVE_REFERENCE_V5_R9_CONSTANTS = Object.freeze({
