...
test("R11 public CLI exposes the complete registered workflow and V10 command receipts", async () => {
  assert.deepEqual([...V5_R11_CLI_COMMANDS], EXPECTED_COMMANDS);
  assert.deepEqual(parseExactArgumentsV5R11([
  ...
  let adopted = 0;
  const result = await runCliV5R11(["register", "--context", "/protected/index.json"], {
    loadWorkflowContext: async () => context(),
    adoptPredecessorWorkflowIndex: async () => { adopted += 1; return { ok: true,
      status: "V5_R11_WORKFLOW_INDEX_ADOPTED_NO_PROVIDER_ACTIVITY",
...
test("R11 CLI rejects ambiguous provider flags before loading protected state", async () => {
  let loads = 0;
  ...
  for (const argv of [
    ...
    const result = await runCliV5R11(argv, deps);
...
test("R11 public default recovery command reaches its exact loader and zero-HTTP engine", async () => {
  const current = context();
  const recovery = { fixture: true };
  let loads = 0;
  let reconciliations = 0;
  const result = await runCliV5R11([
