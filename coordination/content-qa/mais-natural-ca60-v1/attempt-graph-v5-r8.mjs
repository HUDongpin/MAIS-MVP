...
export function reconstructAttemptGraphV5R8(input) {
  let base;
  const wrapperErrors = [];
  try {
    base = reconstructAttemptGraphV5R7(input);
  } catch (error) {
...
export function validateAttemptGraphReconstructionReceiptV5R8({ attemptGraphReceipt, ...input }) {
  const errors = [];
  errors.push(...validateClosedSelfHashedArtifactV5R8(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV2"));
  try {
    const rebuilt = reconstructAttemptGraphV5R8({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R8 attempt graph differs from exact bidirectional evidence reconstruction");
    }
  } catch (error) {
...
export const ATTEMPT_GRAPH_V5_R8_CONSTANTS = Object.freeze({
