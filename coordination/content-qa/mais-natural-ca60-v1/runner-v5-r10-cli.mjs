...
function commandReceipt(command, status, context, output = {}) {
  ...
  return assertClosedSelfHashedArtifactV5R10(sealV5R3Artifact({
    schemaVersion: "NaturalCaRunnerCommandReceiptV9",
...
export function parseExactArgumentsV5R10(argv) {
...
export async function runCliV5R10(argv = [], deps = createRunnerRuntimeV5R10()) {
  const parsed = parseExactArgumentsV5R10(argv);
...
async function main() {
  const result = await runCliV5R10(process.argv.slice(2));
...
export const V5_R10_CLI_COMMANDS = Object.freeze([...COMMANDS]);
