...
function priceEvidence(input) {
  return (input?.routeEvidenceArtifacts ?? []).find(({ evidenceKind }) =>
    evidenceKind === "PRICE_SOURCE") ?? null;
}
...
function validateCostPreview(input, errors) {
  ...
  errors.push(...validateClosedSelfHashedArtifactV5R8(preview, "ProviderCostPreviewReceiptV1"));
  ...
  "R8 cost preview is not an exact signed-route price reconstruction with a 20 percent buffer");
}

export function validateProviderActivationV5R8(input) {
  const errors = [
    ...validateExactRunnerRegistrationEvidenceV5R8(input?.registrationEvidence),
    ...validateActiveRunnerRegistrationV5R8(input?.activeRegistration),
    ...validateFreshRunnerReviewV5R8(input ?? {}),
    ...validateResolvedProviderAuthorizationV5R6(input?.authorization),
    ...validateClosedSelfHashedArtifactV5R8(input?.ownerActivationGrant,
      "OwnerRunnerActivationGrantV1"),
    ...validateClosedSelfHashedArtifactV5R8(input?.priceSnapshot, "ProviderPriceSnapshotV2"),
    ...validateResumeCustodyV5R8(input?.attemptCustody ?? {}),
  ];
  try {
    errors.push(...validateTrustedProviderEvidenceEnvelopeV5R8({
      envelope: input?.routeEvidenceEnvelope,
      activeRegistration: input?.activeRegistration,
      evidenceArtifacts: input?.routeEvidenceArtifacts,
      at: input?.at,
    }));
  } catch (error) {
  ...
  add(errors, evidence?.activeRegistration?.selfHash === registration?.selfHash,
    "R8 exact registration evidence does not bind the supplied active registration");
  add(errors, envelope && authorization?.model === envelope.model
    && authorization?.endpoint === envelope.endpoint,
  "R8 authorization provider, model, or endpoint differs from the frozen direct route");
  ...
  "R8 authorization does not bind the exact registration, review, signed route, and compatibility base");
  for (const field of ["provider", "model", "endpoint", "projectResidency", "dataRegion"]) {
    add(errors, authorization?.[field] === route?.[field],
    `R8 authorization ${field} differs from signed route evidence`);
  }
  add(errors, authorization?.projectIdentityHash === route?.subjectIdentityHash,
    "R8 authorization subject differs from signed route evidence");
  ...
  "R8 OpenAI authorization is not bound to US_STORAGE_PROCESSING and US data region");
  add(errors, !/UNKNOWN|PENDING|UNRESOLVED/iu.test(
    `${authorization?.projectResidency}:${authorization?.dataRegion}`),
  "R8 provider residency and data region remain unresolved");
  ...
  "R8 provider authorization differs from the frozen attempt, token, USD, or concurrency envelope");
  add(errors, allLiveAuthorityFlags(authorization) && allLiveAuthorityFlags(grant),
    "R8 provider authorization or owner activation grant lacks explicit live authority flags");
  ...
  "R8 owner grant does not bind the exact registration, review, route, cost, authorization, and inventory");
  ...
  "R8 owner grant tuple differs from the signed resolved authorization");
  ...
  "R8 owner grant caps, currency, or non-authorizations differ from the frozen envelope");
  ...
  "R8 compatibility ledger authorization, signed price, or inventory binding is invalid");
  ...
  "R8 authorization or grant chronology is invalid or expired");
  ...
  if (authorization?.provider === "DEEPSEEK_DIRECT") {
    ...
    errors.push(...validateClosedSelfHashedArtifactV5R8(input?.executionRegistration,
    ...
    "R8 DeepSeek execution registration is not bound to the sealed reference and exact live tuple");
  }
  return Object.freeze([...new Set(errors)]);
}

export function assertProviderActivationV5R8(input) {
  const errors = validateProviderActivationV5R8(input);
  if (errors.length > 0) throw new TypeError(errors.join("; "));
  return input.authorization;
}

export const ACTIVATION_GUARD_V5_R8_CONSTANTS = Object.freeze({
