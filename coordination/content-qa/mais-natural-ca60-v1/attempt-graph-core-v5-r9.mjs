...
import {
  validateProviderAttemptCommitIntentV5R9,
  validateResolvedProviderAttemptReceiptV5R9,
...
import {
  buildCompatibilityCanaryC0DecisionSetV5R9,
  validateSemanticDispatchAuthorityV5R9,
} from "./semantic-dispatch-v5-r9.mjs";
import {
  buildProviderDispatchPermitV5R9,
  validateProviderDispatchPermitV5R9,
  validateReferenceDispatchAuthorityV5R9,
...
export function reconstructAttemptGraphCoreV5R9(input) {
  const errors = [];
  ...
  for (const reservation of reservations) {
    ...
    const schemaErrors = [
      ...validateClosedSelfHashedArtifactV5R9(request, "ProviderRequestArtifactV5"),
      ...validateClosedSelfHashedArtifactV5R9(audit, audit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R9(permit, permit.schemaVersion),
      ...validateClosedSelfHashedArtifactV5R9(compatibilityPermit, "ProviderDispatchPermitV3"),
      ...validateRawResponseArtifactV5R6(raw),
      ...validateRawResponseBindingReceiptV5R6(binding),
      ...validateClosedSelfHashedArtifactV5R9(event, "ProviderEventReceiptV4"),
      ...validateClosedSelfHashedArtifactV5R9(completion, "ProviderDispatchCompletionV2"),
      ...validateClosedSelfHashedArtifactV5R9(intent, "ProviderAttemptCommitIntentV1"),
      ...validateClosedSelfHashedArtifactV5R9(resolved, "ResolvedProviderAttemptReceiptV2"),
    ];
    if (roleOutput) schemaErrors.push(...validateClosedSelfHashedArtifactV5R9(roleOutput, "ProviderRoleOutputV1"));
    ...
    const runtimeState = {
      ...
      c0ExecutionSet: audit.c0ExecutionSetHash ? input.c0ExecutionSet
        : audit.executionPlan?.mode === "DEEPSEEK_CANARY" && audit.canaryPredicateReceiptHash
          ? buildCompatibilityCanaryC0DecisionSetV5R9(input.canaryPredicateReceipt) : null,
    ...
    try {
      if (permit.schemaVersion === "ProviderDispatchPermitV5") {
        permitErrors.push(...validateProviderDispatchPermitV5R9({
          permit,
    ...
    if (input.authorization.provider === "DEEPSEEK_DIRECT") {
      ...
      if (!semanticAuthority) {
        ...
        semanticErrors.push(...validateSemanticDispatchAuthorityV5R9({
          ...runtimeState,
    ...
    const intentErrors = validateProviderAttemptCommitIntentV5R9({ ...common, intent });
    const resolvedErrors = validateResolvedProviderAttemptReceiptV5R9({ ...common,
...
export function validateAttemptGraphCoreReceiptV5R9({ attemptGraphReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R9(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV1")];
  try {
    const rebuilt = reconstructAttemptGraphCoreV5R9({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R9 attempt graph differs from exact ledger/request/raw/binding/intent/completion reconstruction");
    }
  } catch (error) {
...
export function roleOutputByEvidenceV5R9({ roleAttemptEvidenceReceipts, ledgerEntries }) {
  ...
  return roleAttemptEvidenceReceipts.map((receipt) => {
    const errors = validateClosedSelfHashedArtifactV5R9(receipt, "RoleAttemptEvidenceReceiptV1");
    ...
    if (errors.length > 0 || !output || output.attemptId !== receipt.attemptId
      ...
      throw new TypeError(`R9 role evidence cannot resolve its exact compatibility role output: ${errors.join("; ")}`);
    }
    return output;
  });
}

export const ATTEMPT_GRAPH_CORE_V5_R9_CONSTANTS = Object.freeze({
