...
import {
  validateCanaryGateReceiptV5R7 as validateCanaryGateReceiptV5R9,
  validateNormalC0ExecutionSetV5R7 as validateNormalC0ExecutionSetV5R9,
...
function assertCommon(input) {
  const errors = [
    ...validateClosedSelfHashedArtifactV5R9(input?.activeRegistration,
      input?.activeRegistration?.schemaVersion),
    ...validateClosedSelfHashedArtifactV5R9(input?.authorization, "ProviderAuthorizationV5"),
    ...validateClosedSelfHashedArtifactV5R9(input?.executionRegistration,
      "DeepSeekExecutionRegistrationV4"),
    ...validateClosedSelfHashedArtifactV5R9(input?.inventory, "SampleExecutionInventoryV2"),
  ];
  try {
  ...
  requireCondition(errors.length === 0, `R9 semantic-dispatch upstream validation failed: ${errors.join("; ")}`);
  ...
  "R9 semantic-dispatch registration, authorization, and inventory bindings are invalid");
}
...
function validateSemanticState(input) {
  ...
  if (input.canaryGate) {
    errors.push(...validateCanaryGateReceiptV5R9({
      ...(input.canaryGateContext ?? {}),
      canaryGate: input.canaryGate,
    }));
  }
  if (input.c0ExecutionSet) {
    errors.push(...validateNormalC0ExecutionSetV5R9({
      activeRunnerRegistrationHash: input.activeRegistration.selfHash,
      executionRegistrationHash: input.executionRegistration.selfHash,
      inventory: input.inventory,
...
export function buildCompatibilityCanaryC0DecisionSetV5R9(canaryPredicateReceipt) {
  ...
  "R9 compatibility canary decision set requires the exact sealed canary predicate");
...
export function buildSemanticDispatchAuthorityV5R9(input) {
  assertCommon(input);
  const semanticErrors = validateSemanticState(input);
  requireCondition(semanticErrors.length === 0,
    `R9 semantic canary/C0 state is not exactly rebuildable: ${semanticErrors.join("; ")}`);
  requireCondition(["DEEPSEEK_CANARY", "DEEPSEEK_RESUME"].includes(input.mode),
    "R9 semantic-dispatch mode is invalid");
  const canary = input.inventory.items[0];
  ...
  requireCondition(canaryFirstVerified,
    "R9 semantic dispatch cannot proceed before an exact canary-first gate");
  if (input.mode === "DEEPSEEK_RESUME") {
    ...
    "R9 resume requires the exact canary gate and either pre-C0 base-role state or a frozen normal C0 set");
  }
  const compatibilityC0ExecutionSet = input.mode === "DEEPSEEK_CANARY" && input.canaryPredicateReceipt
    ? buildCompatibilityCanaryC0DecisionSetV5R9(input.canaryPredicateReceipt)
  ...
  requireCondition(plan?.planStatus === "NEXT_ACTION", "R9 frozen planner has no dispatchable next action");
  ...
  requireCondition(selectedC0MembershipVerified,
    "R9 C0 action is outside the exact predicate/random-plus-mandatory selected set");
  const item = input.inventory.items[plan.manifestOrdinal - 1];
  requireCondition(item?.itemHash === plan.itemHash, "R9 planner item is outside the manifest-bound inventory");
  const attemptOrdinal = plan.failedAttemptsForItemRole + 1;
  const attemptId = `V5R9:${input.activeRegistration.selfHash}:${plan.selfHash}:${attemptOrdinal}`;
  ...
  assertClosedSelfHashedArtifactV5R9(receipt, receipt.schemaVersion);
  return receipt;
}

export function validateSemanticDispatchAuthorityV5R9({ semanticDispatchAuthority, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R9(semanticDispatchAuthority,
    "SemanticDispatchAuthorityReceiptV1")];
  try {
    const rebuilt = buildSemanticDispatchAuthorityV5R9({
      ...input,
      issuedAt: semanticDispatchAuthority?.issuedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(semanticDispatchAuthority)) {
      errors.push("R9 semantic dispatch authority differs from exact frozen planner reconstruction");
    }
  } catch (error) {
...
export function buildSemanticDispatchVerificationReceiptV5R9({ activeRegistration, authorization,
  ...
  "R9 semantic verification requires one authority per role-attempt evidence receipt");
  ...
  requireCondition(authorityByHash.size === semanticDispatchAuthorities.length,
    "R9 semantic authorities are duplicated");
  requireCondition(semanticAuthorityContextsByHash instanceof Map,
    "R9 semantic verification requires reconstruction inputs for every authority");
  for (const authority of semanticDispatchAuthorities) {
    ...
    const authorityErrors = reconstruction
      ? validateSemanticDispatchAuthorityV5R9({
        ...reconstruction,
        semanticDispatchAuthority: authority,
      }) : ["authority reconstruction input is absent"];
    requireCondition(authorityErrors.length === 0,
      `R9 semantic authority is not exactly rebuildable: ${authorityErrors.join("; ")}`);
  }
  for (const evidence of roleAttemptEvidenceReceipts) {
    const evidenceErrors = validateClosedSelfHashedArtifactV5R9(evidence, "RoleAttemptEvidenceReceiptV1");
    ...
    `R9 semantic verification found an unbound authority/evidence pair: ${evidenceErrors.join("; ")}`);
  }
...
export function validateSemanticDispatchVerificationReceiptV5R9({ semanticDispatchVerificationReceipt,
  ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R9(semanticDispatchVerificationReceipt,
    "SemanticDispatchVerificationReceiptV1")];
  try {
    const rebuilt = buildSemanticDispatchVerificationReceiptV5R9({
      ...input,
    ...
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(semanticDispatchVerificationReceipt)) {
      errors.push("R9 semantic verification differs from exact ordered authority/evidence reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const SEMANTIC_DISPATCH_V5_R9_CONSTANTS = Object.freeze({
  c0Roles: C0_ROLES,
