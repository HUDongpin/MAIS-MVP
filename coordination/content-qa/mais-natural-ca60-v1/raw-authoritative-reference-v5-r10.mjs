...
function validateGraphAndBlindness(input) {
  ...
  `R10 reference seal requires a complete independently rebuilt OpenAI attempt graph: ${graphErrors.join("; ")}`);
  ...
  "R10 reference graph contains missing successes, non-reparsed success, leaked input, or extra-role evidence");
  ...
  requireCondition(exactSet(successfulOutputHashes,
    ...
    "R10 reference graph evidence does not equal the complete successful compatibility output set");
}

export function buildRawAuthoritativeMachineReferenceSealV5R10(input) {
  ...
  "R10 reference seal requires the exact active registration and fresh A11 concurrence");
  const authErrors = validateClosedSelfHashedArtifactV5R10(input?.authorization, "ProviderAuthorizationV5");
  ...
  `R10 OpenAI reference authorization is invalid: ${authErrors.join("; ")}`);
  ...
  "R10 compatibility reference seal differs from the raw-authoritative attempt graph");
...
export function validateRawAuthoritativeMachineReferenceSealV5R10({ seal, compatibilityReferenceSeal,
  ...input }) {
  ...
  try {
    const rebuilt = buildRawAuthoritativeMachineReferenceSealV5R10({
      ...input,
    ...
    if (canonicalJsonV5R3(rebuilt.seal) !== canonicalJsonV5R3(seal)
      ...
      errors.push("R10 reference seal differs from raw-authoritative graph and label reconstruction");
    }
  } catch (error) {
...
export function buildRawAuthoritativeReferenceValidationV5R10({ seal, compatibilityReferenceSeal,
  validatedAt, ...input }) {
  const errors = validateRawAuthoritativeMachineReferenceSealV5R10({
    ...input,
    seal,
    compatibilityReferenceSeal,
  });
  requireCondition(errors.length === 0 && Date.parse(validatedAt) > Date.parse(seal.sealedAt),
    `R10 reference validation failed or does not postdate sealing: ${errors.join("; ")}`);
  const graph = input.attemptGraphReceipt;
...
export function validateRawAuthoritativeReferenceValidationV5R10({ validationReceipt, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(validationReceipt, VALIDATION_SCHEMA)];
  try {
    const rebuilt = buildRawAuthoritativeReferenceValidationV5R10({
      ...input,
      validatedAt: validationReceipt?.validatedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(validationReceipt)) {
      errors.push("R10 reference validation receipt differs from exact reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const RAW_AUTHORITATIVE_REFERENCE_V5_R10_CONSTANTS = Object.freeze({
