...
import {
  validateProviderActivationV5R9,
} from "./activation-guard-v5-r9.mjs";
import {
  validateRouteProbeArtifactSetV5R9,
  validateTrustedProviderEvidenceEnvelopeV5R9,
} from "./trusted-provider-evidence-v5-r9.mjs";
import {
  applyInterruptedAttemptReconciliationV5R9,
  inspectInterruptedAttemptCustodyV5R9,
  validateResumeCustodyV5R9,
} from "./attempt-recovery-v5-r9.mjs";
import {
  buildTerminalExecutionBundleFromRawCustodyV5R9,
  scoreNaturalCaV5R9,
  verifyNaturalCaScoreFromRawEvidenceV5R9,
} from "./scorer-verifier-v5-r9.mjs";
import {
  verifyTerminalExecutionDecisionReceiptV5R9,
} from "./statistical-kernel-v5-r9.mjs";
import {
  ACTIVE_REGISTRATION_KIND_V5_R9,
  ACTIVE_REVIEW_KIND_V5_R9,
  advanceProtectedWorkflowIndexV5R9,
  loadProtectedWorkflowIndexV5R9,
  executionLedgerRelativePathV5R9,
} from "./workflow-index-v5-r9.mjs";
import {
...
import {
  buildReferenceDispatchAuthorityV5R9,
} from "./dispatch-authority-v5-r9.mjs";
import {
  buildSemanticDispatchAuthorityV5R9,
  buildSemanticDispatchVerificationReceiptV5R9,
} from "./semantic-dispatch-v5-r9.mjs";
import {
  NativeProviderAttemptFailureV5R9,
  createNativeProviderTransportV5R9,
  runNativeProviderAttemptV5R9,
} from "./native-provider-attempt-v5-r9.mjs";
import {
...
import {
  createAttemptCustodyStoreV5R9,
  reloadAttemptCustodyStoreV5R9,
} from "./attempt-custody-store-v5-r9.mjs";
import {
  reconstructAttemptGraphV5R9,
  roleOutputByEvidenceV5R9,
  validateAttemptGraphReconstructionReceiptV5R9,
} from "./attempt-graph-v5-r9.mjs";
import {
...
import {
  buildRawAuthoritativeMachineReferenceSealV5R9,
  buildRawAuthoritativeReferenceValidationV5R9,
} from "./raw-authoritative-reference-v5-r9.mjs";
import {
  buildDeepSeekExecutionRegistrationV5R9,
} from "./execution-freeze-v5-r9.mjs";
import {
  atomicWriteProtectedJsonV5R4,
  readProtectedJsonV5R4,
} from "./protected-storage-v5-r4.mjs";
import {
  buildResolvedProviderAttemptReceiptV5R9,
} from "./attempt-transaction-v5-r9.mjs";
import {
...
function clockIso(clock) {
  ...
  requireCondition(Number.isFinite(date.getTime()), "R9 runner clock is invalid");
...
function artifact(context, kind, { required = true } = {}) {
  ...
  if (required && value === undefined) throw new Error(`protected R9 workflow artifact ${kind} is absent`);
  return value;
}
...
async function writeContentAddressedV5R9({ trustedRoot, family, value }) {
  requireCondition(validateSelfHashV5R3(value), "R9 runtime custody value is not self-hashed");
  const relativePath = path.join("runtime-custody-v5-r9", family, `${value.selfHash}.json`);
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R9 runtime custody path already contains different canonical bytes");
  } catch (error) {
...
function appendStore(context, family) {
  return Object.freeze({ append: (value) => writeContentAddressedV5R9({
...
function providerPrefix(provider) {
  requireCondition(PROVIDERS.has(provider), "R9 provider is invalid");
  return provider === "OPENAI_DIRECT" ? "OPENAI" : "DEEPSEEK";
}

function providerArtifact(context, provider, suffix, options) {
  return artifact(context, `${providerPrefix(provider)}_${suffix}_V5_R9`, options);
}
...
function upstreamErrors(context, { requireAdoption = true } = {}) {
  const errors = [...validateExactRunnerRegistrationEvidenceV5R9(context?.registrationEvidence),
    ...validateFreshRunnerReviewV5R9(reviewInput(context))];
  if (requireAdoption && context?.needsR9Adoption) {
    errors.push("protected workflow has not adopted the exact V5-R9 registration");
  }
  try {
...
function activationInput(context, provider, at) {
  ...
  return {
    ...
    referenceSeal: artifact(context, "REFERENCE_SEAL_V5_R9", { required: false }),
    compatibilityReferenceSeal: artifact(context, "REFERENCE_COMPATIBILITY_SEAL_V5_R9",
      { required: false }),
    referenceSealValidationReceipt: artifact(context, "REFERENCE_SEAL_VALIDATION_V5_R9",
      { required: false }),
    referenceSealContext: context.referenceSealContext,
    executionRegistration: artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R9",
      { required: false }),
    provider,
  };
}
...
async function persistResult(advanceWorkflow, context, command, entries, at) {
  requireCondition(Array.isArray(entries) && entries.length > 0,
    "R9 result persistence requires at least one sealed artifact");
  return advanceWorkflow({ context, command, appendedArtifacts: entries, committedAt: at });
}

export async function persistFinalCommandJournalV5R9({ advanceWorkflow, context, journal,
  command, committedAt }) {
  ...
  "R9 final command-journal persistence requires an active context, command, and chronology");
  const errors = validateClosedSelfHashedArtifactV5R9(journal,
  ...
  `R9 final command journal is invalid or registration-drifted: ${errors.join("; ")}`);
  return persistResult(advanceWorkflow, context, command, [{
    kind: `COMMAND_TRANSITION_JOURNAL_V5_R9:${journal.selfHash}`,
    family: "transition-journals-indexed",
    value: journal,
  }], committedAt);
}

export function createRunnerRuntimeV5R9({
  protectedRoot = DEFAULT_PROTECTED_ROOT,
  repoRoot = DEFAULT_REPO_ROOT,
  clock = () => new Date(),
  advanceWorkflow = advanceProtectedWorkflowIndexV5R9,
  fetchImplementation = globalThis.fetch,
  credentialReaders = Object.freeze({}),
  nativeAttemptRunner = runNativeProviderAttemptV5R9,
  transportFactory = createNativeProviderTransportV5R9,
  registrationEvidenceLoader,
  freshReviewLoader,
} = {}) {
  async function loadWorkflowContext(indexPath) {
    requireCondition(path.isAbsolute(indexPath ?? ""),
      "R9 workflow index path must be absolute");
    return loadProtectedWorkflowIndexV5R9(indexPath, {
      protectedRoot,
      repoRoot,
      registrationEvidenceLoader,
      freshReviewLoader,
  ...
  async function openLedger(context, evidence) {
    return createAtomicExecutionLedgerV5R7({
      ...
      ledgerRelativePath: executionLedgerRelativePathV5R9(evidence.authorization.provider,
  ...
  function roleOutputsForEvidence(evidenceReceipts, ledgerEntries) {
    return roleOutputByEvidenceV5R9({ roleAttemptEvidenceReceipts: evidenceReceipts, ledgerEntries });
  }

  function canaryPredicateState(context, evidence, ledgerEntries, extras = []) {
    const predicate = artifact(context, "CANARY_C0_PREDICATE_V5_R9", { required: false });
    ...
    requireCondition(graph, "R9 canary predicate source graph is absent");
  ...
  function normalC0State(context, evidence, ledgerEntries, extras = []) {
    const c0ExecutionSet = artifact(context, "DEEPSEEK_C0_EXECUTION_SET_V5_R9", { required: false });
    ...
    requireCondition(graph, "R9 full base-role source graph for the C0 set is absent");
    const baseEvidence = successfulEvidence(graph, BASE_ROLES);
    ...
    const errors = validateNormalC0ExecutionSetV5R7({
      ...
      executionRegistrationHash: artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R9").selfHash,
  ...
  function canaryGateState(context, evidence, ledgerEntries, extras = []) {
    const canaryGate = artifact(context, "CANARY_GATE_V5_R9", { required: false });
    ...
    requireCondition(graph && prefixEnd >= 0 && semantic,
      "R9 canary gate graph, ledger prefix, or semantic receipt is absent");
  ...
  function buildGraphContext(context, evidence, ledgerEntries, extras = [], overrides = {}) {
    ...
    return {
      ...
      executionRegistration: evidence.provider === "DEEPSEEK_DIRECT"
        ? artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R9") : null,
      canaryPredicateReceipt: predicateState.predicate,
      canaryPredicateInput: predicateState.input,
  ...
  function reconstructGraph(context, evidence, ledgerEntries, extras, derivedAt, overrides = {}) {
    ...
    const built = reconstructAttemptGraphV5R9({ ...graphContext, derivedAt });
  ...
  function semanticAuthorityContexts(context, evidence, ledgerEntries, graphContext, graph) {
    ...
    for (const roleEvidence of graph.roleAttemptEvidenceReceipts) {
      ...
      requireCondition(authority, "R9 semantic evidence lacks its exact authority");
      ...
      requireCondition(reservationIndex >= 0, "R9 semantic authority reservation is absent");
  ...
  function buildSemanticVerification(context, evidence, ledgerEntries, built, verifiedAt, mode,
    ...
    requireCondition(authorities.every(Boolean),
      "R9 graph evidence lacks ordered semantic authorities");
    return buildSemanticDispatchVerificationReceiptV5R9({
      activeRegistration: context.activeRegistration,
  ...
  async function referenceSealContextFromContext(context, { derivedAt = clockIso(clock) } = {}) {
    ...
    const seal = artifact(context, "REFERENCE_SEAL_V5_R9", { required: false });
    ...
    if (graph) requireCondition(canonicalJsonV5R3(graph) === canonicalJsonV5R3(built.receipt),
      "stored R9 reference graph differs from raw-attempt reconstruction");
  ...
  async function adoptR9WorkflowIndex(context) {
    try {
      ...
      if (!context.needsR9Adoption) {
        return success("V5_R9_WORKFLOW_INDEX_ALREADY_ACTIVE_NO_PROVIDER_ACTIVITY");
      }
      const committedAt = transitionTime(context, clockIso(clock));
      const process = context.processArtifacts;
      const transition = await persistResult(advanceWorkflow, context, "register-v5-r9", [
        { kind: ACTIVE_REGISTRATION_KIND_V5_R9, family: "runner-registrations",
          value: context.activeRegistration },
        { kind: ACTIVE_REVIEW_KIND_V5_R9, family: "runner-reviews", value: context.freshReview },
        { kind: `A11_STATIC_IMPORT_GRAPH_V5_R9:${process.staticImportGraphReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.staticImportGraphReceipt },
        { kind: `A11_FORBIDDEN_PATH_SCAN_V5_R9:${process.forbiddenPrimaryScorerPathScanReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.forbiddenPrimaryScorerPathScanReceipt },
        { kind: `A11_COMMAND_RUNTIME_V5_R9:${process.commandRuntimeReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.commandRuntimeReceipt },
        { kind: `A11_SOURCE_ENUMERATION_V5_R9:${process.independentSourceEnumerationReceipt.selfHash}`,
          family: "runner-review-process-evidence", value: process.independentSourceEnumerationReceipt },
      ], committedAt);
      context.needsR9Adoption = false;
      return success("V5_R9_WORKFLOW_INDEX_ADOPTED_NO_PROVIDER_ACTIVITY", { transition });
    } catch (error) {
      return blocked("V5_R9_WORKFLOW_INDEX_ADOPTION_BLOCKED",
  ...
  async function verifyFrozenUpstream(context) {
    ...
    return errors.length > 0 ? blocked("FROZEN_UPSTREAM_VERIFICATION_BLOCKED", errors)
      : success("V5_R9_FROZEN_FRAME_SAMPLE_METHOD_AND_CLAIM_CEILING_VERIFIED_NO_PROVIDER_ACTIVITY");
  }

  async function dryRun(context) {
    const errors = upstreamErrors(context);
    if (errors.length > 0) return blocked("V5_R9_DRY_RUN_BLOCKED", errors);
    const noAnchor = context.activeRegistration.trustedProviderEvidenceAnchors.length === 0;
    return success(noAnchor
      ? "V5_R9_OFFLINE_DRY_RUN_READY_ROUTE_AUTHENTICITY_BLOCKED_NO_PROVIDER_ACTIVITY"
      : "V5_R9_OFFLINE_DRY_RUN_READY_NO_PROVIDER_ACTIVITY", {
  ...
  async function authorizeCheck(context, provider) {
    try {
      ...
      const errors = [...upstreamErrors(context), ...validateProviderActivationV5R9(input)];
  ...
  async function registerTrustedRouteEvidence(context, provider) {
    try {
      ...
      const setErrors = validateRouteProbeArtifactSetV5R9(routeProbeArtifactSet);
      requireCondition(setErrors.length === 0, setErrors.join("; "));
      const errors = validateTrustedProviderEvidenceEnvelopeV5R9({ envelope,
      ...
      const additions = routeAlreadyRegistered ? [] : [
        { kind: `${providerPrefix(provider)}_TRUSTED_ROUTE_ENVELOPE_V5_R9`,
          family: "trusted-route-evidence", value: envelope },
        { kind: `${providerPrefix(provider)}_AUTHENTICATED_ROUTE_EVIDENCE_V5_R9`,
          family: "authenticated-route-evidence", value: authenticatedRouteEvidence },
        { kind: `${providerPrefix(provider)}_ROUTE_PROBE_ARTIFACT_SET_V5_R9`,
          family: "route-probe-artifact-sets", value: routeProbeArtifactSet },
        { kind: `${providerPrefix(provider)}_PRICE_SNAPSHOT_V5_R9`,
      ...
      requireCondition(presentLiveCandidates.length === 0 || presentLiveCandidates.length === 3,
        "R9 authorization, owner-grant, and cost-preview candidates must be adopted all-or-none");
      requireCondition(presentLiveCandidates.length === 0
        || !providerArtifact(context, provider, "AUTHORIZATION", { required: false }),
      "R9 live authorization is already registered; replacement requires a new registration version");
      if (presentLiveCandidates.length === 3) {
        ...
        if (provider === "DEEPSEEK_DIRECT") {
          ...
          const referenceInputs = {
            ...
            compatibilityReferenceSeal: artifact(context,
              "REFERENCE_COMPATIBILITY_SEAL_V5_R9", { required: false }),
            referenceSealValidationReceipt: artifact(context,
              "REFERENCE_SEAL_VALIDATION_V5_R9", { required: false }),
            referenceSealContext,
          };
          const provisionalExecutionRegistration = buildDeepSeekExecutionRegistrationV5R9({
            ...activation,
            ...referenceInputs,
        ...
        const activationErrors = validateProviderActivationV5R9(activation);
        requireCondition(activationErrors.length === 0,
          `R9 candidate live bundle is invalid: ${activationErrors.join("; ")}`);
      }
      for (const { suffix, family, value } of presentLiveCandidates) additions.push({
        kind: `${providerPrefix(provider)}_${suffix}_V5_R9`, family, value,
      });
      requireCondition(additions.length > 0,
        "R9 route and authorization evidence are already registered or no complete candidate exists");
  ...
  async function makeJournal(context, command, startedAt) {
    ...
    return createCommandTransitionJournalV5R7({
      ...
      activeRunnerRegistrationHash: context.activeRegistration.selfHash,
      startedAt,
      persistIntent: (value) => writeContentAddressedV5R9({ trustedRoot: context.trustedRoot,
        family: "transition-intents", value }),
      persistSubreceipt: (value) => writeContentAddressedV5R9({ trustedRoot: context.trustedRoot,
        family: "transition-subreceipts", value }),
      persistFinalJournal: (value) => writeContentAddressedV5R9({ trustedRoot: context.trustedRoot,
  ...
  function attemptArtifactAdditions(run) {
    return [
      { kind: `DISPATCH_PERMIT_V5_R9:${run.permit.selfHash}`,
        family: "dispatch-permits", value: run.permit },
      { kind: `COMPATIBILITY_DISPATCH_PERMIT_V5_R9:${run.compatibilityPermit.selfHash}`,
        family: "compatibility-dispatch-permits", value: run.compatibilityPermit },
      { kind: `RAW_RESPONSE_ARTIFACT_V5_R9:${run.rawResponseArtifact.selfHash}`,
        family: "raw-response-artifacts", value: run.rawResponseArtifact },
      { kind: `RAW_RESPONSE_BINDING_V5_R9:${run.rawResponseBindingReceipt.selfHash}`,
        family: "raw-response-bindings", value: run.rawResponseBindingReceipt },
      { kind: `PROVIDER_EVENT_V5_R9:${run.providerEventReceipt.selfHash}`,
        family: "provider-events", value: run.providerEventReceipt },
      { kind: `ATTEMPT_COMMIT_INTENT_V5_R9:${run.attemptCommitIntent.selfHash}`,
        family: "attempt-commit-intents", value: run.attemptCommitIntent },
      { kind: `RESOLVED_PROVIDER_ATTEMPT_V5_R9:${run.resolvedAttemptReceipt.selfHash}`,
        family: "resolved-provider-attempts", value: run.resolvedAttemptReceipt },
      ...(run.roleOutput ? [{ kind: `ROLE_OUTPUT_V5_R9:${run.roleOutput.selfHash}`,
  ...
  function derivePostAttemptArtifacts(context, evidence, ledgerEntries, extras, at) {
    ...
    requireCondition(built.receipt.graphStatus === "COMPLETE_VALID",
      built.receipt.lineageErrors.join("; "));
    const additions = [{ kind: `ATTEMPT_GRAPH_V5_R9:${built.receipt.selfHash}`,
    ...
    if (!predicate && canaryBase.length === 2) {
      ...
      additions.push({ kind: "CANARY_C0_PREDICATE_V5_R9",
    ...
    if (!built.graphContext.canaryGate && predicate && canaryEvidence.length === requiredCanaryCount) {
      ...
      additions.push({ kind: `SEMANTIC_DISPATCH_VERIFICATION_V5_R9:${semantic.selfHash}`,
        family: "semantic-dispatch-verifications", value: semantic },
      { kind: "CANARY_GATE_V5_R9", family: "canary-gates", value: gate });
    }
    ...
    if (!c0ExecutionSet && new Set(baseEvidence
      ...
      additions.push({ kind: "DEEPSEEK_C0_EXECUTION_SET_V5_R9",
  ...
  async function executeProviderStep(context, provider, executionMode) {
    ...
    try {
      ...
      const errors = [...upstreamErrors(context), ...validateProviderActivationV5R9(activation)];
      requireCondition(errors.length === 0, errors.join("; "));
      const custodyErrors = validateResumeCustodyV5R9(defaultAttemptCustody(context, provider));
      ...
      const dispatchAuthority = provider === "OPENAI_DIRECT"
        ? buildReferenceDispatchAuthorityV5R9(authorityContext)
        : buildSemanticDispatchAuthorityV5R9(authorityContext);
      ...
      const custodyStore = await createAttemptCustodyStoreV5R9({ trustedRoot: context.trustedRoot,
      ...
      requireCondition(transport?.kind === "V5_R9_NATIVE_EXACT_TRANSPORT",
        "R9 native transport factory did not return the exact transport contract");
      ...
      const journalResult = await runJournaledTransitionSequenceV5R7({ journal, steps: [
        { name: "REQUEST_AND_DISPATCH_AUTHORITY", preparedAt: at,
          ...
          run: () => persistResult(advanceWorkflow, context, "request-and-dispatch-authority", [
            { kind: `PROVIDER_REQUEST_V5_R9:${requestArtifact.selfHash}`,
              family: "provider-requests", value: requestArtifact },
            { kind: `DISPATCH_AUTHORITY_V5_R9:${dispatchAuthority.selfHash}`,
        ...
        { name: "NATIVE_PROVIDER_ATTEMPT_AND_DERIVED_STATE", preparedAt: later(at, 2),
          ...
          run: async () => {
            ...
            try {
              ...
              const activity = error instanceof NativeProviderAttemptFailureV5R9
                ? error.activity : null;
      ...
      if (!journalResult.ok) {
        ...
        try {
          failedJournalTransition = await persistFinalCommandJournalV5R9({ advanceWorkflow,
      ...
      const journalTransition = await persistFinalCommandJournalV5R9({ advanceWorkflow,
      ...
      context.attemptCustodyByProvider[provider] = Object.freeze({ ledgerEntries: after.entries,
        ...
        custodyErrors: validateResumeCustodyV5R9({ ledgerEntries: after.entries,
          preparedCompletions: reloaded.preparedCompletions,
          attemptCommitIntents: reloaded.attemptCommitIntents,
          resolvedAttemptReceipts: reloaded.resolvedAttemptReceipts }) });
      return Object.freeze({ ...run, ok: run.status === "SUCCEEDED",
        status: run.status === "SUCCEEDED"
          ? "V5_R9_PROVIDER_ATTEMPT_SUCCEEDED_AND_RECONSTRUCTED"
          : `V5_R9_PROVIDER_ATTEMPT_RECORDED_${run.status}`,
      ...
      const activity = error instanceof NativeProviderAttemptFailureV5R9 ? error.activity : error?.activity;
  ...
  async function sealReferenceLabels(context) {
    try {
      const errors = upstreamErrors(context);
      requireCondition(errors.length === 0, errors.join("; "));
      requireCondition(!artifact(context, "REFERENCE_SEAL_V5_R9", { required: false }),
        "R9 reference labels are already sealed; replacement requires a new registration version");
      ...
      const referenceSealContext = await referenceSealContextFromContext(context,
        { derivedAt: sealedAt });
      const built = buildRawAuthoritativeMachineReferenceSealV5R9({
        ...referenceSealContext, sealedAt,
      });
      const validatedAt = later(sealedAt, 1);
      const validationReceipt = buildRawAuthoritativeReferenceValidationV5R9({
        ...referenceSealContext,
        seal: built.seal,
      ...
      const transition = await persistResult(advanceWorkflow, context,
        "seal-reference-labels", [
          { kind: `ATTEMPT_GRAPH_V5_R9:${referenceSealContext.attemptGraphReceipt.selfHash}`,
            family: "attempt-graphs", value: referenceSealContext.attemptGraphReceipt },
          { kind: "REFERENCE_COMPATIBILITY_SEAL_V5_R9",
            family: "reference-compatibility-seals", value: built.compatibilityReferenceSeal },
          { kind: "REFERENCE_SEAL_V5_R9", family: "reference-seals", value: built.seal },
          { kind: "REFERENCE_SEAL_VALIDATION_V5_R9",
            family: "reference-seal-validations", value: validationReceipt },
        ], validatedAt);
      return success("MACHINE_REFERENCE_LABELS_FROZEN_RAW_AUTHORITATIVE_V5_R9_NO_PROVIDER_ACTIVITY", {
  ...
  async function freezeDeepSeekExecutionRegistration(context) {
    try {
      const errors = upstreamErrors(context);
      requireCondition(errors.length === 0, errors.join("; "));
      requireCondition(!artifact(context, "DEEPSEEK_EXECUTION_REGISTRATION_V5_R9",
        { required: false }),
      "R9 DeepSeek execution registration is already frozen");
      ...
      const executionRegistration = buildDeepSeekExecutionRegistrationV5R9({
        ...evidence,
        referenceSeal: artifact(context, "REFERENCE_SEAL_V5_R9"),
        compatibilityReferenceSeal: artifact(context, "REFERENCE_COMPATIBILITY_SEAL_V5_R9"),
        referenceSealValidationReceipt: artifact(context, "REFERENCE_SEAL_VALIDATION_V5_R9"),
        referenceSealContext,
        registeredAt,
      ...
      const transition = await persistResult(advanceWorkflow, context,
        "freeze-deepseek-registration", [{ kind: "DEEPSEEK_EXECUTION_REGISTRATION_V5_R9",
          family: "deepseek-execution-registrations", value: executionRegistration }], registeredAt);
      return success("DEEPSEEK_EXECUTION_REGISTRATION_V5_R9_FROZEN_NO_PROVIDER_ACTIVITY", {
  ...
  async function auditAttemptCustody(context, provider) {
    const errors = validateResumeCustodyV5R9(defaultAttemptCustody(context, provider));
  ...
  function rebuildResolvedAttemptFromCustody(context, provider, authorization, custody) {
    ...
    requireCondition(preparedCompletion && custody.intent,
      "R9 recovery cannot rebuild a resolved receipt without exact intent/completion custody");
    ...
    "R9 recovery is missing exact request/audit/permit/raw/binding/event custody");
    return buildResolvedProviderAttemptReceiptV5R9({
      activeRunnerRegistrationHash: context.activeRegistration.selfHash,
  ...
  async function loadRecoveryInput(recoveryPath, provider, context) {
    requireCondition(path.isAbsolute(recoveryPath ?? "") && PROVIDERS.has(provider),
      "R9 recovery loader requires an absolute protected path and exact provider");
    ...
    "R9 recovery authorization path must remain inside the established protected root");
    const bundle = await readProtectedJsonV5R4({ trustedRoot: context.trustedRoot, relativePath });
    const schemaErrors = validateClosedSelfHashedArtifactV5R9(bundle,
    ...
    `R9 recovery authorization bundle is invalid: ${schemaErrors.join("; ")}`);
    ...
    requireCondition(bundle.providerAuthorizationHash === authorization.selfHash,
      "R9 recovery bundle provider authorization differs from the active custody");
    const providerCustody = defaultAttemptCustody(context, provider);
    const custody = inspectInterruptedAttemptCustodyV5R9({ ...providerCustody,
  ...
  async function reconcileInterruptedAttempt(context, recoveryInput) {
    try {
      const result = await applyInterruptedAttemptReconciliationV5R9(recoveryInput);
      const additions = [{ kind: `ATTEMPT_CUSTODY_RECONCILIATION_V5_R9:${result.receipt.selfHash}`,
      ...
      if (recoveryInput.recoveredResolvedAttemptReceipt) additions.push({
        kind: `RESOLVED_PROVIDER_ATTEMPT_V5_R9:${recoveryInput.recoveredResolvedAttemptReceipt.selfHash}`,
      ...
      context.attemptCustodyByProvider = await (async () => {
        ...
        requireCondition(PROVIDERS.has(provider), "R9 recovery input provider is absent");
        const authorization = providerArtifact(context, provider, "AUTHORIZATION");
        ...
        current[provider] = Object.freeze({ ...current[provider], ledgerEntries: verified.entries,
          ...
          custodyErrors: validateResumeCustodyV5R9({ ledgerEntries: verified.entries,
            preparedCompletions: reloaded.preparedCompletions,
            attemptCommitIntents: reloaded.attemptCommitIntents,
            resolvedAttemptReceipts: reloaded.resolvedAttemptReceipts }) });
        return current;
  ...
  function terminalC0Scope(context, inventory) {
    const receipt = assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
  ...
  async function nativeScoringEvidence(context, scoredAt, { terminal = false,
    ...
    "stored R9 DeepSeek graph differs from exact raw-custody reconstruction");
    ...
    requireCondition(c0ExecutionSet,
      "R9 complete scoring requires the frozen normal C0 execution set");
    ...
    if (!terminal) {
      ...
      requireCondition(successfulEvidence(built.receipt).length === expectedSuccessfulCount,
        "R9 execution is incomplete; complete frozen-metric scoring is forbidden");
      ...
      "stored R9 semantic verification differs from exact reconstruction");
    }
    const referenceSealContext = await referenceSealContextFromContext(context,
      { derivedAt: scoredAt });
  ...
  async function persistScoringBundle(context, built, terminalReceipt, committedAt) {
    const entries = [
      { kind: `ATTEMPT_GRAPH_V5_R9:${built.rawEvidence.deepSeekAttemptGraphReceipt.selfHash}`,
        family: "attempt-graphs", value: built.rawEvidence.deepSeekAttemptGraphReceipt },
      { kind: `SEMANTIC_DISPATCH_VERIFICATION_V5_R9:${built.rawEvidence.semanticDispatchVerificationReceipt.selfHash}`,
        family: "semantic-dispatch-verifications",
        value: built.rawEvidence.semanticDispatchVerificationReceipt },
      { kind: `DECISION_EVIDENCE_V5_R9:${built.decisionEvidenceReceipt.selfHash}`,
        family: "decision-evidence", value: built.decisionEvidenceReceipt },
      ...built.itemResults.map((value) => ({ kind: `ITEM_RESULT_V5_R9:${value.itemHash}`,
        family: "item-results", value })),
      ...built.completedItemMarkers.map((value) => ({
        kind: `COMPLETED_ITEM_MARKER_V5_R9:${value.itemHash}`,
        family: "completed-item-markers", value })),
      { kind: `OBSERVED_LEDGER_V5_R9:${built.observedLedger.selfHash}`,
        family: "observed-ledgers", value: built.observedLedger },
      { kind: `COUNTERFACTUAL_LEDGER_V5_R9:${built.counterfactualLedger.selfHash}`,
        family: "counterfactual-ledgers", value: built.counterfactualLedger },
      { kind: `METRIC_INPUT_LEDGER_V5_R9:${built.metricInputLedger.selfHash}`,
        family: "metric-input-ledgers", value: built.metricInputLedger },
      { kind: `FINAL_EVALUATION_V5_R9:${built.finalReceipt.selfHash}`,
        family: "final-evaluations", value: built.finalReceipt },
      { kind: `AGGREGATE_SCORE_V5_R9:${built.scoreReceipt.selfHash}`,
        family: "aggregate-scores", value: built.scoreReceipt },
    ];
    if (terminalReceipt) entries.push({ kind: `TERMINAL_DECISION_V5_R9:${terminalReceipt.selfHash}`,
      family: "terminal-decisions", value: terminalReceipt });
    return persistResult(advanceWorkflow, context, "score-v5-r9", entries, committedAt);
  }
  ...
  async function persistTerminalBundle(context, built, committedAt) {
    const entries = [
      { kind: `ATTEMPT_GRAPH_V5_R9:${built.rawEvidence.deepSeekAttemptGraphReceipt.selfHash}`,
        family: "attempt-graphs", value: built.rawEvidence.deepSeekAttemptGraphReceipt },
      { kind: `TERMINAL_C0_SCOPE_V5_R9:${built.rawEvidence.c0ExecutionSet.selfHash}`,
      ...
      ...built.completedItemMarkers.map((value) => ({
        kind: `COMPLETED_ITEM_MARKER_V5_R9:${value.itemHash}`,
        family: "completed-item-markers", value })),
      { kind: `TERMINAL_DECISION_V5_R9:${built.terminalReceipt.selfHash}`,
        family: "terminal-decisions", value: built.terminalReceipt },
    ];
    return persistResult(advanceWorkflow, context, "score-terminal-v5-r9", entries, committedAt);
  }
  ...
  async function score(context) {
    ...
    try {
      ...
      const built = scoreNaturalCaV5R9({ ...input, scoredAt: later(scoredAt, 1) });
      if (built.finalReceipt.overallDecision === "EXECUTION_INTEGRITY_FAILED") {
        const terminal = buildTerminalExecutionBundleFromRawCustodyV5R9({
    ...
    try {
      ...
      if (scoreReceipt) {
        ...
        requireCondition(graphReceipt && semanticReceipt,
          "R9 score-bound graph or semantic receipt is absent");
        ...
        const verifyErrors = verifyNaturalCaScoreFromRawEvidenceV5R9({ ...input, scoreReceipt });
        return verifyErrors.length > 0
          ? blocked("EXECUTION_RESULT_VERIFICATION_BLOCKED", verifyErrors)
          : success("V5_R9_RAW_EVIDENCE_SCORE_RECOMPUTED_NO_PASS_CLAIM", { scoreReceipt,
      ...
      requireCondition(terminalReceipt, "no R9 aggregate or terminal execution receipt exists");
      const graphReceipt = findByHash(context, terminalReceipt.attemptGraphReceiptHash);
      requireCondition(graphReceipt, "R9 terminal graph receipt is absent");
      const input = await nativeScoringEvidence(context, terminalReceipt.derivedAt,
        { terminal: true, expectedGraph: graphReceipt });
      const rebuilt = buildTerminalExecutionBundleFromRawCustodyV5R9({
        ...input, scoredAt: terminalReceipt.derivedAt,
      });
      const verifyErrors = [...verifyTerminalExecutionDecisionReceiptV5R9({
      ...
      if (canonicalJsonV5R3(rebuilt.terminalReceipt) !== canonicalJsonV5R3(terminalReceipt)) {
        verifyErrors.push("R9 terminal receipt differs from raw-custody reconstruction");
      }
      return verifyErrors.length > 0
        ? blocked("TERMINAL_EXECUTION_RESULT_VERIFICATION_BLOCKED", verifyErrors)
        : success("V5_R9_TERMINAL_DECISION_RECOMPUTED_NO_METRIC_INFERENCE_NO_PASS_CLAIM",
          { terminalReceipt });
    } catch (error) {
...
export const V5_R9_RUNTIME_PATHS = Object.freeze({
  repoRoot: DEFAULT_REPO_ROOT,
  protectedRoot: DEFAULT_PROTECTED_ROOT,
  workflowIndexPattern: "workflow-indexes-v5-r9/<selfHash>.json",
