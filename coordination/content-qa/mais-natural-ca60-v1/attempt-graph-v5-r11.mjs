...
import {
  reconstructAttemptGraphCoreV5R11,
  roleOutputByEvidenceV5R11,
} from "./attempt-graph-core-v5-r11.mjs";
import {
...
export function reconstructAttemptGraphV5R11(input) {
  let base;
  const wrapperErrors = [];
  try {
    base = reconstructAttemptGraphCoreV5R11(input);
  } catch (error) {
...
export { roleOutputByEvidenceV5R11 };

export function validateAttemptGraphReconstructionReceiptV5R11({ attemptGraphReceipt, ...input }) {
  const errors = [];
  errors.push(...validateClosedSelfHashedArtifactV5R11(attemptGraphReceipt,
    "AttemptGraphReconstructionReceiptV2"));
  try {
    const rebuilt = reconstructAttemptGraphV5R11({ ...input, derivedAt: attemptGraphReceipt?.derivedAt });
    if (canonicalJsonV5R3(rebuilt.receipt) !== canonicalJsonV5R3(attemptGraphReceipt)) {
      errors.push("R11 attempt graph differs from exact bidirectional evidence reconstruction");
    }
  } catch (error) {
...
export const ATTEMPT_GRAPH_V5_R11_CONSTANTS = Object.freeze({
