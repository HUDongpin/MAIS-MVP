...
export const BOOTSTRAP_GOLDEN_VECTORS_V5_R8 = Object.freeze(
...
export function bootstrapBoundedIndexV5R8({ key, metric, replicate, draw, bound }) {
  requireCondition(typeof key === "string" && key.length > 0 && !key.includes("|"),
    "R8 bootstrap key must be nonempty and pipe-free");
  requireCondition(typeof metric === "string" && metric.length > 0 && !metric.includes("|"),
    "R8 bootstrap metric must be nonempty and pipe-free");
  requireCondition([replicate, draw, bound].every(Number.isSafeInteger)
    && replicate >= 0 && draw >= 0 && bound > 0, "R8 bootstrap index inputs are invalid");
  const range = 1n << 64n;
...
function assertGoldenVectors() {
  requireCondition(BOOTSTRAP_GOLDEN_VECTORS_V5_R8.every((vector) =>
    bootstrapBoundedIndexV5R8(vector) === vector.expectedIndex),
  "R8 counter-SHA256 bootstrap golden vectors failed");
}

export function wilsonIntervalV5R8(numerator, denominator, z) {
  requireCondition(Number.isInteger(numerator) && Number.isInteger(denominator)
    && numerator >= 0 && denominator >= 0 && numerator <= denominator
    && Number.isFinite(z) && z > 0, "R8 Wilson inputs are invalid");
...
function normalizeFinding(finding, itemId, stream, globalIds) {
  ...
  `R8 ${stream} finding is not a frozen taxonomy leaf for ${itemId}`);
  requireCondition(!globalIds.has(finding.findingId),
    `R8 ${stream} findingId ${finding.findingId} is duplicated across the run`);
  globalIds.add(finding.findingId);
...
export function deterministicFindingMatchesV5R8(referenceFindings, machineFindings) {
  const itemIds = unique([...(referenceFindings ?? []), ...(machineFindings ?? [])]
    .map(({ itemId }) => itemId));
  requireCondition(itemIds.length === 1 && typeof itemIds[0] === "string",
    "R8 finding matching requires exactly one item pseudonym");
...
function validateItemResult(item, index, expected, globalReferenceIds, globalMachineIds) {
  const schemaErrors = validateClosedSelfHashedArtifactV5R8(item, "ItemEvaluationResultV2");
  ...
  `R8 item result ${index + 1} is not a closed sealed ItemEvaluationResultV2: ${schemaErrors.join("; ")}`);
  ...
  `R8 item result ${index + 1} identity is invalid`);
  ...
  })) requireCondition(item[field] === value, `R8 item result ${index + 1} ${field} binding differs`);
  requireCondition(["COMPLETE", "MISSING_RECEIPT"].includes(item.executionDisposition)
    ...
    && ["RESOLVED_POSITIVE", "RESOLVED_NEGATIVE", "UNRESOLVED_REFERENCE", "INVALID"]
      .includes(item.referenceDisposition), `R8 item result ${index + 1} disposition is invalid`);
  ...
  `R8 item result ${index + 1} execution and machine dispositions conflict`);
  requireCondition((complete && HASH.test(item.completedItemMarkerHash ?? ""))
    || (!complete && item.completedItemMarkerHash === null),
  `R8 item result ${index + 1} completion marker binding is invalid`);
  requireCondition(Array.isArray(item.referenceFindings) && Array.isArray(item.machineFindings),
    `R8 item result ${index + 1} finding arrays are absent`);
  const referenceFindings = item.referenceFindings
    .map((finding) => normalizeFinding(finding, item.itemIdPseudonym, "reference", globalReferenceIds))
    .sort(findingSort);
  const machineFindings = item.machineFindings
    .map((finding) => normalizeFinding(finding, item.itemIdPseudonym, "machine", globalMachineIds))
    .sort(findingSort);
  ...
  `R8 item result ${index + 1} reference surface and findings conflict`);
  requireCondition(item.referenceDisposition !== "RESOLVED_NEGATIVE" || referenceFindings.length === 0,
    `R8 item result ${index + 1} resolved-negative reference contains findings`);
  ...
  `R8 item result ${index + 1} machine resolution, surface, and findings conflict`);
...
function buildObservedLedger(input) {
  requireCondition(Array.isArray(input.itemResults) && input.itemResults.length === EXPECTED_ITEM_COUNT,
    "R8 statistical analysis requires exactly 60 sealed item results, including missing-receipt rows");
  const globalReferenceIds = new Set();
  const globalMachineIds = new Set();
  const validated = input.itemResults.map((item, index) => validateItemResult(item, index, input,
    globalReferenceIds, globalMachineIds));
  ...
  "R8 item hashes, pseudonyms, and homology clusters must each be unique across all 60 rows");
  ...
  for (const record of validated) {
    ...
    if (analysisResolved && item.referenceDisposition === "RESOLVED_POSITIVE") {
      matching = deterministicFindingMatchesV5R8(referenceFindings, machineFindings);
    } else if (analysisResolved && item.referenceDisposition === "RESOLVED_NEGATIVE") {
  ...
  requireCondition(accounting.completeReceiptItemCount + accounting.missingReceiptItemCount
    === EXPECTED_ITEM_COUNT, "R8 complete and missing receipt accounting does not equal 60");
  ...
  "R8 complete item accounting is not a disjoint partition");
...
function perItemContribution(row, metric) {
  ...
  throw new TypeError(`R8 unsupported bootstrap metric ${metric}`);
}
...
function bootstrapBounds(observed, metric, direction) {
  ...
  for (let replicate = 0; replicate < BOOTSTRAP_REPLICATES; replicate += 1) {
    ...
    for (let draw = 0; draw < contributions.length; draw += 1) {
      const selected = contributions[bootstrapBoundedIndexV5R8({
...
function exactSurfaceWorldBounds(counterfactual, metric) {
  ...
  const bounds = counterfactual.surfaceWorlds.map(({ confusionMatrix }) => {
    ...
    return denominator === 0 ? { lower: 0, upper: 1 }
      : wilsonIntervalV5R8(numerator, denominator, ONE_SIDED_Z95);
  });
...
function proportionMetric(metric, numerator, denominator, clusterCount, observed, counterfactual) {
  const rule = METRIC_RULES[metric];
  const oneSided = wilsonIntervalV5R8(numerator, denominator, ONE_SIDED_Z95);
  const twoSided = wilsonIntervalV5R8(numerator, denominator, TWO_SIDED_Z95);
  ...
  if (counterfactual.integrityLimitExceeded) {
    ...
    const adverse = wilsonIntervalV5R8(adverseNumerator, adverseDenominator, ONE_SIDED_Z95);
...
function recomputeMetrics(observed, counterfactual) {
  ...
  return Object.freeze([
    ...
    Object.freeze({
      ...
      oneSidedWilsonMissRateLcb95: wilsonIntervalV5R8(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, ONE_SIDED_Z95).lower,
      oneSidedWilsonMissRateUcb95: wilsonIntervalV5R8(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, ONE_SIDED_Z95).upper,
      twoSidedWilsonMissRateL95: wilsonIntervalV5R8(counts.p0FalseNegativeItemCount,
        counts.p0OpportunityItemCount, TWO_SIDED_Z95).lower,
      twoSidedWilsonMissRateU95: wilsonIntervalV5R8(counts.p0FalseNegativeItemCount,
...
function validateInputChronology(input) {
  ...
  "R8 statistical chronology requires explicit timestamps");
  for (const field of ["receiptChainValid", "providerTupleValid", "capsValid",
    ...
    requireCondition(typeof input[field] === "boolean", `R8 ${field} must be Boolean evidence`);
  }
}

export function buildFrozenStatisticalBundleV5R8(input) {
  assertGoldenVectors();
  ...
  const metricInputLedger = assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
  ...
  const finalReceipt = assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
    ...
    statisticalKernel: {
      methodVersion: "MAIS-NATURAL-CA60-V5-R8_NATIVE_FROZEN_STATISTICAL_KERNEL",
      ...
      bootstrapGoldenVectors: BOOTSTRAP_GOLDEN_VECTORS_V5_R8,
...
export function verifyFrozenStatisticalBundleV5R8(input, bundle) {
  const errors = [];
  try {
    const rebuilt = buildFrozenStatisticalBundleV5R8(input);
    for (const field of ["itemResults", "completedItemMarkers", "observedLedger",
      ...
      if (canonicalJsonV5R3(rebuilt[field]) !== canonicalJsonV5R3(bundle?.[field])) {
        errors.push(`R8 ${field} differs from independent frozen-method reconstruction`);
      }
    }
  } catch (error) {
...
function terminalReceiptBody(input) {
  ...
  "R8 terminal cause codes are required");
  ...
  requireCondition(integrity.status === "FAILED",
    "R8 terminal integrity receipt requires independently failed execution integrity");
  const causes = unique([...input.terminalCauseCodes, ...integrity.causeCodes]).sort(codePointCompare);
  return {
    schemaVersion: "TerminalExecutionDecisionReceiptV1",
    designId: DESIGN_ID,
    activeRunnerRegistrationHash: input.activeRunnerRegistrationHash,
    sampleExecutionInventoryHash: input.sampleExecutionInventoryHash,
    referenceSealHash: input.referenceSealHash,
...
export function buildTerminalExecutionDecisionReceiptV5R8(input) {
  return assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact(terminalReceiptBody(input)),
    "TerminalExecutionDecisionReceiptV1");
}

export function verifyTerminalExecutionDecisionReceiptV5R8({ receipt, ...input }) {
  const errors = [];
  try {
    requireCondition(validateSelfHashV5R3(receipt), "R8 terminal receipt self-hash is invalid");
    const rebuilt = buildTerminalExecutionDecisionReceiptV5R8(input);
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(receipt)) {
      errors.push("R8 terminal decision receipt differs from immutable item and integrity evidence reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const STATISTICAL_KERNEL_V5_R8_CONSTANTS = Object.freeze({
