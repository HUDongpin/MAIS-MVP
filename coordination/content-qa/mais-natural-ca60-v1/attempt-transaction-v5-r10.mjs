...
function validateCustodyInputs(input) {
  const errors = [
    ...validateClosedSelfHashedArtifactV5R10(input?.requestArtifact, "ProviderRequestArtifactV5"),
    ...validateClosedSelfHashedArtifactV5R10(input?.dispatchAudit,
      input?.dispatchAudit?.schemaVersion ?? "StateBoundDispatchAuditReceiptV2"),
    ...validateClosedSelfHashedArtifactV5R10(input?.reservation, "ProviderDispatchReservationV2"),
    ...validateClosedSelfHashedArtifactV5R10(input?.dispatchPermit,
      input?.dispatchPermit?.schemaVersion ?? "ProviderDispatchPermitV4"),
    ...validateClosedSelfHashedArtifactV5R10(input?.compatibilityDispatchPermit, "ProviderDispatchPermitV3"),
    ...
    ...validateClosedSelfHashedArtifactV5R10(input?.providerEventReceipt, "ProviderEventReceiptV4"),
    ...validateClosedSelfHashedArtifactV5R10(input?.preparedCompletion, "ProviderDispatchCompletionV2"),
  ];
  if (input?.roleOutput !== null) {
    errors.push(...validateClosedSelfHashedArtifactV5R10(input?.roleOutput, "ProviderRoleOutputV1"));
  }
  return [...new Set(errors)];
}

export function buildProviderAttemptCommitIntentV5R10(input) {
...
export function validateProviderAttemptCommitIntentV5R10({ intent, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(intent, INTENT_SCHEMA)];
  try {
    const rebuilt = buildProviderAttemptCommitIntentV5R10({ ...input, preparedAt: intent?.preparedAt });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(intent)) {
      errors.push("R10 attempt commit intent differs from exact pre-completion custody reconstruction");
    }
  } catch (error) {
...
export function buildResolvedProviderAttemptReceiptV5R10({ intent, committedCompletion, ...input }) {
  const intentErrors = validateProviderAttemptCommitIntentV5R10({ intent, ...input,
    preparedCompletion: input.preparedCompletion });
  requireCondition(intentErrors.length === 0, intentErrors.join("; "));
  requireCondition(validateClosedSelfHashedArtifactV5R10(committedCompletion,
...
export function validateResolvedProviderAttemptReceiptV5R10({ resolvedAttemptReceipt, intent,
  committedCompletion, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(resolvedAttemptReceipt, RESOLVED_SCHEMA)];
  try {
    const rebuilt = buildResolvedProviderAttemptReceiptV5R10({ ...input, intent, committedCompletion });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(resolvedAttemptReceipt)) {
      errors.push("R10 resolved attempt receipt differs from complete two-phase custody reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export async function commitProviderAttemptWithDurableIntentV5R10(input) {
  ...
  "R10 two-phase attempt commit requires append-only intent/final stores and an atomic intent-aware ledger");
  let intent = null;
  const committed = await input.ledger.completeWithDurableIntent({
    ...
    persistIntent: async (preparedCompletion) => {
      intent = buildProviderAttemptCommitIntentV5R10({ ...input, preparedCompletion });
      ...
      requireCondition(completionPersisted?.contentHash === preparedCompletion.selfHash,
        "R10 exact prepared completion bytes were not durable before commit intent");
  ...
  const resolvedAttemptReceipt = buildResolvedProviderAttemptReceiptV5R10({
    ...input,
...
export const ATTEMPT_TRANSACTION_V5_R10_CONSTANTS = Object.freeze({
  commitProtocol:
