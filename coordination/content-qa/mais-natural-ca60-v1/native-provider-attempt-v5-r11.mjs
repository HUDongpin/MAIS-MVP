...
import {
  ...
  NATIVE_PROVIDER_ADAPTER_V5_R11_CONSTANTS,
...
function authorityErrors(input) {
  return validateDispatchAuthorityV5R11(input);
}

export function createNativeProviderTransportV5R11({ fetchImplementation, credentialReader,
  repoRoot, protectedRoot, clock = () => new Date() } = {}) {
  const adapter = createNativeProviderAdapterV5R11({ fetchImplementation, credentialReader,
    repoRoot, protectedRoot, clock });
  return Object.freeze({
    kind: "V5_R11_NATIVE_EXACT_TRANSPORT",
...
function buildCompatibilityPermit(input, reservation) {
  ...
  assertClosedSelfHashedArtifactV5R11(receipt, "ProviderDispatchPermitV3");
  return receipt;
}
...
function buildEvent({ input, reservation, compatibilityPermit, transport, parsed, attemptStatus,
  ...
  assertClosedSelfHashedArtifactV5R11(receipt, "ProviderEventReceiptV4");
  return receipt;
}
...
function buildRoleOutput(input, event, parsed) {
  ...
  assertClosedSelfHashedArtifactV5R11(receipt, "ProviderRoleOutputV1");
  return receipt;
}

export class NativeProviderAttemptFailureV5R11 extends Error {
  constructor(message, activity, cause) {
    super(message, { cause });
    this.name = "NativeProviderAttemptFailureV5R11";
    this.activity = activity;
  }
}

export async function runNativeProviderAttemptV5R11(input) {
  ...
  try {
    const errors = [
      ...validateProviderActivationV5R11(input),
    ...
    for (const name of ["dispatchAuthorityStore", "dispatchPermitStore",
      ...
      requireCondition(typeof input?.[name]?.append === "function", `R11 ${name} is unavailable`);
    }
    requireCondition(typeof input?.ledger?.reserve === "function"
      ...
      && input?.transport?.kind === "V5_R11_NATIVE_EXACT_TRANSPORT",
    "R11 ledger/raw/native-transport dependencies are unavailable");
    ...
    requireCondition(authorityPersisted?.contentHash === input.dispatchAuthority.selfHash,
      "R11 dispatch authority was not durable before reservation");
    const reservation = await input.ledger.reserve({
      requestArtifact: input.requestArtifact.compatibilityRequestArtifact,
    });
    const permit = buildProviderDispatchPermitV5R11({ input, reservation });
    ...
    requireCondition(permitPersisted?.contentHash === permit.selfHash,
      "R11 dispatch permit was not durable before credential read");
    ...
    requireCondition(compatibilityPermitPersisted?.contentHash === compatibilityPermit.selfHash,
      "R11 compatibility permit was not durable before attempt intent");
    ...
    requireCondition(bindingPersisted?.persistedBeforeCompletion === true,
      "R11 raw binding was not durable before attempt intent");
    const committed = await commitProviderAttemptWithDurableIntentV5R11({
      activeRunnerRegistrationHash: input.activeRegistration.selfHash,
      authorizationHash: input.authorization.selfHash,
    ...
    throw new NativeProviderAttemptFailureV5R11(
...
export const NATIVE_PROVIDER_ATTEMPT_V5_R11_CONSTANTS = Object.freeze({
  requestTimeoutMs: NATIVE_PROVIDER_ADAPTER_V5_R11_CONSTANTS.requestTimeoutMs,
  maximumResponseBytes: NATIVE_PROVIDER_ADAPTER_V5_R11_CONSTANTS.maximumResponseBytes,
  credentialReadBoundary:
    "ONLY_AFTER_EXACT_R11_GIT_REGISTRATION_A07_CLOSEOUT_SIGNED_A11_REVIEW_RAW_ROUTE_COST_AUTHORITY_REQUEST_RESERVATION_AND_PERMIT",
