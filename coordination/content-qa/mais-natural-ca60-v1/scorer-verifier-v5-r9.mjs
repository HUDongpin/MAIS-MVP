...
import {
  buildFrozenStatisticalBundleV5R9,
  buildTerminalExecutionDecisionReceiptV5R9,
  verifyFrozenStatisticalBundleV5R9,
} from "./statistical-kernel-v5-r9.mjs";
import {
  reconstructDecisionEvidenceV5R9,
  statisticalDecisionFlagsFromEvidenceV5R9,
} from "./decision-evidence-v5-r9.mjs";
import {
...
function buildItemResults(input, outputs, roleEvidence) {
  ...
  requireCondition(outputByKey.size === outputs.length,
    "R9 successful role-output stream contains duplicate item-role successes");
  ...
  requireCondition(labelByItem.size === 60 && input.inventory.items.length === 60,
    "R9 scoring requires 60 unique reference labels and inventory items");
  ...
  for (const item of input.inventory.items) {
    ...
    `R9 reference label differs from inventory identity for ${item.itemHash}`);
    ...
    const marker = complete ? assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
    ...
    const result = assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
...
function validateSemanticReceipt(input, roleEvidence) {
  ...
  "R9 semantic verification receipt does not equal the complete attempt evidence set");
}

export function buildItemEvaluationEvidenceV5R9(input) {
  const graphErrors = validateAttemptGraphReconstructionReceiptV5R9({
  ...
  `R9 scoring requires an exact complete-custody DeepSeek graph: ${graphErrors.join("; ")}`);
  ...
  const outputs = roleOutputByEvidenceV5R9({
...
export function buildTerminalItemEvaluationEvidenceV5R9(input) {
  const graphErrors = validateAttemptGraphReconstructionReceiptV5R9({
    ...(input.deepSeekAttemptGraphContext ?? {}),
    attemptGraphReceipt: input.deepSeekAttemptGraphReceipt,
  });
  ...
  `R9 terminal item reconstruction requires an exact partial-attempt custody graph: ${graphErrors.join("; ")}`);
  const roleEvidence = input.deepSeekAttemptGraphReceipt.roleAttemptEvidenceReceipts;
  requireCondition(roleEvidence.every(validateSelfHashV5R3),
    "R9 terminal partial attempt graph contains unsealed role evidence");
  const outputs = roleOutputByEvidenceV5R9({
    roleAttemptEvidenceReceipts: roleEvidence.filter(({ attemptStatus }) =>
      attemptStatus === "SUCCEEDED"),
    ledgerEntries: input.deepSeekAttemptGraphContext.ledgerEntries,
...
export function validateCompletedItemMarkerBindingsV5R9({ itemResults, completedItemMarkers }) {
  ...
  try {
    ...
    for (const result of itemResults) {
      ...
      `R9 item ${result.itemHash} and its completed-item marker differ`);
      consumed.add(marker.selfHash);
    }
    requireCondition(consumed.size === markerByHash.size,
      "R9 completed-item marker set contains dangling markers");
  } catch (error) {
...
function statisticalInput(input, itemResults, decisionEvidenceReceipt) {
  const facts = statisticalDecisionFlagsFromEvidenceV5R9(decisionEvidenceReceipt);
...
export function scoreNaturalCaV5R9(input) {
  ...
  "R9 scoring upstream roots are not closed self-hashed artifacts");
  const built = buildItemEvaluationEvidenceV5R9(input);
  const decisionEvidenceReceipt = reconstructDecisionEvidenceV5R9(decisionEvidenceInput(input));
  const frozenInput = statisticalInput(input, built.itemResults, decisionEvidenceReceipt);
  const statisticalBundle = buildFrozenStatisticalBundleV5R9(frozenInput);
  ...
  "R9 completion-marker root inputs differ between raw evidence and statistical custody");
  const markerErrors = validateCompletedItemMarkerBindingsV5R9(built);
  requireCondition(markerErrors.length === 0, markerErrors.join("; "));
  const scoreReceipt = assertClosedSelfHashedArtifactV5R9(sealV5R3Artifact({
...
export function buildTerminalExecutionBundleFromRawCustodyV5R9(input) {
  const built = buildTerminalItemEvaluationEvidenceV5R9(input);
  const decisionEvidenceReceipt = reconstructDecisionEvidenceV5R9(decisionEvidenceInput(input));
  const frozenInput = statisticalInput(input, built.itemResults, decisionEvidenceReceipt);
  const terminalCauseCodes = [...new Set(decisionEvidenceReceipt.causeCodes)].sort();
  requireCondition(terminalCauseCodes.length > 0,
    "R9 terminal decision requires a cause reconstructed from immutable custody");
  ...
  const terminalReceipt = buildTerminalExecutionDecisionReceiptV5R9({
    ...frozenInput,
    terminalCauseCodes,
...
export function verifyNaturalCaScoreFromRawEvidenceV5R9({ scoreReceipt, ...input }) {
  const errors = [];
  try {
    const schemaErrors = validateClosedSelfHashedArtifactV5R9(scoreReceipt,
      "NaturalCaAggregateScoreReceiptV7");
    requireCondition(schemaErrors.length === 0,
      `R9 score receipt fails its closed schema or self-hash: ${schemaErrors.join("; ")}`);
    const rebuilt = scoreNaturalCaV5R9({ ...input, scoredAt: scoreReceipt.scoredAt });
    if (canonicalJsonV5R3(rebuilt.scoreReceipt) !== canonicalJsonV5R3(scoreReceipt)) {
      errors.push("R9 score receipt differs from raw graph/item/statistical reconstruction");
    }
    if (input.frozenBundle) {
      errors.push(...verifyFrozenStatisticalBundleV5R9(rebuilt.statisticalInput,
        input.frozenBundle));
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
...
export const SCORER_VERIFIER_V5_R9_CONSTANTS = Object.freeze({
