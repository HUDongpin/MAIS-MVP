...
import {
  validateAttemptGraphReconstructionReceiptV5R8,
...
export function buildTerminalMissingItemResultsV5R8(input) {
  ...
  "R8 terminal missing-item construction requires sealed upstream roots");
  requireCondition(Array.isArray(input.reasonCodes) && input.reasonCodes.length > 0
    && input.reasonCodes.every((value) => typeof value === "string" && value.length > 0),
  "R8 terminal missing-item construction requires explicit cause codes");
  ...
  requireCondition(input.inventory.items.length === 60 && labelByItem.size === 60,
    "R8 terminal missing-item construction requires the complete 60-item inventory and seal");
  return Object.freeze(input.inventory.items.map((item) => {
    ...
    `R8 terminal reference label differs from inventory identity for ${item.itemHash}`);
    const referenceFindings = findingProjection(label.findings ?? [], item.itemIdPseudonym,
      `terminal-reference:${item.itemHash}`);
...
function buildItemResults(input, outputs, roleEvidence) {
  ...
  requireCondition(outputByKey.size === outputs.length,
    "R8 successful role-output stream contains duplicate item-role successes");
  const labelByItem = new Map(input.compatibilityReferenceSeal.finalLabels
    .map((label) => [label.itemHash, label]));
  requireCondition(labelByItem.size === 60 && input.inventory.items.length === 60,
    "R8 scoring requires 60 unique reference labels and inventory items");
  ...
  for (const item of input.inventory.items) {
    ...
    requireCondition(validateSelfHashV5R3(label) && label.itemIdPseudonym === item.itemIdPseudonym
      && label.clusterId === item.clusterId,
    `R8 reference label differs from inventory identity for ${item.itemHash}`);
    ...
    const marker = complete ? assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
    ...
    const result = assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
      schemaVersion: "ItemEvaluationResultV2",
      designId: "MAIS-NATURAL-CA60-V5",
      runnerRegistrationHash: input.activeRegistration.selfHash,
      sampleExecutionInventoryHash: input.inventory.selfHash,
...
function validateSemanticReceipt(input, roleEvidence) {
  ...
  "R8 semantic verification receipt does not equal the complete attempt evidence set");
}

export function buildItemEvaluationEvidenceV5R8(input) {
  const graphErrors = validateAttemptGraphReconstructionReceiptV5R8({
  ...
  `R8 scoring requires an exact complete-custody DeepSeek graph: ${graphErrors.join("; ")}`);
...
export function validateCompletedItemMarkerBindingsV5R8({ itemResults, completedItemMarkers }) {
  ...
  try {
    ...
    for (const result of itemResults) {
      ...
      `R8 item ${result.itemHash} and its completed-item marker differ`);
      consumed.add(marker.selfHash);
    }
    requireCondition(consumed.size === markerByHash.size,
      "R8 completed-item marker set contains dangling markers");
  } catch (error) {
...
function statisticalInput(input, itemResults) {
  ...
  return Object.freeze({
    ...
    receiptChainValid: input.receiptChainValid,
    providerTupleValid: input.providerTupleValid,
    capsValid: input.capsValid,
    terminalProviderFailure: input.terminalProviderFailure,
    materialDeviation: input.materialDeviation,
    postResultDesignDrift: input.postResultDesignDrift,
    labelLeakage: input.labelLeakage === true || requestLeakage,
    unauthorizedProviderCall: input.unauthorizedProviderCall,
    scoredAt: input.scoredAt,
  });
}

export function scoreNaturalCaV5R8(input) {
  requireCondition(validateSelfHashV5R3(input.activeRegistration)
    && validateSelfHashV5R3(input.inventory)
    && validateSelfHashV5R3(input.referenceSeal)
    && validateSelfHashV5R3(input.compatibilityReferenceSeal)
    ...
    && validateSelfHashV5R3(input.deepSeekAuthorization)
    && validateSelfHashV5R3(input.c0ExecutionSet),
  "R8 scoring upstream roots are not closed self-hashed artifacts");
  const built = buildItemEvaluationEvidenceV5R8(input);
  const frozenInput = statisticalInput(input, built.itemResults);
  const statisticalBundle = buildFrozenStatisticalBundleV5R8(frozenInput);
  ...
  "R8 completion-marker root inputs differ between raw evidence and statistical custody");
  const markerErrors = validateCompletedItemMarkerBindingsV5R8(built);
  requireCondition(markerErrors.length === 0, markerErrors.join("; "));
  const scoreReceipt = assertClosedSelfHashedArtifactV5R8(sealV5R3Artifact({
    schemaVersion: "NaturalCaAggregateScoreReceiptV6",
    designId: "MAIS-NATURAL-CA60-V5",
    activeRunnerRegistrationHash: input.activeRegistration.selfHash,
    sampleExecutionInventoryHash: input.inventory.selfHash,
    referenceSealHash: input.referenceSeal.selfHash,
...
export function verifyNaturalCaScoreFromRawEvidenceV5R8({ scoreReceipt, ...input }) {
  const errors = [];
  try {
    const schemaErrors = validateClosedSelfHashedArtifactV5R8(scoreReceipt,
      "NaturalCaAggregateScoreReceiptV6");
    requireCondition(schemaErrors.length === 0,
      `R8 score receipt fails its closed schema or self-hash: ${schemaErrors.join("; ")}`);
    const rebuilt = scoreNaturalCaV5R8({ ...input, scoredAt: scoreReceipt.scoredAt });
    if (canonicalJsonV5R3(rebuilt.scoreReceipt) !== canonicalJsonV5R3(scoreReceipt)) {
      errors.push("R8 score receipt differs from raw graph/item/statistical reconstruction");
    }
    if (input.frozenBundle) {
      errors.push(...verifyFrozenStatisticalBundleV5R8(rebuilt.statisticalInput,
        input.frozenBundle));
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
...
export const SCORER_VERIFIER_V5_R8_CONSTANTS = Object.freeze({
