...
function parseStructuredSource(artifact) {
  requireCondition(typeof artifact?.rawSourceBody === "string",
    `R10 ${artifact?.evidenceKind ?? "UNKNOWN"} source bytes are absent`);
  let parsed;
  try { parsed = JSON.parse(artifact.rawSourceBody); }
  catch { throw new TypeError(`R10 ${artifact.evidenceKind} source bytes are not structured JSON`); }
  ...
  `R10 ${artifact.evidenceKind} source bytes lack the frozen structured-source contract`);
  return parsed;
}
...
function reconstructSourceClaims(authenticatedRouteEvidence, routeProbeArtifacts) {
  ...
  for (const source of [account, billing, region, price]) {
    ...
    `R10 ${source.evidenceKind} retained bytes differ from the verified route subject/tuple`);
  }
  ...
  "R10 retained account/billing/region/price bytes do not support the route receipt claims");
  ...
  requireCondition(probe.rawSourceBody === routeProbeArtifacts.rawResponseArtifact.rawResponseBody,
    "R10 retained route-probe source bytes differ from raw provider-response custody");
  return Object.freeze({ account, billing, region, price, probe });
}

export function buildRouteProbeArtifactSetV5R10(input) {
  ...
  for (const [key, value] of Object.entries(artifacts)) {
    requireCondition(value && validateSelfHashV5R3(value),
      `R10 route probe ${key} is absent or not exactly self-hashed`);
  }
  const artifactHashes = REQUIRED_ARTIFACT_KEYS.map((key) => artifacts[key].selfHash);
  const set = assertClosedSelfHashedArtifactV5R10(sealV5R3Artifact({
...
export function validateRouteProbeArtifactSetV5R10(set) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(set, "RouteProbeArtifactSetV1")];
  try {
    const rebuilt = buildRouteProbeArtifactSetV5R10(set);
    requireCondition(canonicalJsonV5R3(rebuilt) === canonicalJsonV5R3(set),
      "R10 route-probe artifact set differs from exact ten-artifact reconstruction");
  } catch (error) {
...
export function reconstructRouteProbeCustodyGraphV5R10(input) {
  const artifacts = Object.fromEntries(REQUIRED_ARTIFACT_KEYS.map((key) => [key, input?.[key]]));
  for (const [key, value] of Object.entries(artifacts)) {
    requireCondition(value && validateSelfHashV5R3(value),
      `R10 route probe ${key} is absent or not exactly self-hashed`);
  ...
  "R10 route probe request is not the exact registered zero-natural-content tuple");
  ...
  "R10 route probe authority/reservation/permit lineage differs from the request");
  ...
  "R10 route probe provider event does not bind the exact request/reservation/tuple");
  ...
  "R10 route probe retained raw response bytes or event binding are invalid");
  ...
  "R10 route probe raw-response binding receipt differs from retained bytes/event");
  ...
  "R10 route probe durable attempt intent differs from request/reservation/completion");
  requireCondition(completion.reservationHash === reservation.selfHash
    && completion.providerEventReceiptHash === event.selfHash,
  "R10 route probe completion differs from reservation/provider event");
  ...
  "R10 route probe resolved receipt does not reconstruct the complete exact tuple graph");
...
export function buildTrustedProviderEvidenceEnvelopeV5R10(input) {
...
export function validateTrustedProviderEvidenceEnvelopeV5R10({ envelope, activeRegistration,
  authenticatedRouteEvidence, routeProbeArtifacts, priceSnapshot, at }) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(envelope,
    "TrustedProviderEvidenceEnvelopeV2")];
  try {
    requireCondition(activeRegistration?.selfHash === envelope?.activeRunnerRegistrationHash,
      "R10 route envelope does not bind the active registration");
    requireCondition(exactTuple(envelope.provider, envelope.model, envelope.endpoint,
      envelope.projectResidency, envelope.dataRegion), "R10 route evidence tuple is unresolved or different");
    ...
    requireCondition(anchor?.algorithm === "Ed25519" && typeof anchor.publicKeySpkiPem === "string",
      "R10 route evidence has no exact pinned Ed25519 trust anchor");
    ...
    for (const field of ["activeRunnerRegistrationHash", "provider", "model", "endpoint",
      ...
      requireCondition(authenticatedRouteEvidence[field] === envelope[field],
        `R10 authenticated route evidence ${field} differs from the signed envelope`);
    }
    ...
    const probeRaw = authenticatedRouteEvidence.rawSourceArtifacts
      .find(({ evidenceKind }) => evidenceKind === "ROUTE_PROBE_RESPONSE");
    const probeGraph = reconstructRouteProbeCustodyGraphV5R10({
      ...routeProbeArtifacts,
    ...
    "R10 route capture does not bind the reconstructed zero-content request/event/raw bytes");
    reconstructSourceClaims(authenticatedRouteEvidence, routeProbeArtifacts);
    const priceErrors = validateClosedSelfHashedArtifactV5R10(priceSnapshot, "ProviderPriceSnapshotV2");
    ...
    `R10 signed price snapshot is absent or differs from retained price bytes: ${priceErrors.join("; ")}`);
    ...
    "R10 signed route envelope does not bind the reconstructed source/probe evidence root");
    ...
    "R10 route signature payload differs from reconstructed evidence");
    ...
    "R10 route evidence signature does not verify against the pinned anchor");
    if (at !== undefined) {
      requireCondition(Date.parse(envelope.capturedAt) <= Date.parse(at)
        && Date.parse(at) < Date.parse(envelope.expiresAt),
      "R10 trusted route evidence is not valid at the activation time");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const TRUSTED_PROVIDER_EVIDENCE_V5_R10_CONSTANTS = Object.freeze({
