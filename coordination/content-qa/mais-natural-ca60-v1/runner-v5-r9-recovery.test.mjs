...
import {
  ...
  buildReconciliationAuthorizationV5R9,
  inspectInterruptedAttemptCustodyV5R9,
  reconcileInterruptedAttemptV5R9,
...
function custodyFixture(state) {
  ...
  const recoveryAuthorization = buildReconciliationAuthorizationV5R9({
    activeRunnerRegistrationHash: ACTIVE,
...
test("R9 classifies all four registered crash-injection custody states deterministically", () => {
  const cases = [
  ...
  for (const [injectionPoint, expected] of cases) {
    ...
    assert.equal(inspectInterruptedAttemptCustodyV5R9(input).state, expected);
    assert.equal(validateResumeCustodyV5R9(input).length, expected === "FULLY_RECONCILED" ? 0 : 1);
  }
});

test("R9 reservation-only recovery preserves worst-case accounting and cannot authorize resume", () => {
  const input = custodyFixture("AFTER_RESERVATION");
  const recovered = reconcileInterruptedAttemptV5R9(input);
...
test("R9 intent-only recovery appends the exact prepared completion and resolved receipt with zero HTTP", async () => {
  const input = custodyFixture("AFTER_DURABLE_INTENT");
  const calls = [];
  const result = await applyInterruptedAttemptReconciliationV5R9({
    ...input,
    ledger: {
...
test("R9 completion-only recovery appends no second ledger completion", async () => {
...
test("R9 fully reconciled recovery is idempotent apart from its append-only reconciliation receipt", async () => {
...
test("R9 recovery rejects transport or credential dependencies before any action", async () => {
  const input = custodyFixture("AFTER_DURABLE_INTENT");
  await assert.rejects(() => applyInterruptedAttemptReconciliationV5R9({
    ...input,
