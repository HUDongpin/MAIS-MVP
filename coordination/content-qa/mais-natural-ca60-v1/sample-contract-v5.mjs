...
const OPENAI_REFERENCE_RECEIPT_ROLE_SET_V5 = Object.freeze([
  "A_SOLVE",
  "A_LABEL",
  "B_SOLVE",
  "B_LABEL",
...
// V5 inherits the complete V4 frame/sample method unchanged. Only the active
// design identity and provider-named chronology/receipt checks are migrated.
export const DESIGN_ID = "MAIS-NATURAL-CA60-V5";
...
export const ACTIVE_V5_PROVIDER_CHRONOLOGY_RULE = "sourceEnumerationReceipt.enumeratedAt<runtimeExtractionSnapshot.extractedAt<frameRegistration.frozenAt<sampleManifest.manifestFrozenAt<c0RandomAudit.frozenAt<firstOpenAI GPT-5.6 LunaReferenceAttempt.startedAt";
export const ACTIVE_V5_RUNTIME_SOURCE_ENUMERATION_EVIDENCE_CONTRACT = Object.freeze({
  ...RUNTIME_SOURCE_ENUMERATION_EVIDENCE_CONTRACT,
  chronologyRule: ACTIVE_V5_PROVIDER_CHRONOLOGY_RULE,
});
export const ACTIVE_V5_RUNTIME_SOURCE_ENUMERATION_EVIDENCE_CONTRACT_HASH = sha256Hex(
  canonicalJson(ACTIVE_V5_RUNTIME_SOURCE_ENUMERATION_EVIDENCE_CONTRACT),
);
...
export const SAMPLE_SELECTION_GOLDEN_VECTOR_DESIGN_HASH_SEMANTIC = Object.freeze({
  // This is a frozen non-execution V4 vector carried byte-for-byte into V5.
  // Actual selection uses the active V5 registrationHash, never this value.
 ...
 * authority for role-specific OpenAI reference and DeepSeek egress projections.
 */
...
function buildVerifiedFrameItemEgressDecisionV5({
  item,
  rightsDecisionTable,
  screenEvidence,
  verifiedScannerInventory,
  scannerInventoryEntry,
  trustedScreeningPolicyHash,
  trustedScannerImplementationHash,
  trustedScannerRunnerHash,
}) {
  ...
  if (!scannerInventoryEntry
    || scannerInventoryEntry.itemHash !== itemHash
    || scannerInventoryEntry.scannerExecutionReceiptHash !== screenEvidence.scannerExecutionReceiptHash) {
    throw new TypeError("item screen evidence is not an exact member of the trusted scanner execution receipt inventory");
  }
  if (timestampMillis(screenEvidence.scannerExecutionReceipt.executedAt, "scanner execution receipt executedAt")
    > timestampMillis(verifiedScannerInventory.recordedAt, "scanner execution receipt inventory recordedAt")) {
...
/**
 * V5 execution adapter for the immutable V4 item decision semantics. Shared
 * rights and scanner evidence are canonicalized and verified once, then every
 * item is checked against exact maps. This removes accidental O(n^2) receipt
 * inventory validation without changing any per-item decision bytes.
 */
export function buildFrameItemEgressDecisionsV5(input) {
  const normalizedInput = canonicalizeStrictItemJsonV4(input);
  assertObject(normalizedInput, "frame item egress decision batch input");
  const allowedFields = new Set([
    "items",
    "rightsDecisionInput",
    "rightsDecisionTable",
  ...
  const unknownFields = Object.keys(normalizedInput).filter((field) => !allowedFields.has(field));
  if (unknownFields.length > 0) throw new TypeError(`frame item egress decision batch contains unknown fields: ${unknownFields.join(", ")}`);
  const {
    items,
    rightsDecisionInput,
    rightsDecisionTable,
  ...
  if (!Array.isArray(items) || items.length === 0 || !Array.isArray(screenEvidences)) {
    throw new TypeError("frame item egress decision batch requires nonempty items and screen evidences");
  }
  const expectedRightsDecisionTable = buildRightsEgressDecisionTableV4(rightsDecisionInput);
  if (!canonicalEqual(rightsDecisionTable, expectedRightsDecisionTable)
    || !artifactHashMatches(rightsDecisionTable, "rightsDecisionTableHash")) {
    throw new TypeError("rights decision table mismatch after recomputation from trusted owner-bound input");
  }
  assertSha256(trustedOwnerApprovalRootHash, "out-of-band trusted owner approval root");
  if (rightsDecisionTable.ownerApprovalClaimRootHash !== trustedOwnerApprovalRootHash) {
  ...
  for (const trustedHash of [trustedScreeningPolicyHash, trustedScannerImplementationHash, trustedScannerRunnerHash]) {
    assertSha256(trustedHash, "trusted scanner root");
  }
  const screenById = new Map();
  for (const evidence of screenEvidences) {
    if (screenById.has(evidence.itemId)) throw new TypeError(`frame item egress decision batch repeats screen ${evidence.itemId}`);
    screenById.set(evidence.itemId, evidence);
  }
  const inventoryById = new Map(verifiedScannerInventory.entries.map((entry) => [entry.itemId, entry]));
  const itemIds = new Set();
  const decisions = items.map((item) => {
    if (itemIds.has(item.itemId)) throw new TypeError(`frame item egress decision batch repeats item ${item.itemId}`);
    itemIds.add(item.itemId);
    return buildVerifiedFrameItemEgressDecisionV5({
      item,
      rightsDecisionTable,
      screenEvidence: screenById.get(item.itemId),
      verifiedScannerInventory,
      scannerInventoryEntry: inventoryById.get(item.itemId),
      trustedScreeningPolicyHash,
      trustedScannerImplementationHash,
      trustedScannerRunnerHash,
    });
  });
  if (screenById.size !== items.length || inventoryById.size !== items.length) {
    throw new TypeError("frame item egress decision batch requires exact item/screen/inventory coverage");
  }
  return decisions;
}
...
export function buildFrameFreezeEvidenceV4(input) {
  ...
  const scannerIdentityById = new Map(verifiedScannerInventory.entries.map((entry) => [entry.itemId, entry.itemHash]));
  if (verifiedScannerInventory.entries.length !== rowIdentities.length
    || rowIdentities.some(({ itemId, itemHash }) => scannerIdentityById.get(itemId) !== itemHash)) {
  ...
  const verifiedDecisions = buildFrameItemEgressDecisionsV5({
    items: frameRows,
    rightsDecisionInput,
    rightsDecisionTable,
    trustedOwnerApprovalRootHash,
  ...
  for (const [index, row] of frameRows.entries()) {
    const expectedDecision = verifiedDecisions[index];
...
export function buildC0RandomAuditV3(input) {
  ...
  if (referenceAttemptReceipts.length !== 0) {
    throw new TypeError("C0 freeze must occur before the first OpenAI/reference provider attempt receipt");
  }
  ...
  try {
    ...
    if (!Array.isArray(referenceAttemptReceipts)) {
      ...
      const parsed = referenceAttemptReceipts.map((receipt, index) => {
        ...
        if (receipt.schemaVersion !== "ProviderAttemptReceiptV2"
          || receipt.designId !== DESIGN_ID
          || receipt.requestedProvider !== "OPENAI_DIRECT"
          || !OPENAI_REFERENCE_RECEIPT_ROLE_SET_V5.includes(receipt.role)) {
          addError(errors, `reference attempt receipt ${index} is not an exact OpenAI reference provider/role receipt`);
        }
      ...
      if (Math.min(...parsed) <= c0Ms) {
        addError(errors, "C0 must be frozen before first OpenAI/reference attempt receipt");
      }
    }
  } catch (error) {
