...
export function deepSeekRequestFixture() {
  const providerInput = {
    prompt: "What is 2 + 3?",
    options: ["4", "5", "6"],
    storedAnswer: "5",
  ...
  const adapter = buildDeepSeekEvaluationRequestV5R2({ role: "B_PRIME_CRITIQUE", providerInput });
  return {
...
function authorizationFixture(request) {
  const body = {
    ...
    egressAllowlist: ["prompt", "options", "storedAnswer", "acceptedAnswers", "explanation", "rubric", "difficulty", "itemPseudonym", "bPrimeCritiqueArtifact"],
    egressDenylist: ["OPENAI_REFERENCE_FINAL_REFERENCE_TO_DEEPSEEK", "DEEPSEEK_OUTPUT_TO_OPENAI_REFERENCE", "SOURCE_PATH", "GIT_METADATA", "CREDENTIAL", "STUDENT_OR_USER_DATA", "OTHER_ITEM", "UNAUTHORIZED_COPYRIGHT_CONTENT", "INTERNAL_RESEARCH_RECORD"],
...
export function validDeepSeekContext(transport) {
  const request = deepSeekRequestFixture();
  ...
  return {
    ...
    independentReviewReceipt: { ...reviewBody, reviewHash: jcsHash(reviewBody) },
    authorization,
