...
import {
  loadExactFreshRunnerReviewV5R11,
...
export const ACTIVE_REVIEW_KIND_V5_R11 = "FRESH_RUNNER_REVIEW_V5_R11";
export const ACTIVE_REGISTRATION_KIND_V5_R11 = "ACTIVE_RUNNER_REGISTRATION_V5_R11";
...
export function executionLedgerRelativePathV5R11(provider, compatibilityAuthorizationHash) {
  requireCondition(["OPENAI_DIRECT", "DEEPSEEK_DIRECT"].includes(provider)
    && /^[0-9a-f]{64}$/u.test(compatibilityAuthorizationHash ?? ""),
  "R11 execution-ledger path requires an exact provider and compatibility authorization hash");
  return path.join("execution-ledgers-v5-r11", provider === "OPENAI_DIRECT"
...
export async function reloadCommandJournalSidecarsV5R11(context) {
  requireCondition(context?.trustedRoot?.root && context?.activeRegistration?.selfHash,
    "R11 command-journal sidecar reload requires a trusted root and active registration");
  const relativeDirectory = path.join("runtime-custody-v5-r11", "transition-journals");
  ...
  try {
    ...
    requireCondition(!metadata.isSymbolicLink() && metadata.isDirectory(),
      "R11 command-journal sidecar root must be a real directory");
  ...
  requireCondition(names.every((name) => /^[0-9a-f]{64}\.json$/u.test(name)),
    "R11 command-journal sidecar root contains an unexpected entry");
  const journals = [];
  for (const name of [...names].sort()) {
    ...
    const errors = validateClosedSelfHashedArtifactV5R11(value,
    ...
    `R11 command-journal sidecar is invalid or registration-drifted: ${errors.join("; ")}`);
    journals.push(value);
  }
  requireCondition(new Set(journals.map(({ selfHash }) => selfHash)).size === journals.length,
    "R11 command-journal sidecar set contains duplicate hashes");
  return Object.freeze(journals);
}

export async function reconstructAttemptCustodyByProviderV5R11(context) {
  const result = {};
  ...
  for (const provider of ["OPENAI_DIRECT", "DEEPSEEK_DIRECT"]) {
    ...
    const authorization = context.artifacts.get(`${prefix}_AUTHORIZATION_V5_R11`);
    if (!authorization) {
      const orphans = [...allIntents, ...allResolved].filter((value) => value.provider === provider
        && value.activeRunnerRegistrationHash === context.activeRegistration.selfHash);
      requireCondition(orphans.length === 0,
        `R11 ${provider} attempt artifacts exist without the exact active authorization`);
    ...
    `R11 ${provider} active authorization is invalid or registration-drifted`);
    const priceSnapshot = context.artifacts.get(`${prefix}_PRICE_SNAPSHOT_V5_R11`);
    requireCondition(validateSelfHashV5R3(priceSnapshot) && validateSelfHashV5R3(inventory),
      `R11 ${provider} ledger cannot reload without sealed price and inventory artifacts`);
    const ledger = await createAtomicExecutionLedgerV5R7({
      trustedRoot: context.trustedRoot,
      ledgerRelativePath: executionLedgerRelativePathV5R11(provider,
    ...
    requireCondition(verified.errors.length === 0,
      `R11 ${provider} ledger reload failed: ${verified.errors.join("; ")}`);
    ...
    requireCondition(reloaded.orphanDirectoryHashes.length === 0,
      `R11 ${provider} attempt custody contains directories without ledger reservations`);
    const mergeExact = (left, right, label) => {
      const byHash = new Map();
      for (const value of [...left, ...right]) {
        requireCondition(validateSelfHashV5R3(value),
          `R11 ${provider} ${label} merge contains an invalid self-hashed artifact`);
        ...
        `R11 ${provider} ${label} merge contains different canonical bytes for one self-hash`);
    ...
    result[provider] = Object.freeze({ ...custody,
      custodyErrors: validateResumeCustodyV5R11(custody) });
  }
  return Object.freeze(result);
}

async function persistContentAddressedV5R11({ trustedRoot, family, value }) {
  requireCondition(/^[a-z0-9][a-z0-9-]*$/u.test(family) && validateSelfHashV5R3(value),
    "R11 protected artifact family or self-hash is invalid");
  const relativePath = path.join("derived-v5-r11", family, `${value.selfHash}.json`);
  try {
    ...
    requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
      "R11 content-addressed artifact path contains different canonical bytes");
  } catch (error) {
...
export async function advanceProtectedWorkflowIndexV5R11({ context, command, appendedArtifacts,
  committedAt }) {
  ...
  "loaded R11 workflow context and at least one append-only artifact are required");
  const persisted = [];
  for (const entry of appendedArtifacts) {
    const relativePath = await persistContentAddressedV5R11({ trustedRoot: context.trustedRoot,
  ...
  const nextIndexRelativePath = path.join("workflow-indexes-v5-r11", `${nextIndex.selfHash}.json`);
  ...
  const transitionRelativePath = await persistContentAddressedV5R11({
...
export function resolveActiveRunnerReviewV5R11(artifactMap) {
  const review = artifactMap?.get?.(ACTIVE_REVIEW_KIND_V5_R11);
  requireCondition(review !== undefined,
    `protected workflow is missing ${ACTIVE_REVIEW_KIND_V5_R11}`);
  return review;
}

export async function loadProtectedWorkflowIndexV5R11(indexPath, { protectedRoot, repoRoot,
  registrationEvidenceLoader = () => loadExactTrackedRunnerRegistrationV5R11({ repoRoot }),
  freshReviewLoader = ({ registrationEvidence }) => loadExactFreshRunnerReviewV5R11({
  ...
  const evidenceErrors = validateExactRunnerRegistrationEvidenceV5R11(registrationEvidence);
  requireCondition(evidenceErrors.length === 0,
    `exact R11 runner registration evidence failed: ${evidenceErrors.join("; ")}`);
  ...
  const indexBoundToActiveRegistration = compatibilityContext.index.schemaVersion === "ProtectedWorkflowIndexV2"
    && compatibilityContext.index.activeRunnerRegistrationHash === activeRegistration.selfHash;
  if (indexBoundToActiveRegistration) {
    requireCondition(compatibilityContext.artifacts.get(ACTIVE_REGISTRATION_KIND_V5_R11)?.selfHash
      === activeRegistration.selfHash
      && compatibilityContext.artifacts.get(ACTIVE_REVIEW_KIND_V5_R11)?.selfHash
        === exactReview.freshReview.selfHash,
    "active R11 workflow index lacks the exact append-only registration and review artifacts");
  }
  const context = {
    ...
    needsPredecessorAdoption: !indexBoundToActiveRegistration,
  };
  const sidecarJournals = await reloadCommandJournalSidecarsV5R11(context);
  for (const journal of sidecarJournals) {
    ...
    "R11 indexed command journal differs from its protected sidecar bytes");
    if (!indexed) context.artifacts.set(
      `RUNTIME_COMMAND_TRANSITION_JOURNAL_V5_R11:${journal.selfHash}`, journal);
  }
  context.attemptCustodyByProvider = await reconstructAttemptCustodyByProviderV5R11(context);
  return context;
}

export const WORKFLOW_INDEX_V5_R11_CONSTANTS = Object.freeze({
  activeReviewKind: ACTIVE_REVIEW_KIND_V5_R11,
  activeRegistrationKind: ACTIVE_REGISTRATION_KIND_V5_R11,
  ...
  indexRoot: "workflow-indexes-v5-r11",
