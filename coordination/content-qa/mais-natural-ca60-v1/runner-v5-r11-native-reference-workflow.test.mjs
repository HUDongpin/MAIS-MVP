...
import {
  buildProviderAttemptCommitIntentV5R11,
...
import {
  validateRawAuthoritativeMachineReferenceSealV5R11,
  validateRawAuthoritativeReferenceValidationV5R11,
} from "./raw-authoritative-reference-v5-r11.mjs";
import {
  validateDeepSeekExecutionRegistrationV5R11,
} from "./execution-freeze-v5-r11.mjs";
import {
  buildNativeDeepSeekExecutionRegistrationV5R11,
  buildNativeReferenceSealAndValidationV5R11,
...
const SECRET_SENTINEL = "r11-native-reference-fixture-secret-never-persist";
...
async function buildCompleteRawReferenceFixture(actionCount = 240) {
  const protectedRoot = await mkdtemp(path.join(await realpath(os.tmpdir()), "mais-v5-r11-native-reference-"));
  ...
  for (let actionIndex = 0; actionIndex < actionCount; actionIndex += 1) {
    ...
    const rawResponseStore = {
      ...
      persistBinding: async (binding) => {
        ...
        // role output. R11 deliberately withholds it from durable custody and
      ...
      complete: async ({ reservationHash, providerEventReceipt, roleOutput }) => {
        ...
        const committed = await atomicLedger.completeWithDurableIntent({
          ...
          persistIntent: async (preparedCompletion) => {
            attemptCommitIntent = buildProviderAttemptCommitIntentV5R11({
              activeRunnerRegistrationHash: fixture.activeRegistration.selfHash,
    ...
    const transport = createResolvedExactProviderTransportV5R6({
      ...
      fetchImplementation: async () => new Response(JSON.stringify(openAIResponseEnvelopeV5R4(
        ...
        status: 200,
        headers: { "content-type": "application/json", "x-request-id": `r11-native-reference-${actionIndex}` },
      }),
    });
  ...
  const graph = reconstructAttemptGraphV5R11(attemptGraphContext);
  return { protectedRoot, fixture, attemptGraphContext, graph };
}

test("R11 native partial reference attempts reconstruct one internally complete raw lineage graph", async () => {
...
test("R11 default native path rebuilds 240 raw responses, emits V7/V5 seals, and freezes DeepSeek registration", async () => {
  ...
  try {
    ...
    assert.deepEqual(validateAttemptGraphReconstructionReceiptV5R11({
      ...context.attemptGraphContext,
      attemptGraphReceipt: context.graph.receipt,
    }), []);
    const referenceSealContext = {
    ...
    assert.equal(built.seal.schemaVersion, "MachineReferenceSealV7");
    assert.equal(built.seal.sealReconstructionStatus,
      "FULL_RAW_RESPONSE_REPARSE_AND_ATTEMPT_GRAPH_RECONSTRUCTION_BOUND_TO_R11");
    assert.equal(built.validationReceipt.schemaVersion, "ReferenceSealValidationReceiptV5");
    assert.equal(built.validationReceipt.validationStatus,
    ...
    assert.deepEqual(validateRawAuthoritativeMachineReferenceSealV5R11({
      ...referenceSealContext,
      ...built,
    }), []);
    assert.deepEqual(validateRawAuthoritativeReferenceValidationV5R11({
      ...referenceSealContext,
      ...built,
      validationReceipt: built.validationReceipt,
    }), []);

    const deepSeekFixture = buildResolvedActivationFixtureV5R6("DEEPSEEK_DIRECT");
    const { selfHash: _authorizationHash, ...authorizationBody } = deepSeekFixture.authorization;
    const deepSeekAuthorization = sealV5R3Artifact({
      ...authorizationBody,
      activeRunnerRegistrationHash: context.fixture.activeRegistration.selfHash,
      freshRunnerReviewHash: context.fixture.freshReview.selfHash,
      authenticatedRouteEvidenceHash: deepSeekFixture.authenticatedRouteEvidence.selfHash,
      referenceSealHash: built.seal.selfHash,
      referenceAttemptChainHash: built.seal.attemptChainHash,
      issuedAt: "2026-08-26T11:22:00.000Z",
      expiresAt: "2026-08-27T11:22:00.000Z",
    });
    assert.deepEqual(validateClosedSelfHashedArtifactV5R11(deepSeekAuthorization,
      "ProviderAuthorizationV5"), []);
    const executionInput = {
      activeRegistration: context.fixture.activeRegistration,
      freshReview: context.fixture.freshReview,
      authorization: deepSeekAuthorization,
      routeEvidenceEnvelope: deepSeekFixture.authenticatedRouteEvidence,
      costPreview: deepSeekFixture.costPreview,
      inventory: context.fixture.inventory,
      referenceSeal: built.seal,
      compatibilityReferenceSeal: built.compatibilityReferenceSeal,
      referenceSealValidationReceipt: built.validationReceipt,
      referenceSealContext,
      registeredAt: "2026-08-26T11:23:00.000Z",
    };
    const executionRegistration = buildNativeDeepSeekExecutionRegistrationV5R11(executionInput);
    assert.equal(executionRegistration.schemaVersion, "DeepSeekExecutionRegistrationV4");
    assert.equal(executionRegistration.referenceSealHash, built.seal.selfHash);
    assert.equal(executionRegistration.referenceSealValidationReceiptHash,
      built.validationReceipt.selfHash);
    assert.deepEqual(validateDeepSeekExecutionRegistrationV5R11({
      ...executionInput,
    ...
    assert.ok(validateRawAuthoritativeMachineReferenceSealV5R11({
      ...referenceSealContext,
