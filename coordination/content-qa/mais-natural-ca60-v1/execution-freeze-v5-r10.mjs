...
export function buildDeepSeekExecutionRegistrationV5R10(input) {
  ...
  "R10 DeepSeek authorization/validation does not bind the raw-authoritative reference seal");
  ...
  "R10 reference/authorization/execution-registration chronology is invalid");
  ...
  assertClosedSelfHashedArtifactV5R10(receipt, "DeepSeekExecutionRegistrationV4");
  return receipt;
}

export function validateDeepSeekExecutionRegistrationV5R10({ executionRegistration, ...input }) {
  const errors = [...validateClosedSelfHashedArtifactV5R10(executionRegistration,
    "DeepSeekExecutionRegistrationV4")];
  try {
    const rebuilt = buildDeepSeekExecutionRegistrationV5R10({
      ...input,
      registeredAt: executionRegistration?.registeredAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(executionRegistration)) {
      errors.push("R10 DeepSeek execution registration differs from raw-authoritative reconstruction");
    }
  } catch (error) {
