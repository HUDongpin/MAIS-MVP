...
import {
  loadExactTransportActivationCustodyV5R9,
...
function nowIso(clock) {
  ...
  requireCondition(Number.isFinite(date.getTime()), "R9 adapter clock is invalid");
...
async function persistExactPreDispatchBoundaryV5R9(protectedRoot, artifacts) {
  const trustedRoot = await establishProtectedRootV5R4(protectedRoot);
  const hashes = [];
  for (const value of artifacts) {
    const relativePath = path.join("runtime-custody-v5-r9", "pre-dispatch",
    ...
    try {
      ...
      requireCondition(canonicalJsonV5R3(existing) === canonicalJsonV5R3(value),
        "R9 pre-dispatch content-addressed path contains different canonical bytes");
    } catch (error) {
    ...
    requireCondition(canonicalJsonV5R3(reloaded) === canonicalJsonV5R3(value),
      "R9 pre-dispatch artifact did not reload from protected storage exactly");
    hashes.push(value.selfHash);
  }
  return Object.freeze({
    kind: "V5_R9_DURABLE_PRE_DISPATCH_BOUNDARY",
...
function validateDurableDispatchBoundary(input, boundary) {
  ...
  if (!boundary || boundary.kind !== "V5_R9_DURABLE_PRE_DISPATCH_BOUNDARY"
    ...
    errors.push("R9 request, authority, reservation, and permit are not all durably committed before credential read");
  }
  errors.push(...validateClosedSelfHashedArtifactV5R9(input.requestArtifact,
    "ProviderRequestArtifactV5"));
  errors.push(...validateClosedSelfHashedArtifactV5R9(input.reservation,
    "ProviderDispatchReservationV2"));
  errors.push(...validateClosedSelfHashedArtifactV5R9(input.dispatchPermit,
  ...
  if (input.requestArtifact?.activeRunnerRegistrationHash !== input.activeRegistration?.selfHash
    ...
    errors.push("R9 durable pre-dispatch artifact lineage is inconsistent");
  }
  return [...new Set(errors)];
}

export function createNativeProviderAdapterV5R9({
  ...
  requireCondition(typeof fetchImplementation === "function" && typeof credentialReader === "function",
    "R9 native adapter requires fetch and credential-reader functions");
  requireCondition(path.isAbsolute(repoRoot) && path.isAbsolute(protectedRoot),
    "R9 native adapter requires absolute repository and protected-storage roots");
  return Object.freeze({
    kind: "V5_R9_NATIVE_EXACT_PROVIDER_ADAPTER",
    async send(input) {
      ...
      const exact = await loadExactTransportActivationCustodyV5R9({ repoRoot });
      ...
      "R9 transport caller is not bound to the exact Git-custodied registration");
      const trustedInput = Object.freeze({ ...input, ...exact });
      const activationErrors = validateProviderActivationV5R9(trustedInput);
      ...
      requireCondition(activationErrors.length === 0 && requestErrors.length === 0,
        `R9 adapter rejected before durable dispatch boundary: ${[...activationErrors,
          ...requestErrors].join("; ")}`);
      const boundary = await persistExactPreDispatchBoundaryV5R9(protectedRoot,
      ...
      requireCondition(boundaryErrors.length === 0,
        `R9 adapter rejected before credential read: ${boundaryErrors.join("; ")}`);
      ...
      "R9 exact wire bytes differ from the durable dispatch permit");
      ...
      try {
        ...
        if (bytes.byteLength > MAX_PROVIDER_RESPONSE_BYTES) throw new RangeError("R9 response exceeds byte cap");
...
export const NATIVE_PROVIDER_ADAPTER_V5_R9_CONSTANTS = Object.freeze({
  requestTimeoutMs: PROVIDER_REQUEST_TIMEOUT_MS,
  maximumResponseBytes: MAX_PROVIDER_RESPONSE_BYTES,
  credentialReadBoundary:
    "ONLY_AFTER_R9_EXACT_GIT_REGISTRATION_A07_CLOSEOUT_PINNED_A11_SIGNATURE_FULL_ROUTE_SOURCE_AND_PROBE_RECONSTRUCTION_AUTHORIZATION_GRANT_COST_AND_DURABLE_PRE_DISPATCH_CUSTODY",
