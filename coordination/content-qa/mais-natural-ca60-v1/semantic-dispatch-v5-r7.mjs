...
function assertCommon(input) {
  const errors = [
    ...validateClosedSelfHashedArtifactV5R7(input?.activeRegistration,
      input?.activeRegistration?.schemaVersion),
    ...validateClosedSelfHashedArtifactV5R7(input?.authorization, "ProviderAuthorizationV5"),
    ...validateClosedSelfHashedArtifactV5R7(input?.executionRegistration,
      "DeepSeekExecutionRegistrationV4"),
    ...validateClosedSelfHashedArtifactV5R7(input?.inventory, "SampleExecutionInventoryV2"),
  ];
  try {
  ...
  requireCondition(errors.length === 0, `R7 semantic-dispatch upstream validation failed: ${errors.join("; ")}`);
  ...
  "R7 semantic-dispatch registration, authorization, and inventory bindings are invalid");
}
...
function validateSemanticState(input) {
  ...
  if (input.canaryGate) {
    errors.push(...validateCanaryGateReceiptV5R7({
      ...(input.canaryGateContext ?? {}),
      canaryGate: input.canaryGate,
    }));
  }
  if (input.c0ExecutionSet) {
    errors.push(...validateNormalC0ExecutionSetV5R7({
      activeRunnerRegistrationHash: input.activeRegistration.selfHash,
      executionRegistrationHash: input.executionRegistration.selfHash,
      inventory: input.inventory,
...
export function buildCompatibilityCanaryC0DecisionSetV5R7(canaryPredicateReceipt) {
  ...
  "R7 compatibility canary decision set requires the exact sealed canary predicate");
...
export function buildSemanticDispatchAuthorityV5R7(input) {
  assertCommon(input);
  const semanticErrors = validateSemanticState(input);
  requireCondition(semanticErrors.length === 0,
    `R7 semantic canary/C0 state is not exactly rebuildable: ${semanticErrors.join("; ")}`);
  requireCondition(["DEEPSEEK_CANARY", "DEEPSEEK_RESUME"].includes(input.mode),
    "R7 semantic-dispatch mode is invalid");
  const canary = input.inventory.items[0];
  ...
  requireCondition(canaryFirstVerified,
    "R7 semantic dispatch cannot proceed before an exact canary-first gate");
  if (input.mode === "DEEPSEEK_RESUME") {
    ...
    "R7 resume requires the exact canary gate and either pre-C0 base-role state or a frozen normal C0 set");
  }
  const compatibilityC0ExecutionSet = input.mode === "DEEPSEEK_CANARY" && input.canaryPredicateReceipt
    ? buildCompatibilityCanaryC0DecisionSetV5R7(input.canaryPredicateReceipt)
  ...
  requireCondition(plan?.planStatus === "NEXT_ACTION", "R7 frozen planner has no dispatchable next action");
  ...
  requireCondition(selectedC0MembershipVerified,
    "R7 C0 action is outside the exact predicate/random-plus-mandatory selected set");
  const item = input.inventory.items[plan.manifestOrdinal - 1];
  requireCondition(item?.itemHash === plan.itemHash, "R7 planner item is outside the manifest-bound inventory");
  const attemptOrdinal = plan.failedAttemptsForItemRole + 1;
  const attemptId = `V5R7:${input.activeRegistration.selfHash}:${plan.selfHash}:${attemptOrdinal}`;
  ...
  assertClosedSelfHashedAgainstV5R5(receipt, AUTHORITY_SCHEMA, receipt.schemaVersion);
  return receipt;
}

export function validateSemanticDispatchAuthorityV5R7({ semanticDispatchAuthority, ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(semanticDispatchAuthority, AUTHORITY_SCHEMA)];
  try {
    const rebuilt = buildSemanticDispatchAuthorityV5R7({
      ...input,
      issuedAt: semanticDispatchAuthority?.issuedAt,
    });
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(semanticDispatchAuthority)) {
      errors.push("R7 semantic dispatch authority differs from exact frozen planner reconstruction");
    }
  } catch (error) {
...
export function buildSemanticDispatchVerificationReceiptV5R7({ activeRegistration, authorization,
  ...
  "R7 semantic verification requires one authority per role-attempt evidence receipt");
  ...
  requireCondition(authorityByHash.size === semanticDispatchAuthorities.length,
    "R7 semantic authorities are duplicated");
  requireCondition(semanticAuthorityContextsByHash instanceof Map,
    "R7 semantic verification requires reconstruction inputs for every authority");
  for (const authority of semanticDispatchAuthorities) {
    ...
    requireCondition(authorityErrors.length === 0,
      `R7 semantic authority is not exactly rebuildable: ${authorityErrors.join("; ")}`);
  }
  for (const evidence of roleAttemptEvidenceReceipts) {
    const evidenceErrors = validateClosedSelfHashedArtifactV5R7(evidence, "RoleAttemptEvidenceReceiptV1");
    ...
    `R7 semantic verification found an unbound authority/evidence pair: ${evidenceErrors.join("; ")}`);
  }
  ...
  assertClosedSelfHashedAgainstV5R5(receipt, VERIFICATION_SCHEMA, receipt.schemaVersion);
  return receipt;
}

export function validateSemanticDispatchVerificationReceiptV5R7({ semanticDispatchVerificationReceipt,
  ...input }) {
  const errors = [...validateClosedSelfHashedAgainstV5R5(semanticDispatchVerificationReceipt,
    VERIFICATION_SCHEMA)];
  try {
    const rebuilt = buildSemanticDispatchVerificationReceiptV5R7({
      ...input,
    ...
    if (canonicalJsonV5R3(rebuilt) !== canonicalJsonV5R3(semanticDispatchVerificationReceipt)) {
      errors.push("R7 semantic verification differs from exact ordered authority/evidence reconstruction");
    }
  } catch (error) {
    errors.push(error instanceof Error ? error.message : String(error));
  }
...
export const SEMANTIC_DISPATCH_V5_R7_CONSTANTS = Object.freeze({
  c0Roles: C0_ROLES,
