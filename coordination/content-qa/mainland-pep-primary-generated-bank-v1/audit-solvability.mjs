...
const SOLVER_GAP_CLUSTERS_CSV = path.join(__dirname, "solver-gap-clusters.csv");
const MANUAL_REVIEW_RESULTS_CSV = path.join(__dirname, "manual-review-results.csv");
const PROMOTABILITY_DECISION_MD = path.join(__dirname, "s18-promotability-decision.md");

const BASELINE_BEFORE_REMEDIATION = {
  deterministicSolvedRows: 83,
  deterministicPassRows: 81,
  solverGapRows: 516,
  ambiguousMcRows: 1,
  contentErrorRows: 2,
  answerMismatchRows: 0,
};
...
function statusFromAudit(row, schemaIssues, solverResult) {
  ...
  if (row.type === "multiple-choice") {
    ...
    if (!optionMatchesSolver(selected)) return "answer-mismatch";
    ...
    if (expectedOption) {
      ...
      if (selectedIdentity !== expectedIdentity) return "answer-mismatch";
    }
  }

  if (!answerSetMatches(solverResult, row)) return "answer-mismatch";
  if (!explanationMatches(solverResult, row.explanationZhHans)) return "content-error";
  return "pass";
}
...
function addInventorySummary(rows, audits) {
  ...
  for (const grade of EXPECTED_GRADES) {
    if ((gradeCounts[grade] || 0) !== 100) inventoryIssues.push(`Expected ${grade}=100, found ${gradeCounts[grade] || 0}`);
    for (const semester of EXPECTED_SEMESTERS) {
      const key = `${grade}-${semester}`;
      if ((gradeSemesterCounts[key] || 0) !== 50) inventoryIssues.push(`Expected ${key}=50, found ${gradeSemesterCounts[key] || 0}`);
    }
  }
...
function buildManualQueue(audits) {
  ...
  const add = (audit, reason) => {
    ...
    queue.push({
      ...
      status: audit.status,
      solverMethod: audit.solverMethod,
  ...
  for (const audit of audits) {
    if (audit.status !== "pass") add(audit, `required-${audit.status}`);
  }

  for (const grade of EXPECTED_GRADES) {
    const passRows = audits
      .filter((audit) => audit.grade === grade && audit.status === "pass")
      .sort((a, b) => stableHash(a.questionId) - stableHash(b.questionId));
    const selectedForGrade = [];
    for (const type of EXPECTED_TYPES) {
      const byType = passRows.filter((audit) => audit.type === type);
      for (const audit of byType.slice(0, Math.min(5, byType.length))) selectedForGrade.push(audit);
    }
    for (const audit of passRows) {
      if (selectedForGrade.length >= 15) break;
      if (!selectedForGrade.some((row) => row.questionId === audit.questionId)) selectedForGrade.push(audit);
    }
    for (const audit of selectedForGrade.slice(0, 15)) add(audit, "balanced-pass-sample");
  }
  return queue;
}

function buildManualReviewSummary(manualQueue, audits) {
  const passSampleRows = manualQueue.filter((row) => row.reviewReason === "balanced-pass-sample");
  const passSampleByGrade = {};
  const passSampleByGradeType = {};
  const passAvailabilityByGrade = {};
  const shortfalls = [];
  for (const grade of EXPECTED_GRADES) {
    const availablePass = audits.filter((audit) => audit.grade === grade && audit.status === "pass");
    const sampled = passSampleRows.filter((row) => row.grade === grade);
    passAvailabilityByGrade[grade] = availablePass.length;
    passSampleByGrade[grade] = sampled.length;
    passSampleByGradeType[grade] = countBy(sampled.map((row) => row.type));
    if (sampled.length < 15) {
      shortfalls.push(`${grade}: only ${sampled.length}/15 deterministic-pass rows available for balanced pass sampling`);
    }
    const sampledTypes = new Set(sampled.map((row) => row.type));
    for (const type of EXPECTED_TYPES) {
      if (!sampledTypes.has(type)) shortfalls.push(`${grade}: balanced pass sample lacks ${type}`);
    }
  }
  return {
    passSampleRows: passSampleRows.length,
    passAvailabilityByGrade,
    passSampleByGrade,
    passSampleByGradeType,
    shortfalls,
  };
}
...
function buildManualReviewResults(audits) {
  return audits.map((audit) => {
    if (audit.status === "pass") {
      return {
        questionId: audit.questionId,
        reviewer: "S18-deterministic-audit",
        solvableFromPrompt: "yes",
        independentAnswer: audit.independentAnswer,
        storedAnswerMatches: "yes",
        explanationMatches: audit.explanationCheck === "pass" ? "yes" : "not-checked",
        gradeFit: "pending-final-s18-sampling",
        terminologyOk: "pending-final-s18-sampling",
        sourceDistanceOk: "yes",
        verdict: "approved-deterministic",
        recommendedFix: "",
      };
    }
    if (audit.status === "solver-gap") {
      return {
        questionId: audit.questionId,
        reviewer: "S18-manual-required",
        solvableFromPrompt: "pending",
        independentAnswer: "",
        storedAnswerMatches: "pending",
        explanationMatches: "pending",
        gradeFit: "pending",
        terminologyOk: "pending",
        sourceDistanceOk: audit.notes.includes("source-distance:") ? "no" : "yes",
        verdict: "pending-s18-review",
        recommendedFix: "Manual S18 review required, or add a narrow deterministic solver for this cluster and rerun the full audit.",
      };
    }
    return {
      questionId: audit.questionId,
      reviewer: "S18-deterministic-audit",
      solvableFromPrompt: audit.solverMethod ? "yes" : "not-proven",
      independentAnswer: audit.independentAnswer,
      storedAnswerMatches: "no",
      explanationMatches: audit.explanationCheck === "pass" ? "yes" : "no",
      gradeFit: "pending-after-revision",
      terminologyOk: "pending-after-revision",
      sourceDistanceOk: audit.notes.includes("source-distance:") ? "no" : "yes",
      verdict: "needs-revision",
      recommendedFix: "Correct or regenerate this candidate row, then rerun the full audit.",
    };
  });
}

function writePromotabilityDecision(summary, manualReviewSummary, manualReviewResults, solverGapClusters) {
  const verdictCounts = countBy(manualReviewResults.map((row) => row.verdict));
  const blockerCount =
    (summary.statusCounts["answer-mismatch"] || 0) +
    (summary.statusCounts["ambiguous-mc"] || 0) +
    (summary.statusCounts["content-error"] || 0);
  const readyForManualFinalSignoff = blockerCount === 0 && summary.inventoryIssues.length === 0;
  const finalAppPromotable = readyForManualFinalSignoff && (verdictCounts["pending-s18-review"] || 0) === 0 && (verdictCounts["needs-revision"] || 0) === 0;
  const lines = [];
  lines.push("# S18 Mainland PEP Primary Candidate Promotability Decision");
  lines.push("");
  lines.push(`- Generated: ${new Date().toISOString()}`);
  lines.push("- Scope: offline candidate-only QA; this is not production app integration approval.");
  lines.push(`- Decision: ${finalAppPromotable ? "approved for next integration planning" : readyForManualFinalSignoff ? "ready for S18 final manual signoff, not app-promotable yet" : "not ready for final manual signoff"}`);
  lines.push("");
  lines.push("## Audit Result");
  lines.push("");
  lines.push(`- Rows: ${summary.totalRows}/${summary.expectedRows}`);
  lines.push(`- Type totals: ${JSON.stringify(summary.typeCounts)}`);
  lines.push(`- Status counts: ${JSON.stringify(summary.statusCounts)}`);
  lines.push(`- Automated blocker count: ${blockerCount}`);
  lines.push(`- Deterministic solved rows: ${summary.deterministicSolvedRows} (baseline ${BASELINE_BEFORE_REMEDIATION.deterministicSolvedRows})`);
  lines.push(`- Solver gaps: ${summary.statusCounts["solver-gap"] || 0} (baseline ${BASELINE_BEFORE_REMEDIATION.solverGapRows})`);
  lines.push("");
  lines.push("## Manual Review State");
  lines.push("");
  lines.push(`- Manual review result counts: ${JSON.stringify(verdictCounts)}`);
  lines.push(`- Solver-gap clusters: ${solverGapClusters.length}`);
  lines.push(`- Pass-sample rows prepared: ${manualReviewSummary.passSampleRows}`);
  if (manualReviewSummary.shortfalls.length > 0) {
    lines.push(`- Balanced sample shortfalls: ${manualReviewSummary.shortfalls.join("; ")}`);
  }
  lines.push("");
  lines.push("## Required Before App Integration");
  lines.push("");
  if (blockerCount > 0) {
    lines.push("- Resolve all `answer-mismatch`, `ambiguous-mc`, and `content-error` rows, then rerun the full audit.");
  }
  if ((verdictCounts["pending-s18-review"] || 0) > 0) {
    lines.push("- Complete S18 manual review for every `pending-s18-review` row or add deterministic solver coverage and rerun.");
  }
  if ((verdictCounts["needs-revision"] || 0) > 0) {
    lines.push("- Revise all `needs-revision` rows and rerun the full audit.");
  }
  if (finalAppPromotable) {
    lines.push("- No remaining QA blockers. Open a separate S04/S08 integration task before touching app data.");
  }
  lines.push("");
  lines.push("## Notes");
  lines.push("");
  lines.push("- DeepSeek was not used as the judge of its own answers.");
  lines.push("- Deterministic approvals mean the runner independently computed the answer and matched the stored answer/explanation.");
  lines.push("- `pending-s18-review` rows are intentionally not treated as approved.");
  fs.writeFileSync(PROMOTABILITY_DECISION_MD, `${lines.join("\n")}\n`);
}
...
function writeMarkdown(summary, audits, manualQueue, manualReviewSummary) {
  ...
  const passSampleCount = manualQueue.filter((row) => row.reviewReason === "balanced-pass-sample").length;
  const lines = [];
  lines.push("# Mainland PEP Primary 600 Candidate Solvability Audit");
  lines.push("");
  lines.push(`- Generated: ${new Date().toISOString()}`);
  ...
  lines.push(`- Manual review queue rows: ${manualQueue.length} (${passSampleCount} balanced pass-sample rows)`);
  if (manualReviewSummary.shortfalls.length > 0) {
    lines.push(`- Balanced pass-sample shortfalls: ${manualReviewSummary.shortfalls.length}`);
  }
  lines.push("");
  lines.push("## Promotion Decision");
  lines.push("");
  const promotable =
    summary.totalRows === EXPECTED_TOTAL &&
    ...
    (summary.statusCounts["solver-gap"] || 0) === 0;
  if (promotable) {
    lines.push("Promotable by automated gates. S18 manual pass-sample review is still recommended before production integration.");
  } else {
    lines.push("Not promotable yet. Failed rows and all solver gaps must be reviewed or corrected, then the full audit must be rerun.");
  }
  lines.push("");
  ...
  lines.push("");
  lines.push(
    `${gaps.length} rows have no matched deterministic solver. They are in \`manual-review-queue.csv\` unless future deterministic solvers cover them.`,
  );
  lines.push("");
  lines.push("## Manual Sample Coverage");
  lines.push("");
  lines.push("| Grade | deterministic pass rows available | pass-sample rows | sampled types |");
  lines.push("| --- | ---: | ---: | --- |");
  for (const grade of EXPECTED_GRADES) {
    const typeCounts = manualReviewSummary.passSampleByGradeType[grade] || {};
    lines.push(
      `| ${grade} | ${manualReviewSummary.passAvailabilityByGrade[grade] || 0} | ${manualReviewSummary.passSampleByGrade[grade] || 0} | ${EXPECTED_TYPES.map((type) => `${type}:${typeCounts[type] || 0}`).join(", ")} |`,
    );
  }
  if (manualReviewSummary.shortfalls.length > 0) {
    lines.push("");
    lines.push("Balanced pass-sample target shortfalls exist because some grades have fewer than 15 deterministic-pass rows after this conservative solver pass. All solver-gap rows are still in required manual review.");
  }
  lines.push("");
  lines.push("## QA Notes");
  lines.push("");
...
function main() {
  ...
  const manualReviewResults = buildManualReviewResults(audits);
  ...
  const report = {
    ...
    acceptanceCriteria: {
      ...
      requiredPerGrade: 100,
      requiredPerGradeSemester: 50,
      requiredStatusCountsForPromotion: {
        "answer-mismatch": 0,
        "ambiguous-mc": 0,
        "content-error": 0,
        "solver-gap": "manual reviewer signoff required if nonzero",
      },
      automatedPromotionRecommendation:
        ...
        (summary.statusCounts["content-error"] || 0) === 0 &&
        (summary.statusCounts["solver-gap"] || 0) === 0
          ? "promotable-after-manual-sample"
          : "not-promotable-yet",
    },
    failingRows,
    manualReviewSummary,
  ...
  writeCsv(SOLVER_GAP_CLUSTERS_CSV, solverGapClusters, [
    "clusterId",
    "grade",
    "semester",
    "unitTitle",
  ...
  writePromotabilityDecision(summary, manualReviewSummary, manualReviewResults, solverGapClusters);
    ...
    JSON.stringify(
      {
        ...
        outputFiles: [
          ...
          path.relative(process.cwd(), SOLVER_GAP_CLUSTERS_CSV),
          path.relative(process.cwd(), MANUAL_REVIEW_RESULTS_CSV),
          path.relative(process.cwd(), PROMOTABILITY_DECISION_MD),
        ],
      },
      null,
      2,
