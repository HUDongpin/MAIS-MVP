  ...
  return [
    {
      ...
      content: [
        ...
        "Review fresh DeepSeek-generated v3 candidate questions before any public integration.",
        "Check every item independently for mathematical correctness, answer/explanation alignment, multiple-choice ambiguity, invalid or duplicate options, missing conditions, grade/topic suitability for 人教版初中数学, unsafe visual/source artifacts, excessive template repetition, and public-readiness risk.",
        "Be especially strict for generated answers: if the answer is wrong, incomplete, contradicts the explanation, or multiple options can be correct, mark fail.",
        "Do not reject merely because mathQaStatus is needs-review; that is the expected pre-QA status.",
        "Return JSON only. Do not include markdown.",
        "Use this exact schema:",
        '{"items":[{"id":"string","status":"pass|warn|fail","severity":"none|minor|major|blocker","issueTags":["math_error|answer_mismatch|ambiguous_mc|bad_options|missing_condition|grade_mismatch|source_artifact|copy_risk|language_issue|public_readiness|template_repetition"],"correctedAnswer":null,"rationale":"short Chinese reason"}],"batchSummary":{"pass":0,"warn":0,"fail":0,"notes":"short Chinese summary"}}',
    ...
    {
      role: "user",
      content: `请用 json 审核以下 ${batch.length} 道 v3 人教版初中候选题：\n${JSON.stringify(batch.map(compactQuestion))}`
    }
  ];
}
...
function writeSummary({ records, questions, config, args }) {
  ...
  const resultJson = {
    ...
    scope: "v3-mainland-pep-junior-1200-candidate",
  ...
  const markdown = `# DeepSeek V4 Pro QA - Mainland PEP Junior V3 1200 Candidate
...
${summary.issues.length === 0
    ? "DeepSeek V4 Pro found 0 warn/fail items in this run. This is supporting evidence for candidate evaluation, not automatic public integration."
...
async function main() {
  ...
  if (!new URL(apiUrl).host.includes("api.deepseek.com")) {
    throw new Error(`Refusing non-DeepSeek endpoint for this QA task: ${new URL(apiUrl).host}`);
  }
  if (model !== "deepseek-v4-pro") {
    throw new Error(`Refusing non-DeepSeek-v4-pro model for this QA task: ${model}`);
  }
