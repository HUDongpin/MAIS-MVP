...
const batchSize = Number(process.env.BNU_JUNIOR_GENERATION_BATCH_SIZE ?? 10);
const generationConcurrency = Number(process.env.BNU_JUNIOR_GENERATION_CONCURRENCY ?? 4);
const requestTimeoutMs = Number(process.env.BNU_JUNIOR_DEEPSEEK_TIMEOUT_MS ?? 90000);
const networkRetryLimit = Number(process.env.BNU_JUNIOR_DEEPSEEK_NETWORK_RETRIES ?? 1);
const singleTargetRetryLimit = Number(process.env.BNU_JUNIOR_SINGLE_TARGET_RETRIES ?? 8);
...
const difficultyQuotaByGrade = {
  S1: { Foundation: 165, Core: 245, Exam: 70, Challenge: 20 },
  S2: { Foundation: 85, Core: 260, Exam: 115, Challenge: 40 },
  S3: { Foundation: 40, Core: 190, Exam: 200, Challenge: 70 }
};

const forbiddenSourcePattern =
  /OCR|PDF|DOCX|docx|zip|截图|扫描|页码|第\s*\d+\s*页|文件名|路径|原题|原卷|原教材|教材原文|课本原文|教材原题|课本原题|试卷原题|原文|照抄|源文件|来源文件|来源于|改编自|摘自|如图|见图|上图|下图|右图|左图|图形中|图上|图里|根据图(?:形|表|像)?|观察下面的图|画出[^，。；]*图|作出[^，。；]*图|描出[^，。；]*图|图像|图表|(^|[，。；：、\s])图中/i;
const mathRedFlagPattern =
  /设计有误|选项中没有|没有正确答案|无法确定|重新计算|重新分析|重新审题|前面错误|上面算错|检查[:：]|但选项|但答案|但题目|假设图中|答案不唯一|可能有误|可能错误|题目误写|题目有误|重新生成|重新设计|我将|根据输出要求|缺少条件|缺少信息|缺少图|不够条件|无法得到|需调整|修正|修改为|本意|最终答案应该|答案写了/i;
...
function buildScopeCountsByCardId(curriculumCards) {
  ...
  for (const grade of grades) {
    ...
    if (!gradeCards.length) throw new Error(`Missing BNU junior curriculum safe cards for ${grade}`);
...
function buildPlan({ curriculumCards, assessmentPatternCards, zhongkaoExamPatternCards }) {
  ...
  for (const [cardId, count] of Object.entries(scopeCountsByCardId)) {
    ...
    if (!curriculumCard) throw new Error(`Missing BNU junior curriculum safe card ${cardId}`);
    ...
    if (!supportingAssessmentIds.length) throw new Error(`Missing BNU junior assessment pattern evidence for ${cardId}`);
    ...
    for (let localIndex = 0; localIndex < count; localIndex += 1) {
      ...
      const id = `bnu-junior-ds-v1-${grade.toLowerCase()}-${String(gradeIndex + 1).padStart(3, "0")}`;
      rows.push({
        id,
        batch: "bnu-junior-v1",
        grade,
        ...
        zhongkaoPatternCardIds: supportingExamIds,
        ...
        reviewNotes: "Generation target built from committed BNU junior safe-RAG metadata only."
      });
...
function systemPrompt() {
  return [
    ...
    "不要出现“路径”这个词；涉及最短距离时使用“路线”“路程”或“距离”。",
    "不要写“如图”“见图”“上图”“下图”“图中”。题目若需要几何或图像，必须用文字完整描述，使学生不依赖外部图片即可作答。",
    "几何、统计、函数图像、空间视图、圆、相似、三角函数、概率题必须把所有点名、边长、角度、坐标、数据、图像特征或事件条件写在题面文字中。",
    "相交线、平行线、角、三角形、圆、相似、视图题要写成“已知直线 a 与 b 相交于点 O”“射线 OP 在角 AOB 内部”“点 A(2,3)”这类文字条件；禁止写在图中、图上、图里、图形中或观察图。",
    "为了通过候选包安全门槛，题面、选项、答案、解析尽量完全不要出现汉字“图”；函数题可写“坐标关系”“函数关系”，几何题可写“几何条件”“线段/角/圆/点”。",
    ...
    "S1 不得超前使用二次函数、三角函数、圆幂定理或高年级证明；S2 不得使用高中三角或向量；S3 可以使用中考综合风格但仍须条件完整。",
    "不要设计让学生判断题目、答案、解法或选项是否有误的审稿式任务；若使用错题情境，只能问正确结果或正确步骤。",
    "证明类题必须给出足够条件，并要求证明一个明确结论；answer 也不能为空，必须写待证结论或“结论成立”。如果不确定条件是否足够，就改成计算角度、长度、坐标、函数值或概率的普通题。",
...
function userPrompt({ targets, curriculumCards, assessmentPatternCards, zhongkaoExamPatternCards }) {
  return JSON.stringify({
    task: `Generate exactly ${targets.length} original Beijing Normal University Press junior-secondary math questions.`,
    outputContract: {
      root: "questions",
      ...
      rules: [
        ...
        "When a target uses algebra, equation solving, factorization, radicals, or functions, do not use a bare template stem. Add a distinctive condition, named expression, student step, real context, parameter, or asked quantity tied to the target id suffix.",
        "For factorization multiple-choice targets, avoid repeatedly asking 将多项式……因式分解，正确的结果是. Instead vary coefficient patterns, variable letters, intermediate-step checks, common-error diagnosis, missing factor questions, or parameterized expressions while keeping exactly one correct option.",
        ...
        "Do not use the Chinese word 路径. Use 路线, 路程, or 距离 for shortest-route tasks.",
        "For geometry, statistics, functions, probability, circle, similarity, trigonometry, and spatial-view targets, write every condition needed for a unique answer in the prompt text.",
        "For line-angle geometry, use only textual givens such as 已知直线 a 与 b 相交于点 O, 射线 OP 在角 AOB 内部, and explicit angle measures. Do not use 图中, 图上, 图里, 图形中, 观察图, or 根据图.",
        "Avoid the Chinese character 图 in generated fields whenever possible; use 坐标关系, 函数关系, 几何条件, 线段, 角, 圆, 点, 数据表 instead.",
        ...
        "For proof-like targets, give sufficient conditions and prove a clear conclusion. The answer field must not be blank: write the proved conclusion or 结论成立. If the target is hard to express safely, make it a text-only computation item instead.",
    ...
    safeBnuAssessmentPatternEvidence: assessmentPatternCards.map(compactPatternCard),
...
async function postDeepSeek({ apiUrl, apiKey, model, messages, attempt = 0 }) {
  ...
  try {
    ...
    if (/fetch failed|network|ECONNRESET|ETIMEDOUT|aborted/i.test(message) && attempt < networkRetryLimit) {
...
function normalizeQuestion(target, raw) {
  ...
  if (target.type === "multiple-choice") {
    ...
    if (new Set(optionsZhHans.map(normalizeIdentity)).size !== optionsZhHans.length) {
      errors.push("multiple-choice options include normalized duplicates");
    }
  ...
  const normalized = {
    ...
    reviewNotes: "Generated offline with DeepSeek V4 Pro from MAIS BNU junior safe-RAG metadata only; candidate QA package only, not approved for public integration."
  };
...
function validateBatch(targets, rawQuestions, existingPromptKeys = new Set()) {
  ...
  for (const target of targets) {
    ...
    if (existingPromptKeys.has(promptKey)) {
      errors.push(`${target.id}: prompt duplicates another cached BNU junior V1 question (${result.question.promptZhHans.slice(0, 96)})`);
    }
    if (batchPromptKeys.has(promptKey)) {
      errors.push(`${target.id}: prompt duplicates another question in this batch (${result.question.promptZhHans.slice(0, 96)})`);
    }
...
function retryInstruction(error) {
  return [
    ...
    "Do not use the Chinese word 路径; use 路线, 路程, or 距离.",
    "Do not use 如图, 见图, 上图, 下图, 左图, 右图, 图中, 根据图形, 根据图像, 根据图表, 缺少图, or 缺少条件.",
    "For geometry, do not use 图中, 图上, 图里, 图形中, 观察图, or 根据图 at all; write all line, ray, point, angle, side, arc, coordinate, and data conditions directly in the text.",
    "Last-resort safety rule: avoid the Chinese character 图 anywhere in promptZhHans, optionsZhHans, answer, acceptedAnswers, and explanationZhHans.",
    ...
    "Do not answer or imply 无法确定, 条件不足, 答案不唯一, or 题目有误. Rewrite the task so it has enough conditions and one answer.",
    "For geometry proof targets, use a simple sufficient-condition pattern such as parallel lines imply equal angles, triangle angle sum, congruent triangles, Pythagorean theorem, or similar triangles with all needed measures stated. Always fill answer with the final conclusion, never an empty string.",
    "Use complete textual conditions only: coordinates, point names, side lengths, angle measures, formulas, movement directions, function expressions, or explicit data lists.",
...
function messagesForTargets({
  ...
  const zhongkaoIds = unique(targets.flatMap((target) => target.zhongkaoPatternCardIds));
  ...
  const relevantZhongkaoCards = zhongkaoExamPatternCards.filter((card) => zhongkaoIds.includes(card.id));
  const messages = [
    { role: "system", content: systemPrompt() },
    {
      ...
      content: userPrompt({
        ...
        zhongkaoExamPatternCards: relevantZhongkaoCards
      })
    }
  ];
...
async function generateBatch({
  ...
  const packagePromptKeys = loadCachedPromptKeys(batchId);
  if (fs.existsSync(cachePath)) {
    try {
      ...
      console.log(`${batchId} cache invalidated: ${String(error?.message ?? error).slice(0, 220)}`);
    }
  }
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      const messages = messagesForTargets({ targets, curriculumCards, assessmentPatternCards, zhongkaoExamPatternCards, lastError });
  ...
  if (targets.length > 1) {
    ...
    for (const target of targets) {
      console.log(`${batchId} single-target ${target.id}`);
      let singleLastError = lastError;
      let singleQuestion = null;
      for (let attempt = 1; attempt <= singleTargetRetryLimit; attempt += 1) {
        try {
          ...
          if (attempt < singleTargetRetryLimit) await sleep(2000 * Math.min(attempt, 4));
        }
      }
      if (!singleQuestion) {
...
function buildCoverageRows(questions) {
  const grouped = countBy(questions, (question) =>
    [
      ...
      question.zhongkaoPatternCardIds.join("|")
    ].join("\t")
  );
  return Array.from(grouped.entries())
    .map(([key, count]) => {
      ...
      return { grade, semester, topicId, unitTitle, type, difficulty, count, evidenceCardIds, assessmentPatternCardIds, zhongkaoPatternCardIds };
    })
...
function buildQaReport({ questions, plan, model, apiUrl, startedAt, finishedAt, curriculumCards, assessmentPatternCards, zhongkaoExamPatternCards }) {
  ...
  const missingEvidenceRows = questions.filter(
    (question) =>
      ...
      !question.zhongkaoPatternCardIds.length
  );
  ...
  return `# Mainland BNU Junior Generated Bank V1 Candidate QA Report
...
- Started: ${startedAt}
- Finished: ${finishedAt}
- Verdict: ${allChecksPass ? "Auto structure/count/schema QA passed for the offline package; run audit-solvability.mjs next and use s18-promotability-decision.md as the final auto-gate record." : "Needs remediation before use."}

## Scope

- Generated ${questions.length} original Simplified Chinese questions for Mainland Beijing Normal University Press junior-secondary mathematics.
...
- RAG evidence source: ${curriculumCards.length} committed BNU junior textbook safe cards, ${assessmentPatternCards.length} committed BNU junior assessment-pattern cards, and ${zhongkaoExamPatternCards.length} shared Mainland junior zhongkao pattern cards.
...
- \`mathQaStatus\`, \`terminologyQaStatus\`, and \`manualQaStatus\` remain \`pending-s18-review\`; \`audit-solvability.mjs\` writes the final auto-gate decision to \`s18-promotability-decision.md\`.
...
- Required next step for candidate QA after the auto gate passes: complete the S18 manual review queue before any product integration.
`;
}
...
async function main() {
  ...
  if (!Number.isInteger(batchSize) || batchSize < 1 || batchSize > 20) throw new Error(`Invalid BNU_JUNIOR_GENERATION_BATCH_SIZE: ${batchSize}`);
  if (!Number.isInteger(generationConcurrency) || generationConcurrency < 1 || generationConcurrency > 8) {
    throw new Error(`Invalid BNU_JUNIOR_GENERATION_CONCURRENCY: ${generationConcurrency}`);
  }

  const curriculumCards = loadTsExport(path.join(rootDir, "data/rag/mainlandBnuJunior.ts"), "mainlandBnuJuniorRagCards");
  const assessmentPatternCards = loadTsExport(path.join(rootDir, "data/rag/mainlandBnuJuniorAssessmentPatterns.ts"), "mainlandBnuJuniorAssessmentPatternCards");
  ...
  const plan = buildPlan({ curriculumCards, assessmentPatternCards, zhongkaoExamPatternCards });
  const questions = [];
  ...
  async function runWorker(workerIndex) {
    while (nextBatchIndex < activeBatches.length) {
      ...
      console.log(`worker ${workerIndex + 1}/${workerCount} batch ${index + 1}/${activeBatches.length}: ${batchIdFor(index)} ${targets[0].grade} ${targets[0].unitTitle}`);
  ...
  if (smoke) {
    fs.writeFileSync(
      ...
      `# Mainland BNU Junior DeepSeek Smoke Test
