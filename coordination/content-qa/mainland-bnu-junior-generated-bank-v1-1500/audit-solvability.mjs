...
function evidenceSets() {
  return {
    curriculum: new Set(loadTsExport(path.join(rootDir, "data/rag/mainlandBnuJunior.ts"), "mainlandBnuJuniorRagCards").map((card) => card.id)),
    assessment: new Set(loadTsExport(path.join(rootDir, "data/rag/mainlandBnuJuniorAssessmentPatterns.ts"), "mainlandBnuJuniorAssessmentPatternCards").map((card) => card.id)),
    zhongkao: new Set(loadTsExport(path.join(rootDir, "data/rag/mainlandJuniorZhongkaoExamPatterns.ts"), "mainlandJuniorZhongkaoExamPatternCards").map((card) => card.id))
  };
}

function expectedTopicCounts() {
  const cards = loadTsExport(path.join(rootDir, "data/rag/mainlandBnuJunior.ts"), "mainlandBnuJuniorRagCards");
  const counts = {};
...
function auditRow(row, sets) {
  ...
  if (row.batch !== "bnu-junior-v1") issues.push({ code: "bad-batch", severity: "P1", detail: row.batch });
  ...
  if (!Array.isArray(row.assessmentPatternCardIds) || !row.assessmentPatternCardIds.length || !row.assessmentPatternCardIds.every((cardId) => sets.assessment.has(cardId))) {
    issues.push({ code: "bad-bnu-assessment-evidence", severity: "P1", detail: "assessmentPatternCardIds" });
  }
  if (!Array.isArray(row.zhongkaoPatternCardIds) || !row.zhongkaoPatternCardIds.length || !row.zhongkaoPatternCardIds.every((cardId) => sets.zhongkao.has(cardId))) {
    issues.push({ code: "bad-zhongkao-evidence", severity: "P1", detail: "zhongkaoPatternCardIds" });
  }
  ...
  return {
    ...
    recommendedAction: issues.length ? "Rewrite or manually approve before any app integration." : "Approved by structural gate; include in S18 pass-sample review."
  };
}
...
function candidateRows(rows) {
  return rows.map((row) => ({
    ...
    reviewNotes: "S18 BNU junior V1 automated source-distance/structure gate passed with 300-row manual review queue generated; candidate QA package only, not approved for public integration."
  }));
}
...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues, approved }) {
  ...
  return [
    ...
    `- Result: ${approved ? "candidate-auto-gate-passed" : "blocked"}`,
...
function writeReleaseDecision({ approved, manualQueue, inventoryIssues, auditRows }) {
  const issueRows = auditRows.filter((row) => row.qaStatus !== "pass");
  const text = approved
    ? `# S18 Promotability Decision - Mainland BNU Junior Generated Bank V1

- Date: 2026-05-27
- Session ID: S18
- Decision: candidate-auto-gate-passed-not-approved-for-public-integration
- Generated rows: ${expectedTotal}
- Manual review queue: ${manualQueue.length} rows, including 100-row grade sample per S1/S2/S3 and 100% of auto-issue rows.
- Auto issue rows: ${issueRows.length}
- Candidate threshold: 0 P0/P1 structural/source-distance blockers, 0 duplicate IDs, 0 duplicate exact prompts, valid RAG evidence IDs, valid option/answer structure.
- Reason: Automated offline candidate package checks passed and the required S18 manual queue was generated.
- App integration status: Blocked by design; do not import into \`data/questions.ts\`, routes, lessons, or public practice until a separate owner-approved integration task.
`
    : `# S18 Promotability Decision - Mainland BNU Junior Generated Bank V1
...
- Required next step: rewrite failing rows or regenerate affected batches, then rerun \`node coordination/content-qa/mainland-bnu-junior-generated-bank-v1-1500/audit-solvability.mjs\`.
`;
...
function main() {
  ...
  writeCsv(
    ...
    manualQueue.map((row) => ({
      ...row,
      reviewStatus: "pending-s18-review",
      reviewer: "S18",
      reviewNotes: approved ? "Candidate auto gates passed; manual math/terminology review still required before any integration." : "Blocked until inventory and row issues clear."
    })),
  ...
  console.log(`Audit passed: ${rows.length} rows, manual queue ${manualQueue.length}, candidate pack retained pending S18 manual review.`);
}
