  ...
  for (const grade of grades) {
    ...
    for (const scope of scopes) {
      for (let localIndex = 0; localIndex < scope.count; localIndex += 1) {
        ...
        const id = `hjb-high-ds-v1-${grade.toLowerCase()}-${String(gradeIndex + 1).padStart(3, "0")}`;
        rows.push({
          id,
          grade,
          ...
          reviewNotes: "Generation target built from committed HJB safe-RAG metadata only."
        });
        gradeIndex += 1;
      }
    }
...
async function generateBatch({ targets, batchIndex, hjbCards, hjbExamPatternCards, sharedExamPatternCards, apiUrl, apiKey, model }) {
...
function localCoreForTarget(target, index) {
  ...
  const seed = stableHash(`${target.id}:${target.chapter}:${target.type}`);
  const a = (seed % 11) + 2;
...
function generateLocalQuestion(target, index) {
  ...
  return {
    ...target,
    promptZhHans: `变式${String(index + 1).padStart(4, "0")}：${core.prompt}`,
    ...
    mathQaStatus: "pending-manual",
    terminologyQaStatus: "pending-manual",
    reviewNotes: "Generated offline by deterministic MAIS safe-template fallback from committed HJB safe-RAG metadata after live DeepSeek transport failed; manual S18 sampling required before app integration."
  };
}
...
- The DeepSeek generator remains in this package for a later provider-stable rerun.`;
  ...
  return `# Mainland HJB High Generated Bank V1 QA Report
...
async function main() {
  ...
  if (useLocalFallback) {
    ...
    for (let index = 0; index < batches.length; index += 1) {
      ...
      const batchQuestions = await generateBatch({ targets, batchIndex: index, hjbCards, hjbExamPatternCards, sharedExamPatternCards, apiUrl, apiKey, model });
  ...
  fs.writeFileSync(
    ...
    buildQaReport({
      ...
      generationMode: useLocalFallback ? "local-fallback" : "deepseek"
    })
  );
