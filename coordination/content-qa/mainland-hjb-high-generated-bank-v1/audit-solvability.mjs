...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues }) {
  const gradeCounts = countBy(rows.map((row) => row.grade));
  ...
  return `# Mainland HJB High Generated Bank V1 Solvability And Answer-Shape Audit
...
- After any edit to \`questions.jsonl\`, rerun \`node coordination/content-qa/mainland-hjb-high-generated-bank-v1/audit-solvability.mjs\`.
...
function main() {
  ...
  fs.writeFileSync(AUDIT_MD, buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues }));
  fs.writeFileSync(
    RELEASE_DECISION_MD,
    `# S18 Promotability Decision - Mainland HJB High Generated Bank V1
