...
function auditQualityRow(row) {
  ...
  return {
    ...
    recommendedAction:
      qaStatus === "pass"
        ? "No blocking action; include in S18 manual pass-sample before app integration."
...
function buildMarkdownReport({ rows, auditRows, failingRows, manualQueue, inventoryIssues }) {
  ...
  const releaseRecommendation =
    inventoryIssues.length === 0
      ? "Green: 1500/1500 rows are independently solvable and answer-key matched; proceed to S18 manual sampling before any app integration."
      : "Red: quality QA is blocked until failing rows are rewritten or solver gaps are resolved.";

  return `# Mainland HJB High Generated Bank V1 Quality Audit
