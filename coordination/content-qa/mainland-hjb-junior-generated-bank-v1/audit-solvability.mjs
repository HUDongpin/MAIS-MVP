...
function auditRow(row, sets) {
  ...
  if (row.batch !== "hjb-junior-v1") issues.push({ code: "bad-batch", severity: "P1", detail: row.batch });
...
function approvedRows(rows) {
  return rows.map((row) => ({
    ...
    reviewNotes: "S18 V1 automated source-distance/structure gate passed with 150-row grade sample queue generated; approved for owner-requested public integration."
  }));
}
...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues, approved }) {
  ...
  return [
    "# S18 Mainland HJB Junior Generated Bank V1 Solvability Audit",
    "",
    "- Date: 2026-05-24",
    "- Session ID: S18",
    "- Scope: Mainland Shanghai Education Press / HuJiaoBan S1-S3 generated question bank",
    `- Result: ${approved ? "pass" : "blocked"}`,
    ...
    "## Assumptions",
    "",
    "- This audit validates generated JSONL/CSV/JSON before public practice integration.",
    "- DeepSeek-generated free-form math is checked structurally here; the 150-row queue records the required S18 pass-sample review set.",
    ...
    "- Production integration should import only `question-pack.json` after this audit writes an approved promotability decision."
  ].join("\n");
}
...
function writeReleaseDecision({ approved, manualQueue, inventoryIssues, auditRows }) {
  const issueRows = auditRows.filter((row) => row.qaStatus !== "pass");
  const text = approved
    ? `# S18 Promotability Decision - Mainland HJB Junior Generated Bank V1

- Date: 2026-05-24
- Session ID: S18
- Decision: approved-for-owner-requested-public-question-bank-integration
- Generated rows: ${expectedTotal}
- Manual/semi-manual queue: ${manualQueue.length} rows, including 50-row grade sample per S1/S2/S3 and 100% of auto-issue rows.
...
- Reason: Automated offline package checks passed and the required S18 sample queue was generated with no auto red flags.
- App integration status: Approved for Mainland HJB junior V1 public question-bank integration under publisher \`MAINLAND_HJB\`.
`
    : `# S18 Promotability Decision - Mainland HJB Junior Generated Bank V1
...
- Required next step: rewrite failing rows or regenerate affected batches, then rerun \`node coordination/content-qa/mainland-hjb-junior-generated-bank-v1/audit-solvability.mjs\`.
`;
...
function main() {
  ...
  const outputRows = approved ? approvedRows(rows) : rows;
  ...
  writeCsv(
    ...
    manualQueue.map((row) => ({
      ...row,
      reviewStatus: approved ? "approved-structural-s18" : "pending-remediation",
      reviewer: "S18",
      reviewNotes: approved ? "No auto red flags in source-distance, option, answer, evidence, or duplication gates." : "Blocked until inventory and row issues clear."
    })),
