...
function buildInventoryIssues(rows, auditRows, crossBankStats) {
  ...
  if (crossBankStats.checked && crossBankStats.duplicateCount > 0) {
    issues.push(`Cross-v1/v2 exact prompt duplicates: ${crossBankStats.duplicateCount}.`);
  }
...
function buildMarkdownReport({ rows, auditRows, manualQueue, inventoryIssues, crossBankStats }) {
  ...
  const crossBankLine = crossBankStats.checked
    ? `Checked ${crossBankStats.priorRows} v1/v2 rows; duplicate exact prompts ${crossBankStats.duplicateCount}.`
    : "Not run because prior v1/v2 questions.jsonl files were not found in this workspace.";
...
| Cross-v1/v2 duplicate check | ${crossBankLine} |
