...
Use mechanical validation rather than treating prose as schema validation. For detailed diagnostics or a separate validation artifact:
...
For a safe summary of the same unchanged full packet and unchanged runtime state, `scripts/safe-receipt-summary.mjs` already performs full packet validation and currentness readback; running both CLIs is not mandatory. Use the separate validator when its detailed diagnostics or artifact are needed, and revalidate after relevant packet/runtime or time-sensitive authority changes. This does not waive any contract check or required evidence record.
...
Do not include secret values or exact authorization text. Missing or mismatched fields block before a provider call. Skill availability, provider configuration, credential presence, or an earlier statement that does not satisfy the current exact authorization contract is not a substitute. A still-valid grant from the current task may be reused after verifying every binding, external comparison, scope, cap, and expiry; do not request it again solely because the skill is invoked.

### External trust boundary

Canonical packet hashes establish internal byte/field closure only. They do not authenticate an issuer, prove control of an account, or verify a cryptographic signature; this validator implements no PKI or signature verification. The anchor's `VERIFIED` value records that an external comparison was performed, but the validator cannot prove that assertion from self-contained JSON. A structurally valid packet is therefore never provider authority. Before any live call, the operator must verify fresh exact authorization from the current task, including an existing grant that remains valid, and compare the authorization projection and receipt identity to the externally trusted receipt. Obtain renewed authorization only when the existing grant is absent or no longer satisfies the contract. The validator remains offline/read-only and never calls the provider or runner.
...
## Audit Artifacts and Human Summaries

Formal packet/handoff completeness is defined by this contract and `assets/machine-qa-handoff.template.md`; concise prose does not replace required fields or evidence. Record the evidence class, fixed disposition/claim ceiling, independent proof states, all applicable observed failures and unverified requirements, evidence references, next owner, and exact resume transition. For required C0, name the actual missing canonical role keys and require their valid fresh-context receipts followed by packet revalidation. For D-prime, retain all prior/new identity, authoritative prior-envelope, six-domain invalidation, fresh-binding/count-equality, and independent-review evidence described above and in `references/remediation-and-independence.md`.

Ordinary explanations or status answers may group passed mechanical checks and link their validated evidence. Expand individual fields when needed to explain a failure, choose the next action, or satisfy a requested field-by-field audit; do not repeat an entire schema merely to demonstrate attention. A requested comprehensive audit still reports all applicable failures supported by supplied evidence rather than stopping at the first blocker.

Unsafe or contradictory input retains the safe-summary tool's restricted output boundary: do not construct a semantic A18 handoff from a rejected receipt. A later independently validated, sanitized exact-package `candidate-only` packet routes its redacted machine evidence to A18 without claiming acceptance. Synthetic evidence never routes to A18. Full-packet trust summaries expose only fixed `status` and canonical `receiptIdentitySha256` for live/prior anchors, never issuer/account/source details and never provider authority.

The safe-trust projection for both `trustAnchors.liveAuthorization` and `trustAnchors.priorEvidence` is `allowedFields: [status, receiptIdentitySha256]`, `forbiddenFields: [issuer, account, source]`, and `providerAuthority: false`. These define the exposure/claim boundary; they do not add fields to the summary schema. Successful suppression of other protected fields does not relax this restriction.
