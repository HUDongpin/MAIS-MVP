...
Status: **normative response projection**. Select the one card that matches the primary object, then use the detailed references and validator for proof. A card is a completeness checklist, not evidence and not authorization. Include every applicable line; do not copy example values from a case study into generic policy.
...
- Begin with: no provider call and no credential-source access.
...
- Reject every retroactive authorization or receipt repair and name the narrow fresh-authorization resume gate.
