...
The requirements below describe the completed D-prime evidence cycle. Freeze the new inputs/control plane and verify any live authorization before running reviews; populate completed new receipt bindings, manifests, and the D-prime validation receipt only after the corresponding fresh results exist. Their listing below is not permission to fabricate completion during preflight.
...
The canonical `activeReceiptManifest` rejects retained old receipts even when `candidateMutated` remains falsely set to `false`. Each slot's `resultSha256` derives from its closed redacted result body with fixed status/count/output-digest fields and the exact complete control plane; rehashing only an outer wrapper cannot preserve an old result digest. Formal audit artifacts must prove these bindings. An ordinary status answer may summarize the resulting supersession and exact resume gate, citing the validation evidence; explain the field-level mechanism when it is disputed, affects remediation, or the user requests it.
