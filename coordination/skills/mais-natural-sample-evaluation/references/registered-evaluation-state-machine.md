...
`RUNNER_INDEPENDENTLY_VERIFIED` is never inferred from a worktree suffix, README, closeout label, or signature alone. Its proof requires exact runner registration, closure, and commit hashes; their direct-parent receipt links and binding digests; and the current signed independent runner-review receipt. A formal status audit records that complete derivation; ordinary prose may cite the verified evidence instead of repeating every hash. If any part is missing or stale, retain the earlier runner state or block. The phrase “receipt-derived” alone does not establish the proof.
...
Pre-register the design profile and power artifact together with inference frame, eligibility, homology rule, sampling, role isolation, estimands, unresolved treatment, provider order, custody, metrics provenance, and claim ceiling. Pin the exact clean source baseline and bounded source-rights receipt; verify the runtime-visible frame and exclude synthetic/test/candidate material. Freeze a deterministic homology-aware sample under protected custody, obtain independent freeze review, then register the exact runner source closure, record closeout, and obtain an independent signed runner review. Frozen frame counts and sample size are nonnull and internally consistent. Selection is result-blind and reroll-free.
