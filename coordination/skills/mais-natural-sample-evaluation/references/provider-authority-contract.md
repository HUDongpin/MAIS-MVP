...
The bundled scripts never read credentials or contact a provider. If execution is separately authorized, dispatch belongs to the exact registered repository runner and adapter covered by current runner closure/review and custody receipts. Immediately before invocation, require the full current runner, trusted route, phase grant, source rights, material-currentness, activity/cap, and custody chain; runner closure must be clean, tracked, and exact. Never copy a provider client into this skill or treat `EXECUTE_OR_RESUME` as implicit dispatch permission.
