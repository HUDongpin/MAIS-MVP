...
const COMPONENTS = {
  "mais-rsi-machine-qa-workflow": { evals: 9, triggers: [10, 10] },
  "mais-natural-sample-evaluation": { evals: 7, triggers: [10, 10] },
  "mais-content-promotion-gate": { evals: 7, triggers: [10, 10] },
  "mais-content-qa-approval-workflow": { evals: 2, triggers: [10, 10] },
  "mais-release-hygiene-deploy-workflow": { evals: 4, triggers: [10, 10] },
};
...
test("three specialist response-card contracts cover the fifteen core scenario families", () => {
  ...
  for (const specialist of Object.keys(cards)) {
    assert.deepEqual(markdownH2Headings(cards[specialist]).filter((heading) => /^## (?:RSI|NAT|PRO)-[1-5] /u.test(heading)), headings[specialist], `${specialist}: exact ordered cards`);
  ...
  assertContainsAll(markdownSection(cards.rsi, headings.rsi[2]), [
    "If authorization is absent, expired, stale, mismatched, or over scope, stop before calling",
    "Do not access credential sources",
    "`package.json`",
    "`package-lock.json`",
    "sorted unique",
    "`100755`",
  ...
  assertContainsAll(naturalPreamble, ["no credential access or provider call"], "Natural card default");
  ...
  assertContainsAll(markdownSection(cards.natural, headings.natural[1]), [
    "make no credential access or provider call",
  ...
  assertContainsAll(markdownSection(cards.promotion, headings.promotion[4]), [
    "Do not deploy, access provider configuration, call a network service",
    "structured artifact with every known binding",
...
test("compatibility Skills use Axx routing and retain their new authority ceilings", () => {
  ...
  assert.match(release, /Registry are owned by `mais-content-promotion-gate`/u);
  assert.match(release, /A18 content acceptance for the same candidate/u);
  assert.match(release, /A23 current Promotion handoff/u);
  assert.doesNotMatch(release, /(?:validate|verif(?:y|ies)).{0,80}(?:A18\/A23 )?Promotion chain/iu);
  assert.match(release, /never manufactures or validates Promotion authority/u);
  assert.match(release, /No Shadow\/CI\/build\/READY\/provider-config\/HTTP-200 state is translated into live behavior pass/u);
  ...
  assert.match(release, /never edits provider\/environment configuration/u);
  assert.match(release, /`sameShaRouteReadback`/u);
  assert.match(release, /`liveRequiredBehavior`/u);
  assert.match(release, /Follow the user's language for prose/u);
});
...
test("formal response contracts are section-scoped and schema-backed", () => {
  ...
  const rsiEvidence = read("mais-rsi-machine-qa-workflow/references/evidence-and-receipt-contract.md");
  const rsiAudit = markdownSection(rsiEvidence, "## Audit Artifacts and Human Summaries");
  const naturalCards = read("mais-natural-sample-evaluation/references/response-cards.md");
  const naturalAudit = markdownSection(naturalCards, "## Formal audit completeness and ordinary answers");
  const recoveryContract = markdownSection(naturalRecovery, "## Reservation-Only Interruption");
  const promotionAudit = read("mais-content-promotion-gate/references/release-handoff.md");
  const releaseReporting = read("mais-release-hygiene-deploy-workflow/references/outcomes-and-reporting.md");
  const releaseOutcomes = markdownSection(releaseReporting, "## Release Outcomes");

  // Routing and response brevity may change; machine authority and formal
  // artifact completeness must remain closed under the unchanged schemas.
  assert.match(rsi, /natural estimation and exact-item acceptance needs both separate downstream routes/u);
  assert.match(rsi, /activeReceiptManifest.*after the required review results exist/u);
  assert.match(rsi, /Old receipts cannot remain active even if `candidateMutated` is falsely set to `false`/u);
  assert.match(rsiAudit, /concise prose does not replace required fields or evidence/u);
  assert.match(rsiAudit, /actual missing canonical role keys/u);
  assert.match(rsiAudit, /valid fresh-context receipts followed by packet revalidation/u);
  assert.match(rsiAudit, /do not construct a semantic A18 handoff from a rejected receipt/u);
  ...
  assert.match(remediation, /rehashing only an outer wrapper cannot preserve an old result digest/u);
  ...
  assertContainsAll(naturalAudit, [
    "current per-phase zero reconciliations and the activity-summary receipt prove zero",
    "Without that current reconciliation-and-summary chain",
    "Lack of visible calls is not proof of zero",
    "attempts: {known:false,count:null}",
    "completedCalls: {known:false,count:null}",
    "egress: {known:false,count:null}",
    "activity.unknown.present:true",
    "packet-derived category union",
  ], "Natural audit response contract");
  assert.match(naturalCards, /no contradictory current evidence.*do not replace those values with unknown/u);
  assert.match(naturalAudit, /current verification/u);
  ...
  assertContainsAll(naturalCase, [
    ...
    "re-verify the relevant current repository evidence",
  ...
  assertContainsAll(promotionAudit, [
    "bindings.candidateDigest", "bindings.sourceCommit", "bindings.targetBaselineCommit",
    "bindings.checkerVersionSha256", "bindings.checkerBundleDigest", "bindings.checkerReleaseCommit",
    "complete `attemptRelation` projection", "complete `receiptComparison`",
    "`canonical`", "`fresh`", "`replay`", "`bindingsEqual`", "`semanticDigestsEqual`",
    "`rawDigestsEqual`", "`runIdsDistinct`", "`semanticDigestsVerified`", "`comparisonStatus`",
    "`comparisonDigest` is required when status is `pass`",
    "Missing bindings stay visible even when another blocker already prevents deployment",
  ...
  assert.match(releaseOutcomes, /current outcome stays at the highest independently proven non-live state/u);
  assert.match(releaseOutcomes, /machine-consumed current-outcome field contains only the proven state/u);
...
test("current source hashes and pending installation cannot inherit historical verification", () => {
  const manifest = json("suite-manifest.json");
  assert.equal(manifest.status, "source-candidate-uncommitted");
  assert.equal(manifest.evaluation.behaviorScenarios, 29);
  assert.equal(manifest.evaluation.triggerQueries, 100);
  assert.equal(manifest.evaluation.status, "not-run-for-current-source");
  assert.equal(manifest.packageBuild.status, "not-built-for-current-source");
  assert.equal(manifest.packageBuild.archiveCount, 0);
  assert.equal(manifest.installationReceipt.status, "not-installed-for-current-source");
  assert.equal(manifest.gitDelivery.pushed, false);
  assert.equal(manifest.gitDelivery.merged, false);
  const historicalBytes = fs.readFileSync(path.join(SUITE_DIR, manifest.historicalEvidence.manifestPath));
  assert.equal(createHash("sha256").update(historicalBytes).digest("hex"), manifest.historicalEvidence.sha256);
  const historical = JSON.parse(historicalBytes);
  assert.equal(historical.suiteVersion, "1.0.0");
  assert.equal(historical.status, "source-package-installation-verified");
  for (const component of manifest.components) {
    const canonical = hashSkillTree(path.join(SUITE_DIR, component.name));
    const packaged = hashSkillTree(path.join(SUITE_DIR, component.name), { packageView: true });
    assert.equal(component.sourceHash, canonical.treeSha256, component.name);
    assert.equal(component.sourceFileCount, canonical.fileCount, component.name);
    assert.equal(component.packageViewHash, packaged.treeSha256, component.name);
    assert.equal(component.packageFileCount, packaged.fileCount, component.name);
    assert.equal(component.sourceCommit, null);
    assert.equal(component.packageHash, null);
    assert.deepEqual(component.installation, { state: "not-installed", readbackHash: null, readbackFileCount: null });
  }
});
