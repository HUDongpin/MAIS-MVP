...
test("clean temporary checkout of current origin/main remains audit-only when workflow JSON.parse is untrusted", async (t) => {
  const sourceRepo = resolve(process.cwd());
  const tempRoot = await createPromotionTestTempDir(t, "promotion-current-main-discovery-");
  const cloneRoot = join(tempRoot, "repo");
  const clone = spawnSync("git", ["clone", "-q", "--shared", "--no-checkout", sourceRepo, cloneRoot], { encoding: "utf8" });
  assert.equal(clone.status, 0, clone.stderr);
  const fetch = spawnSync("git", ["fetch", "-q", sourceRepo, "refs/remotes/origin/main"], { cwd: cloneRoot, encoding: "utf8" });
  assert.equal(fetch.status, 0, fetch.stderr);
  const checkout = spawnSync("git", ["checkout", "-q", "--detach", "FETCH_HEAD"], { cwd: cloneRoot, encoding: "utf8" });
  assert.equal(checkout.status, 0, checkout.stderr);
  assert.equal(spawnSync("git", ["status", "--porcelain=v1", "--untracked-files=all"], { cwd: cloneRoot, encoding: "utf8" }).stdout, "");
  await assert.rejects(
    discoverNativePromotionContext(cloneRoot),
    (error) => error.code === "WORKFLOW_JSON_PARSE_UNTRUSTED",
  );
});
