...
A formal audit artifact retains the complete discovered envelope and the projection below. Ordinary prose may summarize its conclusion, actual blockers/unknowns, active-versus-historical boundary, and exact next gate while citing the complete evidence; it need not repeat every field. Conceptual questions do not require repository discovery. Neither a concise summary nor a formal artifact may infer favorable values for missing/unrun checks.
...
Use `assets/promotion-readiness-report.template.md` for the formal report. Every required field must be populated, marked `unknown`/`not-run`, or named as a blocker; an ordinary prose summary is not itself that artifact.

The complete `attemptRelation` projection retains `relation`, `baseAttemptRef`, `revisionRef`, `baseCount`, `candidateChanged`, `sourceChanged`, `checkerChanged`, `baselineChanged`, `reviewedBaselineOnly`, `directParentManifestSha256`, `directParentReceiptSha256`, `evidenceCommit`, `bindingCommit`, `executionCommit`, `storageCommit`, `finalizationCommit`, and `historicalClosureOverwritten`, alongside status, currentness, lifecycle state, live boundary, and `liveAllowed=false`. Keep canonical Receipt verification, native validate authority, three-Receipt comparison, and Closure/Registry finalization separate; one layer cannot supply an unproved field in another.
