...
Status: **normative evidence projection**. Apply the relevant cards, combining them when needed, and use repository-native discovery/schema/checker evidence for artifact-backed claims. Ordinary prose may lead with the conclusion and summarize verified evidence, blockers, unknowns, and the exact next gate. Formal artifacts and explicitly requested field-by-field reports retain all required projections. A card is not a Manifest, Receipt, Closure, Registry, authorization, or live proof. Named dated facts stay in their case study; conceptual explanations do not require package discovery.
...
- Do not deploy, access provider configuration, call a network service, or claim live proof from Shadow evidence.
...
- For an actual requested Shadow-to-live handoff, return the structured artifact with every known binding and each missing binding marked by a fixed blocker. A conceptual explanation of this boundary may summarize the requirements without creating a formal handoff.
