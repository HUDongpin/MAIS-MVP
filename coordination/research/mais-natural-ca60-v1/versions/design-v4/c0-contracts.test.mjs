...
function deepSeekAuthorization() {
  ...
  return withHash({
    ...
    qwenAttemptChainHash: hash("c"),
    directBillingEvidenceHash: hash("d"),
    dataRegionEvidenceHash: hash("e"),
  }, "authorizationHash");
}
...
function c0RoleBundle({ identityByRole = {}, parsedPayloadByRole = {} } = {}) {
  ...
  const itemProjection = {
    ...
    difficulty: "Low",
    itemPseudonym: `ca60-${"1".repeat(32)}`,
  };
  const context = {
    registrationHash: hash("3"),
    frameRegistrationHash: hash("a"),
  ...
  for (const [index, role] of C0_ROLES.entries()) {
    ...
    responseBodies[role] = providerResponseEnvelope(parsedPayload, `c0-${index + 1}`);
    const wireEvidence = buildProviderWireEvidenceV4({
      ...
      userPayload: requestBodies[role].input,
      responseSchema: contract.outputSchema,
      adapterTransformHash: authorization.adapterHash,
      rawWireResponseBytes: `${JSON.stringify(responseBodies[role], null, 2)}\n`,
...
test("DeepSeek B-prime and C0 roles freeze exact egress, literal prompts, and real output schemas", () => {
  const baseDeepSeekFields = ["prompt", "options", "storedAnswer", "acceptedAnswers", "explanation", "rubric", "difficulty", "itemPseudonym"];
  assert.deepEqual(DEEPSEEK_EGRESS_ALLOWLIST, [...baseDeepSeekFields, "bPrimeCritiqueArtifact"]);
  assert.deepEqual(Object.keys(DEEPSEEK_ROLE_CONTRACTS), DEEPSEEK_ROLE_SET);
  for (const [role, contract] of Object.entries(DEEPSEEK_ROLE_CONTRACTS)) {
    assert.deepEqual(
      contract.inputFieldNames,
      role === "B_PRIME_REVISION"
        ? [...baseDeepSeekFields, "bPrimeCritiqueArtifact"]
        : baseDeepSeekFields,
    );
...
test("C0 reducer rejects five roles mixed across items, prompts, schemas, or execution roots", () => {
  const mixedItems = c0RoleBundle({
    identityByRole: Object.fromEntries(C0_ROLES.map((role, index) => [role, {
      itemIdPseudonym: `ca60-${(index + 2).toString(16).padStart(32, "0")}`,
...
test("C0 reducer handles unknown codes and cross-role conflicts conservatively without a sixth call", () => {
  ...
  const duplicateIdConflict = c0RoleBundle({
    parsedPayloadByRole: {
      C0_PRIME_ROLE_5: {
        valid: true,
        surfaceDisposition: "FINDING",
  ...
  const duplicateIdConflictReduced = reduceC0RoleOutputBundle(duplicateIdConflict);
  assert.equal(duplicateIdConflictReduced.status, "UNRESOLVED");
  assert.equal(
    duplicateIdConflictReduced.conflictCodes.includes("DUPLICATE_FINDING_ID_PROJECTION_CONFLICT"),
    true,
  );
  assert.equal(duplicateIdConflictReduced.successfulRoleCallCount, 5);
});
