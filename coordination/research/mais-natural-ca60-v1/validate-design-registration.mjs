...
export function determineDecision({ registrationValid, authorized, unresolvedN, positiveN, negativeN }) {
  if (!registrationValid) return "INVALID_REGISTRATION";
  const counts = [unresolvedN, positiveN, negativeN];
  if (counts.some((count) => !Number.isInteger(count) || count < 0) || counts.reduce((sum, count) => sum + count, 0) > 60) return "INVALID_REGISTRATION";
  if (!authorized) return "BLOCKED_AUTHORIZATION";
  if (unresolvedN > 0) return "INCONCLUSIVE_UNRESOLVED";
  if (positiveN < 25 || negativeN < 52) return "INCONCLUSIVE_MACHINE_REFERENCE";
  return "LIMITED_MACHINE_REFERENCE";
}
