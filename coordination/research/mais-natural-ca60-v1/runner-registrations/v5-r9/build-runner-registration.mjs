...
import V5_R8_REGISTRATION from "../v5-r8/runner-registration.json" with { type: "json" };
import {
  canonicalJsonV5R3,
  sealV5R3Artifact,
  sha256V5R3,
...
import {
  computeClosureKernelsV5R9,
  PRODUCTION_ENTRYPOINTS_V5_R9,
  R8_FINDING_IDS_V5_R9,
  runtimeSourceEnumerationRootV5R9,
  V5_R8_A11_DISCREPANCY_REVIEW_COMMIT,
  V5_R8_A11_DISCREPANCY_REVIEW_HASH,
  V5_R8_REGISTRATION_COMMIT,
  V5_R8_REGISTRATION_HASH,
  V5_R8_SOURCE_COMMIT,
  V5_R9_OWNER_RUNNER_IMPLEMENTATION_AUTHORIZATION_TEXT_HASH,
  V5_R9_REVIEWER_IDENTITY_ANCHOR_COMMIT,
  V5_R9_REVIEWER_IDENTITY_ANCHOR_HASH,
...
import {
  validatePublicReviewIdentityAnchorV5R9,
} from "../../../../content-qa/mais-natural-ca60-v1/review-evidence-v5-r9.mjs";
import {
...
} from "../../../../content-qa/mais-natural-ca60-v1/schema-contract-v5-r9.mjs";

const ROOT = "coordination/content-qa/mais-natural-ca60-v1/";
const R8_REVIEWED_AT = "2026-08-26T14:49:30.000Z";
...
const A07_CLOSEOUT_RECEIPT_PATH =
  "coordination/reports/mais-natural-ca60-v5-r9-runner-closeout-a07/a07-runner-closeout-receipt.json";
const A07_SESSION_LOG_PATH =
  "coordination/session-logs/2026-08-26-A07-mais-natural-ca60-v5-r9.md";

export const TEST_ENTRYPOINTS_V5_R9 = Object.freeze([
  ...
  `${ROOT}runner-v5-r9-cli.test.mjs`,
  `${ROOT}runner-v5-r9-public-cli-workflow.test.mjs`,
  `${ROOT}runner-v5-r9-recovery.test.mjs`,
  `${ROOT}runner-v5-r9-remediation.test.mjs`,
  "coordination/research/mais-natural-ca60-v1/runner-registrations/v5-r9/runner-registration-artifact.test.mjs",
].sort());
...
function componentHash(manifest, sourcePaths) {
  ...
  requireCondition(selected.length === expected.length,
    `R9 core-guard source is absent: ${expected.filter((sourcePath) =>
...
function validateManifest(manifest, label) {
  ...
  `R9 ${label} source manifest is unsafe, unordered, duplicated, or incomplete`);
}

export function registeredTestPathsV5R9(testClosure) {
  ...
  "R9 test transitive closure is invalid");
  return Object.freeze([...new Set(testClosure.paths)].sort(compare));
}

export function buildRunnerSupersedingRegistrationV5R9({
  runnerSourceCommit,
  productionClosure,
  testClosure,
  ...
  requireCondition(/^[0-9a-f]{40}$/u.test(runnerSourceCommit),
    "R9 runner source commit is invalid");
  requireCondition(new Date(registeredAt).toISOString() === registeredAt
    && Date.parse(registeredAt) > Date.parse(R8_REVIEWED_AT),
  "R9 runner registration must strictly postdate the bound V5-R8 A11 discrepancy review");
  requireCondition(validatePublicReviewIdentityAnchorV5R9(REVIEWER_IDENTITY_ANCHOR).length === 0
    && REVIEWER_IDENTITY_ANCHOR.selfHash === V5_R9_REVIEWER_IDENTITY_ANCHOR_HASH,
  "R9 public A11 reviewer identity anchor is invalid or differs from its frozen hash");
  ...
  "R9 production transitive closure is invalid");
  ...
  "R9 production manifest is not the complete transitive import closure");
  requireCondition(PRODUCTION_ENTRYPOINTS_V5_R9.every((entrypoint) =>
    productionClosure.paths.includes(entrypoint)),
  "R9 production entrypoints are absent from the closure");
  requireCondition(TEST_ENTRYPOINTS_V5_R9.every((entrypoint) => testClosure.paths.includes(entrypoint))
    && canonicalJsonV5R3(registeredTestPathsV5R9(testClosure))
      === canonicalJsonV5R3(testSourceManifest.map(({ path: sourcePath }) => sourcePath)),
  "R9 test manifest differs from the complete registered test closure");
  const closureKernels = computeClosureKernelsV5R9(productionSourceManifest);
  requireCondition(Object.values(closureKernels)
    .every((value) => /^[0-9a-f]{64}$/u.test(value ?? "")),
  "R9 closure-kernel component source is missing");
  ...
  ].map((field) => [field, V5_R8_REGISTRATION[field]]));
  const registration = sealV5R3Artifact({
    schemaVersion: "NaturalCaExecutionRunnerSupersedingRegistrationV5",
    runnerVersion: "V5-R9",
    designId: "MAIS-NATURAL-CA60-V5",
    registeredAt,
    supersedesRunnerRegistrationHash: V5_R8_REGISTRATION_HASH,
    supersededRunnerRegistrationCommit: V5_R8_REGISTRATION_COMMIT,
    supersededRunnerSourceCommit: V5_R8_SOURCE_COMMIT,
    discrepancyReviewHash: V5_R8_A11_DISCREPANCY_REVIEW_HASH,
    discrepancyReviewCommit: V5_R8_A11_DISCREPANCY_REVIEW_COMMIT,
    previousReceiptHash: V5_R8_A11_DISCREPANCY_REVIEW_HASH,
    ownerRunnerImplementationAuthorizationTextHash:
      V5_R9_OWNER_RUNNER_IMPLEMENTATION_AUTHORIZATION_TEXT_HASH,
    reviewerIdentityAnchorPath: REVIEWER_IDENTITY_ANCHOR_PATH,
    reviewerIdentityAnchorCommit: V5_R9_REVIEWER_IDENTITY_ANCHOR_COMMIT,
    reviewerIdentityAnchorOriginCommit: V5_R9_REVIEWER_IDENTITY_ANCHOR_ORIGIN_COMMIT,
    ...
    ...frozen,
    runnerSourceCommit,
    productionEntryPoints: [...PRODUCTION_ENTRYPOINTS_V5_R9],
    productionSourceManifest,
    ...
    supersededProductionSourceRootHash: V5_R8_REGISTRATION.productionSourceRootHash,
    supersededTestSourceRootHash: V5_R8_REGISTRATION.testSourceRootHash,
    supersededImportClosureRootHash: V5_R8_REGISTRATION.importClosureRootHash,
    ...
    closureKernels,
    runtimeSourceEnumerationRootHash: runtimeSourceEnumerationRootV5R9(productionSourceManifest),
    ...
    providerEntrypoints: {
      openAI: {
        ...
        registeredCli: "runner-v5-r9-cli.mjs",
        coreGuardHash,
        providerCallsMade: 0,
      },
      deepSeek: {
    ...
    status: "OFFLINE_IMPLEMENTED_PENDING_FRESH_A11_V5_R9_REVIEW_PROVIDER_EXECUTION_BLOCKED",
    claimCeiling: "FRAME_AND_SAMPLE_FROZEN_RUNNER_OFFLINE_IMPLEMENTED_NOT_EXECUTED",
  });
  assertClosedSelfHashedArtifactV5R9(registration,
    "NaturalCaExecutionRunnerSupersedingRegistrationV5");
  const errors = validateActiveRunnerRegistrationV5R9(registration);
  requireCondition(errors.length === 0, errors.join("; "));
  return registration;
}

export async function buildRunnerSupersedingRegistrationFromGitV5R9({
  repoRoot,
  runnerSourceCommit,
  registeredAt,
}) {
  ...
  const testSourceManifest = await sourceManifestFromGitV5R6({
    repoRoot,
    commit: runnerSourceCommit,
    paths: registeredTestPathsV5R9(testClosure),
  });
  return buildRunnerSupersedingRegistrationV5R9({
    runnerSourceCommit,
    productionClosure,
    testClosure,
    productionSourceManifest,
