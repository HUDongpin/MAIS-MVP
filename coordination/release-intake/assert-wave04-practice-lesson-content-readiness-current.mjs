...
const outputPath = path.join(root, "coordination", "release-intake", "latest-A25-wave04-practice-lesson-content-readiness-current-gate.json");
...
const paths = {
  ...
  readiness: "coordination/release-intake/latest-A25-wave04-practice-lesson-content-readiness.json",
  readinessMarkdown: "coordination/release-intake/latest-A25-wave04-practice-lesson-content-readiness.md"
};
...
const packageConfigs = [
  {
    ...
    checkNames: ["testQuestionBank", "typeCheck", "practicePlaywright"]
  },
  {
    ...
    checkNames: ["typeCheck", "lessonPlaywright"]
  },
  {
    ...
    checkNames: ["testRag", "testQuestionBank", "typeCheck"]
  }
];
...
function main() {
  ...
  const sequenceWave = wave04(sequence);

  if (!sequenceWave) failures.push("Wave 04 is missing from the closure execution sequence");
  ...
  if (readiness.wave?.waveId !== "wave-04-practice-lesson-content") failures.push("readiness wave id is stale");
...
function finish(payload) {
  ...
  if (json) {
    ...
    console.log("A25 Wave 04 practice lesson content readiness gate");
    console.log(`Commit ready: ${payload.commitReady ? "yes" : "no"}`);
    console.log(`Failures: ${payload.failures.length}`);
  }
  if (payload.failures.length > 0) {
    console.error("A25 Wave 04 practice lesson content readiness gate failed.");
