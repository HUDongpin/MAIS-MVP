...
const paths = {
  ...
  latestJson: "coordination/release-intake/latest-A25-wave03-shell-dashboard-roadmap-readiness.json",
  latestMarkdown: "coordination/release-intake/latest-A25-wave03-shell-dashboard-roadmap-readiness.md",
  datedJson: `coordination/release-intake/${date}-A25-wave03-shell-dashboard-roadmap-readiness.json`,
  datedMarkdown: `coordination/release-intake/${date}-A25-wave03-shell-dashboard-roadmap-readiness.md`
};
...
const packageConfigs = [
  {
    ...
    approvalIds: ["a01-app-shell-lead"],
    ...
    checks: [
      ...
      {
        name: "appShellPlaywright",
        command: "npx",
        args: ["playwright", "test", "tests/e2e/app-shell-auth.spec.ts", "tests/e2e/home-functional.spec.ts", "--project=desktop-chrome"]
      }
    ]
  },
  {
    ...
    approvalIds: ["a02-dashboard-lead", "a15-adaptive-engine-lead"],
    ...
    checks: [
      { name: "testAnalytics", command: "npm", args: ["run", "test:analytics"] },
      { name: "typeCheck", command: "npm", args: ["run", "type-check", "--", "--pretty", "false"] },
      {
        name: "dashboardAdaptivePlaywright",
        command: "npx",
        args: ["playwright", "test", "tests/e2e/student-smoke.spec.ts", "tests/e2e/adaptive-llm-smoke.spec.ts", "--project=desktop-chrome"]
      }
    ]
  },
  {
    ...
    approvalIds: ["a03-curriculum-roadmap-lead"],
    ...
    checks: [
      { name: "typeCheck", command: "npm", args: ["run", "type-check", "--", "--pretty", "false"] },
      {
        name: "roadmapPlaywright",
        command: "npx",
        args: ["playwright", "test", "tests/e2e/mainland-hjb-roadmaps.spec.ts", "tests/e2e/mainland-pep-roadmaps.spec.ts", "--project=desktop-chrome"]
      }
    ]
  }
];
...
function packageReadiness(config, previousPackagesById, previousGeneratedAt) {
  ...
  for (const check of config.checks) {
    ...
    if (check.name === "typeCheck") typeCheckRawByName[check.name] = raw;
  }

  const typeCheckRaw = typeCheckRawByName.typeCheck ?? { stdout: "", stderr: "", status: null, passed: false };
  const typeCheck = {
...
function summarize(packages) {
  const checks = packages.flatMap((pkg) => Object.entries(pkg.checks).map(([name, check]) => ({ packageId: pkg.packageId, name, check })));
  return {
...
function markdown(payload) {
  ...
  return `# A25 Wave 03 Shell Dashboard Roadmap Readiness
...
function main() {
  ...
  const wave = wave03(sequence);
  if (!wave) throw new Error("Wave 03 is missing from the closure execution sequence.");
