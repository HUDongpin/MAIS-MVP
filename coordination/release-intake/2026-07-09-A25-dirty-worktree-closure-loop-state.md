...
Generated: 2026-07-09T14:00:37.285Z
...
- Focus batch owner-authorization intake: no-focus-batch
- Focus batch accepted rows: 0/0
- Focus batch pending rows: 0
- Focus batch owner-input visible rows: 0
...
- Exact command targets ready: 3/3
...
| 验证 | validate | active | 5/6 | Completion audit is missing or stale |
| 合并 | merge | blocked | 1/4 | 41 canonical authorization row(s) are still pending; Owner inputs are not ready; Validation hold is waiting for owner compose deletion confirmation |
| 清理 | cleanup | blocked | 0/4 | 6604 expanded dirty entries remain; A22 release source is not clean; A25 strict worktree lifecycle is not clean |
...
| A25 aggregate currentness is represented | no | Completion audit is missing or stale |
...
| Check | Passed | Blocker |
| --- | --- | --- |
| Owner canonical authorization rows are recorded | no | 41 canonical authorization row(s) are still pending |
| Separate execution instructions are recorded for ready owner execution inputs | yes | none |
...
| Check | Passed | Blocker |
| --- | --- | --- |
| Root dirty entries are zero | no | 6604 expanded dirty entries remain |
