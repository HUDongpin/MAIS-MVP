# 2026-07-04 A25 Effective Work Order - A08 state and analytics lead

- Owner: A08 state and analytics lead
- Priority: P4
- Reason: Standard owner package.
- Entries: 11
- From P0 proposals: 0
- Dominant slice: runtime app/API/data/public: 5
- Pathspec: `coordination/release-intake/latest-A25-effective-owner-a08-state-and-analytics-lead.pathspec`

## Required Final State

Reviewed commit, owner-approved discard, evidence archive, or blocker.

## Suggested Commands

```bash
node coordination/release-intake/review-owner-pathspec.mjs coordination/release-intake/latest-A25-effective-owner-a08-state-and-analytics-lead.pathspec --status
node coordination/release-intake/review-owner-pathspec.mjs coordination/release-intake/latest-A25-effective-owner-a08-state-and-analytics-lead.pathspec --diffstat
```

## Status Buckets

- `??`: 6
- `M`: 5

## P0 Proposal Confidence

- No P0 proposal entries in this package.

## Path Sample

- `M` `components/providers/AppProviders.tsx`
- `M` `lib/learningAnalytics.test.ts`
- `M` `lib/learningAnalytics.ts`
- `M` `lib/utils.ts`
- `M` `types/index.ts`
- `??` `components/providers/appProvidersStudentWarmupPerformanceBoundary.test.ts`
- `??` `components/providers/appProvidersTeacherAnalyticsBoundary.test.ts`
- `??` `lib/difficulty.test.ts`
- `??` `lib/difficulty.ts`
- `??` `scripts/run-analytics-tests.mjs`
- `??` `tsconfig.analytics.json`
