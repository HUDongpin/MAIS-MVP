...
Generated: 2026-07-09T14:01:17.855Z
...
- Projected pending canonical rows after current focus: 41
...
| --- | --- | --- | --- | ---: | --- |
| `current-root-clean-worktree` | clean worktree | blocked | no | 3 | 6604 root dirty status entries remain in integration inventory; dirty-root deploy guard is expected to block direct preview/production publish; A22 policy requires a clean worktree, clean clone, reviewed clean release slice, or owner-approved pruned staging package |
...
| `reviewed-clean-release-slice` | reviewed clean release slice | blocked | no | 5 | 15 owner-package row(s) remain blocked before a reviewed release slice can be trusted; 9907 package/worktree type-check error line(s) remain on the critical path; 41 canonical authorization row(s) remain pending now; 41 canonical authorization row(s) would still remain after the current 0-row focus batch; validate-to-merge exit criteria are not green |
...
| --- | --- | --- | ---: | --- |
| `a22-release-source-not-clean` | A22 production reliability and release engineering | blocked | 6604 | The current root remains a dirty integration inventory and cannot be used as the release source. |
...
| `canonical-authorization-backlog` | A25 git hygiene and release intake | blocked | 41 | Canonical authorization backlog remains before owner inputs can be green. |
