...
function parseArgs(argv) {
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (!["--policy", "--target-root", "--expected-head", "--expected-release"].includes(flag)) {
  ...
  if (
    ...
    typeof values.expectedRelease !== "string"
  ) {
    throw argumentError(
      "AUDIT_ARGUMENT_INVALID",
      "audit requires --policy, --target-root, --expected-head, --expected-release, and --json."
    );
  }
  return values;
}
...
try {
  ...
  report = await auditPromotionTerminalCurrentHead({
    ...
    expectedReleaseCommit: parsed.expectedRelease,
